...
const USAGE = 'Usage: owner-entry-runtime [path] [--json]';
...
export async function runOwnerEntryRuntime(
  ...
  try {
    const resolution = await resolveOwnerWorkflowResolution(parsed.targetPath);
    io.stdout(
      parsed.json
        ? `${JSON.stringify(resolution, null, 2)}\n`
        : `${formatOwnerWorkflowResolution(resolution)}\n`,
    );
    return 0;
  } catch (error) {
    io.stderr(`${error instanceof Error ? error.message : String(error)}\n`);

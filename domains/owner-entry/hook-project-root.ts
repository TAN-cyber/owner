...
import type { OwnerHookRequest } from './hook-types.js';
...
async function assertRebasedWorktreeReady(projectRoot: string): Promise<void> {
  for (const marker of ['.git', path.join('.owner', 'config.yaml')]) {
    try {
      await fs.lstat(path.join(projectRoot, marker));
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error;
      throw new Error(
        `linked worktree ${projectRoot} is not initialized for Owner: missing ${marker.replaceAll('\\', '/')}`,
        { cause: error },
      );
    }
  }
...
export async function resolveOwnerHookProjectRoot(

...
import type { OwnerHookDecision, OwnerHookProcessOutput, OwnerHookRequest } from './hook-types.js';
...
export const OWNER_HOOK_PLATFORM_IDS = new Set(['claude', 'codex']);
...
export function parseOwnerHookRequest(source: string, filePath?: string): OwnerHookRequest {
  if (filePath?.trim()) {
...
export function readOwnerHookRequest(): OwnerHookRequest {
  const filePath = process.env.FILE_PATH;
  if (filePath?.trim()) return parseOwnerHookRequest('', filePath);
  if (process.stdin.isTTY) return parseOwnerHookRequest('', filePath);
  try {
    return parseOwnerHookRequest(readFileSync(0, 'utf8'), filePath);
  } catch {
    return parseOwnerHookRequest('', filePath);
  }
}

export function renderOwnerHookDecision(
  platformId: string,
  decision: OwnerHookDecision,
): OwnerHookProcessOutput {
  if (!OWNER_HOOK_PLATFORM_IDS.has(platformId)) {
    return {
      exitCode: 64,
      stdout: '',
      stderr: `Unsupported Owner Hook platform: ${platformId}\n`,
    };
  }

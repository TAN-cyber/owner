...
async function readProtectedIfExists(
  ...
  return readPipelineProjectFile(projectRoot, file, { label });
}
...
async function writeProtectedText(
  ...
  await writePipelineProjectText(projectRoot, file, content, { label });
}
...
async function handoffSourceFiles(projectRoot: string, changeDir: string): Promise<string[]> {
  ...
  const specsInspection = await inspectPipelineProjectTarget(projectRoot, specs, {
    label: 'Pipeline handoff specs directory',
    expected: 'directory',
  });
  if (specsInspection.exists) {
    ...
    for (const entry of entries) {
      ...
      const specDirectory = await inspectPipelineProjectTarget(
        projectRoot,
        `${specs}/${entry.name}`,
        {
          label: `Pipeline handoff spec directory ${entry.name}`,
          expected: 'directory',
        },
      );
      if (!specDirectory.exists) continue;
      if (
        await pipelineProjectTargetExists(projectRoot, spec, {
          label: `Pipeline handoff spec ${entry.name}`,
          expected: 'file',
        })
      ) {
        files.push(spec);
...
async function computeContextHash(
  ...
  for (const file of await handoffSourceFiles(projectRoot, changeDir)) {
    ...
    const content = await readProtectedIfExists(
      projectRoot,
      file,
      `Pipeline handoff source ${reference}`,
    );
...
async function writeMarkdownContext(
  ...
  await writeProtectedText(
    ...
    output,
    lines.join('\n'),
    'Pipeline handoff markdown output',
  );
}
...
async function writeJsonContext(
  ...
  await writeProtectedText(projectRoot, output, document, 'Pipeline handoff JSON output');
}
...
async function writeSpecMarkdownContext(
  ...
  if (specsInspection.exists) {
    ...
    for (const entry of entries) {
      ...
      const content = await readProtectedIfExists(
        projectRoot,
        spec,
        `Pipeline handoff spec ${entry.name}`,
      );
      if (content === null) continue;
      projected = true;
      lines.push(
  ...
  await writeProtectedText(
    ...
    output,
    lines.join('\n'),
    'Pipeline handoff spec markdown output',
  );
}
...
async function writeSpecJsonContext(
  ...
  await writeProtectedText(
    ...
    'Pipeline handoff spec JSON output',
  );
}
...
async function readField(projectRoot: string, changeDir: string, field: string): Promise<string> {
  ...
  const document = parseDocument(
    await readPipelineProjectFile(projectRoot, file, { label: 'Pipeline change state' }),
    { uniqueKeys: false },
  );
  if (document.errors.length > 0) {
    throw new HandoffFailure(`ERROR: Invalid .owner.yaml: ${document.errors[0].message}`);
  }
...
async function appendRecoveryEvent(
  ...
  const alreadyRecorded = trajectory.some(
    (event) =>
      ...
      event.data.kind === 'pipeline-handoff' &&
...
async function completedHandoffIsCurrent(
  ...
  const [contextJsonContent, contextMdContent] = await Promise.all([
    readProtectedIfExists(projectRoot, contextJson, 'Pipeline handoff JSON output'),
    readProtectedIfExists(projectRoot, contextMd, 'Pipeline handoff markdown output'),
  ]);
    ...
    try {
      ...
      if (phase === '--hash-only') {
        ...
        for (const required of ['proposal.md', 'design.md', 'tasks.md']) {
          if (
            !(await pipelineProjectFileNonempty(
              layout.projectRoot,
              `${changeDir}/${required}`,
              `Pipeline handoff source ${required}`,
            ))
          ) {
      ...
      if (phase !== 'design' || mode !== '--write') {
        throw new HandoffFailure(
          red('Usage: owner-handoff.mjs <change-name> design --write [--full]'),
        );
      }
      ...
      if (!active.stateExists) {
        throw new HandoffFailure(red(`ERROR: .owner.yaml not found at ${changeRef}/.owner.yaml`));
      }
      ...
      const handoffDir = `${changeDir}/.owner/handoff`;
      await inspectPipelineProjectTarget(layout.projectRoot, `${changeDir}/.owner`, {
        label: 'Pipeline change runtime directory',
        expected: 'directory',
      });
      await inspectPipelineProjectTarget(layout.projectRoot, handoffDir, {
        label: 'Pipeline handoff directory',
        expected: 'directory',
      });
      ...
      const contextJsonRef = pipelineProjectRelative(layout.projectRoot, contextJson);
      const contextMdRef = pipelineProjectRelative(layout.projectRoot, contextMd);
      const contextHash = await computeContextHash(layout.projectRoot, changeDir, changeRef);
      const actionId = `pipeline-handoff:${contextHash}`;
      const initialProjection = await readPipelineState(changeDir);
      if (!initialProjection.pipeline) {
        throw new HandoffFailure(red('ERROR: design handoff requires Pipeline state'));
      }
      ...
      if (
        initialProjection.pipeline.handoffHash &&
        initialProjection.pipeline.handoffHash !== contextHash &&
        !recovering
      ) {
        throw new HandoffFailure(
          red(
            `ERROR: stale handoff detected: source hash ${contextHash} does not match completed hash ${initialProjection.pipeline.handoffHash}`,
          ),
        );
      }
      ...
      if (
        runtime.pipeline.handoffHash === contextHash &&
        runtime.pipeline.handoffContext === contextJsonRef &&
        !runtime.run.pending &&
        !pendingAction &&
      ...
      const context = await readPipelineProjectFile(layout.projectRoot, contextMd, {
        label: 'Pipeline handoff markdown output',
      });
      ...
      const transitionedRun =
        pendingRun.currentStep === 'full.design.handoff'
          ? await transitionPipelineRuntimeRun(changeDir, completedPipeline, pendingRun, {
              actionId,
              kind: 'pipeline-handoff',
            })
          : pendingRun;

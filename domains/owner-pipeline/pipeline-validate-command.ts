...
import { pipelineProjectTargetExists, readPipelineProjectFile } from './pipeline-protected-path.js';
import { PIPELINE_WIRE_KEYS, RUN_WIRE_KEYS } from './pipeline-state.js';
import {
...
const ENUMS: Record<string, readonly string[]> = {
  ...
  pipeline_profile: ['full', 'hotfix', 'tweak'],
...
export const pipelineValidateCommand: PipelineCommandHandler = async (args, options) =>
  withPipelineCommandContext(options, async () => {
    ...
    const { directory, label } = await resolvePipelineChangeDirectory(name, projectRoot);
    const yamlFile = path.join(directory, '.owner.yaml');
    const lines = [`[VALIDATE] ${label}/.owner.yaml`];
    let errors = 0;
    let warnings = 0;
    ...
    let source: string;
    try {
      source = await readPipelineProjectFile(projectRoot, yamlFile, {
        label: `Pipeline state ${label}/.owner.yaml`,
      });
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        fail('.owner.yaml does not exist');
        lines.push(
          '',
    ...
    for (const field of ['design_doc', 'plan', 'handoff_context', 'verification_report'] as const) {
      ...
      try {
        if (
          !(await pipelineProjectTargetExists(projectRoot, path.resolve(projectRoot, value), {

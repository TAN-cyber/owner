...
import {
  assertPipelineWorkflowEnabled,
  pipelineLayoutPaths,
  discoverPipelineProject,
  readPipelineArtifactLayout,
  writePipelineArtifactLayout,
} from './pipeline-layout.js';

const JOURNAL_RELATIVE_PATH = '.owner/pipeline-root-move.json';
const STAGING_PLAN_IDENTITY = '.owner/transactions/pipeline-root-move/<transaction-id>/openspec';
...
const journalClaimBrand = Symbol('PipelineRootMoveJournalClaim');

export interface PipelineRootMoveFileSummary {
  path: string;
  size: number;
  hash: string;
}

interface PipelineRootMoveManifest {
  directories: string[];
  files: PipelineRootMoveFileSummary[];
  totalBytes: number;
  hash: string;
}

interface PipelineRootMoveJournalBase {
  id: string;
  source: string;
  target: string;
  staging: string;
  ...
  planId: string;
  manifest: PipelineRootMoveManifest;
  readonly [journalClaimBrand]?: PipelineRootMoveJournalClaim;
}

interface PipelineRootMoveJournalV1 extends PipelineRootMoveJournalBase {
  schema: 'owner.pipeline-root-move.v1';
  stage: 'copying' | 'ready' | 'switched' | 'configured';
  targetInitialState: 'missing' | 'empty';
}

interface PipelineRootMoveJournalV2 extends PipelineRootMoveJournalBase {
  schema: 'owner.pipeline-root-move.v2';
  stage: 'locked' | 'copying' | 'ready' | 'switched' | 'configured';
  artifactLayout: 'legacy';
  sourceIdentity: PipelineRootMovePathIdentity[];
  targetInitialIdentity: PipelineRootMoveObjectIdentity | null;
  approvedPlanId: string;
  targetInitialState: 'missing' | 'empty' | 'non-empty';
}

type PipelineRootMoveJournal = PipelineRootMoveJournalV1 | PipelineRootMoveJournalV2;

interface PipelineRootMoveJournalClaim {
  fileIdentity: BigIntStats;
  contentHash: string;
}

interface PipelineRootMoveTestHooks {
...
export interface PipelineRootMoveOptions {
  planId?: string;
  testHooks?: PipelineRootMoveTestHooks;
}

export type PipelineRootMoveRecoveryStrategy = 'continue' | 'rollback';

export interface PipelineRootMoveFileObjectIdentity {
  dev: string;
  ino: string;
  birthtime: string;
}

export interface PipelineRootMoveObjectIdentity extends PipelineRootMoveFileObjectIdentity {
  ctime: string;
  mtime: string;
}

export interface PipelineRootMovePathIdentity extends PipelineRootMoveFileObjectIdentity {
  path: string;
}

export interface PipelineRootMovePlan {
  projectRoot: string;
  source: string;
  target: string;
  staging: string;
  ...
  expectedConfigHash: string;
  planId: string;
  fileSummary: PipelineRootMoveFileSummary[];
  configChange: { from: 'legacy'; to: 'docs' };
  conflicts: string[];
  blockers: string[];
  pendingRecovery: PipelineRootMoveInspection | null;
  historicalPointersPreserved: string[];
  applyPreconditions: string[];
  allowedRecoveryStrategies: PipelineRootMoveRecoveryStrategy[];
  targetInitialState: 'missing' | 'empty' | 'non-empty';
  readyToApply: boolean;
}

export interface PipelineRootMoveInspection {
  id: string;
  stage: PipelineRootMoveJournal['stage'];
  source: string;
  target: string;
  staging: string;
  planId: string;
  allowedStrategies: PipelineRootMoveRecoveryStrategy[];
  reason?: string;
}
...
async function assertProtectedMovePath(
  ...
  await inspectProtectedProjectPath(projectRoot, projectRelative(projectRoot, target), {
    label: `Pipeline root move physical path ${label}`,
    expected,
  });
}
...
async function writeProtectedFileExclusive(
  ...
  const label = `Pipeline root move ${operation}`;
...
async function removeProtectedEmptyDirectory(
  ...
  operation: string,
  testHooks?: PipelineRootMoveTestHooks,
  expectedIdentity?: PipelineRootMoveObjectIdentity,
): Promise<boolean> {
  const label = `Pipeline root move ${operation}`;
  let identity: BigIntStats;
  try {
  ...
  if (
    expectedIdentity &&
    !sameStableIdentity(
      stableObjectIdentity(identity, `Pipeline root move ${operation}`),
      expectedIdentity,
    )
  ) {
    throw new Error(`Pipeline root move ${operation} changed after migration preflight`);
  }
...
async function assertRootMovePreflightBoundaries(
  ...
  const legacy = pipelineLayoutPaths(projectRoot, 'legacy');
  const docs = pipelineLayoutPaths(projectRoot, 'docs');
  const transactionBase = path.join(projectRoot, '.owner', 'transactions', 'pipeline-root-move');
  const checks: Array<[string, string, 'file' | 'directory' | 'any']> = [
    [path.join(projectRoot, '.owner'), '.owner', 'directory'],
    [path.join(projectRoot, '.owner', 'config.yaml'), '.owner/config.yaml', 'file'],
    ...
    [path.join(projectRoot, '.owner', 'transactions'), '.owner/transactions', 'directory'],
    [transactionBase, '.owner/transactions/pipeline-root-move', 'directory'],
  ];
  if (transactionId) {
...
async function atomicWriteJournal(
  projectRoot: string,
  expected: PipelineRootMoveJournal,
  updated: PipelineRootMoveJournal,
  testHooks?: PipelineRootMoveTestHooks,
): Promise<PipelineRootMoveJournal> {
  const file = journalFile(projectRoot);
  await ensureRootMoveDirectory(
    projectRoot,
    path.dirname(file),
    'Pipeline root move journal parent',
    testHooks,
  );
  const temporary = `${file}.${randomUUID()}.tmp`;
  const serialized = JSON.stringify(updated, null, 2) + '\n';
  let quarantine: QuarantinedPipelineRootMoveJournal | undefined;
  try {
    ...
    const temporaryIdentity = await captureMutationObject(
      temporary,
      'file',
      'Pipeline root move updated journal',
    );
    ...
    try {
      ...
      throw new Error(
        'Pipeline root move journal publish failed; a successor journal was preserved',
        { cause: error },
      );
    }
    ...
    if (
      ...
      throw new Error('Pipeline root move journal ownership changed while updating');
    }
...
async function createJsonExclusive(
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'EEXIST') {
      throw new Error(
        'Pipeline root move is incomplete; use owner doctor --repair --strategy continue|rollback',
        { cause: error },
      );
    }
    throw error;
...
function manifestHash(manifest: Omit<PipelineRootMoveManifest, 'hash'>): string {
...
interface PipelineRootMovePlanIdentity {
  source: string;
  target: string;
  staging: string;
  artifactLayout: 'legacy';
...
function planIdFor(plan: PipelineRootMovePlanIdentity): string {
  return createHash('sha256')
    .update(
      JSON.stringify({
        source: plan.source,
...
function stableObjectIdentity(stat: BigIntStats, label: string): PipelineRootMoveObjectIdentity {
...
function sameStableIdentity(
  left: PipelineRootMoveFileObjectIdentity | PipelineRootMovePathIdentity,
  right: PipelineRootMoveFileObjectIdentity | PipelineRootMovePathIdentity,
): boolean {
  return JSON.stringify(left) === JSON.stringify(right);
}

function sameStableChain(
  left: PipelineRootMovePathIdentity[],
  right: PipelineRootMovePathIdentity[],
): boolean {
  return (
...
async function captureStableSourceIdentity(
  ...
  directory: string,
  label: string,
): Promise<PipelineRootMovePathIdentity[]> {
  const chain = await captureProtectedDirectoryChain(projectRoot, directory, label);
  await validateProtectedDirectoryChain(chain, label);
...
async function projectConfigSnapshot(projectRoot: string) {
  ...
  if (!snapshot.document || !snapshot.identity.exists || !snapshot.identity.sha256) {
    throw new Error('.owner/config.yaml does not exist');
  }
...
async function scanTree(
  ...
  root: string,
  testHooks?: PipelineRootMoveTestHooks,
): Promise<PipelineRootMoveManifest> {
  const directories: string[] = [];
  const files: PipelineRootMoveFileSummary[] = [];
  let totalBytes = 0;
  ...
  async function visit(directory: string, relativeDirectory: string): Promise<void> {
    ...
    for (const entry of entries.sort((left, right) => compareManifestPath(left.name, right.name))) {
      ...
      if (stat.isSymbolicLink()) {
        throw new Error(
          `Pipeline root move does not support symbolic links or junctions: ${relative}`,
        );
      }
      if (stat.isDirectory()) {
      ...
      if (!stat.isFile()) {
        throw new Error(
          `Pipeline root move supports only regular files and directories: ${relative}`,
        );
      }
      await testHooks?.afterSourceFileInspect?.(relative);
      if (files.length + 1 > MAX_FILES) {
        throw new Error(`Pipeline root move exceeds ${MAX_FILES} files`);
      }
      ...
      const bytes = await readRootMoveFile(
        ...
        `Pipeline root move source file ${relative}`,
        stat,
      );
      totalBytes += bytes.byteLength;
      if (totalBytes > MAX_TOTAL_BYTES) {
        throw new Error(`Pipeline root move exceeds ${MAX_TOTAL_BYTES} bytes`);
      }
...
function sameManifest(
  actual: PipelineRootMoveManifest,
  expected: PipelineRootMoveManifest,
): boolean {
...
async function inspectInitialTarget(
  ...
  conflicts: string[];
  identity: PipelineRootMoveObjectIdentity | null;
}> {
  try {
    const chain = await captureProtectedDirectoryChain(projectRoot, target, 'docs-target');
    const initialEntry = chain.entries.at(-1);
    if (!initialEntry) throw new Error('Pipeline docs target identity is unavailable');
    const initialIdentity = stableObjectIdentity(initialEntry.stat, 'Pipeline docs target');
    ...
    const currentIdentity = currentEntry
      ? stableObjectIdentity(currentEntry.stat, 'Pipeline docs target')
      : null;
    if (!currentIdentity || !sameStableIdentity(initialIdentity, currentIdentity)) {
      throw new Error('Pipeline docs target changed after inspection');
    }
    if (entries.length > 0) {
      return {
        state: 'non-empty',
        conflicts: ['Pipeline docs target is not empty'],
...
async function validatedLegacyConfig(projectRoot: string): Promise<void> {
  await assertPipelineWorkflowEnabled(projectRoot);
  if ((await readPipelineArtifactLayout(projectRoot)) !== 'legacy') {
    throw new Error('Pipeline root move requires pipeline.artifact_layout: legacy');
  }
}

async function preflight(
  projectRoot: string,
  options: PipelineRootMoveOptions = {},
): Promise<{
  plan: PipelineRootMovePlan;
  manifest: PipelineRootMoveManifest;
}> {
  await assertRootMovePreflightBoundaries(projectRoot);
  await validatedLegacyConfig(projectRoot);
  if ((await readPipelineArtifactLayout(projectRoot)) !== 'legacy') {
    throw new Error('Pipeline root move requires the legacy layout');
  }
  const legacy = pipelineLayoutPaths(projectRoot, 'legacy');
  const docs = pipelineLayoutPaths(projectRoot, 'docs');
  if (!(await protectedDirectoryExists(projectRoot, legacy.openSpecRoot, 'Pipeline legacy root'))) {
    throw new Error('Pipeline legacy root does not exist: openspec/');
  }
  ...
  if (!sameStableChain(sourceIdentityBefore, sourceIdentityAfter)) {
    throw new Error('Pipeline legacy root identity changed during migration preflight');
  }
  ...
  const identity: PipelineRootMovePlanIdentity = {
...
async function copyManifest(
  ...
  await ensureRootMoveDirectory(
    projectRoot,
    targetRoot,
    'Pipeline root move staging root',
    testHooks,
  );
  for (const directory of manifest.directories) {
    await ensureRootMoveDirectory(
      ...
      `Pipeline root move staging directory ${directory}`,
      testHooks,
    );
  }
  for (const file of manifest.files) {
    ...
    const sourceBytes = await readRootMoveFile(
      ...
      `Pipeline root move source file ${file.path}`,
    );
    if (sourceBytes.byteLength !== file.size || hashBytes(sourceBytes) !== file.hash) {
      throw new Error('Pipeline legacy root changed after migration preflight');
    }
    const target = path.join(targetRoot, ...file.path.split('/'));
    await ensureRootMoveDirectory(
      projectRoot,
      path.dirname(target),
      `Pipeline root move staging parent ${file.path}`,
      testHooks,
    );
    const targetInspection = await inspectProtectedProjectPath(
      projectRoot,
      projectRelative(projectRoot, target),
      { label: `Pipeline root move staging file ${file.path}`, expected: 'file' },
    );
    if (targetInspection.exists) {
      throw new Error('Pipeline root move staging changed after migration preflight');
    }
    await writeProtectedFileExclusive(
      projectRoot,
      target,
    ...
    const copiedBytes = await readRootMoveFile(
      ...
      target,
      Math.max(1, file.size),
      `Pipeline root move staging file ${file.path}`,
    );
    if (copiedBytes.byteLength !== file.size || hashBytes(copiedBytes) !== file.hash) {
      throw new Error('Pipeline root move staging verification failed');
    }
  }
}
...
function invalidJournal(detail: string): Error {
  return new Error(`invalid Pipeline root move journal: ${detail}`);
}
...
function parseManifest(value: unknown): PipelineRootMoveManifest {
...
function parseFileObjectIdentity(
  value: unknown,
  label: string,
): PipelineRootMoveFileObjectIdentity {
  ...
  return {
    ...
    ino: identity.ino,
    birthtime: identity.birthtime,
  } as PipelineRootMoveFileObjectIdentity;
}

function parseObjectIdentity(value: unknown, label: string): PipelineRootMoveObjectIdentity {
  const identity = record(value, label);
...
function parseSourceIdentity(value: unknown): PipelineRootMovePathIdentity[] {
...
function parseJournalV1(journal: Record<string, unknown>): PipelineRootMoveJournalV1 {
  assertExactKeys(
    journal,
    [
      'schema',
  ...
  const expectedStaging = `.owner/transactions/pipeline-root-move/${journal.id}/openspec`;
  if (
...
function parseJournalV2(journal: Record<string, unknown>): PipelineRootMoveJournalV2 {
  assertExactKeys(
    journal,
    [
      'schema',
...
function parseJournal(value: unknown): PipelineRootMoveJournal {
  const journal = record(value, 'root');
  if (journal.schema === 'owner.pipeline-root-move.v1') return parseJournalV1(journal);
  if (journal.schema === 'owner.pipeline-root-move.v2') return parseJournalV2(journal);
...
async function readJournal(
  projectRoot: string,
  testHooks?: PipelineRootMoveTestHooks,
): Promise<PipelineRootMoveJournal | null> {
...
async function readJournalAtPath(
  ...
  try {
    ...
    Object.defineProperty(journal, journalClaimBrand, {
      ...
      } satisfies PipelineRootMoveJournalClaim,
      enumerable: false,
    });
    return journal;
  } catch (error) {
...
function journalClaim(journal: PipelineRootMoveJournal): PipelineRootMoveJournalClaim {
  const claim = journal[journalClaimBrand];
  if (!claim) throw new Error('Pipeline root move journal has no ownership claim');
  return claim;
}

function sameJournalOwnership(
  expected: PipelineRootMoveJournal,
  actual: PipelineRootMoveJournal,
): boolean {
...
async function validateOwnedJournal(
  ...
  if (!current || !sameJournalOwnership(expected, current)) {
    throw new Error('Pipeline root move journal ownership changed');
  }
}

interface QuarantinedPipelineRootMoveJournal {
  file: string;
  journal: PipelineRootMoveJournal;
}

async function restoreQuarantinedJournal(
  projectRoot: string,
  quarantine: QuarantinedPipelineRootMoveJournal,
  operation: string,
  testHooks?: PipelineRootMoveTestHooks,
): Promise<boolean> {
  const file = journalFile(projectRoot);
  ...
  if (!restored || !sameJournalOwnership(quarantine.journal, restored)) {
    throw new Error('Pipeline root move journal ownership changed while restoring quarantine');
  }
  await unlinkProtectedFile(
    projectRoot,
    quarantine.file,
...
async function quarantineOwnedJournal(
  ...
  operation: string,
  testHooks?: PipelineRootMoveTestHooks,
): Promise<QuarantinedPipelineRootMoveJournal> {
  const file = journalFile(projectRoot);
  ...
  const parent = await captureProtectedDirectoryChain(
    projectRoot,
    path.dirname(file),
    `Pipeline root move ${operation} quarantine`,
  );
  await testHooks?.beforeMutation?.(`${operation}-quarantine`);
  await validateProtectedDirectoryChain(parent, `Pipeline root move ${operation} quarantine`);
  await validateOwnedJournal(projectRoot, expected);
  ...
  const moved = await readJournalAtPath(
    ...
    throw new Error('Pipeline root move journal ownership changed while quarantining', {
      cause: error,
    });
  });
...
async function unlinkOwnedJournal(
  ...
  try {
    ...
    if (!removed) {
      throw new Error('Pipeline root move quarantined journal disappeared before removal');
    }
    return true;
  } catch (error) {
    await restoreQuarantinedJournal(
...
async function updateJournal(
  ...
  if (journal.schema === 'owner.pipeline-root-move.v1' && stage === 'locked') {
    throw new Error('Legacy Pipeline root move journals do not support the locked stage');
  }
  const updated = { ...journal, stage } as PipelineRootMoveJournal;
  return atomicWriteJournal(projectRoot, journal, updated, testHooks);
}

function transactionPaths(projectRoot: string, journal: PipelineRootMoveJournal) {
  const layouts = {
    legacy: pipelineLayoutPaths(projectRoot, 'legacy'),
    docs: pipelineLayoutPaths(projectRoot, 'docs'),
  };
...
async function treeStatus(
  ...
  if (!(await protectedDirectoryExists(projectRoot, root, 'Pipeline root move tree'))) {
    return 'missing';
  }
  try {
...
async function targetTreeStatus(
  ...
  if (!(await protectedDirectoryExists(projectRoot, root, 'Pipeline root move target'))) {
    return 'missing';
  }
  try {
...
async function manifestSubsetStatus(
  ...
  root: string,
  manifest: PipelineRootMoveManifest,
): Promise<{ status: ManifestSubsetStatus; actual?: PipelineRootMoveManifest }> {
  if (!(await protectedDirectoryExists(projectRoot, root, 'Pipeline root move manifest tree'))) {
...
async function cleanupManifestBoundTree(
  ...
  if (inspection.status === 'mismatch' || !inspection.actual) {
    throw new Error(
      'Pipeline quarantine contains unknown or changed content; files were preserved',
    );
  }
    ...
    const bytes =
      stat.isFile() && !stat.isSymbolicLink()
        ? await readRootMoveFile(
            ...
            absolute,
            Math.max(1, file.size),
            `Pipeline root move quarantine file ${file.path}`,
            stat,
          )
        : null;
...
async function consumeBoundEmptyTarget(
  ...
  if (journal.targetInitialState === 'missing') {
    if (state !== 'missing') {
      throw new Error('Pipeline docs target changed after migration preflight');
    }
    return;
  }
  if (state === 'empty') {
    if (journal.schema === 'owner.pipeline-root-move.v2' && !journal.targetInitialIdentity) {
      throw new Error('Pipeline docs target identity is missing from the approved plan');
    }
    await removeProtectedEmptyDirectory(
      projectRoot,
      target,
      'remove-empty-target',
      testHooks,
      journal.schema === 'owner.pipeline-root-move.v2'
...
async function assertBoundSourceIdentity(
  ...
  if (journal.schema === 'owner.pipeline-root-move.v1') return;
  ...
  if (!sameStableChain(current, journal.sourceIdentity)) {
    throw new Error('Pipeline legacy root identity changed after migration preflight');
  }
}

async function assertOriginalConfig(projectRoot: string, journal: PipelineRootMoveJournal) {
  if ((await projectConfigHash(projectRoot)) !== journal.originalConfigHash) {
    throw new Error('Pipeline project config changed after migration preflight');
  }
  if ((await readPipelineArtifactLayout(projectRoot)) !== 'legacy') {
    throw new Error('Pipeline project config changed after migration preflight');
  }
...
async function assertExpectedConfig(projectRoot: string, journal: PipelineRootMoveJournal) {
  if ((await projectConfigHash(projectRoot)) !== journal.expectedConfigHash) {
    throw new Error('Pipeline project config does not match the expected post-switch config hash');
  }
  if ((await readPipelineArtifactLayout(projectRoot)) !== 'docs') {
    throw new Error('Pipeline project config does not match the expected post-switch config hash');
  }
}
...
async function finishJournal(
  ...
  if (journal.stage === 'locked') {
    throw new Error('Pipeline root move locked preflight must be approved before execution');
  }
  ...
  if (current.stage === 'copying') {
    ...
    await assertTreeMatches(
      ...
      source,
      current.manifest,
      'Pipeline legacy root changed after migration preflight',
    );
    ...
    await assertTreeMatches(
      ...
      staging,
      current.manifest,
      'Pipeline root move staging verification failed',
    );
  ...
  if (current.stage === 'switched') {
    ...
    await assertTreeMatches(
      ...
      source,
      current.manifest,
      'Pipeline legacy root changed before config switch',
    );
    await assertTreeMatches(
      projectRoot,
      target,
      current.manifest,
      'Pipeline docs target verification failed after switch',
    );
    const layout = await readPipelineArtifactLayout(projectRoot);
    if (layout === 'legacy') {
      await assertOriginalConfig(projectRoot, current);
      await writePipelineArtifactLayout(projectRoot, 'docs', {
        expectedIdentity: {
          exists: true,
  ...
  if (current.stage !== 'configured') {
    throw new Error(`Unsupported Pipeline root move stage: ${current.stage}`);
  }
  await assertExpectedConfig(projectRoot, current);
  await assertTreeMatches(
    projectRoot,
    target,
    current.manifest,
    'Pipeline docs target verification failed after config switch',
  );
  if (await protectedDirectoryExists(projectRoot, source, 'Pipeline legacy root')) {
    await assertBoundSourceIdentity(projectRoot, source, current);
    await assertTreeMatches(
      projectRoot,
      source,
      current.manifest,
      'Pipeline legacy root changed before cleanup; both roots were preserved',
    );
    if (await protectedDirectoryExists(projectRoot, quarantine, 'Pipeline root move quarantine')) {
      throw new Error('Pipeline root move quarantine conflicts with the legacy root');
    }
...
async function rollbackJournal(
  ...
  if (
    journal.stage === 'configured' ||
    (await readPipelineArtifactLayout(projectRoot)) !== 'legacy'
  ) {
    throw new Error('Pipeline root move cannot roll back after the config switch');
  }
  ...
  if (
    ...
    throw new Error('Pipeline root move quarantine exists; rollback is not proven safe');
  }
  for (const [directory, label] of [[staging, 'staging']] as const) {
    ...
    if (status === 'mismatch') {
      throw new Error(`Pipeline root move ${label} changed after migration preflight`);
    }
    if (status === 'match') {
  ...
  if (
    ...
    throw new Error('Pipeline root move docs target changed after migration preflight');
  }
  ...
  if (journal.targetInitialState === 'empty' && targetState !== 'empty') {
    await ensureRootMoveDirectory(projectRoot, target, 'Pipeline docs rollback target', testHooks);
  }
  await removeProtectedEmptyDirectory(
    projectRoot,
    transactionRoot,
...
async function recoveryPolicy(
  projectRoot: string,
  journal: PipelineRootMoveJournal,
): Promise<{ allowedStrategies: PipelineRootMoveRecoveryStrategy[]; reason?: string }> {
  await assertRootMovePreflightBoundaries(projectRoot, journal.id);
  if (journal.stage === 'locked') {
    return {
  ...
  if (sourceState === 'mismatch') {
    return {
      allowedStrategies: [],
      reason: 'Pipeline legacy root changed after migration preflight',
    };
  }
  if (targetState === 'mismatch') {
    return {
      allowedStrategies: [],
      reason: 'Pipeline docs target changed after migration preflight',
    };
  }
  if (stagingState === 'mismatch') {
    return { allowedStrategies: [], reason: 'Pipeline staging changed after migration preflight' };
  }
  if (quarantineState === 'mismatch') {
    return {
      allowedStrategies: [],
      reason: 'Pipeline quarantine contains unknown or changed content',
    };
  }
...
export async function planPipelineRootMove(
  startPath: string,
  options: PipelineRootMoveOptions = {},
): Promise<PipelineRootMovePlan> {
  const projectRoot = await discoverPipelineProject(startPath);
  const journal = await readJournal(projectRoot, options.testHooks);
  if (journal) {
    const pendingRecovery = await inspectPipelineRootMove(projectRoot);
    if (!pendingRecovery)
      throw new Error('Pipeline root move journal disappeared during inspection');
    return {
      projectRoot,
      source: journal.source,
      target: journal.target,
      ...
      sourceIdentity:
        journal.schema === 'owner.pipeline-root-move.v2'
          ...
          : [],
      targetInitialIdentity:
        journal.schema === 'owner.pipeline-root-move.v2' && journal.targetInitialIdentity
      ...
      blockers: [`pending Pipeline root move: ${journal.id} at ${journal.stage}`],
      pendingRecovery,
      historicalPointersPreserved: [...HISTORICAL_POINTERS_PRESERVED],
      applyPreconditions: [...APPLY_PRECONDITIONS],
...
export async function applyPipelineRootMove(
  startPath: string,
  options: PipelineRootMoveOptions = {},
): Promise<PipelineRootMovePlan> {
  const projectRoot = await discoverPipelineProject(startPath);
  ...
  if (existing) {
    throw new Error(
      `Pipeline root move ${existing.id} is incomplete; use owner doctor --repair --strategy continue|rollback`,
    );
  }
  ...
  if (!HASH_PATTERN.test(approvedPlanId)) {
    throw new Error('Pipeline root move apply received an invalid internal plan ID');
  }
  const id = randomUUID();
  await assertRootMovePreflightBoundaries(projectRoot, id);
  const lockedJournal: PipelineRootMoveJournal = {
    schema: 'owner.pipeline-root-move.v2',
    id,
    stage: 'locked',
    source: initial.plan.source,
    target: initial.plan.target,
    staging: `.owner/transactions/pipeline-root-move/${id}/openspec`,
  ...
  if (!ownedLock || ownedLock.id !== id || ownedLock.stage !== 'locked') {
    throw new Error('Pipeline root move lock ownership changed before locked preflight');
  }
  ...
  try {
    ...
    if (plan.planId !== approvedPlanId) {
      throw new Error(
        `Pipeline root move plan changed since dry-run: approved ${approvedPlanId}, current ${plan.planId}`,
      );
    }
    if (!plan.readyToApply) {
      throw new Error(
        `Pipeline root move apply is blocked: ${[...plan.conflicts, ...plan.blockers].join('; ')}`,
      );
    }
    if (plan.targetInitialState === 'non-empty') {
      throw new Error('Pipeline root move apply is blocked: Pipeline docs target is not empty');
    }
    const journal: PipelineRootMoveJournal = {
      schema: 'owner.pipeline-root-move.v2',
      id,
      stage: 'copying',
      source: plan.source,
...
export async function repairPipelineRootMove(
  projectRoot: string,
  strategy?: PipelineRootMoveRecoveryStrategy,
  options: PipelineRootMoveOptions = {},
): Promise<boolean> {
  const journal = await readJournal(projectRoot, options.testHooks);
  ...
  if (!strategy) {
    throw new Error(
      `Pipeline root move ${journal.id} recovery strategy is required: continue or rollback`,
    );
  }
  const policy = await recoveryPolicy(projectRoot, journal);
  if (!policy.allowedStrategies.includes(strategy)) {
    throw new Error(
      `Pipeline root move ${journal.id} does not allow ${strategy}: ${
...
export async function inspectPipelineRootMove(
  projectRoot: string,
  options: PipelineRootMoveOptions = {},
): Promise<PipelineRootMoveInspection | null> {
  const journal = await readJournal(projectRoot, options.testHooks);

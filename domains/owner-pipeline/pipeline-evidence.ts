...
import { readLegacyArchivedHandoffFallback } from './pipeline-archive-pointer.js';
import {
...
const PIPELINE_ARTIFACT_MAX_BYTES = 2 * 1024 * 1024;
...
async function linkedFileEvidence(
  ...
  if (layoutError) {
    return {
      ...
      satisfied: false,
      source,
      detail: `Pipeline layout is unsafe or unavailable: ${
  ...
  try {
    if (layout) {
      ...
      const alternateRoot = pipelineProjectRelative(
        projectRoot,
        pipelineLayoutPaths(projectRoot, alternateLayout).openSpecRoot,
      );
      if (source === alternateRoot || source.startsWith(`${alternateRoot}/`)) {
        ...
        if (alternate.exists) {
          return {
            ...
            satisfied: false,
            source,
            detail: 'standalone OpenSpec root is not a Owner artifact root',
          };
        }
      }
    }
...
async function archivedHandoffEvidence(
  ...
  try {
    layout ??= await assertPipelineLayoutReadable(projectRoot);
  } catch (error) {
    return {
      code: 'design.handoff',
      satisfied: false,
      ...(relativePath ? { source: relativePath.replaceAll('\\', '/') } : {}),
      detail: `Pipeline layout is unsafe or unavailable for handoff evidence: ${
        error instanceof Error ? error.message : String(error)
      }`,
    };
  }
...
async function taskEvidence(projectRoot: string, tasksFile: string): Promise<PipelineEvidence> {
  let source: string;
  const relative = relativeSource(projectRoot, tasksFile);
  try {
    source = (
      await readProtectedProjectFile(projectRoot, relative, PIPELINE_ARTIFACT_MAX_BYTES, {
...
export function evidenceSatisfied(evidence: readonly PipelineEvidence[], code: string): boolean {
...
export async function collectPipelineEvidence(
  ...
  let layout: PipelineLayoutPaths | undefined;
  ...
  const evidence = await Promise.all([
    ...
    linkedFileEvidence(
      projectRoot,
      'design.document',
      pipeline?.designDoc ?? null,
      layout,
      layoutError,
    ),
    linkedFileEvidence(projectRoot, 'build.plan', pipeline?.plan ?? null, layout, layoutError),
    taskEvidence(projectRoot, tasks),
    linkedFileEvidence(
      projectRoot,
      'verification.report',
      pipeline?.verificationReport ?? null,
      layout,
      layoutError,
    ),
    archivedHandoffEvidence(projectRoot, changeDir, pipeline?.handoffContext ?? null, layout),
    directFileEvidence(projectRoot, 'run.checkpoint', checkpoint),
  ]);

  const handoff = evidence.find((item) => item.code === 'design.handoff');
  if (handoff && !pipeline?.handoffHash) {
  ...
  evidence.push({
    ...
    satisfied: pipeline?.archiveConfirmation === 'confirmed',
  });

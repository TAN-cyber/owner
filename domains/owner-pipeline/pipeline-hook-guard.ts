...
import { inspectPipelineProjectTarget } from './pipeline-protected-path.js';
import type { OwnerHookDecision, OwnerHookRequest } from '../owner-entry/hook-types.js';
...
import { resolveCurrentChange } from './pipeline-current-change.js';
import { readPipelineState, readLegacyState } from './pipeline-store.js';
...
function result(exitCode: number, message: string): PipelineCommandResult {
  return { exitCode, stderr: message + '\n' };
}

function allowed(message: string): PipelineCommandResult {
  return result(0, `[OWNER-HOOK] allowed: ${message}`);
}
...
interface GoverningBlock {
  blockedResult: PipelineCommandResult;
}
...
async function loadGoverningChange(changeDir: string): Promise<GoverningChange | null> {
  try {
    ...
    if (!projection.pipeline) throw new Error('Pipeline state projection is unavailable');
    return {
      changeDir,
      phase: projection.pipeline.phase,
      pipeline: projection.pipeline,
      archived: projection.pipeline.archived,
    };
  } catch (error) {
    // Legacy/partial state without the required Pipeline fields: fall back to a
...
async function activeChangesImpl(projectRoot: string): Promise<GoverningChange[]> {
  ...
  for (const entry of (await fs.readdir(changesDir, { withFileTypes: true })).sort((left, right) =>
    ...
    const active = await inspectPipelineActiveChangeDirectory(entry.name, projectRoot);
...
// `activeChanges` is invoked once by the router's `listActivePipelineHookChanges`
// and again by `inspectPipelineHookGuard`. Within a single Hook decision the
...
const activeChanges = memoizedHookRead('pipelineActiveChanges', (projectRoot: string) =>
  activeChangesImpl(projectRoot),
);
const hookPlanReadiness = memoizedHookRead('pipelinePlanReadiness', inspectPipelinePlanReadiness);

export interface ActivePipelineHookChange {
  workflow: 'pipeline';
  name: string;
  phase: PipelinePhase;
}

export interface PipelineHookGuardDependencies {
  scopeTargets?: typeof scopeOwnerHookTargets;
}

export async function listActivePipelineHookChanges(
  projectRoot: string,
): Promise<ActivePipelineHookChange[]> {
...
function superpowersArtifactPrefix(projectRoot: string, layout: PipelineLayoutPaths): string {
  return `${pipelineProjectRelative(projectRoot, layout.superpowersRoot)}/`;
}
...
function superpowersArtifactSlots(
  projectRoot: string,
  layout: PipelineLayoutPaths,
): readonly SuperpowersArtifactSlot[] {
  return [
    {
      prefix: `${pipelineProjectRelative(projectRoot, layout.superpowersSpecsDir)}/`,
      field: 'designDoc',
      wireField: 'design_doc',
      phase: 'design',
    },
    {
      prefix: `${pipelineProjectRelative(projectRoot, layout.superpowersPlansDir)}/`,
      field: 'plan',
      wireField: 'plan',
      phase: 'build',
    },
    {
      prefix: `${pipelineProjectRelative(projectRoot, layout.superpowersReportsDir)}/`,
...
function superpowersArtifactValue(
  ...
  return governing.pipeline?.[slot.field] ?? null;
}
...
async function allowsSuperpowersArtifactWrite(
  ...
  return (
    (await hookPlanReadiness(projectRoot, governing.pipeline?.plan ?? null)).status === 'broken'
  );
}
...
function matchesRecordedSuperpowersArtifact(
  ...
  const artifactPaths = [
    governing.pipeline?.designDoc,
    governing.pipeline?.plan,
    governing.pipeline?.verificationReport,
  ];
...
async function governingChange(
  ...
  const prefix = `${pipelineProjectRelative(projectRoot, layout.changesDir)}/`;
  if (relativePath.startsWith(prefix)) {
    const rest = relativePath.slice(prefix.length);
    const [name] = rest.split('/');
    if (name && name !== 'archive') {
      const active = await inspectPipelineActiveChangeDirectory(name, projectRoot);
      if (active.stateExists) {
        const governing = await loadGoverningChange(active.directory);
        if (governing) return governing;
        return { changeDir: active.directory, phase: 'open', pipeline: null, archived: false };
      }
      return { changeDir: active.directory, phase: 'open', pipeline: null, archived: false };
    }
  }
...
function isOwnerConfig(relativePath: string): boolean {
  return relativePath.startsWith('.owner/') || relativePath.includes('/.owner/');
}
...
function openSpecAllowed(
  ...
  const stateFile =
    relativePath.endsWith('/.owner.yaml') || relativePath.endsWith('/.openspec.yaml');
  const proposal =
    relativePath.endsWith('/proposal.md') ||
    relativePath.endsWith('/design.md') ||
    relativePath.endsWith('/tasks.md');
  const handoff = relativePath.includes('/.owner/');
...
function blocked(relativePath: string, phase: PipelinePhase): PipelineCommandResult {
  const guidance =
    phase === 'open'
      ? [
      ...
      : phase === 'design'
        ? [
            ...
            '  NEXT: finish the Design Doc, then run owner guard <change-name> design --apply to enter build',
          ]
        : [
  ...
  return result(
    ...
    [
      '',
      '╔══════════════════════════════════════════╗',
      '║     OWNER PHASE GUARD — WRITE BLOCKED    ║',
      '╚══════════════════════════════════════════╝',
      '',
      `  Current phase: ${phase}`,
...
function blockedMissingDesignDoc(relativePath: string): PipelineCommandResult {
  return result(
    2,
    [
      '',
...
function blockedPlanNotReady(
  ...
  const changeDirectory = governing.changeDir
    ? pipelineProjectRelative(projectRoot, governing.changeDir)
    : '<pipeline-change-dir>';
  const plansDirectory = pipelineProjectRelative(projectRoot, layout.superpowersPlansDir);
  ...
  return result(
    ...
    [
      ...
      '',
      'RECOVERY:',
      `1. Invoke /owner-build for ${name} and resume Step 1.`,
      ...repair,
  ...
  const details = slot
    ? governing.phase !== slot.phase
      ? [
          ...
          '  NEXT: resume the matching Owner phase or use an already recorded artifact path',
        ]
      : recorded
        ? [
...
function blockedMultipleChanges(
  relativePath: string,
  changeNames: string[],
): PipelineCommandResult {
  return result(
    ...
    [
      ...
      '',
      '  NEXT: run owner state select <change-name>, then retry the source write',
      '',
    ].join('\n'),
  );
}

function blockedStaleSelection(relativePath: string, reason: string): PipelineCommandResult {
  return result(
    2,
    [
      '',
...
async function inspectPipelineHookTarget(
  projectRoot: string,
  target: string,
  selectedChangeName?: string,
  scopeTargets: typeof scopeOwnerHookTargets = scopeOwnerHookTargets,
  ...
  try {
    ...
    return result(
      2,
      [
        `[OWNER-HOOK] blocked: scope could not be determined safely for ${target}.`,
  ...
  let layout: PipelineLayoutPaths;
  try {
    layout = await assertPipelineLayoutWritable(projectRoot);
  } catch (error) {
    return result(
      2,
      `[OWNER-HOOK] blocked: ${error instanceof Error ? error.message : String(error)}`,
    );
  }

  if (isOwnerConfig(relativePath)) {
    return allowed(`${relativePath} (whitelist: owner config)`);
  }
  ...
  if (!governing) return allowed('no active owner change');
  ...
  const openSpec = openSpecAllowed(
    relativePath,
    phase,
    `${pipelineProjectRelative(projectRoot, layout.openSpecRoot)}/`,
  );
  ...
  if (governing.invalidState) {
    return result(
      2,
      `[OWNER-HOOK] blocked: active Pipeline state is invalid; repair .owner.yaml before writing ${relativePath}`,
    );
  }
  if (
    phase === 'build' &&
    governing.pipeline?.workflow === 'full' &&
    !governing.pipeline.designDoc
  ) {
    return blockedMissingDesignDoc(relativePath);
  }
  if (phase === 'build' && governing.pipeline?.workflow === 'full') {
    const planReadiness = await hookPlanReadiness(projectRoot, governing.pipeline.plan);
...
export async function inspectPipelineHookGuard(
  projectRoot: string,
  changeName: string,
  request: OwnerHookRequest,
  dependencies: PipelineHookGuardDependencies = {},
): Promise<OwnerHookDecision> {
  if (request.intent !== 'non-write') {
    try {
      await assertPipelineLayoutWritable(projectRoot);
    } catch (error) {
      return {
        allowed: false,
        reason: error instanceof Error ? error.message : String(error),
  ...
  if (!selected) {
    return {
      allowed: false,
      reason: `Selected Pipeline change ${changeName} is missing or archived; resume /owner-pipeline before retrying`,
      workflow: 'pipeline',
      change: changeName,
    };
  }
  ...
  for (const target of request.targets) {
    const inspected = await inspectPipelineHookTarget(
      projectRoot,
      target,
      changeName,
    ...
    if (inspected.exitCode !== 0) {
      return {
        allowed: false,
        reason: inspected.stderr?.trim() || 'Pipeline phase guard blocked the write',
        workflow: 'pipeline',
        change: changeName,
        phase: selected.phase,
      };
  ...
  return {
    ...
    reason: configuredTarget
      ? 'Pipeline write allowed by configured Hook allow path'
      : `Pipeline write allowed in ${selected.phase}`,
    workflow: 'pipeline',
    change: changeName,
    phase: selected.phase,
  };
...
export const pipelineHookGuardCommand: PipelineCommandHandler = async (args) => {
  const projectRoot = parseProjectRoot(args);
  const target = inputTarget();
  if (!target) return allowed('no file path in tool input');
  return inspectPipelineHookTarget(projectRoot, target);
};

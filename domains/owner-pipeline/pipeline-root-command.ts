...
import {
  ...
  type PipelineRootMovePlan,
} from './pipeline-root-move.js';
import {
...
function usage(language: RootMoveLanguage = 'en') {
  return {
    ...
    stderr:
      language === 'zh-CN'
        ? '用法：owner pipeline root show | owner pipeline root move docs --dry-run | owner pipeline root move docs --apply'
        : 'Usage: owner pipeline root show | owner pipeline root move docs --dry-run | owner pipeline root move docs --apply',
  };
}
...
function rootMoveLanguage(
  ...
  return config?.pipeline?.language === 'en' ? 'en' : 'zh-CN';
}
function formatPipelineRootMoveError(error: unknown, language: RootMoveLanguage): string {
  const message = error instanceof Error ? error.message : String(error);
  if (language === 'en') return message;
  if (message === 'invalid Pipeline root move journal: manifest paths must be unique and sorted') {
    return 'Pipeline 根目录迁移失败：迁移记录中的清单路径必须唯一且按顺序排列';
  }
  if (message.startsWith('invalid Pipeline root move journal:')) {
    return 'Pipeline 根目录迁移失败：迁移记录格式或内容无效，请运行 owner doctor 检查恢复状态';
  }
  return 'Pipeline 根目录迁移失败：迁移过程中发生错误，请运行 owner doctor 检查迁移状态';
}

export function formatPipelineRootMoveReport(
  ...
  const localizedReason = (value: string) => {
    if (!zh) return value;
    const pending = /^pending Pipeline root move: (.+) at (.+)$/u.exec(value);
    if (pending) return `存在待恢复的 Pipeline 根目录迁移：${pending[1]}，阶段 ${pending[2]}`;
    return (
      {
        'Pipeline docs target is not empty': 'docs 目标目录非空',
        ...
        'Pipeline legacy root changed after migration preflight':
          '迁移预检后 legacy 根目录发生变化',
        'Pipeline docs target changed after migration preflight':
          '迁移预检后 docs 目标目录发生变化',
        'Pipeline staging changed after migration preflight': '迁移预检后暂存目录发生变化',
        'Pipeline quarantine contains unknown or changed content': '隔离目录包含未知或已变化的内容',
      ...
      }[value] ?? '迁移状态存在无法识别的冲突，请运行 owner doctor 检查恢复状态'
    );
  };
  ...
  const lines = [
    mode === 'dry-run'
      ? zh
        ? 'Pipeline 根目录迁移现状'
        : 'Pipeline root move status'
      : zh
        ? 'Pipeline 根目录迁移完成'
        : 'Pipeline root move complete',
    ...(mode === 'dry-run'
      ? [
          zh
            ? '说明：仅查看现状，未修改任何文件。'
  ...
  if (mode === 'dry-run') {
    lines.push(
      plan.readyToApply
        ? zh
          ? '下一步：运行 owner pipeline root move docs --apply 执行迁移。'
          : 'Next: run owner pipeline root move docs --apply to apply the migration.'
        : zh
          ? '下一步：请先解决上述冲突或阻塞项。'
...
function formatAlreadyMigratedReport(
  ...
  const root = pipelineProjectRelative(layout.projectRoot, layout.openSpecRoot);
  if (language === 'zh-CN') {
    return (
      [
        mode === 'dry-run' ? 'Pipeline 根目录迁移现状' : 'Pipeline 根目录迁移完成',
  ...
  return (
    [
      mode === 'dry-run' ? 'Pipeline root move status' : 'Pipeline root move complete',
...
export const pipelineRootCommand: PipelineCommandHandler = async (args) => {
  ...
  if (action === 'show' && target === undefined) {
    ...
    const inspection = await inspectPipelineLayout(projectRoot, layout.artifactLayout);
    return {
      exitCode: 0,
      stdout:
        JSON.stringify({
          ...
          openSpecRoot: pipelineProjectRelative(projectRoot, layout.openSpecRoot),
          alternateRoot: pipelineProjectRelative(projectRoot, inspection.alternateRoot),
          ...
          changesRoot: pipelineProjectRelative(projectRoot, layout.changesDir),
          archiveRoot: pipelineProjectRelative(projectRoot, layout.archiveDir),
          specsRoot: pipelineProjectRelative(projectRoot, layout.specsDir),
          superpowersRoot: pipelineProjectRelative(projectRoot, layout.superpowersRoot),
        }) + '\n',
    };
  }
  ...
  try {
    if (mode === '--dry-run') {
      ...
      return {
        exitCode: 0,
        stdout: formatPipelineRootMoveReport(plan, 'dry-run', language),
      };
    }

    const plan = await applyPipelineRootMove(projectRoot);
    return {
      exitCode: 0,
      stdout: formatPipelineRootMoveReport(plan, 'complete', language),
    };
  } catch (error) {
    return {
      exitCode: 70,
      stderr: formatPipelineRootMoveError(error, language),
    };
  }
};

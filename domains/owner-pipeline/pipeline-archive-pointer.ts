...
import { assertPipelineLayoutReadable, pipelineProjectRelative } from './pipeline-layout.js';
import { assertOpenSpecChangeName } from './pipeline-paths.js';
 ...
 * the evidence source; only its `.owner/` suffix is resolved against the
...
export async function readLegacyArchivedHandoffFallback(
  ...
  const source = normalizeWorkflowRelativePath(pointer, 'Pipeline handoff artifact pointer');
  const original = await inspectProtectedProjectPath(projectRoot, source, {
    label: 'Pipeline handoff artifact',
    expected: 'file',
  });
  if (original.exists) return null;

  const layout = await assertPipelineLayoutReadable(projectRoot);
  const configuredChangesPrefix = `${pipelineProjectRelative(projectRoot, layout.changesDir)}/`;
  ...
  await inspectProtectedProjectPath(
    projectRoot,
    pipelineProjectRelative(projectRoot, archivedChange),
    {
      label: 'Pipeline archived change directory',
      expected: 'directory',
    },
  );

  const mapped = path.join(archivedChange, '.owner', ...suffix);
  const mappedRelative = pipelineProjectRelative(projectRoot, mapped);
  try {
    await readProtectedProjectFile(projectRoot, mappedRelative, maxBytes, {
      label: 'Historical Pipeline archived handoff artifact',
    });

 ...
 * isolation/bound_branch straight from `.owner.yaml` (typed — a yaml `null`
...
export async function resolveBranchBinding(
  ...
  if (document.errors.length > 0) {
    throw new Error(`Invalid .owner.yaml: ${document.errors[0].message}`);
  }
...
export function driftBlockedMessage(
  ...
  return (
    ...
    `Next: ask the user to confirm — switch back to '${boundBranch}', or run \`owner state rebind ${change}\` after explicit confirmation.`
  );
}

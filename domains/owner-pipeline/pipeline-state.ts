...
export const PIPELINE_PROFILES = ['full', 'hotfix', 'tweak'] as const;
export const PIPELINE_MIGRATION_VERSION = 1;
...
export type PipelineArtifactLanguage = (typeof ARTIFACT_LANGUAGES)[number];

export interface PipelineState {
  workflow: PipelineProfile;
  language: PipelineArtifactLanguage | null;
  phase: PipelinePhase;
  ...
  pipelineMigration: number | null;
}

export interface PipelineStateProjection {
  pipeline: PipelineState | null;
  run: RunState | null;
  unknownKeys: string[];
}

export const PIPELINE_WIRE_KEYS = [
  'workflow',
  'language',
  'phase',
  'context_compression',
...
/** Fields that appear in .owner.yaml to link to the Run state. */
export const RUN_WIRE_KEYS = ['run_id'] as const;

const KNOWN_KEYS = new Set<string>([...PIPELINE_WIRE_KEYS, ...RUN_WIRE_KEYS]);
...
// guard in pipeline-state-command.ts enforces review_mode selection for full workflow
...
function nullableString(doc: StateDocument, key: string): string | null {
  ...
  if (typeof value !== 'string') {
    throw new Error(`Invalid Pipeline state: ${key} must be a string or null`);
  }
  return value;
}
...
function enumValue<const T extends readonly string[]>(
  ...
  if (value === null || value === undefined || value === '') {
    ...
    throw new Error(`Invalid Pipeline state: ${key} is required`);
  }
  if (typeof value !== 'string' || !values.includes(value)) {
    throw new Error(
      `Invalid Pipeline state: ${key} must be one of ${values.join(', ')}${nullable ? ' or null' : ''}`,
    );
  }
...
function booleanValue(doc: StateDocument, key: string, nullable = true): boolean | null {
  ...
  if (typeof value !== 'boolean') {
    throw new Error(`Invalid Pipeline state: ${key} must be true or false`);
  }
  return value;
}
...
function nonNegativeInteger(doc: StateDocument, key: string, fallback = 0): number {
  ...
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) {
    throw new Error(`Invalid Pipeline state: ${key} must be a non-negative integer`);
  }
  return value;
}
...
function relativePath(doc: StateDocument, key: string): string | null {
  ...
  if (/^(?:[A-Za-z]:|[\\/]|~)/u.test(value) || value.split(/[\\/]/u).includes('..')) {
    throw new Error(`Invalid Pipeline state: ${key} must be a relative repository path`);
  }
  return value;
}
...
function sha256(doc: StateDocument, key: string): string | null {
  ...
  if (value !== null && !/^[a-f0-9]{64}$/u.test(value)) {
    throw new Error(`Invalid Pipeline state: ${key} must be a sha256 hex digest`);
  }
  return value;
}

function migrationVersion(doc: StateDocument): number | null {
  const value = doc.pipeline_migration;
  if (value === null || value === undefined || value === '') return null;
  if (value !== PIPELINE_MIGRATION_VERSION) {
    throw new Error(
      `Invalid Pipeline state: pipeline_migration must be ${PIPELINE_MIGRATION_VERSION}`,
    );
  }
  return value;
}

function pipelineStateFromDocument(doc: StateDocument): PipelineState | null {
  const hasPipelineProjection = PIPELINE_WIRE_KEYS.some((key) => has(doc, key));
  if (!hasPipelineProjection) return null;
  ...
  return {
    workflow: enumValue(doc, 'workflow', PIPELINE_PROFILES, false)!,
    ...
    pipelineProfile: enumValue(doc, 'pipeline_profile', PIPELINE_PROFILES),
    pipelineMigration: migrationVersion(doc),
  };
}

export function parsePipelineStateDocument(
  doc: StateDocument,
  run?: RunState | null,
): PipelineStateProjection {
  // Run state is stored separately in .owner/run-state.json.
  ...
  if (resolvedRun === null && run === undefined) {
    ...
    if (doc.run_id && doc.skill) {
      try {
        ...
        throw new Error(message.replace(/^Invalid Run state:/u, 'Invalid Pipeline state:'), {
          cause: error,
        });
      }
    }
  ...
  return {
    pipeline: pipelineStateFromDocument(doc),
    run: resolvedRun,
...
export function readLegacyStateSummary(doc: StateDocument): LegacyStateSummary {
  ...
  return {
    workflow:
      typeof workflowRaw === 'string' && PIPELINE_PROFILES.includes(workflowRaw as PipelineProfile)
        ? (workflowRaw as PipelineProfile)
        : null,
    phase:
      typeof phaseRaw === 'string' && PHASES.includes(phaseRaw as PipelinePhase)
...
export function pipelineStateToDocument(state: PipelineState): StateDocument {
  return {
    ...
    pipeline_profile: state.pipelineProfile,
    pipeline_migration: state.pipelineMigration,
  };
}

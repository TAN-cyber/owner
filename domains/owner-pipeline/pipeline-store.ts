...
const PIPELINE_STATE_MAX_BYTES = 2 * 1024 * 1024;

function documentRecord(document: Document): StateDocument {
  const value = document.toJS() as unknown;
  if (value === null) return {};
  if (typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('Invalid Pipeline state document: root must be a mapping');
  }
...
function applyProjection(document: Document, projection: PipelineStateProjection): void {
  if (projection.pipeline) {
    for (const [key, value] of Object.entries(pipelineStateToDocument(projection.pipeline))) {
      setIfChanged(document, key, value);
    }
  } else {
    for (const key of PIPELINE_WIRE_KEYS) document.delete(key);
  }

  // Only write run_id as a link — full Run state lives in .owner/run-state.json
...
async function readDocument(file: string): Promise<Document> {
  ...
  try {
    source = (
      await readProtectedProjectFile(
        path.dirname(file),
        path.basename(file),
        PIPELINE_STATE_MAX_BYTES,
        {
          label: 'Pipeline state document',
        },
      )
    ).bytes.toString('utf8');
  } catch (error) {
  ...
  if (document.errors.length > 0) {
    throw new Error(`Invalid Pipeline state document: ${document.errors[0].message}`);
  }
  documentRecord(document);
  return document;
}

export interface ReadPipelineStateOptions {
  migrate?: boolean;
}

export async function readPipelineState(
  changeDir: string,
  options: ReadPipelineStateOptions = {},
): Promise<PipelineStateProjection> {
  ...
  if (!run && doc.run_id && doc.skill) {
    // Legacy format: Run fields embedded in .owner.yaml — migrate
  ...
  return parsePipelineStateDocument(documentRecord(document), run);
}

export async function readLegacyState(changeDir: string): Promise<LegacyStateSummary> {
  const document = await readDocument(path.join(changeDir, '.owner.yaml'));
  return readLegacyStateSummary(documentRecord(document));
}

export async function writePipelineState(
  changeDir: string,
  projection: Omit<PipelineStateProjection, 'unknownKeys'> & { unknownKeys?: string[] },
  ...
  parsePipelineStateDocument(documentRecord(document), projection.run ?? null);

...
import { hasExplicitPipelineArtifactLayout } from '../workflow-contract/project-config.js';
import {
...
import {
  ...
  type PipelineArtifactLayout,
  type PipelineLayoutPaths,
} from './pipeline-layout.js';

const ROOT_MOVE_JOURNAL = '.owner/pipeline-root-move.json';
const INIT_OWNERSHIP_JOURNAL = '.owner/pipeline-init-ownership.json';
...
const initializationPermitBrand = Symbol('PipelineLayoutInitializationPermit');
const ownershipClaimBrand = Symbol('PipelineInitOwnershipClaim');

interface PipelineInitObjectIdentity {
  dev: string;
  ino: string;
  birthtime: string;
  ctime: string;
...
interface PipelineInitManifestFile {
  path: string;
  size: number;
  hash: string;
  identity: PipelineInitObjectIdentity;
}

interface PipelineInitManifest {
  directories: string[];
  files: PipelineInitManifestFile[];
  totalBytes: number;
  hash: string;
}

interface PipelineInitOwnershipJournal {
  schema: 'owner.pipeline-init-ownership.v1';
  id: string;
  ...
  rootIdentity: PipelineInitObjectIdentity | null;
  manifest: PipelineInitManifest | null;
  readonly [ownershipClaimBrand]?: PipelineInitOwnershipClaim;
}

interface PipelineInitOwnershipClaim {
  fileIdentity: BigIntStats;
  contentHash: string;
}

interface PipelineInitTestHooks {
  afterJournalQuarantine?: (operation: string) => void | Promise<void>;
  afterRootQuarantine?: (quarantine: string) => void | Promise<void>;
}

interface PipelineInitOperationOptions {
  testHooks?: PipelineInitTestHooks;
}

export interface PipelineLayoutInitializationPermit {
  readonly projectRoot: string;
  readonly artifactLayout: PipelineArtifactLayout;
  readonly configIdentity: WorkflowProjectConfigIdentity;
  readonly ownershipId?: string;
  readonly [initializationPermitBrand]: true;
}

export interface PipelineLayoutInitialization extends PipelineLayoutPaths {
  readonly initializationPermit: PipelineLayoutInitializationPermit;
}

function permitsDesiredRoot(
  permit: PipelineLayoutInitializationPermit | undefined,
  projectRoot: string,
  desiredLayout: PipelineArtifactLayout,
): permit is PipelineLayoutInitializationPermit {
  return (
...
function manifestHash(manifest: Omit<PipelineInitManifest, 'hash'>): string {
...
function objectIdentity(stat: import('fs').BigIntStats): PipelineInitObjectIdentity {
  if (stat.dev === 0n || stat.ino === 0n) {
    throw new Error(
      'Pipeline init ownership requires a filesystem with stable device and file identities',
    );
  }
  return {
    dev: String(stat.dev),
...
function sameObjectIdentity(
  left: PipelineInitObjectIdentity,
  right: PipelineInitObjectIdentity,
): boolean {
...
function safeManifestPath(value: unknown, label: string): string {
  if (
    ...
    throw new Error(`Invalid Pipeline init ownership journal: ${label} is unsafe`);
  }
  return value;
}

function parseManifest(value: unknown): PipelineInitManifest {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('Invalid Pipeline init ownership journal: manifest is invalid');
  }
  ...
  if (
    ...
    throw new Error('Invalid Pipeline init ownership journal: manifest shape is invalid');
  }
  ...
  const files = raw.files.map((entry, index) => {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
      throw new Error(`Invalid Pipeline init ownership journal: files[${index}] is invalid`);
    }
  ...
  if (files.length > INIT_OWNERSHIP_MAX_FILES) {
    throw new Error('Invalid Pipeline init ownership journal: manifest contains too many files');
  }
  if (
    ...
    throw new Error('Invalid Pipeline init ownership journal: manifest paths are not sorted');
  }
  const totalBytes = files.reduce((total, file) => total + file.size, 0);
  const normalized = { directories, files, totalBytes };
  if (totalBytes !== raw.totalBytes || manifestHash(normalized) !== raw.hash) {
    throw new Error('Invalid Pipeline init ownership journal: manifest hash is invalid');
  }
  return { ...normalized, hash: raw.hash };
}

function parseObjectIdentity(value: unknown): PipelineInitObjectIdentity {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('Invalid Pipeline init ownership journal: rootIdentity is invalid');
  }
  const raw = value as Record<string, unknown>;
  if (
...
function parseOwnershipJournal(value: unknown): PipelineInitOwnershipJournal {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('Invalid Pipeline init ownership journal: root is invalid');
  }
  const raw = value as Record<string, unknown>;
  if (
    raw.schema !== 'owner.pipeline-init-ownership.v1' ||
    ...
    throw new Error('Invalid Pipeline init ownership journal: identity is invalid');
  }
  const expectedRoot = raw.artifactLayout === 'docs' ? 'docs/openspec' : 'openspec';
  if (raw.root !== expectedRoot) {
    throw new Error('Invalid Pipeline init ownership journal: root does not match layout');
  }
  if (
    !(
      ...
      ((raw.stage === 'quarantining' || raw.stage === 'quarantined') &&
        ...
        raw.quarantine.startsWith('.owner/transactions/pipeline-init/'))
    )
  ) {
    throw new Error('Invalid Pipeline init ownership journal: quarantine does not match stage');
  }
  ...
  if (
    ...
    throw new Error('Invalid Pipeline init ownership journal: config identity is invalid');
  }
  ...
  if ((raw.rootIdentity === null) !== (raw.manifest === null)) {
    throw new Error(
      'Invalid Pipeline init ownership journal: root identity and manifest must be checkpointed together',
    );
  }
  return {
    schema: 'owner.pipeline-init-ownership.v1',
    id: raw.id,
    stage: raw.stage as PipelineInitOwnershipJournal['stage'],
...
async function readOwnershipJournal(
  projectRoot: string,
): Promise<PipelineInitOwnershipJournal | null> {
...
async function readOwnershipJournalAtPath(
  ...
  try {
    ...
    Object.defineProperty(journal, ownershipClaimBrand, {
      ...
      } satisfies PipelineInitOwnershipClaim,
      enumerable: false,
    });
    return journal;
  } catch (error) {
...
async function scanOwnedRoot(
  projectRoot: string,
  rootRelative: string,
): Promise<{ identity: PipelineInitObjectIdentity; manifest: PipelineInitManifest }> {
  ...
  await inspectProtectedProjectPath(projectRoot, rootRelative, {
    label: 'Pipeline init owned root',
    expected: 'directory',
  });
  const initial = await fs.lstat(root, { bigint: true });
  if (!initial.isDirectory() || initial.isSymbolicLink()) {
    throw new Error('Pipeline init owned root must be a real directory');
  }
  const identity = objectIdentity(initial);
  const directories: string[] = [];
  const files: PipelineInitManifestFile[] = [];
  let totalBytes = 0;
  ...
  async function visit(directory: string, prefix: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (stat.isSymbolicLink()) {
        throw new Error(`Pipeline init ownership does not support links: ${relative}`);
      }
      if (stat.isDirectory()) {
      ...
      if (!stat.isFile()) {
        throw new Error(`Pipeline init ownership supports only regular files: ${relative}`);
      }
      if (files.length + 1 > INIT_OWNERSHIP_MAX_FILES) {
        throw new Error('Pipeline init ownership manifest contains too many files');
      }
      const bytes = await fs.readFile(absolute);
      totalBytes += bytes.byteLength;
      if (totalBytes > INIT_OWNERSHIP_MAX_TOTAL_BYTES) {
        throw new Error('Pipeline init ownership manifest exceeds its byte budget');
      }
      ...
      if (
        ...
        throw new Error(`Pipeline init owned file changed while reading: ${relative}`);
      }
      files.push({
        path: relative,
  ...
  if (!current.isDirectory() || !sameCheckpointIdentity(identity, objectIdentity(current))) {
    throw new Error('Pipeline init owned root changed while scanning');
  }
  const normalized = { directories, files, totalBytes };
  return { identity, manifest: { ...normalized, hash: manifestHash(normalized) } };
}

function sameManifest(left: PipelineInitManifest, right: PipelineInitManifest): boolean {
...
function ownershipClaim(journal: PipelineInitOwnershipJournal): PipelineInitOwnershipClaim {
  const claim = journal[ownershipClaimBrand];
  if (!claim) throw new Error('Pipeline init ownership journal has no bound file identity');
  return claim;
}

function sameOwnership(
  expected: PipelineInitOwnershipJournal,
  actual: PipelineInitOwnershipJournal,
): boolean {
...
async function validateOwnedJournal(
  ...
  if (!current || !sameOwnership(expected, current)) {
    throw new Error('Pipeline init ownership changed');
  }
}

async function createOwnershipJournal(
  projectRoot: string,
  journal: PipelineInitOwnershipJournal,
): Promise<PipelineInitOwnershipJournal> {
  await ensureProtectedProjectDirectory(projectRoot, '.owner', {
    label: 'Pipeline init ownership journal parent',
  });
  ...
  if (!persisted || persisted.id !== journal.id || persisted.stage !== journal.stage) {
    throw new Error('Pipeline init ownership changed while publishing');
  }
  return persisted;
}
...
async function quarantineOwnershipJournal(
  ...
  expected: PipelineInitOwnershipJournal,
  operation: string,
  options: PipelineInitOperationOptions = {},
): Promise<QuarantinedOwnershipJournal> {
  await validateOwnedJournal(projectRoot, expected);
  const relativePath = `.owner/pipeline-init-ownership.${expected.id}.${randomUUID()}.quarantine`;
  ...
  const moved = await readOwnershipJournalAtPath(projectRoot, relativePath).catch(async (error) => {
    ...
    throw new Error('Pipeline init ownership changed while entering journal quarantine', {
      cause: error,
    });
  });
...
async function validateOwnedJournalAtPath(
  ...
  if (!current || !sameOwnership(quarantine.journal, current)) {
    throw new Error('Pipeline init quarantined journal ownership changed');
  }
}
...
async function updateOwnershipJournal(
  projectRoot: string,
  expected: PipelineInitOwnershipJournal,
  updated: PipelineInitOwnershipJournal,
  operation: string,
  options: PipelineInitOperationOptions = {},
): Promise<PipelineInitOwnershipJournal> {
  const temporaryRelative = `.owner/pipeline-init-ownership.${randomUUID()}.tmp`;
  ...
  try {
    ...
    try {
      ...
      throw new Error('Pipeline init ownership publish failed; a successor journal was preserved', {
        cause: error,
      });
    }
    const persisted = await readOwnershipJournal(projectRoot);
    ...
    if (
      ...
      throw new Error('Pipeline init ownership changed while updating');
    }
    await validateOwnedJournalAtPath(projectRoot, quarantine);
...
function ownedRootRelative(artifactLayout: PipelineArtifactLayout): 'openspec' | 'docs/openspec' {
  return artifactLayout === 'docs' ? 'docs/openspec' : 'openspec';
}

function assertOwnershipPermit(
  projectRoot: string,
  permit: PipelineLayoutInitializationPermit,
  journal: PipelineInitOwnershipJournal,
): void {
  if (
    permit.projectRoot !== projectRoot ||
    ...
    throw new Error('Pipeline init ownership permit does not match the active initialization');
  }
}
...
async function assertCheckpointMatches(
  ...
  if (!journal.rootIdentity || !journal.manifest) {
    throw new Error('Pipeline init ownership has no completed root checkpoint');
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error('Pipeline init root changed after the ownership checkpoint', {
        cause: error,
      });
    }
    throw error;
...
async function quarantineCheckpointedRoot(
  ...
  const quarantineRelative = `.owner/transactions/pipeline-init/${journal.id}/openspec`;
  const quarantine = path.join(projectRoot, ...quarantineRelative.split('/'));
  await ensureProtectedProjectDirectory(
    projectRoot,
    `.owner/transactions/pipeline-init/${journal.id}`,
    { label: 'Pipeline init rollback quarantine parent' },
  );
  const quarantineInspection = await inspectProtectedProjectPath(projectRoot, quarantineRelative, {
    label: 'Pipeline init rollback quarantine',
    expected: 'directory',
  });
  if (quarantineInspection.exists) {
    throw new Error(`Pipeline init rollback quarantine already exists: ${quarantineRelative}`);
  }
  ...
  if (
    ...
    throw new Error(
      `Pipeline init root changed while entering quarantine; content was preserved at ${quarantineRelative}`,
    );
  }
 ...
 * previously missing Pipeline root. Existing roots are never claimed.
 */
export async function beginPipelineLayoutInitialization(
  projectRoot: string,
  initialization: PipelineLayoutInitialization,
  options: PipelineInitOperationOptions = {},
): Promise<PipelineLayoutInitialization> {
  ...
  if (
    ...
    throw new Error('Pipeline layout initialization does not match the project root');
  }
  ...
  if (!workflowProjectConfigIdentityEquals(currentConfigIdentity, permit.configIdentity)) {
    throw new Error('Project config changed before Pipeline layout initialization began');
  }

  const existingJournal = await readOwnershipJournal(root);
  if (existingJournal) {
    if (!permit.ownershipId) {
      throw new Error('Another Pipeline layout initialization is already in progress');
    }
    assertOwnershipPermit(root, permit, existingJournal);
    if (existingJournal.stage !== 'initializing') {
      throw new Error(
        `Pipeline init ownership is ${existingJournal.stage}; recover it with owner doctor before continuing`,
      );
    }
    return initialization;
  }
  if (permit.ownershipId) {
    throw new Error('Pipeline init ownership journal is missing');
  }
  ...
  const rootInspection = await inspectProtectedProjectPath(root, rootRelative, {
    label: 'Pipeline init root before ownership',
    expected: 'directory',
  });
  if (rootInspection.exists) return initialization;

  const journal: PipelineInitOwnershipJournal = {
    schema: 'owner.pipeline-init-ownership.v1',
    id: randomUUID(),
    stage: 'initializing',
  ...
  const afterOwnership = await inspectProtectedProjectPath(root, rootRelative, {
    label: 'Pipeline init root after ownership',
    expected: 'directory',
  });
  if (afterOwnership.exists) {
    await removeOwnershipJournal(root, ownedJournal, options).catch(() => false);
    throw new Error('Pipeline init root appeared while ownership was being established');
  }
  return {
    ...initialization,
 ...
 */
export async function checkpointPipelineLayoutInitialization(
  projectRoot: string,
  permit: PipelineLayoutInitializationPermit,
  options: PipelineInitOperationOptions = {},
): Promise<boolean> {
  ...
  if (!journal) throw new Error('Pipeline init ownership journal is missing');
  assertOwnershipPermit(root, permit, journal);
  if (journal.stage !== 'initializing') {
    throw new Error(`Pipeline init ownership cannot checkpoint from ${journal.stage}`);
  }
  const currentConfigIdentity = await readWorkflowProjectConfigIdentity(root);
  if (!workflowProjectConfigIdentityEquals(currentConfigIdentity, journal.configIdentity)) {
    throw new Error('Project config changed during Pipeline layout initialization');
  }
  ...
  if (
    ...
    throw new Error('Pipeline init ownership changed while checkpointing');
  }
  return true;
}

export async function completePipelineLayoutInitialization(
  projectRoot: string,
  permit: PipelineLayoutInitializationPermit,
  options: PipelineInitOperationOptions = {},
): Promise<boolean> {
  ...
  assertOwnershipPermit(root, permit, journal);
  if (journal.stage !== 'initializing') {
    throw new Error(`Pipeline init ownership cannot complete from ${journal.stage}`);
  }
  await assertCheckpointMatches(root, journal);
  return removeOwnershipJournal(root, journal, options);
}

export async function rollbackPipelineLayoutInitialization(
  projectRoot: string,
  permit: PipelineLayoutInitializationPermit,
  options: PipelineInitOperationOptions = {},
): Promise<boolean> {
  ...
  assertOwnershipPermit(root, permit, journal);
  if (journal.stage !== 'initializing') {
    throw new Error(`Pipeline init ownership cannot rollback from ${journal.stage}`);
  }
  const currentConfigIdentity = await readWorkflowProjectConfigIdentity(root);
  if (!workflowProjectConfigIdentityEquals(currentConfigIdentity, journal.configIdentity)) {
    throw new Error('Project config changed during Pipeline layout initialization');
...
export interface PipelineLayoutInitializationInspection {
  id: string;
  stage: PipelineInitOwnershipJournal['stage'];
  artifactLayout: PipelineArtifactLayout;
  root: string;
  quarantine: string | null;
...
export async function inspectPipelineLayoutInitialization(
  projectRoot: string,
): Promise<PipelineLayoutInitializationInspection | null> {
  const root = path.resolve(projectRoot);
  const journal = await readOwnershipJournal(root);
...
export async function repairPipelineLayoutInitialization(
  projectRoot: string,
  ...
  if (strategy === 'continue') {
    if (journal.stage !== 'initializing') {
      throw new Error(
        `Pipeline init ${journal.id} is ${journal.stage}; its preserved quarantine cannot be restored automatically`,
      );
    }
    ...
    const configuredLayout = snapshot.document?.pipeline?.artifact_layout ?? journal.artifactLayout;
    if (
      !workflows.includes('pipeline') ||
      configuredLayout !== journal.artifactLayout ||
      workflowProjectConfigIdentityEquals(snapshot.identity, journal.configIdentity)
    ) {
      throw new Error(
        `Pipeline init ${journal.id} is still resumable; rerun owner init to continue it`,
      );
    }
    await assertCheckpointMatches(root, journal);
  ...
  if (!journal.quarantine) {
    throw new Error('Pipeline init quarantining journal has no quarantine path');
  }
  ...
  if (!quarantineInspection.exists) {
    const sourceInspection = await inspectProtectedProjectPath(root, journal.root, {
      label: 'Pipeline init rollback source',
      expected: 'directory',
    });
    if (!sourceInspection.exists) {
      throw new Error('Pipeline init rollback source and quarantine are both missing');
    }
    await fs.rename(
  ...
  if (
    ...
    throw new Error('Pipeline init rollback quarantine changed; all content was preserved');
  }
...
/**
 * Validate OpenSpec initialization against the explicitly selected Pipeline
 ...
 */
export async function assertPipelineLayoutInitializationSafe(
  projectRoot: string,
  desiredLayout: PipelineArtifactLayout,
  permit?: PipelineLayoutInitializationPermit,
  ...
  if (!workflowProjectConfigIdentityEquals(configIdentity, configSnapshot.identity)) {
    throw new Error('Project config changed while inspecting Pipeline layout initialization');
  }
  if (
  ...
  const legacyPipelineConfigured =
    config !== null &&
  ...
  const pipelineEnabled =
    ...
    (!config?.config && (config?.pipeline !== undefined || legacyPipelineConfigured));
  ...
  const legacy = pipelineLayoutPaths(root, 'legacy');
  const docs = pipelineLayoutPaths(root, 'docs');
  const [legacyRoot, docsRoot] = await Promise.all([
    inspectProtectedProjectPath(root, pipelineProjectRelative(root, legacy.openSpecRoot), {
      label: 'Pipeline managed physical path openspec',
      expected: 'directory',
    }),
    inspectProtectedProjectPath(root, pipelineProjectRelative(root, docs.openSpecRoot), {
      label: 'Pipeline managed physical path docs/openspec',
      expected: 'directory',
    }),
  ]);
  ...
  if (ownership) {
    if (ownership.artifactLayout !== desiredLayout) {
      throw new Error(
        `Pipeline init ownership is bound to ${ownership.artifactLayout}, not ${desiredLayout}`,
      );
    }
    ...
    if (ownership.stage !== 'initializing') {
      throw new Error(
        `Pipeline init ownership is ${ownership.stage}; recover it with owner doctor before continuing`,
      );
    }
    ...
    const configuredLayout = hasExplicitPipelineArtifactLayout(config?.value.pipeline)
      ? (config?.pipeline?.artifact_layout ?? desiredLayout)
      : desiredLayout;
    const committedConfigurationMatches =
      Boolean(config && pipelineEnabled && configuredLayout === desiredLayout) &&
  ...
  if (config && pipelineEnabled) {
    ...
    if (configuredLayout !== desiredLayout) {
      throw new Error(
        `Configured Pipeline layout is ${configuredLayout}, but OpenSpec initialization requested ${desiredLayout}`,
      );
    }
    ...
    if (!config.config) {
      ...
      throw new Error(
        `Configured Pipeline OpenSpec root is missing for ${desiredLayout} layout while the alternate root exists`,
      );
    }
    const configured = await assertPipelineLayoutWritable(root, desiredLayout);
    return {
      ...configured,
      initializationPermit: permitsDesiredRoot(permit, root, desiredLayout)
        ? permit
  ...
  if (legacyRoot.exists || docsRoot.exists) {
    ...
    throw new Error(
      'Cannot initialize Pipeline layout without .owner/config.yaml when openspec/ or docs/openspec/ already exists',
    );
  }

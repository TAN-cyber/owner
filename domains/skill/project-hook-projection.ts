...
import { reconcileProjectOwnerHooksForPlatform } from './hook-lifecycle.js';
import { getAssetsDir } from './platform-install.js';

const HOOK_ROUTER_RUNTIME = ['owner', 'scripts', 'owner-hook-router.mjs'] as const;
...
export async function projectOwnerHooksFromInstalledScope(
  ...
  for (const { platform, canonicalSkillsDir } of owners) {
    ...
    try {
      ...
      const result = await reconcileProjectOwnerHooksForPlatform(
        targetProjectRoot,
        platform,
        workflowSelection,
        options,

...
export const OWNER_AMBIENT_RESUME_TAG = 'owner-ambient-resume';
...
export function renderOwnerAmbientResumeContent(languageId: SkillLanguageId): string {
  if (languageId === 'zh') {
    return [
      '<!-- Managed by Owner. Edits inside this block may be replaced by owner init/update. -->',
...
export async function installOwnerProjectInstructions(
  projectPath: string,
  languageId: SkillLanguageId,
): Promise<ProjectInstructionResult> {
  const content = renderOwnerAmbientResumeContent(languageId);
  const files = [];
  ...
  for (const file of PROJECT_INSTRUCTION_FILES) {
    ...
    const result = await mergeManagedMarkdownBlock(instruction.target, {
      tagName: OWNER_AMBIENT_RESUME_TAG,
      content,
    });
...
export async function syncOwnerProjectInstructions(
  projectPath: string,
  languageId: SkillLanguageId,
  ambientResumeEnabled: boolean,
): Promise<{ changed: number }> {
  if (!ambientResumeEnabled) {
    const result = await removeOwnerProjectInstructions(projectPath);
    return { changed: result.removed };
  }

  const result = await installOwnerProjectInstructions(projectPath, languageId);
  return { changed: result.changed };
}

export async function removeOwnerProjectInstructions(
  projectPath: string,
  ...
  for (const file of PROJECT_INSTRUCTION_FILES) {
    const instruction = await resolveProtectedProjectInstructionPath(projectPath, file);
    const result = await removeManagedMarkdownBlock(instruction.target, OWNER_AMBIENT_RESUME_TAG);
    files.push({ file, result });
  }

...
import type { OwnerProjectConfig } from './native-types.js';
...
function nativeConfigIdentity(config: OwnerProjectConfig): string {
...
async function ensureConfig(
  ...
  if (targetConfig && sourceConfig) {
    if (nativeConfigIdentity(targetConfig) !== nativeConfigIdentity(sourceConfig)) {
      throw new Error(
        `Native worktree configuration differs from the source project: ${path.join(targetRoot, '.owner', 'config.yaml')}`,
      );
    }
    return false;
  }

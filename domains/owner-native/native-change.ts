...
export const NATIVE_CHANGE_STATE_FILE = 'owner-state.yaml';
...
export class NativeSchemaMigrationRequiredError extends Error {
  ...
  constructor(
    ...
    super(
      `Native change ${change} uses ${schema}; run owner native doctor ${change} --repair before mutating it`,
    );
...
export class NativeBaselineIncompleteError extends Error {
  ...
  constructor(
    ...
    super(
      `Native change ${change} baseline is incomplete (${omittedCount} omitted entr${omittedCount === 1 ? 'y' : 'ies'}). Adjust native.snapshot scope or resource budgets in .owner/config.yaml, then retry.`,
    );
  ...
  for (const root of listGitWorktreeRoots(paths.projectRoot)) {
    ...
    try {
      ...
      try {
        ...
        if (/^schema:\s*owner\.native\.v4\s*$/mu.test(portableSource)) {
          ...
          if (
            local.schema === 'owner.native.local-execution.v4' &&

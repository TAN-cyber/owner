...
const NATIVE_EVIDENCE_PROJECTION_GENERATOR = 'owner-native';
...
export function renderNativeEvidenceProjectionMarkdown(input: {
  ...
  const lines: string[] = [
    '# Owner Native Evidence Projection',
    '',
    `- Change: ${input.change}`,
    `- Phase: ${input.phase}`,
    ...
    '  .owner/runtime/native directory. The Native Runtime regenerates it on every evidence-bearing',

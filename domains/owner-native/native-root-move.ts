...
interface RootMoveCleanupManifest {
  schema: 'owner.native.root-move-cleanup.v1';
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (previousWorkspace) {
      await writeNativeWorkspaceIdentity({
        ...
        ...(previousWorkspace.schema === 'owner.native.workspace.v3'
          ? {
              binding: {
...
function stagingDirectory(targetPaths: NativeProjectPaths, id: string): string {
  return path.join(targetPaths.artifactRoot, `.owner-native-move-${id}`);
}

function sourceRemovalDirectory(sourcePaths: NativeProjectPaths, id: string): string {
  return path.join(sourcePaths.artifactRoot, `.owner-native-source-${id}.removing`);
}
...
function parseCleanupManifest(
  ...
  if (
    ...
    manifest.schema !== 'owner.native.root-move-cleanup.v1' ||
...
async function readRootMoveJournal(
  ...
  try {
    ...
    if (journal.schema !== 'owner.native.transaction.v1' || journal.kind !== 'root-move') {
...
export async function recoverNativeRootMove(options: {
  ...
  hooks?: NativeTransactionHooks;
}): Promise<{ activeNativeRoot: string; config: OwnerProjectConfig }> {

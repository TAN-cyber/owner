...
export async function inspectNativeStatus(
  ...
  try {
    ...
    if (inspection.status === 'migration-required' && inspection.state) {
      return {
        ...
        detailsCommand: `owner native status ${name} --details`,
        checkpoint: null,
        continuation: null,
        workspace,
        runtime,
  ...
  if (runtime.status !== 'available') {
    ...
    return {
      ...
      detailsCommand: `owner native status ${state.name} --details`,
      checkpoint: null,
      continuation,
      workspace,
      runtime,
...
function nativeStatusCursor(namesHash: string, offset: number): string {
  ...
  const integrity = canonicalHash('owner.native.status-cursor.v1', { namesHash, offset });
...
function nativeStatusOffset(options: {
  ...
  const expected = canonicalHash('owner.native.status-cursor.v1', {
...
export async function listNativeStatusPage(
  ...
  const namesHash = canonicalHash('owner.native.status-names.v1', names);
  ...
  for (const candidate of candidates) {
    ...
    const trial: NativeStatusPageProjection = {
      ...
      nextPageArgs:
        nextOffset < names.length
          ? ['owner', 'native', 'status', '--cursor', nativeStatusCursor(namesHash, nextOffset)]
          : null,
  ...
  const nextPageArgs = nextCursor ? ['owner', 'native', 'status', '--cursor', nextCursor] : null;
  return {
    schema: 'owner.native.status-page.v1',
    total: names.length,
    offset,

...
export function nativePortableContinuation(
  ...
  const base = {
    schema: 'owner.native.continuation.v2' as const,
    skill: 'owner-native' as const,
    change: state.name,
    phase: state.phase,
    status: state.status,
  ...
  if (state.status === 'blocked') {
    ...
    return {
      ...
      commandArgs: retry
        ? ['owner', 'native', 'next', state.name, '--retry-verifier', '--summary', '<summary>']
        : null,
  ...
  if (state.phase === 'shape') {
    return {
      ...
      commandArgs: ['owner', 'native', 'next', state.name, '--summary', '<summary>', '--confirmed'],
    ...
    if (children) {
      if (!children.confirmed) {
        return {
          ...
          commandArgs: ['owner', 'native', 'next', state.name, '--summary', '<summary>'],
  ...
  return {
    ...
    disposition: 'continue',
    action: 'archive',
    commandArgs: ['owner', 'native', 'archive', state.name, '--confirmed'],
    requiredInputs: [],
    runnerAction: runner('none'),
  };
}

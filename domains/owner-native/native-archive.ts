...
async function finalizeArchive(
  ...
  if (journal.schema === 'owner.native.transaction.v2' && !finalizationStarted) {
  ...
  if (
    journal.schema === 'owner.native.transaction.v2' &&
  ...
  if (!finalizationStarted) {
    if (journal.schema === 'owner.native.transaction.v2') {
    ...
    withNativeTransitionLock(options.paths, options.name, `archive ${options.name}`, async () => {
      ...
      try {
        ...
        if (runtimeStorage.layout === 'legacy') {
          throw new Error(
            `Native Archive requires project-local Runtime; run owner native doctor ${options.name} --repair`,
          );
        }
    ...
    async () => {
      ...
      try {
        ...
        const journal: AnyArchiveTransactionJournal =
          (generic as unknown as { schema: string }).schema === 'owner.native.transaction.v2'

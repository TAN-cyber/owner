...
const ACCEPTANCE_HASH_TAG = 'owner.native.acceptance.v1';
...
function acceptanceCursor(acceptanceHash: string, offset: number): string {
  ...
  const cursorHash = canonicalHash('owner.native.acceptance-cursor.v1', {
    acceptanceHash,
    offset,
  });
...
function acceptanceOffset(options: {
  ...
  if (
    match[3] !==
    canonicalHash('owner.native.acceptance-cursor.v1', {

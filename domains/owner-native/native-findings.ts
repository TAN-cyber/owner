...
function retryCommand(
  ...
  if (retry === 'next') {
    return `owner native next ${state.name} --summary "<summary>"${
  ...
  if (retry === 'status') return `owner native status ${state.name} --details`;
  return null;
}
      ...
      return {
        ...
        repairCommand:
          metadata.repair === 'doctor'
            ? `owner native doctor ${options.state.name} --repair${

...
export async function assertNoPendingNativeRootMove(projectRoot: string): Promise<void> {
  ...
  if (config?.native.pending_root_move) {
    throw new Error(
      `Native root move ${config.native.pending_root_move.id} is incomplete; use owner native doctor --repair`,
    );
  }
}
...
export async function resolveNativeProject(options: {
...
}): Promise<{ config: OwnerProjectConfig; paths: NativeProjectPaths; configured: boolean }> {
  ...
  if (existing?.native.pending_root_move) {
    throw new Error(
      `Native root move ${existing.native.pending_root_move.id} is incomplete; use owner native doctor --repair`,
    );
  }

...
import type { OwnerProjectConfig, NativeProjectPaths } from './native-types.js';
...
export const NATIVE_PORTABLE_STATE_FILE = 'owner-state.yaml';
...
export async function isNativePortableChange(
  ...
  try {
    ...
    return /^schema:\s*owner\.native\.v4\s*$/mu.test(source);
  } catch (error) {

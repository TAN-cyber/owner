...
export function formatNativeReceiptBindingMismatchMessage(options: {
  ...
  return `Native verification receipt binding is invalid: ${options.detail}. Re-issue the affected receipts with \`owner native receipt refresh ${options.change} --apply\`.`;
}
...
async function retreatStaleNativeEvidence(options: {
  ...
  if (evidenceIsFresh) {
    ...
    return {
      ...
      next: 'manual',
      nextCommand:
        previousPhase === 'archive' ? `owner native archive ${options.state.name} --dry-run` : null,
      findings,
      continuation: nativeContinuation({
        state: options.state,
      ...
      if (!verificationRetryIsFresh) {
        ...
        return {
          ...
          nextCommand: stopped
            ? null
            : state.phase === 'archive'
              ? `owner native archive ${state.name} --dry-run`
              : null,
  ...
  return {
    ...
    nextCommand: repairStopped
      ...
      : persisted.phase === 'archive'
        ? `owner native archive ${persisted.name} --dry-run`
        : null,
    findings: repairFindings,
    continuation: nativeContinuation({
      state: persisted,

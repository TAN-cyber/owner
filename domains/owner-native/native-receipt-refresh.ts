 ...
 * produced by `owner native check` and re-issued automatically by `next
...
export interface NativeReceiptRefreshResult {
  ...
  /** Required-check receipts that must be re-produced via `owner native check`. */
...
function describeManualObservation(): string {
  return 'Manual evidence re-issued via `owner native receipt refresh` to restore revision binding.';
}

...
function phaseCommand(state: NativeChangeState): {
  ...
  const commandArgs = ['owner', 'native', 'next', state.name, '--summary', '<summary>'];
...
export function nativeContinuation(options: {
  ...
  if (runtimeMissing) {
    return buildContinuation(options.state, {
      ...
      commandArgs: ['owner', 'native', 'next', options.state.name, '--summary', '<summary>'],
  ...
  if (options.evidenceRetreat) {
    const commandArgs = ['owner', 'native', 'next', options.state.name, '--summary', '<summary>'];
    return buildContinuation(options.state, {
      disposition: 'continue',
      action: 'advance-phase',
      commandArgs,
  ...
  if (actionableFindings.length > 0) {
    ...
    if (requiredInputs.includes('refresh-verification-receipts')) {
      return buildContinuation(options.state, {
        disposition: 'continue',
        action: 'work-phase',
        commandArgs: ['owner', 'native', 'receipt', 'refresh', options.state.name, '--apply'],
        requiredInputs,
      });
    }
    return buildContinuation(options.state, {
  ...
  if (options.state.phase === 'archive') {
    ...
    return buildContinuation(options.state, {
      ...
      commandArgs: options.archiveReady
        ? ['owner', 'native', 'archive', options.state.name, '--dry-run']
        : null,

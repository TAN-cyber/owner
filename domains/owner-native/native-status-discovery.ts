...
export interface NativeDiscoveredStatusPageProjection {
  schema: 'owner.native.status-page.v1' | 'owner.native.status-page.v2';
  total: number;
  offset: number;
...
function discoveryCursor(candidatesHash: string, offset: number): string {
  ...
  const integrity = canonicalHash('owner.native.workspace-status-cursor.v1', {
    candidatesHash,
    offset,
  });
...
function pageAction(
  ...
  const args = cursor
    ? ['owner', 'native', 'status', '--cursor', cursor, '--project-root', projectRoot, '--json']
    : null;
  return {
  ...
  if (inspection?.state) {
    return {
      ...
      continuation: {
        ...
        disposition: 'blocked',
        action: 'none',
        commandArgs: ['owner', 'native', 'doctor', candidate.name, '--repair'],
        requiredInputs: [],
        runnerAction: {
          kind: 'none',
          candidateId: null,
...
export async function listDiscoveredNativeStatusPage(options: {
  ...
  const candidatesHash = canonicalHash(
    'owner.native.workspace-status-candidates.v1',
  ...
  const schema = candidates.some(({ kind }) => kind === 'portable')
    ? ('owner.native.status-page.v2' as const)
    : ('owner.native.status-page.v1' as const);

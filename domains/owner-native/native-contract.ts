...
const CONTRACT_HASH_TAG = 'owner.native.contract.v1';
const ACCEPTANCE_SET_HASH_TAG = 'owner.native.acceptance-set.v1';
...
export interface NativeContractSnapshot {
  schema: 'owner.native.contract.v1';
  brief: {
    source: string;
    contentHash: string;
  };
...
function contentHash(value: string): string {
  return canonicalHash('owner.native.contract-content.v1', value.replace(/\r\n?/gu, '\n'));
}
...
export function buildNativeContractSnapshot(input: {
  ...
  const contractContent = {
    schema: 'owner.native.contract.v1' as const,
    brief,
    specs: specSnapshots,
    acceptance,
    acceptanceHash,

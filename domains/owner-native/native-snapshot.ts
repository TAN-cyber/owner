...
async function finalizeNativeGitSnapshotSelection(
  ...
  if (initial.combined.overflow || final.combined.overflow) {
    selection.overflow = {
      ...
      hash: sha256Text(
        `owner.native.git-selection-overflow.v2\n${JSON.stringify({
...
async function nativePhysicalSnapshotSelection(options: {
  ...
  const addRecord = (record: NativePhysicalSelectionRecord): void => {
    ...
    const digest = createHash('sha256')
      .update('owner.native.physical-selection-record.v1\0')
      .update(encoded)
      .digest();
  ...
  const hash = overflow
    ? sha256Text(
        `owner.native.physical-selection-incomplete.v1\n${JSON.stringify({
    ...
    : sha256Text(
        `owner.native.physical-selection.v1\n${JSON.stringify({
          visitedNodeCount,
          recordCount,
          encodedBytes,
...
function parseGitSelectionEvidence(value: unknown): NativeGitSelectionEvidence {
  ...
  if (selection.schema !== 'owner.native.git-selection.v1') {
...
function parsePhysicalSelectionEvidence(value: unknown): NativePhysicalSelectionEvidence {
  ...
  if (selection.schema !== 'owner.native.physical-selection.v1') {
...
export function parseNativeContentSnapshotManifest(value: unknown): NativeContentSnapshotManifest {
  ...
  if (manifest.schema !== 'owner.native.content-snapshot.v1') {
  ...
  if (manifest.policy !== undefined) {
    ...
    if (policyValue.schema !== 'owner.native.snapshot-policy.v1') {
...
export async function filterNativeContentSnapshotToProjectScope(
  ...
  let overflowHash =
    manifest.omissionOverflow?.hash ?? sha256Text('owner.native.snapshot-omission-overflow.v1');
...
export async function createNativeContentSnapshot(
  ...
  let omittedCount = 0;
  let overflowCount = 0;
  let overflowHash = sha256Text('owner.native.snapshot-omission-overflow.v1');
  let totalBytes = 0;

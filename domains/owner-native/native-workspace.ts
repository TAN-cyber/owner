...
export interface NativeWorkspaceIdentityV2 extends NativeWorkspaceIdentityFields {
  schema: 'owner.native.workspace.v2';
}
...
function assertIdentity(value: unknown): asserts value is NativeWorkspaceIdentity {
  ...
  if (root.schema !== 'owner.native.workspace.v2' && root.schema !== 'owner.native.workspace.v3') {
  ...
  if (root.schema === 'owner.native.workspace.v2') {
    if (
...
export async function inspectNativeWorkspaceSchema(
  ...
  'owner.native.workspace.v1' | 'owner.native.workspace.v2' | 'owner.native.workspace.v3' | null
> {
  ...
  if (
    schema === 'owner.native.workspace.v1' ||
    schema === 'owner.native.workspace.v2' ||
    schema === 'owner.native.workspace.v3'
  ) {
    if (schema !== 'owner.native.workspace.v1') assertIdentity(value);
    return schema;
  }
  throw new Error('Unsupported Native workspace identity');
}
...
export async function projectNativeWorkspace(
  ...
  try {
    ...
    if (identity.schema !== 'owner.native.workspace.v3') {
      return {
        ...base,
        bindingState: 'legacy',
        isolation: null,
...
export async function nativeWorkspaceIdentityNeedsMigration(
  ...
  if ((value as { schema?: unknown }).schema === 'owner.native.workspace.v1') return true;
  assertIdentity(value);
  if (value.schema === 'owner.native.workspace.v3') return false;
...
export async function inspectNativeWorkspaceIdentity(
  ...
  const [projectRootId, nativeRootId, projectRootPathId, nativeRootPathId] = await Promise.all([
    physicalDirectoryIdentity('owner.native.workspace-project-root.v2', options.paths.projectRoot),
    physicalDirectoryIdentity('owner.native.workspace-native-root.v2', options.paths.nativeRoot),
    directoryPathIdentity('owner.native.workspace-project-root-path.v2', options.paths.projectRoot),
    directoryPathIdentity('owner.native.workspace-native-root-path.v2', options.paths.nativeRoot),
  ]);
    ...
    ...(options.sessionId
      ? {
          sessionHash: identityHash(
            'owner.native.workspace-session.v2',
            `${projectRootId}\n${nativeRootId}\n${options.sessionId}`,
          ),
        }
      : {}),
  ...
  const identity: NativeWorkspaceIdentity = options.binding
    ...
    : { schema: 'owner.native.workspace.v2', ...fields };
...
export async function setNativeWorkspaceFinish(
  ...
  return withNativeMutationLock(paths, `set workspace finish for ${name}`, async () => {
    ...
    if (identity === null || identity.schema !== 'owner.native.workspace.v3') {
...
export async function readNativeWorkspaceIdentity(
  ...
  if (
    ...
    typeof value === 'object' &&
    !Array.isArray(value) &&
    (value as { schema?: unknown }).schema === 'owner.native.workspace.v1'
  ) {
...
export async function inspectNativeWorkspaceBinding(options: {
  ...
  assertIdentity(options.identity);
  if (options.identity.schema === 'owner.native.workspace.v2') {
    return {
      state: 'legacy',

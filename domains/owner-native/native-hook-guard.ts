...
export type NativeHookIntent = OwnerHookIntent;
export interface NativeHookRequest extends Omit<OwnerHookRequest, 'toolName'> {
  toolName?: string | null;
}

export type NativeHookGuardResult = OwnerHookDecision;
...
function requestTargetsAreControlOnly(
  ...
  return (
    ...
    request.targets.every((targetPath) => {
      ...
      const relative = path.relative(projectRoot, target).replaceAll('\\', '/');
      return relative === '.owner/config.yaml' || isWithin(nativeRoot, target);
    })
  );
}
...
export async function inspectNativeHookGuard(
  ...
  if (selectedChangeName) {
    ...
    if (!change) {
      return {
        allowed: false,
        reason: `Selected Native change ${selectedChangeName} is missing or archived; resume /owner-native before retrying`,
        workflow: 'native',
  ...
  for (const targetPath of request.targets) {
    ...
    return {
      allowed: false,
      reason: `Native change ${state.name} is in ${state.phase}; implementation writes are only allowed in build. If this belongs to the current change, confirm the scope and run owner native next ${state.name} --summary "<reason>" --return-to-build; otherwise create or select a separate Native change`,
      workflow: 'native',
      phase: state.phase,
      change: state.name,
    };

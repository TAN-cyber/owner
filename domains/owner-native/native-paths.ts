...
export async function nativeProjectPaths(
  ...
  const nativeRoot = path.join(artifactRoot, 'owner');
  if (await isSymbolicLink(nativeRoot)) {
    throw new Error('The configured Native owner root must not be a symbolic link');
  }
  ...
  if (!inside(physicalArtifactRoot, physicalNativeRoot)) {
    throw new Error('The configured Native owner root resolves outside its artifact root');
  }
  const resolvedProjectRoot = path.resolve(projectRoot);
  const runtimeDir = path.join(resolvedProjectRoot, '.owner', 'runtime', 'native');

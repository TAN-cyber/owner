...
export function parseWorkflowProjectConfigDocument(
  ...
  if (document.errors.length > 0) {
    throw new Error(`Invalid .owner/config.yaml: ${document.errors[0].message}`);
  }
  ...
  if (typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Invalid .owner/config.yaml: root must be a mapping');
  }
...
export function mergeWorkflowProjectConfigDocument(
  ...
  if (!validated) throw new Error('Unsupported Owner project schema');
...
function workflowYamlError(message, line) {
  ...
  throw new Error('Invalid .owner/config.yaml: ' + message + suffix);
}
...
function managedWorkflowConfigFields(source) {
  const root = workflowConfigRecord(parseWorkflowProjectYaml(source), '.owner/config.yaml');
  const hasProjectMarker =
...
async function readWorkflowProjectPathConfig(projectRoot) {
  const file = path.join(projectRoot, '.owner', 'config.yaml');
  let inspection;
  try {

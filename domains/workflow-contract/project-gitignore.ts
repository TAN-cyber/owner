...
const MANAGED_BLOCK_START = '# >>> Owner managed project state >>>';
const MANAGED_BLOCK_END = '# <<< Owner managed project state <<<';
...
function withoutManagedBlocks(source: string): string {
  ...
  for (const line of linesWithEndings(source)) {
    ...
    if (text === MANAGED_BLOCK_START) {
      if (insideManagedBlock) throw new Error('Owner .gitignore managed block is malformed');
  ...
  if (insideManagedBlock) throw new Error('Owner .gitignore managed block is incomplete');
  return output.join('');
}

export function renderOwnerProjectGitignore(source: string): string {
...
/**
 * Keep project-level Owner configuration portable without exposing local
 * Runtime, selections, caches, drafts, or installed Skills.
 *
 * This only updates `.gitignore`; it never stages files or invokes Git.
 */
export async function ensureOwnerProjectGitignore(projectRoot: string): Promise<void> {
  ...
  const output = renderOwnerProjectGitignore(source);

...
const CONFIG_TRANSACTION_SCHEMA = 'owner.project-config-write.v1';
const CONFIG_TRANSACTION_PATH = '.owner/config-write-transaction.json';
...
function transactionCandidate(id: string): string {
  return `.owner/config.yaml.${id}.next`;
}

function transactionQuarantine(id: string): string {
  return `.owner/config.yaml.${id}.quarantine`;
}

function transactionCleanup(id: string): string {
  return `.owner/config-write-transaction.${id}.cleanup`;
}
...
export async function beginWorkflowProjectConfigTransaction(
  ...
  if (await readTransaction(projectRoot)) {
    throw new Error(
      'An unfinished project config write transaction exists; run owner doctor --repair',
    );
  }

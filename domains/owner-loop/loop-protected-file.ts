...
import { atomicWriteBytes } from './loop-atomic-file.js';
import { sameLoopFileObject } from './loop-file-identity.js';

export interface LoopProtectedFileHooks {
...
export interface LoopProtectedFile {
  bytes: Buffer;
  hash: string;
  size: number;
}

export interface LoopProtectedTextFile extends LoopProtectedFile {
  text: string;
}
...
function positiveLimit(value: number): number {
  if (!Number.isSafeInteger(value) || value < 1) {
    throw new Error('Loop protected file byte limit must be a positive integer');
  }
  return value;
}
...
export async function readLoopProtectedFile(options: {
  root: string;
  file: string;
  maxBytes: number;
  label: string;
...
export async function readLoopProtectedTextFile(options: {
  root: string;
  file: string;
  maxBytes: number;
  label: string;
  forbiddenRoots?: readonly string[];
  hooks?: LoopProtectedFileHooks;
}): Promise<LoopProtectedTextFile> {
  const snapshot = await readLoopProtectedFile(options);
  let text: string;
  try {
...
export async function readLoopProtectedDirectory(options: {
  root: string;
  directory: string;
  label: string;
  maxEntries?: number;
...
export async function captureLoopProtectedDirectoryGuard(options: {
  root: string;
  directory: string;
  label: string;
...
interface LoopProtectedQuarantineGuard {
  quarantine: string;
  parentChain: DirectoryIdentity[];
  identity: DirectoryIdentity;
}

async function quarantineLoopProtectedDirectoryInternal(options: {
  root: string;
  directory: string;
  quarantine: string;
  label: string;
...
}): Promise<LoopProtectedQuarantineGuard> {
...
export async function quarantineLoopProtectedDirectory(options: {
  root: string;
  directory: string;
  quarantine: string;
  label: string;
  ...
  await quarantineLoopProtectedDirectoryInternal(options);
}

export async function removeLoopProtectedDirectory(options: {
  root: string;
  directory: string;
  quarantine: string;
  label: string;
...
export async function removeLoopProtectedFile(options: {
  root: string;
  file: string;
  maxBytes: number;
  expectedHash: string;
...
export async function removeLoopProtectedEmptyDirectory(options: {
  root: string;
  directory: string;
  label: string;
  beforeRemove?: () => void | Promise<void>;
...
export async function removeLoopProtectedQuarantine(options: {
  root: string;
  quarantine: string;
  label: string;
  beforeRemove?: (quarantine: string) => void | Promise<void>;
...
export async function ensureLoopProtectedDirectory(options: {
  root: string;
  directory: string;
  label: string;
}): Promise<void> {
...
export async function moveLoopProtectedDirectory(options: {
  root: string;
  source: string;
  target: string;
  label: string;
...
export async function copyLoopProtectedFile(options: {
  sourceRoot: string;
  source: string;
  targetRoot: string;
  target: string;
  ...
  const source = await readLoopProtectedFile({
  ...
  await atomicWriteBytes(options.target, source.bytes, {
    ...
    beforeCommit: async () => {
      ...
      const target = await readLoopProtectedFile({
  ...
  const persisted = await readLoopProtectedFile({
    root: options.targetRoot,
    file: options.target,
    maxBytes: options.maxBytes,

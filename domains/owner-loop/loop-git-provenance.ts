...
import type { LoopSnapshotEntry } from './loop-types.js';
import { isLocalGitBranch } from '../../platform/paths/git-worktree.js';
import type { LoopExternalDrift, LoopSnapshotProjection } from './loop-verification-scope.js';
import { loopDeclaredArtifactOwnsPath } from './loop-verification-scope.js';
import type { LoopWorkspaceGitProvenance } from './loop-workspace.js';
...
function entriesByPath(
  projection: Pick<LoopSnapshotProjection, 'entries'>,
): Map<string, LoopSnapshotEntry> {
  return new Map(projection.entries.map((entry) => [entry.path, entry]));
}

function sameTreeEntry(
  entry: LoopSnapshotEntry | undefined,
 ...
 * normal Loop partial-scope path remains responsible for asking for a decision.
 */
export function detectLoopGitExternalDrift(options: {
  projectRoot: string;
  provenance: LoopWorkspaceGitProvenance;
  baseline: Pick<LoopSnapshotProjection, 'complete' | 'entries'>;
  current: Pick<LoopSnapshotProjection, 'complete' | 'entries'>;
  declaredArtifacts: readonly LoopDeclaredArtifact[];
}): LoopExternalDrift | null {
  ...
  for (const changedPath of changedPaths) {
    if (
      options.declaredArtifacts.some((artifact) =>
        loopDeclaredArtifactOwnsPath(artifact, changedPath),
      )
    ) {
      continue;
    }

...
import { sha256File } from './loop-hash.js';
...
export const LOOP_ARTIFACT_VALIDATION_LIMITS = {
...
async function readContainedFile(root: string, relativeRef: string): Promise<string> {
  ...
  if (!isInsidePath(root, target)) throw new Error(`Artifact escapes Loop change: ${relativeRef}`);
  const realRoot = await fs.realpath(root);
  const realTarget = await fs.realpath(target);
  if (!isInsidePath(realRoot, realTarget)) {
    throw new Error(`Artifact symlink escapes Loop change: ${relativeRef}`);
  }
...
function result(findings: LoopFinding[]): LoopArtifactValidation {
  return { valid: findings.length === 0, findings };
}

export async function validateLoopBrief(
  changeDir: string,
  briefRef: string,
  ...
  try {
    source = (
      await readLoopBoundedTextFile({
        root: changeDir,
        ref: briefRef,
        maxBytes: LOOP_ARTIFACT_VALIDATION_LIMITS.maxFileBytes,
      })
    ).text;
  } catch (error) {
...
export async function validateLoopVerification(
  changeDir: string,
  reportRef: string,
): Promise<LoopArtifactValidation> {
  const findings: LoopFinding[] = [];
...
export function canonicalSpecPath(paths: LoopProjectPaths, capability: string): string {
  return path.join(paths.specsDir, capability, 'spec.md');
}

export async function validateLoopSpecChanges(
...
export async function resolveLoopArtifactFile(
  changeDir: string,
  relativeRef: string,
): Promise<string> {

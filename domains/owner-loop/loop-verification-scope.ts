...
import { canonicalHash, canonicalJson } from './loop-canonical-hash.js';
import { parseLoopContentSnapshotManifest } from './loop-snapshot.js';
import type {
...
export const LOOP_IMPLEMENTATION_SCOPE_SCHEMA = 'owner.loop.implementation-scope.v2' as const;
export const LOOP_SNAPSHOT_PROJECTION_SCHEMA = 'owner.loop.content-snapshot-projection.v1' as const;

const SNAPSHOT_PROJECTION_HASH_TAG = 'owner.loop.content-snapshot-projection.v1';
const IMPLEMENTATION_SCOPE_HASH_TAG = 'owner.loop.implementation-scope.v2';
const UNRESOLVED_SCOPE_ID_TAG = 'owner.loop.unresolved-scope-id.v1';
const SCOPE_DETAIL_OVERFLOW_HASH_TAG = 'owner.loop.scope-detail-overflow.v1';
const SNAPSHOT_PROJECTION_OVERFLOW_HASH_TAG = 'owner.loop.snapshot-projection-overflow.v2';
const SHA256_HASH_PATTERN = /^[a-f0-9]{64}$/u;
export const MAX_LOOP_IMPLEMENTATION_EVIDENCE_DOCUMENT_BYTES = 1024 * 1024;
...
export const MAX_LOOP_DETAILED_SCOPE_CHANGES = 128;
export const MAX_LOOP_DETAILED_UNRESOLVED_SCOPES = 128;

export interface LoopDeclaredArtifact {
  path: string;
  kind: 'file' | 'directory';
}

export type LoopSnapshotProjectionRef = `runtime/evidence/snapshots/${string}.json`;

export interface LoopImplementationChange {
  path: string;
  kind: 'added' | 'modified' | 'removed';
  before: LoopImplementationFileIdentity | null;
  after: LoopImplementationFileIdentity | null;
  attributedTo: LoopDeclaredArtifact[];
}

export interface LoopImplementationFileIdentity {
  hash: string;
  size: number;
}

export type LoopUnresolvedScopeKind =
  | 'unattributed-change'
  | 'snapshot-omission'
  | 'snapshot-incomplete'
...
export interface LoopUnresolvedScope {
  id: string;
  kind: LoopUnresolvedScopeKind;
  source: 'baseline' | 'current' | 'implementation-scope';
  path: string | null;
  reason: string;
}

export interface LoopGitScopeAdvisory {
  advisoryOnly: true;
  changedPaths: string[];
...
export interface LoopExternalDrift {
  provider: 'git';
  baseCommit: string;
  targetBranch: string;
  targetCommit: string;
...
export interface LoopImplementationScope {
  schema: typeof LOOP_IMPLEMENTATION_SCOPE_SCHEMA;
  contractHash: string;
  baselineProjectionRef: LoopSnapshotProjectionRef;
  baselineProjectionHash: string;
  currentProjectionRef: LoopSnapshotProjectionRef;
  currentProjectionHash: string;
  complete: boolean;
  declaredArtifacts: LoopDeclaredArtifact[];
  changes: LoopImplementationChange[];
  unattributed: LoopImplementationChange[];
  unresolvedScopes: LoopUnresolvedScope[];
  noCodeReason: string | null;
  gitAdvisory?: LoopGitScopeAdvisory;
  externalDrift?: LoopExternalDrift;
  scopeHash: string;
}

export interface BuildLoopImplementationScopeInput {
  baseline: LoopContentSnapshotManifest;
  current: LoopContentSnapshotManifest;
  contractHash: string;
...
export interface LoopSnapshotProjection {
  schema: typeof LOOP_SNAPSHOT_PROJECTION_SCHEMA;
  origin: LoopContentSnapshotManifest['origin'];
  capture?: LoopContentSnapshotManifest['capture'];
  complete: boolean;
  limits: LoopContentSnapshotManifest['limits'];
  policy?: LoopSnapshotPolicy;
  entries: LoopSnapshotEntry[];
  omitted: LoopSnapshotOmission[];
  omittedCount: number;
  omissionOverflow?: LoopContentSnapshotManifest['omissionOverflow'];
}

export interface LoopImplementationScopeAuthority {
  contractHash: string;
  declaredArtifacts: LoopDeclaredArtifact[];
  noCodeReason: string | null;
 ...
 */
export interface LoopImplementationScopeBundle {
  authority: LoopImplementationScopeAuthority;
  baseline: LoopSnapshotProjection;
  current: LoopSnapshotProjection;
  scope: LoopImplementationScope;
}

interface UnresolvedScopeIdentity {
  kind: LoopUnresolvedScopeKind;
  source: LoopUnresolvedScope['source'];
  path: string | null;
...
export function loopSnapshotProjectionRef(hash: string): LoopSnapshotProjectionRef {
  if (!SHA256_HASH_PATTERN.test(hash)) {
    throw new Error('Loop snapshot projection hash must be a SHA-256 hash');
  }
...
function normalizeEntry(entry: LoopSnapshotEntry, label: string): LoopSnapshotEntry {
...
function normalizeOmission(omission: LoopSnapshotOmission, label: string): LoopSnapshotOmission {
  return {
...
function compareEntries(left: LoopSnapshotEntry, right: LoopSnapshotEntry): number {
  return (
...
function compareOmissions(left: LoopSnapshotOmission, right: LoopSnapshotOmission): number {
  return (
    compareText(left.path, right.path) ||
 ...
 */
function fitSnapshotProjectionBudget(value: LoopSnapshotProjection): LoopSnapshotProjection {
  if (serializedEvidenceBytes(value) <= MAX_LOOP_IMPLEMENTATION_EVIDENCE_DOCUMENT_BYTES) {
    return value;
  }
  ...
  const foldOmission = (omission: LoopSnapshotOmission, alreadyCounted: boolean): void => {
    if (!alreadyCounted) omittedCount += 1;
    overflowCount += 1;
    updateFramedHash(overflowHash, 'omission', omission);
  };
  const foldEntry = (entry: LoopSnapshotEntry): void => {
    omittedCount += 1;
    overflowCount += 1;
  ...
  const takeCompactableOmission = (): LoopSnapshotOmission | null => {
  ...
  const candidate = (): LoopSnapshotProjection => ({
    ...value,
    ...
    omittedCount,
    omissionOverflow: {
      ref: `loop-snapshot://omitted-overflow/${'0'.repeat(64)}`,
      hash: '0'.repeat(64),
      count: overflowCount,
    },
  });
  ...
  while (serializedEvidenceBytes(projection) > MAX_LOOP_IMPLEMENTATION_EVIDENCE_DOCUMENT_BYTES) {
    ...
    if (compactableOmissionCount > 0) {
      ...
      throw new Error('Loop snapshot projection metadata exceeds its evidence byte budget');
    }
  ...
  return {
    ...projection,
    omissionOverflow: {
      ref: `loop-snapshot://omitted-overflow/${digest}`,
      hash: digest,
      count: overflowCount,
    },
  };
...
function snapshotProjection(manifest: LoopContentSnapshotManifest): LoopSnapshotProjection {
  const parsed = parseLoopContentSnapshotManifest(manifest);
  ...
  return fitSnapshotProjectionBudget({
    schema: LOOP_SNAPSHOT_PROJECTION_SCHEMA,
...
function normalizeDeclaredArtifacts(
  artifacts: readonly LoopDeclaredArtifact[],
): LoopDeclaredArtifact[] {
  const byPath = new Map<string, LoopDeclaredArtifact>();
  for (const [index, artifact] of artifacts.entries()) {
    if (artifact.kind !== 'file' && artifact.kind !== 'directory') {
      throw new Error(`Declared artifact ${index} kind is invalid`);
    }
    const normalized: LoopDeclaredArtifact = {
...
export function loopDeclaredArtifactOwnsPath(
  artifact: LoopDeclaredArtifact,
  changedPath: string,
): boolean {
...
function fileIdentity(entry: LoopSnapshotEntry | undefined): LoopImplementationFileIdentity | null {
  return entry ? { hash: entry.hash, size: entry.size } : null;
}

interface LoopImplementationChangeCore {
  path: string;
  kind: LoopImplementationChange['kind'];
  before: LoopImplementationFileIdentity | null;
  after: LoopImplementationFileIdentity | null;
}
...
function visitDerivedChanges(
  baseline: LoopSnapshotProjection,
  current: LoopSnapshotProjection,
  visitor: (change: LoopImplementationChangeCore) => void,
): void {
  let beforeIndex = 0;
  let afterIndex = 0;
...
function materializeChange(
  core: LoopImplementationChangeCore,
  declaredArtifacts: LoopDeclaredArtifact[],
): LoopImplementationChange {
  return {
    ...core,
    attributedTo: declaredArtifacts.filter((artifact) =>
      loopDeclaredArtifactOwnsPath(artifact, core.path),
    ),
  };
}
 ...
 * the complete set.
 */
export function deriveLoopImplementationChanges(input: {
  baseline: LoopSnapshotProjection;
  current: LoopSnapshotProjection;
  declaredArtifacts: readonly LoopDeclaredArtifact[];
}): LoopImplementationChange[] {
  const declaredArtifacts = normalizeDeclaredArtifacts(input.declaredArtifacts);
  const changes: LoopImplementationChange[] = [];
...
function unresolvedScope(identity: UnresolvedScopeIdentity, reason: string): LoopUnresolvedScope {
  return {
...
function compareUnresolvedScopes(left: LoopUnresolvedScope, right: LoopUnresolvedScope): number {
  return (
...
function uniqueUnresolvedScopes(scopes: LoopUnresolvedScope[]): LoopUnresolvedScope[] {
...
function normalizeExternalDrift(value: LoopExternalDrift): LoopExternalDrift {
  if (!value || value.provider !== 'git') {
    throw new Error('Loop external drift provider is invalid');
  }
  const paths = normalizeGitChangedPaths(value.paths);
  if (paths.length === 0) throw new Error('Loop external drift requires at least one path');
  return {
    provider: 'git',
    baseCommit: normalizeGitCommit(value.baseCommit, 'Loop external drift base commit'),
    targetBranch: normalizeGitBranch(value.targetBranch, 'Loop external drift target branch'),
    targetCommit: normalizeGitCommit(value.targetCommit, 'Loop external drift target commit'),
    paths,
  };
}
...
interface LoopScopeChangeScan {
  candidates: LoopImplementationChange[];
...
function scanScopeChanges(
  ...
  const candidates: LoopImplementationChange[] = [];
  ...
  visitDerivedChanges(baseline, current, (core) => {
    ...
    if (!candidateBudgetExhausted && candidates.length < MAX_LOOP_DETAILED_SCOPE_CHANGES) {
      const candidate = materializeChange(core, declaredArtifacts);
      const nextBytes = serializedEvidenceBytes(candidate);
      if (candidateBytes + nextBytes <= MAX_LOOP_IMPLEMENTATION_EVIDENCE_DOCUMENT_BYTES) {
...
function parseScopeDetailOverflow(scope: LoopUnresolvedScope): {
  changes: number;
  hash: string;
  unattributed: number;
  unresolved: number;
  ...
  visitDerivedChanges(options.baseline, options.current, (core) => {
    if (changeIndex >= options.detailedChangeCount) {
      ...
      for (const artifact of options.declaredArtifacts) {
        if (!loopDeclaredArtifactOwnsPath(artifact, core.path)) continue;
        attributionCount += 1;
...
function buildScopeFromProjections(
  ...
  if (authority.externalDrift) {
    ...
    const invalidExternalPaths = authority.externalDrift.paths.filter(
      (value) =>
        ...
        declaredArtifacts.some((artifact) => loopDeclaredArtifactOwnsPath(artifact, value)),
    );
    if (invalidExternalPaths.length > 0) {
      throw new Error(
        `Loop external drift contains paths outside the unowned change set: ${invalidExternalPaths.slice(0, 8).join(', ')}`,
      );
    }
    if (externalPathSet.size !== authority.externalDrift.paths.length) {
      throw new Error('Loop external drift paths must be unique');
    }
  }
  ...
  const omissionCandidates: LoopUnresolvedScope[] = [];
  ...
  for (const [source, projection] of [
    ...
    for (const omission of projection.omitted) {
      if (omissionCandidates.length >= MAX_LOOP_DETAILED_UNRESOLVED_SCOPES) break;
  ...
  const buildGitAdvisory = (): LoopGitScopeAdvisory | undefined => {
  ...
  const buildScopeContent = (options: {
    ...
    return {
      schema: LOOP_IMPLEMENTATION_SCOPE_SCHEMA,
      contractHash,
      baselineProjectionRef: loopSnapshotProjectionRef(baselineProjectionHash),
      baselineProjectionHash,
      currentProjectionRef: loopSnapshotProjectionRef(currentProjectionHash),
      currentProjectionHash,
  ...
  while (true) {
    ...
    throw new Error('Loop implementation scope metadata exceeds its evidence byte budget');
  }
  ...
  if (serializedEvidenceBytes(scope) > MAX_LOOP_IMPLEMENTATION_EVIDENCE_DOCUMENT_BYTES) {
    throw new Error('Loop implementation scope exceeded its evidence byte budget after fitting');
  }
  return scope;
}

/**
 * Build the authority bundle consumed by Loop evidence storage.
 * Snapshot timestamps are parsed but deliberately excluded from the normalized projections.
 */
export function buildLoopImplementationScopeBundle(
  input: BuildLoopImplementationScopeInput,
): LoopImplementationScopeBundle {
...
export function buildLoopImplementationScope(
  input: BuildLoopImplementationScopeInput,
): LoopImplementationScope {
  return buildLoopImplementationScopeBundle(input).scope;
}
...
function snapshotProjectionRefValue(
  ...
  hash: string,
  label: string,
): LoopSnapshotProjectionRef {
  const expected = loopSnapshotProjectionRef(hash);
...
export function parseLoopSnapshotProjection(
  value: unknown,
  expectedHash?: string,
): LoopSnapshotProjection {
  const root = scopeRecord(value, 'Loop snapshot projection');
  exactScopeKeys(
    root,
  ...
  if (root.schema !== LOOP_SNAPSHOT_PROJECTION_SCHEMA) {
    throw new Error('Loop snapshot projection schema is invalid');
  }
  const parsedManifest = parseLoopContentSnapshotManifest({
  ...
  if (
    ...
    throw new Error('Loop snapshot projection is not canonical');
  }
  ...
  if (
    ...
    throw new Error('Loop snapshot projection content hash mismatch');
  }
  return projection;
}

function parseDeclaredArtifact(value: unknown, index: number): LoopDeclaredArtifact {
...
function parseFileIdentity(value: unknown, label: string): LoopImplementationFileIdentity | null {
...
function parseImplementationChange(value: unknown, index: number): LoopImplementationChange {
...
function parseUnresolvedScope(value: unknown, index: number): LoopUnresolvedScope {
  ...
  const kinds = new Set<LoopUnresolvedScopeKind>([
    'unattributed-change',
    'snapshot-omission',
    'snapshot-incomplete',
    'snapshot-omission-overflow',
  ...
  if (typeof scope.kind !== 'string' || !kinds.has(scope.kind as LoopUnresolvedScopeKind)) {
  ...
  return {
    id: scope.id,
    kind: scope.kind as LoopUnresolvedScopeKind,
    source: scope.source,
    path:
      scope.path === null
        ? null
...
function parseExternalDrift(value: unknown): LoopExternalDrift {
  const root = scopeRecord(value, 'Loop implementation external drift');
  exactScopeKeys(
    root,
    ['provider', 'baseCommit', 'targetBranch', 'targetCommit', 'paths'],
    [],
    'Loop implementation external drift',
  );
  if (root.provider !== 'git') {
    throw new Error('Loop implementation external drift provider is invalid');
  }
  const paths = parseSortedPaths(root.paths, 'Loop external drift paths');
 ...
 * Storage additionally calls `rebuildLoopImplementationScopeBundle` so snapshot-derived facts
 * are verified against the two projections instead of trusted from this document.
 */
export function parseLoopImplementationScope(value: unknown): LoopImplementationScope {
  const root = scopeRecord(value, 'Loop implementation scope');
  const required = [...SCOPE_KEYS].filter(
    (key) => key !== 'gitAdvisory' && key !== 'externalDrift',
  );
  exactScopeKeys(root, required, ['gitAdvisory', 'externalDrift'], 'Loop implementation scope');
  if (root.schema !== LOOP_IMPLEMENTATION_SCOPE_SCHEMA) {
    throw new Error('Loop implementation scope schema is invalid');
  }
      ...
      (change) =>
        ...
        JSON.stringify(
          declaredArtifacts.filter((artifact) =>
            loopDeclaredArtifactOwnsPath(artifact, change.path),
          ),
        ),
    )
  ) {
  ...
  let gitAdvisory: LoopGitScopeAdvisory | undefined;
...
function parseScopeAuthority(value: unknown): LoopImplementationScopeAuthority {
  const root = scopeRecord(value, 'Loop implementation scope authority');
  exactScopeKeys(
    root,
    ['contractHash', 'declaredArtifacts', 'noCodeReason'],
    ['gitChangedPaths', 'externalDrift'],
    'Loop implementation scope authority',
  );
  if (!Array.isArray(root.declaredArtifacts)) {
    throw new Error('Loop implementation scope authority declarations must be an array');
  }
  const declaredArtifacts = root.declaredArtifacts.map(parseDeclaredArtifact);
  if (
    JSON.stringify(declaredArtifacts) !==
    ...
    throw new Error('Loop implementation scope authority declarations must be canonical');
  }
  if (
    root.noCodeReason !== null &&
    (typeof root.noCodeReason !== 'string' ||
    ...
    throw new Error('Loop implementation scope authority no-code reason is invalid');
  }
  ...
  if (root.gitChangedPaths !== undefined) {
    gitChangedPaths = parseSortedPaths(
      ...
      'Loop implementation scope authority Git paths',
    );
  }
  const externalDrift =
    root.externalDrift === undefined ? undefined : parseExternalDrift(root.externalDrift);
  ...
  if (authority.contractHash !== root.contractHash) {
    throw new Error('Loop implementation scope authority contract hash is not canonical');
  }
  return authority;
}
 ...
 */
export function parseLoopImplementationScopeBundle(value: unknown): LoopImplementationScopeBundle {
  const root = scopeRecord(value, 'Loop implementation scope bundle');
  exactScopeKeys(
    root,
    ['authority', 'baseline', 'current', 'scope'],
    [],
    'Loop implementation scope bundle',
  );
  const authority = parseScopeAuthority(root.authority);
  const baseline = parseLoopSnapshotProjection(root.baseline);
  const current = parseLoopSnapshotProjection(root.current);
  const suppliedScope = parseLoopImplementationScope(root.scope);
  ...
  if (JSON.stringify(suppliedScope) !== JSON.stringify(rebuiltScope)) {
    throw new Error('Loop implementation scope does not match its authoritative bundle');
  }
...
export function rebuildLoopImplementationScopeBundle(input: {
  baseline: LoopSnapshotProjection;
  current: LoopSnapshotProjection;
  scope: LoopImplementationScope;
}): LoopImplementationScopeBundle {
  const suppliedScope = parseLoopImplementationScope(input.scope);
  return parseLoopImplementationScopeBundle({
    authority: {

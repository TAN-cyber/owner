...
export interface LoopAtomicWriteOptions {
...
async function captureDirectoryIdentity(directory: string): Promise<DirectoryIdentity> {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error(`Loop atomic write parent must be a real directory: ${directory}`);
  }
  return {
    path: directory,
...
async function verifyDirectoryChain(chain: readonly DirectoryIdentity[]): Promise<void> {
  for (const identity of chain) {
    ...
    if (
      ...
      throw new Error(`Loop atomic write parent changed before commit: ${identity.path}`);
    }
  }
}
...
async function prepareContainedDirectoryChain(
  ...
  if (!isInside(lexicalRoot, lexicalDirectory)) {
    throw new Error(`Loop atomic write parent is outside its managed root: ${directory}`);
  }
  ...
  for (const segment of segments) {
    ...
    if (!isInside(chain[0].realPath, identity.realPath)) {
      throw new Error(`Loop atomic write parent resolves outside its managed root: ${cursor}`);
    }
    chain.push(identity);
  }
  await verifyDirectoryChain(chain);
  ...
  try {
    ...
    if (directoryChain) {
      ...
      if (
        ...
        throw new Error('Loop atomic write temporary file opened outside its managed parent');
      }
    }
    ...
    if (!sameFileIdentity(temporaryIdentity, await handle.stat())) {
      throw new Error('Loop atomic write temporary file changed while writing');
    }
    await handle.close();
    handle = undefined;
    ...
    if (directoryChain) {
      ...
      if (
        ...
        throw new Error('Loop atomic write temporary file changed before commit');
      }
    }

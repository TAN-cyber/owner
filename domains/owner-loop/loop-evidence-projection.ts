...
import type { LoopImplementationScope } from './loop-verification-scope.js';
import type { LoopReadableVerificationEvidenceEnvelope } from './loop-verification-evidence.js';
import type { LoopVerificationReceipt } from './loop-verification-receipt.js';

/**
 * Relative path of the human-readable evidence projection inside a Loop change.
 *
 * The projection is a read-only derivative of the content-addressed evidence in
 * the project-local Loop Runtime. It exists so that people (and developers debugging a
 ...
 */
export const LOOP_EVIDENCE_PROJECTION_REF = 'evidence.md';

const LOOP_EVIDENCE_PROJECTION_GENERATOR = 'owner-loop';

/**
 * Bounded projection limits. Mirrors the bounded-projection pattern used by
 * `projectLoopAcceptancePage`: the projection always has an upper bound and
 * reports truncation rather than silently dropping entries.
 */
export const LOOP_EVIDENCE_PROJECTION_LIMITS = Object.freeze({
  maxScopeChanges: 128,
...
function renderScopeSection(scope: LoopImplementationScope): string[] {
  const lines: string[] = [
  ...
  const limit = LOOP_EVIDENCE_PROJECTION_LIMITS.maxScopeChanges;
  ...
  const unresolvedLimit = LOOP_EVIDENCE_PROJECTION_LIMITS.maxUnresolvedReasons;
...
function renderVerificationSection(
  envelope: LoopReadableVerificationEvidenceEnvelope,
  ...
  const limit = LOOP_EVIDENCE_PROJECTION_LIMITS.maxAcceptanceEntries;
  ...
  if (receipts.length > 0) {
    ...
    const receiptLimit = LOOP_EVIDENCE_PROJECTION_LIMITS.maxReceipts;
...
function renderReceipt(receipt: LoopVerificationReceipt): string[] {
  const lines: string[] = [
 ...
 */
export function renderLoopEvidenceProjectionMarkdown(input: {
  change: string;
  phase: LoopChangeState['phase'];
  revision: number;
  scope: LoopImplementationScope | null;
  envelope: LoopReadableVerificationEvidenceEnvelope | null;
  receipts: readonly LoopVerificationReceipt[];
  generatedAt: string;
}): string {
  const lines: string[] = [
    '# Owner Loop Evidence Projection',
    '',
    `- Change: ${input.change}`,
    `- Phase: ${input.phase}`,
    ...
    '',
    `Generated-by: ${LOOP_EVIDENCE_PROJECTION_GENERATOR}`,
    '',
    '<!--',
    '  This file is a read-only projection of the content-addressed evidence under',
    '  .owner/runtime/loop directory. The Loop Runtime regenerates it on every evidence-bearing',
...
function loopEvidenceProjectionFile(paths: LoopProjectPaths, name: string): string {
  const changeDir = loopChangeDir(paths, name);
  return path.join(changeDir, ...LOOP_EVIDENCE_PROJECTION_REF.split('/'));
}
 ...
 */
export async function writeLoopEvidenceProjection(
  ...
  const scope = scopeRef ? await readLoopImplementationScope(paths, name, scopeRef) : null;
  const envelope = verificationRef
    ? await readLoopVerificationEvidence(paths, name, verificationRef)
    : null;

  const receiptRefs = envelope ? [...new Set(envelope.receiptRefs)].sort(sortText) : [];
  const receipts: LoopVerificationReceipt[] = [];
  for (const ref of receiptRefs) {
    try {
      receipts.push(await readLoopVerificationReceipt(paths, name, ref));
    } catch {
  ...
  const markdown = renderLoopEvidenceProjectionMarkdown({
    change: name,
    phase: state.phase,
  ...
  await atomicWriteText(loopEvidenceProjectionFile(paths, name), markdown, {
    containedRoot: loopChangeDir(paths, name),
  });
}

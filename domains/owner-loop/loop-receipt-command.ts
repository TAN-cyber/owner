import {
  ...
  MAX_LOOP_AUTOMATED_COMMAND_TIMEOUT_MS,
} from './loop-verification-receipt-runtime.js';
import { refreshLoopVerificationReceipts } from './loop-receipt-refresh.js';
import {
  assertNoArguments,
  configuredPaths,
  LoopUsageError,
...
export async function loopReceiptCommand(
  args: string[],
  projectRoot: string,
  ...
  if (subcommand === 'manual') {
    ...
    return success('receipt manual', issued, `Loop manual receipt: ${issued.ref}\n`);
  }
  if (subcommand === 'automated') {
    const separator = args.indexOf('--');
    if (separator < 0 || separator === args.length - 1) {
      throw new LoopUsageError('receipt automated requires -- <executable> [args...]');
    }
    ...
    const timeoutMs =
      timeoutText === undefined
        ...
        : /^[1-9]\d*$/u.test(timeoutText) &&
            ...
            Number(timeoutText) <= MAX_LOOP_AUTOMATED_COMMAND_TIMEOUT_MS
          ? Number(timeoutText)
          : null;
    if (timeoutMs === null) {
      throw new LoopUsageError(
        `--timeout-ms must be an integer from 1 through ${MAX_LOOP_AUTOMATED_COMMAND_TIMEOUT_MS}`,
      );
    }
    const issued = await issueLoopAutomatedCheckReceipt({
      paths,
      name,
      acceptanceIds,
    ...
    const lines = [`Loop automated receipt ${issued.receipt.status}: ${issued.ref}`];
    if (issued.recovery) {
  ...
  if (subcommand === 'refresh') {
    ...
    assertNoArguments(args);
    if (apply && dryRun) {
      throw new LoopUsageError('receipt refresh --apply and --dry-run cannot be combined');
    }
    const result = await refreshLoopVerificationReceipts({ paths, name, apply });
    const lines: string[] = [];
    ...
    if (result.requiresCheck.length > 0) {
      lines.push(
        `Re-run \`owner loop check ${name}\` to refresh ${result.requiresCheck.length} required-check receipt(s).`,
      );
    }
    if (result.applied) {
  ...
  throw new LoopUsageError(`Unknown receipt command: ${subcommand}`);
}

...
import { LoopArchivePreflightError, LoopSpecConflictError } from './loop-archive.js';
import {
  LoopBaselineIncompleteError,
  LoopChangeRevisionConflictError,
  LoopWorkspaceIsolationRequiredError,
} from './loop-change.js';
import { discoverLoopProject, loopProjectPaths } from './loop-paths.js';
import { readProjectConfig, resolveLoopProject } from './loop-config.js';
import { LoopReceiptScopeStaleError } from './loop-receipt-errors.js';
import { LoopVerificationReceiptBindingError } from './loop-verification-runtime.js';
import { LoopWorkspacePreparationError } from './loop-workspace-preparation.js';
import type { OwnerProjectConfig, LoopProjectPaths } from './loop-types.js';
export { USAGE } from './loop-cli-help.js';
...
export interface LoopCliErrorShape {
  code:
    | 'usage'
    | 'invalid-data'
    | 'blocked'
...
export interface DispatchResult {
  ...
  data?: unknown;
  text?: string;
  error?: LoopCliErrorShape;
}

export const LOOP_SHOW_MAX_SERIALIZED_BYTES = 10 * 1024 * 1024;

export class LoopUsageError extends Error {}

export function takeFlag(args: string[], name: string): boolean {
  const indexes = args.flatMap((value, index) => (value === name ? [index] : []));
  if (indexes.length > 1) throw new LoopUsageError(`${name} may only be provided once`);
...
export function takeOption(args: string[], name: string): string | undefined {
  ...
  if (value === undefined || value.startsWith('--')) {
    throw new LoopUsageError(`${name} requires a value`);
  }
  args.splice(index, 2);
  return value;
}
...
export function assertNoArguments(args: string[]): void {
  if (args.length > 0) throw new LoopUsageError(`Unexpected argument: ${args[0]}`);
}

export function requiredPositional(args: string[], label: string): string {
  const value = args.shift();
  if (!value || value.startsWith('--')) throw new LoopUsageError(`${label} is required`);
  return value;
}
...
export function languageOption(args: string[], fallback: 'en' | 'zh-CN' = 'en'): 'en' | 'zh-CN' {
  ...
  if (language !== 'en' && language !== 'zh-CN') {
    throw new LoopUsageError('--language must be en or zh-CN');
  }
  return language;
}
...
export function revisionOption(args: string[]): number | undefined {
  ...
  if (!/^[1-9]\d*$/u.test(value) || !Number.isSafeInteger(Number(value))) {
    throw new LoopUsageError('--expect-revision must be a positive integer');
  }
  return Number(value);
}
...
/**
 * Resolve the project root for a Loop invocation without allowing a host
...
export async function projectRootFrom(explicit: string | undefined): Promise<string> {
  return explicit
    ...
    : discoverLoopProject(process.cwd());
}
...
export async function configuredPaths(projectRoot: string): Promise<{
  ...
  const resolved = await resolveLoopProject({
...
export async function doctorPaths(projectRoot: string): Promise<LoopProjectPaths> {
  const config = await readProjectConfig(projectRoot);
  return loopProjectPaths(projectRoot, config?.loop.artifact_root ?? 'docs');
}
...
export function errorResult(command: string | null, error: unknown): DispatchResult {
  if (error instanceof LoopUsageError) {
    return {
      command,
      exitCode: 64,
  ...
  if (error instanceof LoopSpecConflictError) {
    return {
      command,
      exitCode: 73,
      data: {
  ...
  if (error instanceof LoopArchivePreflightError) {
    return {
      command,
      exitCode: 73,
  ...
  if (error instanceof LoopChangeRevisionConflictError) {
    return {
      command,
      exitCode: 73,
      data: {
  ...
  if (error instanceof LoopWorkspaceIsolationRequiredError) {
    return {
      command,
      exitCode: 73,
      data: {
  ...
  if (error instanceof LoopWorkspacePreparationError) {
    return {
      command,
      exitCode: 73,
  ...
  if (error instanceof LoopBaselineIncompleteError) {
    return {
      command,
      exitCode: 65,
      data: {
        ...
        supportedFixes: [
          'increase loop.snapshot.max_files, loop.snapshot.max_total_bytes, or loop.snapshot.max_duration_ms',
          'add an explicit loop.snapshot.exclude pattern for data outside implementation scope',
        ],
  ...
  if (error instanceof LoopReceiptScopeStaleError) {
    return {
      command,
      exitCode: 65,
      data: error.recovery,
...
export function render(result: DispatchResult, json: boolean): LoopCommandResult {
  if (json) {
    return {

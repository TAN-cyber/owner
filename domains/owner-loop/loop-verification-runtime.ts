...
import type {
  LoopArchiveEvidenceFact,
  LoopVerificationFreshness,
...
import type { LoopContractSnapshot } from './loop-contract.js';
...
export type LoopVerificationFreshnessFindingCode =
...
export interface LoopVerificationPreparation {
  ready: boolean;
  findingCodes: LoopVerificationFreshnessFindingCode[];
  envelope: LoopVerificationEvidenceEnvelope | null;
  evidenceRef: string | null;
  reportSnapshot: { hash: string; text: string } | null;
  /** Parsed report and acceptance trace reused after required-check execution. */
  preflight?: LoopVerificationPreflight;
   ...
   */
  receiptBindingFailures?: LoopReceiptBindingFailureDetail[];
}

export interface LoopVerificationPreflight {
  report: Awaited<ReturnType<typeof reportEvidence>>;
  trace: ReturnType<typeof buildLoopAcceptanceEvidenceTrace>;
}
 ...
 */
export interface LoopReceiptBindingFailureDetail {
  ref: string;
 ...
 */
export class LoopVerificationReceiptBindingError extends Error {
  readonly details: LoopReceiptBindingFailureDetail[];

  constructor(details: LoopReceiptBindingFailureDetail[]) {
    const summary =
      details.length === 0
        ? 'Loop verification receipt binding is invalid'
        : `Loop verification receipt binding is invalid (${details.length} receipt(s)): ${details
            .map(
              (d) =>
            ...
            .join(' | ')}`;
    super(summary);
    this.name = 'LoopVerificationReceiptBindingError';
    this.details = details;
  }
}

export interface LoopVerificationFreshnessInspection {
...
function loopRootRef(paths: LoopProjectPaths): string {
  const value = path.relative(paths.projectRoot, paths.loopRoot).replaceAll('\\', '/');
  if (!value || value === '..' || value.startsWith('../') || path.posix.isAbsolute(value)) {
    throw new Error('Loop root is outside the project root');
  }
  return value;
}
...
async function inspectCurrentScopeFacts(options: {
  ...
  if (!options.state.implementation_scope) {
    throw new Error('Loop change has no implementation scope');
  }
  const bundle = await readLoopImplementationScopeBundle(
    options.paths,
    options.state.name,
  ...
  const findingCodes: LoopVerificationFreshnessFindingCode[] = [];
...
export interface LoopImplementationScopeFreshnessInspection {
 ...
 */
export async function inspectLoopImplementationScopeFreshness(options: {
  paths: LoopProjectPaths;
  state: LoopChangeState;
  now?: Date;
}): Promise<LoopImplementationScopeFreshnessInspection> {
  if (!options.state.implementation_scope) {
...
async function reportEvidence(options: {
  ...
  hash: string;
  text: string;
  entries: LoopAcceptanceEvidenceEntry[];
}> {
  const report = await readLoopBoundedTextFile({
    root: loopChangeDir(options.paths, options.state.name),
  ...
  return {
    ...
    hash: report.hash,
    text: report.text,
    entries: parseLoopVerificationMachineBlock(report.text),
  };
}
...
}): LoopVerificationFreshnessFindingCode[] {
  const { receipt, implementationScope } = options;
  const codes: LoopVerificationFreshnessFindingCode[] = [];
...
function verificationReceiptBindings(options: {
  ...
  implementationScope: LoopImplementationScopeBundle;
  sourceRevision?: number;
}): LoopVerificationReceiptBindings {
  return {
    ...
    artifactHash: loopArtifactBindingHash(options.implementationScope.scope.declaredArtifacts),
  };
}
...
async function validateTypedReceipt(options: {
  ...
  state: LoopChangeState;
  ref: string;
  expectedBindings: LoopVerificationReceiptBindings;
  acceptanceId?: string;
  ...
  const receipt = await readLoopVerificationReceipt(options.paths, options.state.name, options.ref);
  ...
  const bindingComparison = compareLoopReceiptBindings(receipt, options.expectedBindings);
  ...
  if (mismatches.length > 0) {
    throw new LoopVerificationReceiptBindingError([
      {
        ref: options.ref,
        role: options.role,
  ...
  if (options.result === 'pass' && receipt.status !== 'passed') {
    throw new Error(`Loop verification receipt is ${receipt.status}`);
  }
  const check = await validateLoopStaticReceiptDependency({
    paths: options.paths,
    state: options.state,
    receipt,
  });
  if (check) {
    ...
    if (codes.length > 0) {
      throw new Error(`Loop static receipt dependency is invalid: ${codes.join(', ')}`);
    }
  }
  return receipt;
}
...
async function validateCurrentReceiptGraph(options: {
  ...
  const collectedFailures: LoopReceiptBindingFailureDetail[] = [];
  for (const ref of options.requiredReceiptRefs) {
    const receipt = await validateTypedReceipt({
      ...
      if (error instanceof LoopVerificationReceiptBindingError) {
    ...
    if (receipt.kind !== 'static-inspection') {
      throw new Error('Loop required-check evidence must be a static-inspection receipt');
    }
  }
  for (const entry of options.trace.entries) {
    ...
    for (const ref of entry.evidenceRefs) {
      ...
      if (receipt.kind !== 'automated-check' && receipt.kind !== 'manual-evidence') {
        throw new Error('Loop acceptance evidence must be automated-check or manual-evidence');
      }
      if (entry.status === 'failed' && receipt.status === 'passed') {
        throw new Error('Loop failed acceptance evidence must reference a non-passing receipt');
      }
    }
  }
  if (collectedFailures.length > 0) {
    throw new LoopVerificationReceiptBindingError(collectedFailures);
  }
}

export interface LoopVerificationEvidenceOptions {
  paths: LoopProjectPaths;
  state: LoopChangeState;
  result: 'pass' | 'fail';
  reportRef: string;
...
/** Build and validate an envelope without mutating the Loop evidence store. */
export async function inspectLoopVerificationEvidence(
  options: LoopVerificationEvidenceOptions,
): Promise<LoopVerificationPreparation> {
  if (options.state.phase !== 'verify') {
    throw new Error(`Loop verification evidence requires Verify, got ${options.state.phase}`);
  }
  ...
  if (options.result === 'pass' && !options.receiptRef && options.requireReceipt !== false) {
    throw new Error('Loop passing verification requires a typed required-check receipt');
  }
  const requiredReceiptRefs = options.receiptRef ? [options.receiptRef] : [];
  const trace =
    options.preflight?.trace ??
    buildLoopAcceptanceEvidenceTrace(facts.contract.acceptance, report.entries, {
      loopRootRef: loopRootRef(options.paths),
  ...
  if (
    ...
    throw new Error(
      'Loop passing verification cannot include failed or missing acceptance criteria',
    );
  }
  try {
  ...
  const allowance = options.state.partial_allowance
    ? await readLoopPartialAllowance(
        options.paths,
        options.state.name,
  ...
  const evidenceRef = loopEvidenceRef('verifications', envelope.envelopeHash);
  return {
    ready: true,
    findingCodes: [],
    envelope,
...
export async function persistLoopVerificationEvidence(options: {
  paths: LoopProjectPaths;
  state: LoopChangeState;
  ...
  if (
    ...
    throw new Error('Loop verification evidence is not ready to persist');
  }
  ...
  if (evidenceRef !== options.preparation.evidenceRef) {
    throw new Error('Loop verification evidence persistence ref changed');
  }
}

/** Backwards-compatible one-shot API for callers that explicitly want durable evidence. */
export async function prepareLoopVerificationEvidence(
  options: LoopVerificationEvidenceOptions,
): Promise<LoopVerificationPreparation> {
  const preparation = await inspectLoopVerificationEvidence(options);
...
function emptyEvidence(
  result: LoopChangeState['verification_result'],
  freshness: LoopVerificationFreshness,
): LoopArchiveEvidenceFact {
  return {
    result,
...
export async function inspectLoopVerificationFreshness(options: {
  paths: LoopProjectPaths;
  state: LoopChangeState;
  now?: Date;
}): Promise<LoopVerificationFreshnessInspection> {
  if (
    !options.state.implementation_scope ||
  ...
  try {
    ...
    if (envelope.schema !== 'owner.loop.verification-evidence.v2') {
      return {
        freshness: 'stale',
    ...
    const freshness: LoopVerificationFreshness =

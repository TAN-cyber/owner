...
import type { LoopContractSpecSnapshot } from './loop-contract.js';
import type { LoopDeclaredArtifact } from './loop-verification-scope.js';

export const LOOP_CONFLICT_RADAR_SCHEMA = 'owner.loop.conflict-radar.v1' as const;

export const LOOP_CONFLICT_RADAR_LIMITS = Object.freeze({
  maxChanges: 32,
...
const RADAR_HASH_TAG = 'owner.loop.conflict-radar.v1';
const SIGNAL_SET_HASH_TAG = 'owner.loop.conflict-radar-signals.v1';
...
export type LoopConflictClassification = 'definite-conflict' | 'possible-overlap' | 'disjoint';

export type LoopWorkspaceRelationship = 'same' | 'different' | 'unknown';

export interface LoopConflictRadarChangeInput {
  name: string;
  revision: number;
  specs: readonly Pick<LoopContractSpecSnapshot, 'capability' | 'operation' | 'baseHash'>[];
...
export interface LoopCapabilityConflictSignal {
  kind: 'capability';
  certainty: Exclude<LoopConflictClassification, 'disjoint'>;
  capability: string;
  leftOperation: LoopContractSpecSnapshot['operation'];
  rightOperation: LoopContractSpecSnapshot['operation'];
  leftBaseHash: string | null;
  rightBaseHash: string | null;
}

export interface LoopArtifactConflictSignal {
  kind: 'artifact';
  certainty: Exclude<LoopConflictClassification, 'disjoint'>;
  leftArtifact: LoopDeclaredArtifact;
  rightArtifact: LoopDeclaredArtifact;
}

export type LoopConflictSignal = LoopCapabilityConflictSignal | LoopArtifactConflictSignal;

export interface LoopConflictRelationship {
  left: string;
  right: string;
  classification: LoopConflictClassification;
  workspaceRelationship: LoopWorkspaceRelationship;
  signalCount: number;
  signalHash: string;
  signals: LoopConflictSignal[];
  signalsTruncated: boolean;
}

export interface LoopConflictRadarSnapshot {
  schema: typeof LOOP_CONFLICT_RADAR_SCHEMA;
  ...
  relationships: LoopConflictRelationship[];
...
interface NormalizedSpec {
  capability: string;
  operation: LoopContractSpecSnapshot['operation'];
...
type FullRelationship = LoopConflictRelationship;
...
function normalizedProjectPath(value: unknown, label: string): string {
  const candidate = boundedText(value, label, LOOP_CONFLICT_RADAR_LIMITS.maxArtifactPathBytes);
...
function normalizeSpec(value: LoopConflictRadarChangeInput['specs'][number], label: string) {
  const record = strictRecord(value, SPEC_KEYS, label);
  const capability = boundedText(
    record.capability,
    `${label} capability`,
    LOOP_CONFLICT_RADAR_LIMITS.maxCapabilityBytes,
  );
...
function normalizeArtifact(value: LoopDeclaredArtifact, label: string): LoopDeclaredArtifact {
...
function normalizeChange(value: LoopConflictRadarChangeInput, index: number): NormalizedChange {
  const record = strictRecord(value, CHANGE_KEYS, `Conflict radar change ${index}`);
  const name = boundedText(
    record.name,
    `Conflict radar change ${index} name`,
    LOOP_CONFLICT_RADAR_LIMITS.maxNameBytes,
  );
  ...
  if (record.specs.length > LOOP_CONFLICT_RADAR_LIMITS.maxSpecsPerChange) {
  ...
  if (record.declaredArtifacts.length > LOOP_CONFLICT_RADAR_LIMITS.maxArtifactsPerChange) {
    throw new Error(`Conflict radar change ${name} exceeds the declared artifact budget`);
  }

  const specs = (record.specs as LoopConflictRadarChangeInput['specs'])
    .map((spec, specIndex) =>
  ...
  const declaredArtifacts = (record.declaredArtifacts as LoopDeclaredArtifact[])
...
function normalizeChanges(input: readonly LoopConflictRadarChangeInput[]): NormalizedChange[] {
  if (!Array.isArray(input)) throw new Error('Loop conflict radar input must be an array');
  if (input.length > LOOP_CONFLICT_RADAR_LIMITS.maxChanges) {
    throw new Error('Loop conflict radar exceeds the change budget');
  }
  const changes = input
    .map(normalizeChange)
    .sort((left, right) => compareText(left.name, right.name));
  if (new Set(changes.map((change) => change.name)).size !== changes.length) {
    throw new Error('Loop conflict radar contains duplicate change names');
  }
  const specCount = changes.reduce((total, change) => total + change.specs.length, 0);
  if (specCount > LOOP_CONFLICT_RADAR_LIMITS.maxTotalSpecs) {
    throw new Error('Loop conflict radar exceeds the total spec budget');
  }
  const artifactCount = changes.reduce(
    (total, change) => total + change.declaredArtifacts.length,
    0,
  );
  if (artifactCount > LOOP_CONFLICT_RADAR_LIMITS.maxTotalArtifacts) {
    throw new Error('Loop conflict radar exceeds the total declared artifact budget');
  }
  return changes;
}
...
function artifactOverlap(
  left: LoopDeclaredArtifact,
  right: LoopDeclaredArtifact,
): LoopArtifactConflictSignal | null {
  const leftOwnsRight =
    left.kind === 'directory' &&
...
function relationship(left: NormalizedChange, right: NormalizedChange): FullRelationship {
  ...
  const addSignal = (signal: LoopConflictSignal) => {
    ...
    if (visibleSignals.length < LOOP_CONFLICT_RADAR_LIMITS.maxSignalsPerRelationship) {
...
function classificationRank(value: LoopConflictClassification): number {
...
function snapshot(
  ...
  return {
    schema: LOOP_CONFLICT_RADAR_SCHEMA,
    workspaceIdentityAdvisoryOnly: true,
...
/**
 * Compare already-visible changes from one physical Loop root.
 *
 ...
 */
export function buildLoopConflictRadar(
  input: readonly LoopConflictRadarChangeInput[],
): LoopConflictRadarSnapshot {
  ...
  let relationships = allRelationships
    .slice(0, LOOP_CONFLICT_RADAR_LIMITS.maxRelationships)
    .map((value) => ({ ...value, signals: [...value.signals] }));
  let result = snapshot(changes, allRelationships, relationships, radarHash);
  while (
    relationships.length > 0 &&
    byteLength(JSON.stringify(result)) > LOOP_CONFLICT_RADAR_LIMITS.maxSerializedBytes
  ) {
    relationships = relationships.slice(0, -1);
    result = snapshot(changes, allRelationships, relationships, radarHash);
  }
  if (byteLength(JSON.stringify(result)) > LOOP_CONFLICT_RADAR_LIMITS.maxSerializedBytes) {
    throw new Error('Loop conflict radar summary exceeds its serialized output budget');
  }
  return result;
}

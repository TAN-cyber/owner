...
import { parseLoopCheckReceipt } from './loop-check-receipt-model.js';
import { inspectLoopChange } from './loop-change.js';
import { readLoopCheckpointJournal } from './loop-checkpoint-storage.js';
import { readProjectConfig } from './loop-config.js';
import {
  MAX_LOOP_EVIDENCE_DOCUMENT_BYTES,
  loopEvidenceRef,
  loopReportEvidenceRef,
  type LoopEvidenceKind,
...
import { inspectPendingLoopSchemaMigration } from './loop-schema-migration.js';
...
import { parseLoopVerificationReceipt } from './loop-verification-receipt.js';

export const LOOP_EVIDENCE_RETENTION_POLICY = Object.freeze({
...
type LoopManagedEvidenceKind = (typeof MANAGED_KINDS)[number];

interface LoopFileIdentity {
  dev: number;
  ino: number;
  birthtimeMs: number;
  ctimeMs: number;
...
interface LoopDirectoryIdentity extends LoopFileIdentity {
  path: string;
  realPath: string;
}

interface LoopEvidenceDocument {
  ref: string;
  kind: LoopManagedEvidenceKind;
  change: string;
  changeRoot: string;
  file: string;
  size: number;
  mtimeMs: number;
  identity: LoopFileIdentity;
  directoryChain: LoopDirectoryIdentity[];
  dependencies: string[];
}

export interface LoopEvidenceRetentionHooks {
...
export interface LoopEvidenceRetentionOptions {
  ...
  deferAll?: boolean;
  hooks?: LoopEvidenceRetentionHooks;
}

interface LoopEvidenceRetentionPlan {
  candidates: LoopEvidenceDocument[];
  candidateBytes: number;
}

interface LoopCleanupRecoveryResult {
  findings: LoopDoctorFinding[];
  pending: boolean;
}

function sameObjectIdentity(
  left: LoopFileIdentity,
  right: Pick<import('node:fs').Stats, keyof LoopFileIdentity>,
): boolean {
...
function fileIdentity(stat: import('node:fs').Stats): LoopFileIdentity {
  return {
    dev: stat.dev,
    ino: stat.ino,
...
function portableProjectRef(paths: LoopProjectPaths, target: string): string {
  ...
  if (
    ...
    throw new Error('Loop evidence retention finding path is outside the project');
  }
  return relative.split(path.sep).join('/');
}

function evidenceRootPath(paths: LoopProjectPaths, name: string): string {
  return path.join(loopChangeRuntimeDir(paths, name), 'evidence');
}

function evidenceRootRef(paths: LoopProjectPaths, name: string): string {
...
async function verifyDirectoryChain(chain: readonly LoopDirectoryIdentity[]): Promise<void> {
  for (const identity of chain) {
    ...
    if (
      ...
      throw new Error(`Loop evidence retention parent changed: ${identity.path}`);
    }
  }
}

function refForKind(kind: LoopManagedEvidenceKind, hash: string): string {
  return kind === 'check-receipts'
    ? `runtime/evidence/check-receipts/${hash}.json`
    : loopEvidenceRef(kind as LoopEvidenceKind, hash);
}
...
function parseDocument(
  ...
  if (kind === 'snapshots') {
    return {
      canonical: parseLoopSnapshotProjection(value, hash),
      dependencies: [],
    };
  }
  if (kind === 'scopes') {
    const scope = parseLoopImplementationScope(value);
    if (scope.scopeHash !== hash) throw new Error('Loop scope filename does not match its hash');
    return {
      canonical: scope,
  ...
  if (kind === 'allowances') {
    ...
    if (allowance.change !== expectedChange || allowance.allowanceHash !== hash) {
      throw new Error('Loop allowance filename/change does not match its content');
    }
    return {
      canonical: allowance,
      dependencies: [loopEvidenceRef('scopes', allowance.scopeHash)],
    };
  }
  if (kind === 'verifications') {
    ...
    if (evidence.change !== expectedChange || evidence.envelopeHash !== hash) {
      throw new Error('Loop verification filename/change does not match its content');
    }
    ...
    return {
      ...
      dependencies: [
        ...
        loopReportEvidenceRef(evidence.reportHash),
  ...
  if (kind === 'reports') {
    ...
    if (
      ...
      throw new Error('Loop report evidence filename/hash does not match its content');
    }
    return { canonical: report, dependencies: [] };
  }
  if (kind === 'receipts') {
    const receipt = parseLoopVerificationReceipt(value);
    if (receipt.bindings.change !== expectedChange || receipt.receiptHash !== hash) {
      throw new Error('Loop verification receipt filename/change does not match its content');
    }
    return {
      canonical: receipt,
  ...
  const receipt = parseLoopCheckReceipt(value);
  if (receipt.change !== expectedChange || receipt.receiptHash !== hash) {
    throw new Error('Loop check receipt filename/change does not match its content');
  }
...
async function readCanonicalDocument(options: {
  ...
  name: string;
  directoryChain: LoopDirectoryIdentity[];
}): Promise<LoopEvidenceDocument> {
  const before = await fs.lstat(options.file);
  if (!before.isFile() || before.isSymbolicLink()) {
    throw new Error(`Loop evidence entry is not a regular file: ${options.ref}`);
  }
  const maximumBytes =
    options.kind === 'check-receipts'
      ? MAX_LOOP_CHECK_RECEIPT_BYTES
      : MAX_LOOP_EVIDENCE_DOCUMENT_BYTES;
  if (before.size > maximumBytes) {
    throw new Error(`Loop evidence entry exceeds its byte budget: ${options.ref}`);
  }
  const beforeRealPath = await fs.realpath(options.file);
  if (!isInsidePath(options.directoryChain[0].realPath, beforeRealPath)) {
    throw new Error(`Loop evidence entry resolves outside its change: ${options.ref}`);
  }
  const openFlags =
  ...
  const handle = await fs.open(options.file, openFlags).catch((error: NodeJS.ErrnoException) => {
    if (error.code === 'ELOOP' || error.code === 'ENXIO') {
      throw new Error(`Loop evidence entry became unsafe while opening: ${options.ref}`);
    }
    throw error;
  });
  try {
    ...
    if (
      ...
      throw new Error(`Loop evidence entry changed while opening: ${options.ref}`);
    }
    const chunks: Buffer[] = [];
    ...
    if (
      ...
      throw new Error(`Loop evidence entry changed while reading: ${options.ref}`);
    }
    ...
    try {
      ...
      throw new Error(`Loop evidence entry is not valid JSON: ${options.ref}`, {
        cause: error,
      });
    }
    const parsed = parseDocument(options.kind, options.hash, json, options.name);
    if (text !== JSON.stringify(parsed.canonical, null, 2) + '\n') {
      throw new Error(`Loop evidence entry is not canonically serialized: ${options.ref}`);
    }
    return {
      ref: options.ref,
      kind: options.kind,
...
async function inspectCleanupQuarantines(options: {
  ...
  repair: boolean;
  hooks: LoopEvidenceRetentionHooks;
}): Promise<LoopCleanupRecoveryResult> {
  const runtimeRoot = loopChangeRuntimeDir(options.paths, options.name);
  const changeRoot = runtimeRoot;
  const evidenceRoot = evidenceRootPath(options.paths, options.name);
  await resolveContainedLoopPath(loopStorageRoot(options.paths, evidenceRoot), evidenceRoot);
  let rootEntries;
  try {
  ...
  const recoveries: Array<{
    ...
    original: string;
    quarantine: string;
    document: LoopEvidenceDocument;
    action: 'restore-original' | 'finish-linked-recovery';
    originalDocument: LoopEvidenceDocument | null;
  }> = [];
  for (const kind of MANAGED_KINDS) {
    ...
    for (const [originalName, entriesForOriginal] of [...byOriginal.entries()].sort(
      ...
      if (entriesForOriginal.length !== 1) {
        throw new Error(
          `Loop evidence cleanup has multiple quarantines for ${kind}/${originalName}`,
        );
      }
      const entry = entriesForOriginal[0];
      if (!entry.isFile() || entry.isSymbolicLink()) {
        throw new Error(
          `Loop evidence cleanup quarantine is not a regular file: ${kind}/${entry.name}`,
        );
      }
      const original = path.join(directory, originalName);
      const hash = HASH_FILE_PATTERN.exec(originalName)?.[1];
      if (!hash) throw new Error(`Loop evidence cleanup quarantine name is invalid: ${entry.name}`);
      ...
      findings: [
        {
          ...
          message: boundedMessage(
            `Loop evidence cleanup has ${refs.length} safely recoverable quarantined document(s) for ${options.name}: ${boundedRefs(refs)}`,
          ),
  ...
  for (const recovery of recoveries.sort((left, right) => compareText(left.ref, right.ref))) {
    ...
    if (recovery.action === 'finish-linked-recovery') {
      ...
      if (
        ...
        throw new Error(
          `Loop evidence cleanup original or quarantine changed during recovery: ${recovery.ref}`,
        );
      }
    ...
    if (!(await pathIsMissing(recovery.original))) {
      throw new Error(`Loop evidence cleanup original appeared during recovery: ${recovery.ref}`);
    }
    ...
    if (
      ...
      throw new Error(`Loop evidence cleanup quarantine changed during recovery: ${recovery.ref}`);
    }
    ...
    if (
      ...
      throw new Error(`Loop evidence cleanup recovery identity changed: ${recovery.ref}`);
    }
    await fs.rm(recovery.quarantine);
    await verifyDirectoryChain(recovery.document.directoryChain);
  }
    ...
    findings: [
      {
        ...
        message: boundedMessage(
          `Recovered ${refs.length} interrupted Loop evidence cleanup document(s) for ${options.name}: ${boundedRefs(refs)}`,
        ),
        path: evidenceRootRef(options.paths, options.name),
      },
    ],
...
async function scanEvidenceStore(
  ...
  await resolveContainedLoopPath(loopStorageRoot(paths, evidenceRoot), evidenceRoot);
  let rootEntries;
  try {
    const rootChain = await captureDirectoryChain(runtimeRoot, evidenceRoot);
    rootEntries = await fs.readdir(evidenceRoot, { withFileTypes: true });
  ...
  const unknownRootEntries = rootEntries
    .filter(
      (entry) =>
        !MANAGED_KINDS.includes(entry.name as LoopManagedEvidenceKind) ||
        !entry.isDirectory() ||
  ...
  if (unknownRootEntries.length > 0) {
    throw new Error(
      `Loop evidence root has unknown or special entries: ${boundedRefs(unknownRootEntries)}`,
    );
  }

  const documents: LoopEvidenceDocument[] = [];
  let scannedBytes = 0;
  for (const kind of MANAGED_KINDS) {
    if (!rootEntries.some((entry) => entry.name === kind)) continue;
    const directory = path.join(evidenceRoot, kind);
    ...
    for (const entry of entries) {
      if (documents.length >= LOOP_EVIDENCE_RETENTION_POLICY.maxDocuments) {
        throw new Error(
          `Loop evidence store exceeds ${LOOP_EVIDENCE_RETENTION_POLICY.maxDocuments} documents`,
        );
      }
      const match = HASH_FILE_PATTERN.exec(entry.name);
      if (!match || !entry.isFile() || entry.isSymbolicLink()) {
        throw new Error(
          `Loop evidence kind ${kind} has an unknown or special entry: ${entry.name}`,
        );
      }
      ...
      if (scannedBytes > LOOP_EVIDENCE_RETENTION_POLICY.maxScannedBytes) {
        throw new Error(
          `Loop evidence store exceeds ${LOOP_EVIDENCE_RETENTION_POLICY.maxScannedBytes} scanned bytes`,
        );
      }
...
function closeDependencies(
  ...
  documents: ReadonlyMap<string, LoopEvidenceDocument>,
): Set<string> {
  ...
  while (pending.length > 0) {
    ...
    if (!document) throw new Error(`Loop evidence dependency is missing: ${ref}`);
    retained.add(ref);
...
function dependentsBeforeDependencies(candidates: LoopEvidenceDocument[]): LoopEvidenceDocument[] {
  ...
  const ready = [...dependencyCounts]
    ...
    .map(([ref]) => ref)
    .sort(compareText);
  const ordered: LoopEvidenceDocument[] = [];
  ...
  if (ordered.length !== candidates.length) {
    throw new Error('Loop evidence candidate dependency graph contains a cycle');
  }
  return ordered;
}

function planRetention(
  documents: LoopEvidenceDocument[],
  rootRefs: readonly string[],
  nowMs: number,
): LoopEvidenceRetentionPlan {
  const byRef = new Map(documents.map((document) => [document.ref, document]));
  if (byRef.size !== documents.length) throw new Error('Loop evidence store has duplicate refs');
  // A damaged orphan is not safe to reason about either: every managed dependency must exist.
  for (const document of documents) {
    for (const dependency of document.dependencies) {
      if (!byRef.has(dependency)) {
        throw new Error(`Loop evidence dependency is missing: ${dependency}`);
      }
    }
  }
  ...
  for (const document of documents) {
    if (nowMs - document.mtimeMs < LOOP_EVIDENCE_RETENTION_POLICY.minimumAgeMs) {
  ...
  for (const kind of MANAGED_KINDS) {
    const newestUnreferenced = documents
      ...
      .slice(0, LOOP_EVIDENCE_RETENTION_POLICY.keepLatestUnreferencedPerKind);
  ...
  const candidates = documents
    .filter(
      (document) =>
        ...
        nowMs - document.mtimeMs >= LOOP_EVIDENCE_RETENTION_POLICY.minimumAgeMs,
    )
...
function boundedRefs(refs: readonly string[]): string {
  const shown = refs.slice(0, LOOP_EVIDENCE_RETENTION_POLICY.maxReportedRefs);
  const omitted = refs.length - shown.length;
  return `${shown.join(', ')}${omitted > 0 ? `, ... (${omitted} more)` : ''}`;
}

function boundedMessage(message: string): string {
  const maximum = LOOP_EVIDENCE_RETENTION_POLICY.maxReportMessageBytes;
...
function summaryFinding(
  name: string,
  plan: LoopEvidenceRetentionPlan,
  repaired: boolean,
  evidenceRoot: string,
): LoopDoctorFinding {
  ...
  return {
    ...
    message: boundedMessage(
      `${repaired ? 'Removed' : 'Found'} ${plan.candidates.length} old unreferenced Loop evidence document(s) (${plan.candidateBytes} bytes) for ${name}: ${boundedRefs(refs)}`,
    ),
    path: evidenceRoot,
  };
}
...
async function deleteCandidate(
  candidate: LoopEvidenceDocument,
  hooks: LoopEvidenceRetentionHooks,
): Promise<void> {
  ...
  if (
    ...
    throw new Error(`Loop evidence candidate changed before cleanup: ${candidate.ref}`);
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error(`Loop evidence candidate changed during cleanup: ${candidate.ref}`);
    }
...
async function hasPendingRelocationOrArchive(paths: LoopProjectPaths): Promise<boolean> {
  const config = await readProjectConfig(paths.projectRoot);
  if (config?.loop.pending_root_move) return true;
  let entries;
  try {
...
async function changeNames(paths: LoopProjectPaths, requested?: string): Promise<string[]> {
...
async function retentionDeferred(paths: LoopProjectPaths, name: string): Promise<boolean> {
  ...
  const [transition, migration, checkpoint] = await Promise.all([
    inspectPendingLoopTransition(paths, name),
    inspectPendingLoopSchemaMigration(paths, name),
    readLoopCheckpointJournal(paths, name),
  ]);
...
async function collectChangeRetention(options: {
  ...
  const deleted: LoopEvidenceDocument[] = [];
    ...
    try {
      ...
      return [
          ...
          : []),
        {
          ...
          message: boundedMessage(
            `Loop evidence cleanup stopped safely for ${options.name}: ${(error as Error).message}`,
          ),
          path: evidenceRef,
        },
      ];
...
/**
 * Inspect or explicitly prune old unreferenced Loop-owned evidence.
 *
 ...
 */
export async function inspectLoopEvidenceRetention(
  options: LoopEvidenceRetentionOptions,
): Promise<LoopDoctorFinding[]> {
  if (options.deferAll || (await hasPendingRelocationOrArchive(options.paths))) return [];
  const now = options.now ?? new Date();
  if (Number.isNaN(now.getTime())) throw new Error('Loop evidence retention time is invalid');
  const run = async (repair: boolean): Promise<LoopDoctorFinding[]> => {
    ...
    for (const name of await changeNames(options.paths, options.name)) {
      try {
        ...
        findings.push({
          ...
          message: boundedMessage(
            `Loop evidence retention refused to clean ${name}: ${(error as Error).message}`,
          ),
  ...
  try {
    return await withLoopMutationLock(options.paths, 'prune Loop evidence', () => run(true));
  } catch (error) {
    return [
      {
        severity: 'info',
        ...
        message: boundedMessage(
          `Loop evidence cleanup was deferred until the project mutation lock is available: ${(error as Error).message}`,
        ),
      },
    ];
  }

...
import { loopSelectionFile } from './loop-selection.js';
import type { LoopChangeState, LoopProjectPaths } from './loop-types.js';
import type { LoopWorkspaceFinish, LoopWorkspaceIdentityV3 } from './loop-workspace.js';
import { inspectLoopWorkspaceBinding } from './loop-workspace.js';

export interface LoopWorkspaceFinishPlan {
...
export interface LoopWorkspaceFinishResult {
  action: LoopWorkspaceFinish;
...
export class LoopWorkspaceFinishError extends Error {
  constructor(readonly result: LoopWorkspaceFinishResult) {
    super(result.message ?? 'Loop workspace finish is blocked');
    this.name = 'LoopWorkspaceFinishError';
  }
}

type LoopWorkspaceFinishState =
  | Pick<LoopChangeState, 'name' | 'spec_changes'>
  | Pick<LoopPortableState, 'name' | 'spec_changes'>;
...
function portableRelative(projectRoot: string, target: string): string {
  ...
  if (!relative || relative === '..' || relative.startsWith('../') || path.isAbsolute(relative)) {
    throw new Error(`Loop workspace finish path escaped the project: ${target}`);
  }
  return relative;
}
...
function assertGitIdentity(projectRoot: string): void {
  ...
  throw new Error('Loop workspace finish requires configured Git user.name and user.email');
}
...
function assertCommandAvailable(command: string, args: readonly string[]): void {
  try {
    ...
    throw new Error(`${command} is required for the selected Loop workspace finish`, {
      cause: error,
    });
  }
}

export async function prepareLoopWorkspaceFinish(options: {
  paths: LoopProjectPaths;
  state: LoopChangeState;
  workspace: LoopWorkspaceIdentityV3;
}): Promise<LoopWorkspaceFinishPlan | null> {
  ...
  if (!workspace.finish || !workspace.changeBranch || !workspace.targetBranch) {
    throw new Error('Loop isolated workspace finish is not persisted');
  }
  if (workspace.changeBranch === workspace.targetBranch) {
    throw new Error('Loop change and target branches must be different for workspace finish');
  }
  const inspection = await inspectLoopWorkspaceBinding({ paths, identity: workspace });
  if (inspection.state !== 'aligned') {
    throw new Error(`Loop workspace finish is blocked: ${inspection.message ?? inspection.code}`);
  }
  const context = inspectGitWorktree(paths.projectRoot);
  if (!context.primaryWorktreeRoot) {
    throw new Error('Loop workspace finish requires a registered Git worktree');
  }
  assertGitIdentity(paths.projectRoot);
  const allowedBeforeArchive = [portableRelative(paths.projectRoot, loopSelectionFile(paths))];
  const unrelated = gitStatusPaths(paths.projectRoot).filter(
    (candidate) => !pathCovered(candidate, allowedBeforeArchive),
  );
  if (unrelated.length > 0) {
    throw new Error(
      `Loop workspace finish is blocked until change-owned implementation and artifacts are committed; remaining paths: ${unrelated.slice(0, 20).join(', ')}${unrelated.length > 20 ? ', ...' : ''}`,
    );
  }
  const targetRoot =
  ...
  if (workspace.finish === 'merge' && workspace.isolation === 'worktree') {
    if (!targetRoot) {
      throw new Error(
        `Loop merge finish requires a registered worktree on target branch ${workspace.targetBranch}`,
      );
    }
    if (!gitWorktreeIsClean(targetRoot)) {
      throw new Error(`Loop merge target worktree is not clean: ${targetRoot}`);
    }
  }
  const remote =
 ...
 */
export async function prepareLoopPortableWorkspaceFinish(options: {
  paths: LoopProjectPaths;
  ...
  if (!context.primaryWorktreeRoot || context.currentBranch !== workspace.change_branch) {
    throw new Error(
      `Loop workspace finish requires branch ${workspace.change_branch} in the current registered worktree`,
    );
  }
  assertGitIdentity(paths.projectRoot);
  const allowedBeforeArchive = [portableRelative(paths.projectRoot, loopSelectionFile(paths))];
  if (options.archiveDir) {
    allowedBeforeArchive.push(
      portableRelative(paths.projectRoot, loopChangeDir(paths, state.name)),
  ...
  if (workspace.finish === 'merge' && workspace.isolation === 'worktree') {
    if (!targetRoot) {
      throw new Error(
        `Loop merge finish requires a registered worktree on target branch ${workspace.target_branch}`,
      );
    }
    if (!gitWorktreeIsClean(targetRoot)) {
      throw new Error(`Loop merge target worktree is not clean: ${targetRoot}`);
...
function baseResult(plan: LoopWorkspaceFinishPlan): LoopWorkspaceFinishResult {
  return {
    action: plan.finish,
...
export async function finishArchivedLoopWorkspace(options: {
  paths: LoopProjectPaths;
  state: LoopWorkspaceFinishState;
  name: string;
  archiveDir: string;
  transactionId: string;
  plan: LoopWorkspaceFinishPlan;
}): Promise<LoopWorkspaceFinishResult> {
  const result = baseResult(options.plan);
  let switchedMergeRoot: string | null = null;
  try {
    const allowedPaths = [
      portableRelative(options.paths.projectRoot, loopChangeDir(options.paths, options.name)),
      ...
      portableRelative(options.paths.projectRoot, loopSelectionFile(options.paths)),
    ];
    ...
    if (unexpected.length > 0) {
      throw new Error(
        `Loop Archive produced or encountered paths outside the authorized finish scope: ${unexpected.slice(0, 20).join(', ')}${unexpected.length > 20 ? ', ...' : ''}`,
      );
    }
    ...
    throw new LoopWorkspaceFinishError(result);
  }
}

...
import { parseLoopPortableState, readLoopPortableState } from './loop-portable-state.js';
import type {
...
export const LOOP_CHILDREN_FILE = 'children.yaml';
export const LOOP_CHILDREN_SCHEMA = 'owner.loop.children.v1' as const;
...
export interface LoopChildDefinition {
  name: string;
  depends_on: string[];
  covers: string[];
}

export interface LoopChildrenContract {
  schema: typeof LOOP_CHILDREN_SCHEMA;
  children: LoopChildDefinition[];
}

export interface LoopChildrenDocument {
  contract: LoopChildrenContract;
  size: number;
}

export type LoopChildDerivedStatus = 'pending' | 'ready' | 'active' | 'done' | 'blocked';

export interface LoopChildStatusProjection {
  name: string;
  dependsOn: string[];
  covers: string[];
  status: LoopChildDerivedStatus;
  phase: LoopPortablePhase | null;
  projectRoot: string | null;
  message: string | null;
}

export interface LoopChildrenInspection {
...
interface ArchivedChildEvidence extends ArchivedChildState {
  ...
  committedState: LoopPortableState | null;
}
...
function assertAcyclic(children: readonly LoopChildDefinition[]): void {
  const byName = new Map(children.map((child) => [child.name, child]));
  const visiting = new Set<string>();
  const visited = new Set<string>();
  const visit = (name: string): void => {
    if (visited.has(name)) return;
    if (visiting.has(name)) throw new Error(`Loop children dependency cycle includes ${name}`);
    visiting.add(name);
...
function validateCoverage(
  children: readonly LoopChildDefinition[],
  ...
  for (const child of children) {
    ...
    if (unknown.length > 0) {
      throw new Error(`Loop child ${child.name} covers unknown acceptance: ${unknown.join(', ')}`);
    }
  }
  const covered = new Set(children.flatMap((child) => child.covers));
  const missing = acceptanceIds.filter((id) => !covered.has(id));
  if (missing.length > 0) {
    throw new Error(`Loop children do not cover parent acceptance: ${missing.join(', ')}`);
  }
}

export function parseLoopChildrenContract(
  source: string,
  acceptanceIds?: readonly string[],
): LoopChildrenContract {
  const document = parseDocument(source, { uniqueKeys: true });
  if (document.errors.length > 0) {
    throw new Error(`Loop children contract is invalid YAML: ${document.errors[0].message}`);
  }
  const root = record(document.toJS({ mapAsMap: false }), 'Loop children contract');
  exactKeys(root, ROOT_KEYS, 'Loop children contract');
  if (root.schema !== LOOP_CHILDREN_SCHEMA) {
    throw new Error(`Loop children schema must be ${LOOP_CHILDREN_SCHEMA}`);
  }
  if (!Array.isArray(root.children) || root.children.length === 0) {
    throw new Error('Loop children must be a non-empty array');
  }
  const children = root.children.map((entry, index): LoopChildDefinition => {
    const child = record(entry, `Loop child ${index}`);
    exactKeys(child, CHILD_KEYS, `Loop child ${index}`);
    if (typeof child.name !== 'string' || !NAME_PATTERN.test(child.name)) {
      throw new Error(`Loop child ${index} name is invalid`);
    }
    return {
      name: child.name,
      depends_on: stringList(child.depends_on, `Loop child ${child.name} depends_on`),
      covers: stringList(child.covers, `Loop child ${child.name} covers`),
    };
  });
  const names = children.map(({ name }) => name);
  if (new Set(names).size !== names.length) throw new Error('Loop child names must be unique');
  const known = new Set(names);
  for (const child of children) {
    const missing = child.depends_on.filter((dependency) => !known.has(dependency));
    if (missing.length > 0) {
      throw new Error(`Loop child ${child.name} depends on unknown child: ${missing.join(', ')}`);
    }
  }
  assertAcyclic(children);
  if (acceptanceIds) validateCoverage(children, acceptanceIds);
  return { schema: LOOP_CHILDREN_SCHEMA, children };
}

export async function readLoopChildrenContract(options: {
  changeDir: string;
  acceptanceIds?: readonly string[];
}): Promise<LoopChildrenDocument | null> {
  const file = path.join(options.changeDir, LOOP_CHILDREN_FILE);
  try {
    await fs.lstat(file);
  } catch (error) {
  ...
  const source = await readLoopBoundedTextFile({
    ...
    ref: LOOP_CHILDREN_FILE,
  });
  return {
    contract: parseLoopChildrenContract(source.text, options.acceptanceIds),
    size: source.size,
  };
}

export function hashLoopParentContract(options: {
  acceptance: readonly Pick<LoopPortableAcceptanceState, 'id' | 'source' | 'text'>[];
  children: LoopChildrenContract;
}): string {
  return canonicalHash('owner.loop.parent-contract.v1', {
    acceptance: options.acceptance.map(({ id, source, text }) => ({ id, source, text })),
    children: options.children,
  });
}

async function workspaceSources(paths: LoopProjectPaths): Promise<WorkspaceSource[]> {
  ...
  for (const projectRoot of [...new Set(roots.map((root) => path.resolve(root)))]) {
    ...
    sources.push({
      projectRoot,
      paths: await loopProjectPaths(projectRoot, config.loop.artifact_root),
...
async function readActiveChild(
  source: WorkspaceSource,
  name: string,
): Promise<LoopPortableState | null> {
  const file = path.join(source.paths.changesDir, name, 'owner-state.yaml');
  try {
    return await readLoopPortableState(file);
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') return null;
    throw error;
  }
...
async function readArchivedChildren(
  ...
  for (const entry of entries) {
    ...
    try {
      const file = path.join(source.paths.archiveDir, entry.name, 'owner-state.yaml');
      const state = await readLoopPortableState(file);
      if (names.has(state.name)) states.push({ file, state });
    } catch {
      // Legacy archives cannot satisfy a Portable Loop child declaration.
    }
  }
  return states;
}
...
function readCommittedPortableState(
  ...
  try {
    ...
    return parseLoopPortableState(document.toJS({ mapAsMap: false }));
  } catch {
    return null;
  }
}
...
function activeFact(
  ...
  if (
    ...
    return {
      ...
      message: `Loop child ${state.name} must use a linked worktree targeting parent branch ${parentBranch ?? '(missing)'}`,
    };
  }
  if (state.status === 'blocked' || state.status === 'await-user') {
    return {
      ...
      projectRoot: source.projectRoot,
      message:
        state.blockers[0]?.reason.text ?? `Loop child ${state.name} requires user resolution`,
    };
  }
  return {
    kind: 'active',
...
function isMergedChildArchive(state: LoopPortableState, parentBranch: string | null): boolean {
  return (
    state.status === 'done' &&
    state.archived &&
...
export async function inspectLoopChildren(options: {
  paths: LoopProjectPaths;
  state: LoopPortableState;
}): Promise<LoopChildrenInspection | null> {
  const changeDir = path.join(options.paths.changesDir, options.state.name);
  const document = await readLoopChildrenContract({
    changeDir,
  ...
  const contractHash = hashLoopParentContract({
  ...
  const active = new Map<string, Array<{ source: WorkspaceSource; state: LoopPortableState }>>();
  ...
  for (const child of document.contract.children) {
    ...
    if (unmerged) {
      facts.set(child.name, {
        ...
        message: `Loop child ${child.name} is archived but not merged into parent branch ${parentBranch ?? '(missing)'}`,
      });
      continue;
    }
    ...
    if (candidates.length > 1) {
      facts.set(child.name, {
        ...
        phase: null,
        projectRoot: null,
        message: `Loop child ${child.name} has multiple active workspace owners`,
      });
  ...
  const resolved = new Map<string, LoopChildStatusProjection>();
  const resolve = (name: string): LoopChildStatusProjection => {
    ...
    const missingBaseDependencies =
      fact?.kind === 'active' && dependenciesDone
        ...
        : [];
    let status: LoopChildDerivedStatus;
    let message = fact?.message ?? null;
    if (fact?.kind === 'done') {
      status = dependenciesDone ? 'done' : 'blocked';
      if (!dependenciesDone) message = `Loop child ${name} was merged before its dependencies`;
    } else if (fact?.kind === 'active') {
      status = dependenciesDone && missingBaseDependencies.length === 0 ? 'active' : 'blocked';
      if (!dependenciesDone) message = `Loop child ${name} started before its dependencies merged`;
      else if (missingBaseDependencies.length > 0)
        message = `Loop child ${name} does not include merged dependencies: ${missingBaseDependencies.join(', ')}`;
    ...
    const projection: LoopChildStatusProjection = {
      name,

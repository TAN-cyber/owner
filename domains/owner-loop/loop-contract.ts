...
import {
  ...
  LOOP_ACCEPTANCE_LIMITS,
  type LoopAcceptanceCriterion,
} from './loop-acceptance.js';
import { canonicalHash } from './loop-canonical-hash.js';

const CONTRACT_HASH_TAG = 'owner.loop.contract.v1';
const ACCEPTANCE_SET_HASH_TAG = 'owner.loop.acceptance-set.v1';
const HASH_PATTERN = /^[a-f0-9]{64}$/u;

export const LOOP_CONTRACT_LIMITS = {
  maxAcceptanceCriteria: LOOP_ACCEPTANCE_LIMITS.maxCriteria,
} as const;

export interface LoopContractSpecInput {
  capability: string;
...
export interface LoopContractSpecSnapshot {
  capability: string;
  operation: LoopContractSpecInput['operation'];
  source: string | null;
  baseHash: string | null;
  contentHash: string | null;
}

export interface LoopContractSnapshot {
  schema: 'owner.loop.contract.v1';
  brief: {
    source: string;
    contentHash: string;
  };
  specs: LoopContractSpecSnapshot[];
  acceptance: LoopAcceptanceCriterion[];
...
function contentHash(value: string): string {
  return canonicalHash('owner.loop.contract-content.v1', value.replace(/\r\n?/gu, '\n'));
}

function normalizeSpec(input: LoopContractSpecInput): {
  snapshot: LoopContractSpecSnapshot;
  markdown: string | null;
} {
  if (!/^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/u.test(input.capability)) {
    throw new Error(`Invalid Loop contract capability: ${input.capability}`);
  }
  if (
    ...
    throw new Error(`Invalid Loop contract operation for ${input.capability}`);
  }
...
function compareCriteria(left: LoopAcceptanceCriterion, right: LoopAcceptanceCriterion): number {
 ...
 * the Loop path, size, symlink, and revision boundaries appropriate to their seam.
 */
export function buildLoopContractSnapshot(input: {
  briefSource?: string;
  briefMarkdown: string;
  specs: readonly LoopContractSpecInput[];
}): LoopContractSnapshot {
  const briefSource = portableRef(input.briefSource ?? 'brief.md', 'Loop brief source');
  ...
  if (new Set(capabilities).size !== capabilities.length) {
    throw new Error('Loop contract contains duplicate capabilities');
  }
  const sources = [
    briefSource,
  ...
  if (new Set(sources).size !== sources.length) {
    throw new Error('Loop contract contains duplicate artifact sources');
  }

  const acceptance = deriveBriefAcceptanceCriteria(
    input.briefMarkdown,
    briefSource,
    LOOP_CONTRACT_LIMITS.maxAcceptanceCriteria,
  );
  for (const { snapshot, markdown } of specs) {
    ...
    acceptance.push(
      ...deriveSpecAcceptanceCriteria(
        markdown,
        snapshot.source,
        LOOP_CONTRACT_LIMITS.maxAcceptanceCriteria - acceptance.length,
      ),
    );
    acceptance.push(
      ...deriveSpecMandatoryAcceptanceCriteria(
  ...
  if (acceptance.length === 0) {
    throw new Error('Loop contract has no structured acceptance criteria');
  }
  if (acceptance.length > LOOP_CONTRACT_LIMITS.maxAcceptanceCriteria) {
    throw new Error(
      `Loop contract exceeds its ${LOOP_CONTRACT_LIMITS.maxAcceptanceCriteria}-criterion acceptance budget`,
    );
  }
  const acceptanceIds = acceptance.map(({ id }) => id);
  if (new Set(acceptanceIds).size !== acceptanceIds.length) {
    throw new Error('Loop contract contains duplicate acceptance criteria');
  }
  ...
  const contractContent = {
    schema: 'owner.loop.contract.v1' as const,
    brief,
    specs: specSnapshots,
    acceptance,
    acceptanceHash,

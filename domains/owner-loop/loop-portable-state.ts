...
import { atomicWriteText, type LoopAtomicWriteOptions } from './loop-atomic-file.js';
import {
  emptyLoopPortableHistoryOverflow,
  LOOP_PORTABLE_HISTORY_LIMIT,
  LOOP_PORTABLE_STATE_SCHEMA,
  type LoopBuilderCheckSummary,
  type LoopBuilderHandoff,
  type LoopPortableAcceptanceState,
  type LoopPortableBlockerState,
  type LoopPortableCheckSummary,
  type LoopPortableHistoryEntry,
  type LoopPortableHistoryOutcome,
  type LoopPortableHistoryOverflow,
  type LoopPortableLoopState,
  type LoopPortableSpecChange,
  type LoopPortableState,
  type LoopPortableText,
  type LoopPortableVerificationState,
...
export class LoopPortableStateVersionConflictError extends Error {
  readonly code = 'loop-state-version-conflict';
  ...
  constructor(
    ...
    super(
      `Loop state version conflict: expected ${expectedStateVersion}, got ${actualStateVersion}`,
    );
    this.name = 'LoopPortableStateVersionConflictError';
  }
}
...
function parsePortableText(value: unknown, label: string): LoopPortableText {
...
function parseSpecChange(value: unknown, index: number): LoopPortableSpecChange {
  const label = `Loop spec_changes[${index}]`;
  const root = record(value, label);
...
function parseWorkspace(value: unknown): LoopPortableWorkspace {
  const label = 'Loop workspace';
  const root = record(value, label);
  ...
  if (isolation === 'current' && finish !== null) {
    throw new Error('Loop current workspace cannot contain a finish action');
  }
  if (isolation === 'current' && (change_branch === null) !== (target_branch === null)) {
    throw new Error('Loop workspace branch bindings must both be present or both be null');
  }
  if (isolation !== 'current' && (change_branch === null || target_branch === null)) {
    throw new Error('Loop isolated workspace requires change_branch and target_branch');
  }
  return { isolation, change_branch, target_branch, finish };
}

function parseLoop(value: unknown): LoopPortableLoopState {
...
function parseAcceptance(value: unknown, index: number): LoopPortableAcceptanceState {
  const label = `Loop acceptance[${index}]`;
  const root = record(value, label);
...
function parseBuilderCheck(value: unknown, index: number): LoopBuilderCheckSummary {
  const label = `Loop builder_handoff.checks[${index}]`;
  const root = record(value, label);
...
function parseBuilderHandoff(value: unknown): LoopBuilderHandoff {
  const label = 'Loop builder_handoff';
  const root = record(value, label);
  rejectUnknown(
    root,
    new Set([
...
function parseBlocker(value: unknown, index: number): LoopPortableBlockerState {
  const label = `Loop blockers[${index}]`;
  const root = record(value, label);
...
function parseCheck(value: unknown, index: number): LoopPortableCheckSummary {
  const label = `Loop verification.checks[${index}]`;
  const root = record(value, label);
  rejectUnknown(
    root,
    new Set([
...
function parseVerification(value: unknown): LoopPortableVerificationState {
  const label = 'Loop verification';
  const root = record(value, label);
  rejectUnknown(
    root,
    new Set([
...
function parseHistoryEntry(value: unknown, index: number): LoopPortableHistoryEntry {
  const label = `Loop history[${index}]`;
  const root = record(value, label);
  rejectUnknown(
    root,
    new Set([
...
function parseOutcomeCounts(value: unknown): Record<LoopPortableHistoryOutcome, number> {
  const label = 'Loop history_overflow.outcome_counts';
  const root = record(value, label);
...
function parseHistoryOverflow(value: unknown): LoopPortableHistoryOverflow {
  const label = 'Loop history_overflow';
  const root = record(value, label);
  rejectUnknown(
    root,
  ...
  if (dropped_entries === 0 && (first_dropped_at !== null || last_dropped_at !== null)) {
    throw new Error('Loop empty history overflow cannot contain dropped timestamps');
  }
  if (dropped_entries > 0 && (first_dropped_at === null || last_dropped_at === null)) {
    throw new Error('Loop non-empty history overflow requires dropped timestamps');
  }
  ...
  if (counted !== dropped_entries) {
    throw new Error('Loop history overflow outcome count must equal dropped_entries');
  }
  return { dropped_entries, first_dropped_at, last_dropped_at, outcome_counts };
}

function assertReferences(state: LoopPortableState): void {
  const acceptanceIds = new Set(state.acceptance.map((entry) => entry.id));
  for (const id of state.loop.previous_unresolved_ids) {
    if (!acceptanceIds.has(id)) throw new Error(`Loop loop references unknown acceptance ID ${id}`);
  }
  for (const id of state.builder_handoff?.addressed_acceptance_ids ?? []) {
    if (!acceptanceIds.has(id)) throw new Error(`Loop builder handoff references unknown ID ${id}`);
  }
  for (const blocker of state.blockers) {
    for (const id of blocker.acceptance_ids) {
      if (!acceptanceIds.has(id)) throw new Error(`Loop blocker references unknown ID ${id}`);
    }
  }
  if (state.builder_handoff) {
    ...
    if (!allowedIteration) {
      throw new Error('Loop builder handoff iteration is not current or the prior repair result');
    }
  }
  if (state.verification) {
    if (!state.builder_handoff) throw new Error('Loop verification requires a builder handoff');
    if (state.verification.candidate_id !== state.builder_handoff.candidate_id) {
      throw new Error('Loop verification candidate must match the builder handoff');
    }
    if (state.verification.identity_provider !== state.builder_handoff.identity_provider) {
      throw new Error('Loop Builder and Verifier identity providers must match');
    }
    ...
    if (skillAssurance !== (state.verification.identity_provider === 'skill-coordinated')) {
      throw new Error('Loop verification assurance does not match its identity provider');
    }
    if (
      state.verification.assurance === 'semantic-verification-unavailable' &&
      state.verification.verdict !== 'blocked'
    ) {
      throw new Error('Loop unavailable semantic verification must remain blocked');
    }
    if (
      state.verification.assurance === 'user-confirmed-degraded' &&
      state.verification.verdict !== 'pass'
    ) {
      throw new Error('Loop user-confirmed degraded verification must be passing');
    }
    if (state.verification.verifier_execution_ref === state.builder_handoff.builder_execution_ref) {
      throw new Error('Loop Builder and Verifier execution refs must differ');
    }
    const currentResult =
    ...
    if (!currentResult && !priorRepairResult) {
      throw new Error('Loop verification iteration and attempt do not match a stable loop result');
    }
  }
}

function assertLifecycle(state: LoopPortableState): void {
  const stagesByPhase: Record<LoopPortableState['phase'], ReadonlySet<string>> = {
  ...
  if (!stagesByPhase[state.phase].has(state.loop.stage)) {
    throw new Error(`Loop loop stage ${state.loop.stage} is invalid for phase ${state.phase}`);
  }
  const terminalStage =
  ...
  if (terminalStage !== null && state.loop.stage !== terminalStage) {
    throw new Error(`Loop status ${state.status} requires loop stage ${terminalStage}`);
  }
  if (state.status === 'active' && ['await-user', 'blocked', 'done'].includes(state.loop.stage)) {
    throw new Error(`Loop active status cannot use loop stage ${state.loop.stage}`);
  }
  if (state.archived !== (state.status === 'done')) {
    throw new Error('Loop archived must be true exactly when status is done');
  }
  if (state.archived && state.phase !== 'archive') {
    throw new Error('Loop archived state must be in Archive');
  }
  if (state.verification_result === 'pass') {
    if (state.verification?.verdict !== 'pass') {
      throw new Error('Loop pass requires a persisted passing verification');
    }
    if (state.acceptance.some((entry) => entry.result !== 'passed')) {
      throw new Error('Loop pass requires every acceptance item to pass');
    }
    if (state.verification.checks.some((check) => check.status !== 'passed')) {
      throw new Error('Loop pass requires every persisted check to pass');
    }
  }
  if (state.verification?.assurance === 'semantic-verification-unavailable') {
    if (
      ...
      throw new Error(
        'Loop unavailable semantic verification must await explicit user confirmation',
      );
    }
  }
  if (state.verification_report !== null && state.verification === null) {
    throw new Error('Loop verification report requires persisted verification');
  }
}

export function parseLoopPortableState(value: unknown): LoopPortableState {
  const root = record(value, 'Loop portable state');
  rejectUnknown(root, ROOT_KEYS, 'Loop portable state');
  if (root.schema !== LOOP_PORTABLE_STATE_SCHEMA) {
    throw new Error(`Loop portable state schema must be ${LOOP_PORTABLE_STATE_SCHEMA}`);
  }
  const name = stringValue(root.name, 'Loop state name');
  if (!NAME_PATTERN.test(name)) throw new Error('Loop state name is invalid');
  if (root.brief !== 'brief.md') throw new Error('Loop state brief must be brief.md');
  const spec_changes = arrayValue(root.spec_changes, 'Loop spec_changes', parseSpecChange);
  assertUnique(
    spec_changes.map((entry) => entry.capability),
    'Loop spec change capabilities',
  );
  const acceptance = arrayValue(root.acceptance, 'Loop acceptance', parseAcceptance);
  assertUnique(
    acceptance.map((entry) => entry.id),
    'Loop acceptance IDs',
  );
  const history = arrayValue(root.history, 'Loop history', parseHistoryEntry);
  if (history.length > LOOP_PORTABLE_HISTORY_LIMIT) {
    throw new Error(`Loop history cannot exceed ${LOOP_PORTABLE_HISTORY_LIMIT} entries`);
  }
  const state: LoopPortableState = {
    schema: LOOP_PORTABLE_STATE_SCHEMA,
    name,
    language: enumValue(root.language, ['en', 'zh-CN'] as const, 'Loop state language'),
    phase: enumValue(
      root.phase,
    ...
    state_version: integerValue(root.state_version, 'Loop state_version', 1),
    brief: 'brief.md',
    ...
    blockers: arrayValue(root.blockers, 'Loop blockers', parseBlocker),
    ...
    verification_report:
      root.verification_report === null
        ...
        : enumValue(
            ...
            'Loop verification_report',
          ),
    archived: booleanValue(root.archived, 'Loop archived'),
    created_at: timestamp(root.created_at, 'Loop created_at'),
  };
...
export function createLoopPortableState(options: {
  name: string;
  language: 'en' | 'zh-CN';
...
export function appendLoopPortableHistory(
  ...
  return parseLoopPortableState({ ...parsedState, history, history_overflow: overflow });
}
...
export async function readLoopPortableState(file: string): Promise<LoopPortableState> {
  const source = await fs.readFile(file, 'utf8');
  return parseLoopPortableState(parseYaml(source, 'Loop portable state'));
}

export async function writeLoopPortableState(
  file: string,
  state: LoopPortableState,
  options: LoopAtomicWriteOptions = {},
): Promise<void> {
  const parsed = parseLoopPortableState(state);
 ...
 * still hold Loop's project mutation lock so the same guarantee spans processes.
 */
export async function compareAndSwapLoopPortableState(options: {
  file: string;
  ...
  return serializeStateWrite(options.file, async () => {
    const expected = integerValue(options.expectedStateVersion, 'Loop expected state version', 1);
    const next = parseLoopPortableState(options.next);
    if (next.state_version !== expected + 1) {
      throw new Error('Loop CAS next state_version must increment exactly once');
    }
    const current = await readLoopPortableState(options.file);
    if (
    ...
    if (current.state_version !== expected) {
      throw new LoopPortableStateVersionConflictError(expected, current.state_version);
    }
    await writeLoopPortableState(options.file, next, {
      ...(options.containedRoot ? { containedRoot: options.containedRoot } : {}),
      beforeCommit: async () => {
        const latest = await readLoopPortableState(options.file);
        if (latest.state_version !== expected) {
          throw new LoopPortableStateVersionConflictError(expected, latest.state_version);
        }
      },
    });
    return next;

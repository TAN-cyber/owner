...
import { atomicWriteJson, type LoopAtomicWriteOptions } from './loop-atomic-file.js';
import {
  LOOP_LOCAL_EXECUTION_SCHEMA,
  type LoopLocalCheckState,
  type LoopLocalExecutionState,
...
function parseCheck(value: unknown, index: number): LoopLocalCheckState {
  const label = `Loop local checks[${index}]`;
...
export function parseLoopLocalExecution(value: unknown): LoopLocalExecutionState {
  const label = 'Loop local execution state';
  const root = record(value, label);
  rejectUnknown(
    root,
  ...
  if (root.schema !== LOOP_LOCAL_EXECUTION_SCHEMA) {
    throw new Error(`Loop local execution schema must be ${LOOP_LOCAL_EXECUTION_SCHEMA}`);
  }
  const workspaceRoot = record(root.workspace, 'Loop local workspace');
  rejectUnknown(
    workspaceRoot,
    new Set(['projectRoot', 'worktreeRoot', 'branch']),
    'Loop local workspace',
  );

  let execution: LoopLocalExecutionState['execution'] = null;
  if (root.execution !== null) {
    const executionRoot = record(root.execution, 'Loop local execution');
    rejectUnknown(
      executionRoot,
      new Set([
        'operationId',
    ...
    const startedAt = timestampOrNull(executionRoot.startedAt, 'Loop local execution.startedAt');
    if (startedAt === null) throw new Error('Loop local execution.startedAt is required');
    execution = {
      operationId: stringValue(executionRoot.operationId, 'Loop local execution.operationId'),
      stage: enumValue(
        executionRoot.stage,
        ['building', 'checking', 'verifying', 'archiving'] as const,
        'Loop local execution.stage',
      ),
      actor:
        executionRoot.actor === null
          ...
          : enumValue(
              ...
              'Loop local execution.actor',
            ),
      executionId: nullableString(executionRoot.executionId, 'Loop local execution.executionId'),
      status: enumValue(
        executionRoot.status,
        ['running', 'completed', 'interrupted'] as const,
        'Loop local execution.status',
      ),
      startedAt,
      requestCheckRounds: integerValue(
        executionRoot.requestCheckRounds,
        'Loop local execution.requestCheckRounds',
      ),
    };
  }

  if (!Array.isArray(root.checks)) throw new Error('Loop local checks must be an array');
  const checks = root.checks.map(parseCheck);
  if (new Set(checks.map((check) => check.id)).size !== checks.length) {
    throw new Error('Loop local check IDs must be unique');
  }
  if (execution && checks.some((check) => check.operationId !== execution.operationId)) {
    throw new Error('Loop local checks must belong to the current operation');
  }
  return {
    schema: LOOP_LOCAL_EXECUTION_SCHEMA,
    change: stringValue(root.change, 'Loop local change'),
    basedOnStateVersion: integerValue(
      root.basedOnStateVersion,
      'Loop local basedOnStateVersion',
      1,
    ),
    workspace: {
      projectRoot: absolutePath(workspaceRoot.projectRoot, 'Loop local workspace.projectRoot'),
      worktreeRoot: absolutePath(workspaceRoot.worktreeRoot, 'Loop local workspace.worktreeRoot'),
      branch: nullableString(workspaceRoot.branch, 'Loop local workspace.branch'),
    },
    execution,
    checks,
  };
...
export function rebuildLoopLocalExecution(options: {
  portableState: LoopPortableState;
  projectRoot: string;
  worktreeRoot?: string;
  branch?: string | null;
}): LoopLocalExecutionState {
  return parseLoopLocalExecution({
    schema: LOOP_LOCAL_EXECUTION_SCHEMA,
...
export async function readLoopLocalExecution(
  file: string,
  ...
  try {
    ...
    throw new Error(`Loop local execution state is invalid JSON: ${(error as Error).message}`, {
      cause: error,
    });
  }
  return parseLoopLocalExecution(value);
}

export async function writeLoopLocalExecution(
  file: string,
  state: LoopLocalExecutionState,
  options: LoopAtomicWriteOptions = {},
): Promise<void> {
  const parsed = parseLoopLocalExecution(state);
  await fs.mkdir(path.dirname(file), { recursive: true });
  await atomicWriteJson(file, parsed, options);
}

export type LoopLocalExecutionRebuildReason = 'missing' | 'invalid' | 'version-mismatch';

export async function readOrRebuildLoopLocalExecution(options: {
  file: string;
  portableState: LoopPortableState;
  projectRoot: string;
  worktreeRoot?: string;
  ...
  state: LoopLocalExecutionState;
  rebuilt: boolean;
  reason: LoopLocalExecutionRebuildReason | null;
}> {
  let current: LoopLocalExecutionState | null = null;
  let reason: LoopLocalExecutionRebuildReason | null = null;
  try {
    current = await readLoopLocalExecution(options.file);
  ...
  const rebuilt = rebuildLoopLocalExecution(options);
  await writeLoopLocalExecution(options.file, rebuilt, {

...
const ROOT_MOVE_JOURNAL = '.owner/classic-root-move.json';
const INIT_OWNERSHIP_JOURNAL = '.owner/classic-init-ownership.json';
...
interface ClassicInitOwnershipJournal {
  schema: 'owner.classic-init-ownership.v1';
  id: string;
...
function parseOwnershipJournal(value: unknown): ClassicInitOwnershipJournal {
  ...
  const raw = value as Record<string, unknown>;
  if (
    raw.schema !== 'owner.classic-init-ownership.v1' ||
  ...
  if (
    !(
      ...
      ((raw.stage === 'quarantining' || raw.stage === 'quarantined') &&
        ...
        raw.quarantine.startsWith('.owner/transactions/classic-init/'))
    )
  ) {
...
async function quarantineOwnershipJournal(
  ...
  const relativePath = `.owner/classic-init-ownership.${expected.id}.${randomUUID()}.quarantine`;
...
async function updateOwnershipJournal(
  ...
  const temporaryRelative = `.owner/classic-init-ownership.${randomUUID()}.tmp`;
...
async function quarantineCheckpointedRoot(
  ...
  const quarantineRelative = `.owner/transactions/classic-init/${journal.id}/openspec`;
  const quarantine = path.join(projectRoot, ...quarantineRelative.split('/'));
  await ensureProtectedProjectDirectory(
    projectRoot,
    `.owner/transactions/classic-init/${journal.id}`,
...
export async function beginClassicLayoutInitialization(
  ...
  if (existingJournal) {
    ...
    if (existingJournal.stage !== 'initializing') {
      throw new Error(
        `Classic init ownership is ${existingJournal.stage}; recover it with owner doctor before continuing`,
      );
    }
...
export async function repairClassicLayoutInitialization(
  ...
  if (strategy === 'continue') {
    ...
    if (
      ...
      throw new Error(
        `Classic init ${journal.id} is still resumable; rerun owner init to continue it`,
      );
    }
    await assertCheckpointMatches(root, journal);
...
export async function assertClassicLayoutInitializationSafe(
  ...
  if (pendingMove.exists) {
    throw new Error(
      'Classic root move transaction is incomplete; inspect it with owner doctor and recover it explicitly before writing',
    );
  }
  ...
  if (ownership) {
    ...
    if (ownership.stage !== 'initializing') {
      throw new Error(
        `Classic init ownership is ${ownership.stage}; recover it with owner doctor before continuing`,
      );
    }
  ...
  if (legacyRoot.exists || docsRoot.exists) {
    ...
    throw new Error(
      'Cannot initialize Classic layout without .owner/config.yaml when openspec/ or docs/openspec/ already exists',
    );
  }

...
function usage(language: RootMoveLanguage = 'en') {
  return {
    ...
    stderr:
      language === 'zh-CN'
        ? '用法：owner classic root show | owner classic root move docs --dry-run | owner classic root move docs --apply'
        : 'Usage: owner classic root show | owner classic root move docs --dry-run | owner classic root move docs --apply',
  };
}
...
function formatClassicRootMoveError(error: unknown, language: RootMoveLanguage): string {
  ...
  if (message.startsWith('invalid Classic root move journal:')) {
    return 'Classic 根目录迁移失败：迁移记录格式或内容无效，请运行 owner doctor 检查恢复状态';
  }
  return 'Classic 根目录迁移失败：迁移过程中发生错误，请运行 owner doctor 检查迁移状态';
}
...
export function formatClassicRootMoveReport(
  ...
  if (mode === 'dry-run') {
    lines.push(
      plan.readyToApply
        ? zh
          ? '下一步：运行 owner classic root move docs --apply 执行迁移。'
          : 'Next: run owner classic root move docs --apply to apply the migration.'
        : zh
          ? '下一步：请先解决上述冲突或阻塞项。'
...
export const classicRootCommand: ClassicCommandHandler = async (args) => {
  ...
  if (action === 'show' && target === undefined) {
    ...
    return {
      ...
      stdout:
        JSON.stringify({
          schema: 'owner.classic-layout.v1',

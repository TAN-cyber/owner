...
export const classicArchiveCommand: ClassicCommandHandler = async (args) => {
  ...
  try {
    ...
    if (
      !changeDir ||
      !(await classicProjectTargetExists(layout.projectRoot, `${changeDir}/.owner.yaml`, {
    ...
    if (classic.verifyResult !== 'pass') {
      throw new ArchiveFailure(
        red(
          `FATAL: verify_result is '${classic.verifyResult}', expected 'pass'. Run owner-verify first.`,
        ),
      );
    }

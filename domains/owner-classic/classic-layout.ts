...
export class ClassicLayoutUnavailableError extends Error {
  ...
  constructor(message = 'Classic artifact layout is unavailable from .owner/config.yaml') {
    super(message);
...
export async function assertClassicLayoutWritable(
  ...
  const pendingMove = path.join(path.resolve(projectRoot), '.owner', 'classic-root-move.json');

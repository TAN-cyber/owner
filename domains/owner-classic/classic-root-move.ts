...
const JOURNAL_RELATIVE_PATH = '.owner/classic-root-move.json';
const STAGING_PLAN_IDENTITY = '.owner/transactions/classic-root-move/<transaction-id>/openspec';
...
async function assertRootMovePreflightBoundaries(
  ...
  const transactionBase = path.join(projectRoot, '.owner', 'transactions', 'classic-root-move');
  const checks: Array<[string, string, 'file' | 'directory' | 'any']> = [
    ...
    [transactionBase, '.owner/transactions/classic-root-move', 'directory'],
  ];
  if (transactionId) {
...
async function createJsonExclusive(
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'EEXIST') {
      throw new Error(
        'Classic root move is incomplete; use owner doctor --repair --strategy continue|rollback',
        { cause: error },
      );
    }
    throw error;
...
function parseJournalV1(journal: Record<string, unknown>): ClassicRootMoveJournalV1 {
  ...
  const expectedStaging = `.owner/transactions/classic-root-move/${journal.id}/openspec`;
  if (
...
function parseJournal(value: unknown): ClassicRootMoveJournal {
  ...
  if (journal.schema === 'owner.classic-root-move.v1') return parseJournalV1(journal);
  if (journal.schema === 'owner.classic-root-move.v2') return parseJournalV2(journal);
...
async function updateJournal(
  ...
  if (journal.schema === 'owner.classic-root-move.v1' && stage === 'locked') {
...
async function consumeBoundEmptyTarget(
  ...
  if (state === 'empty') {
    if (journal.schema === 'owner.classic-root-move.v2' && !journal.targetInitialIdentity) {
    ...
    await removeProtectedEmptyDirectory(
      ...
      'remove-empty-target',
      testHooks,
      journal.schema === 'owner.classic-root-move.v2'
...
async function assertBoundSourceIdentity(
  ...
  if (journal.schema === 'owner.classic-root-move.v1') return;
    ...
    return {
      ...
      sourceIdentity:
        journal.schema === 'owner.classic-root-move.v2'
          ...
          : [],
      targetInitialIdentity:
        journal.schema === 'owner.classic-root-move.v2' && journal.targetInitialIdentity
...
export async function applyClassicRootMove(
  ...
  if (existing) {
    throw new Error(
      `Classic root move ${existing.id} is incomplete; use owner doctor --repair --strategy continue|rollback`,
    );
  }
  ...
  const lockedJournal: ClassicRootMoveJournal = {
    ...
    staging: `.owner/transactions/classic-root-move/${id}/openspec`,

...
async function inspectClassicHookTarget(
  ...
  if (governing.invalidState) {
    return result(
      2,
      `[OWNER-HOOK] blocked: active Classic state is invalid; repair .owner.yaml before writing ${relativePath}`,
    );
  }
...
export async function inspectClassicHookGuard(
  ...
  if (!selected) {
    return {
      allowed: false,
      reason: `Selected Classic change ${changeName} is missing or archived; resume /owner-classic before retrying`,
      workflow: 'classic',
      change: changeName,
    };
  }

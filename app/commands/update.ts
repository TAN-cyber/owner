...
import { removeLegacyOwnerSkillsForPlatform } from '../../domains/skill/uninstall.js';
import { syncOwnerProjectInstructions } from '../../domains/skill/project-instructions.js';
import {
...
interface InstalledOwnerPackage {
  packageRoot: string;
  version: string;
  binPath: string;
...
interface InstalledOwnerTarget {
  scope: InstallScope;
  platform: Platform;
...
interface CommandFailureDetail {
  component: 'npm' | 'OpenSpec' | 'Superpowers' | 'Skill' | 'Rule' | 'Hook';
  reason: string;
  scope?: InstallScope;
  platform?: string;
  platformName?: string;
...
function getInstalledOwnerSkillsDirs(
  baseDir: string,
  platform: Platform,
...
async function hasLocalOwnerSkills(
  baseDir: string,
  platform: Platform,
  scope: InstallScope,
): Promise<boolean> {
  for (const skillsDir of getInstalledOwnerSkillsDirs(baseDir, platform, scope)) {
    if (!(await targetPathExists(skillsDir))) continue;
    const entries = await readTargetDir(skillsDir);
    if (entries.some((entry) => entry.startsWith('owner'))) return true;
  }
  return false;
}

async function detectInstalledOwnerLanguage(
  baseDir: string,
  platform: Platform,
  scope: InstallScope = 'project',
  ...
  for (const skillsDir of getInstalledOwnerSkillsDirs(baseDir, platform, scope)) {
    if (!(await targetPathExists(skillsDir))) continue;
    const entries = (await readTargetDir(skillsDir)).filter((entry) => entry.startsWith('owner'));
...
async function detectInstalledOwnerTargets(
  projectPath: string,
  options: DetectTargetsOptions = {},
): Promise<InstalledOwnerTarget[]> {
  const scopes = options.scopes ?? (['project', 'global'] as InstallScope[]);
  const targets: InstalledOwnerTarget[] = [];
  ...
  for (const scope of scopes) {
    ...
    for (const { platform } of owners) {
      if (!(await hasLocalOwnerSkills(baseDir, platform, scope))) continue;

      targets.push({
        scope,
        platform,
        language: await detectInstalledOwnerLanguage(baseDir, platform, scope),
      });
    }
  }
...
async function detectOwnerPackageScope(
  projectPath: string,
...
function resolveNpmSelfUpdatePlan(
  ...
  if (!current) {
    return {
      action: 'fail',
      reason: `current Owner version is not valid semver: ${currentVersion}`,
    };
  }
  if (!registry) {
    return {
      action: 'fail',
      reason: `registry Owner version is not valid semver: ${registryVersion}`,
    };
  }
  ...
  if (comparison === 0) {
    return { action: 'skip', reason: `Owner ${currentVersion} is already installed` };
  }
...
function formatSkillUpdateCommand(
  ...
  if (installMode === 'symlink') {
    return `symlink via .owner/skills/ in ${destPrefix}${getPlatformSkillsDir(platform, scope)}/skills/ (${scope})`;
  }
...
async function readOwnerPackage(packageRoot: string): Promise<InstalledOwnerPackage> {
  ...
  if (!packageRootStat.isDirectory() || packageRootStat.isSymbolicLink()) {
    throw new Error(`Owner package root is not a real directory: ${packageRoot}`);
  }
  ...
  if (typeof pkg.version !== 'string') {
    throw new Error(`Owner package has no valid version: ${packageJsonPath}`);
  }
  const binReference = typeof pkg.bin === 'string' ? pkg.bin : pkg.bin?.owner;
  if (!binReference) throw new Error(`Owner package has no owner bin: ${packageJsonPath}`);
  const binPath = path.resolve(resolvedPackageRoot, binReference);
  if (!isSameOrInside(binPath, resolvedPackageRoot)) {
    throw new Error(`Owner package bin is invalid: ${binReference}`);
  }
  ...
  try {
    ...
    throw new Error(`Owner package bin is invalid: ${binReference}`, { cause: error });
  }
  return { packageRoot: resolvedPackageRoot, version: pkg.version, binPath: realBinPath };
}

async function readInstalledOwnerPackage(
  scope: InstallScope,
  projectPath: string,
  npmCliPath: string,
): Promise<InstalledOwnerPackage> {
  if (scope === 'project') {
...
async function validateOwnerPackageCommands(
  candidate: InstalledOwnerPackage,
  ...
  const checks: Array<{
    ...
    },
    {
      ...
      accepts: (stdout) => stdout.includes('Usage: owner workflow resolve'),
...
async function validateRegistryOwnerPackage(
  version: string,
  npmCliPath: string,
): Promise<CapturedProcessResult> {
  let validationDir: string;
  try {
    validationDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-self-update-'));
  } catch (error) {
    return {
      success: false,
      exitCode: null,
  ...
  try {
    ...
    if (!install.success) {
      ...
      if (candidate.version !== version) {
        ...
        result = await validateOwnerPackageCommands(candidate, version);
      }
    }
  } catch (error) {
    result = {
...
async function installOwnerNpmVersion(
  npmCliPath: string,
  scope: InstallScope,
  projectPath: string,
  version: string,
...
async function updateOwnerNpmPackage(
  npmCliPath: string,
  scope: InstallScope,
  projectPath: string,
  targetVersion: string,
  installedPackage: InstalledOwnerPackage,
  log: (message: string) => void,
): Promise<{ success: boolean; exitCode: number | null; reason?: string }> {
  const validation = await validateRegistryOwnerPackage(targetVersion, npmCliPath);
  ...
  const install = await installOwnerNpmVersion(npmCliPath, scope, projectPath, targetVersion);
  let installReason = install.reason;
  if (install.success) {
    try {
      const installedAfterUpdate = await readOwnerPackage(installedPackage.packageRoot);
  ...
  const rollback = await installOwnerNpmVersion(
    npmCliPath,
    scope,
    projectPath,
  ...
  if (rollback.success) {
    try {
      const restored = await readOwnerPackage(installedPackage.packageRoot);
...
function summarizeTargets(targets: InstalledOwnerTarget[]): AllProjectsUpdateResult['targets'] {
...
async function updateSingleProject(
  ...
  const packageScope =
    options.scope && !options.targetScopes
      ? options.scope
      : await detectOwnerPackageScope(projectPath);
  ...
  if (skipRepeatedGlobalNpm) {
    ...
    let installedPackage: InstalledOwnerPackage | null = null;
    try {
      npmCliPath = await resolveNpmCliPath();
      installedPackage = await readInstalledOwnerPackage(packageScope, projectPath, npmCliPath);
    } catch (error) {
      npmStatus = 'failed';
      npmReason = `unable to inspect installed Owner package: ${(error as Error).message}`;
    }

    const registryVersion = installedPackage ? await getLatestVersion() : null;
    if (installedPackage && registryVersion === null) {
      npmStatus = 'failed';
      npmReason = 'unable to resolve the latest Owner version from the npm registry';
      ...
      if (plan.action === 'skip') {
        ...
        log(`    $ ${npmCommand}`);
        const npmResult = await updateOwnerNpmPackage(
          npmCliPath,
          packageScope,
          projectPath,
          plan.version,
    ...
    ? await Promise.all(
        (options.targetScopes ?? [options.scope ?? 'project']).map(async (scope) => {
          const existing = options.language
            ? null
            : (
                await detectInstalledOwnerTargets(projectPath, {
                  scopes: [scope],
  ...
  // Global scope mirrors `owner init`: `owner update --scope global` must keep
  ...
  for (const target of targets) {
    ...
    const { copied, skipped, failed } = await copyOwnerSkillsForPlatform(
      baseDir,
      target.platform,
      true,
      languageSkillsDir,
    ...
    const cleanupResult =
      failed === 0
        ? await removeLegacyOwnerSkillsForPlatform(baseDir, target.platform, target.scope)
        : { removed: 0, failed: 0 };
    ...
    try {
      ...
      if (ruleResult.copied > 0) {
        log(
          `  Owner rules -> ${target.platform.name}: ${ruleResult.copied} ${t(lang, 'rulesUpdated')}`,
        );
      }
      if (ruleResult.failed > 0) {
        log(`  Owner rules -> ${target.platform.name}: ${t(lang, 'rulesFailed')} (${ruleReason})`);
      }
    } catch (err) {
      totalRulesFailed++;
      ...
      log(`  Owner rules -> ${target.platform.name}: ${t(lang, 'rulesFailed')} (${reason})`);
    }
    ...
    try {
      ...
      if (status === 'installed') {
        totalHooksInstalled++;
        log(`  Owner hooks -> ${target.platform.name}: ${t(lang, 'hooksUpdated')}`);
        if (cleanupFailed > 0) {
          log(`  Owner hooks -> ${target.platform.name}: ${reason}`);
        }
      } else if (status === 'failed') {
        log(`  Owner hooks -> ${target.platform.name}: ${t(lang, 'hooksFailed')} (${reason})`);
      } else if (reason && target.platform.supportsHooks) {
        log(`  Owner hooks -> ${target.platform.name}: ${t(lang, 'hooksSkipped')} (${reason})`);
      }
    } catch (err) {
      totalHooksFailed++;
      const reason = (err as Error).message;
  ...
  if (projectTarget && !projectMutationBlocked) {
    try {
      ...
      if (projectInstructionsUpdated > 0) {
        log(`  Owner project instructions -> ${projectInstructionsUpdated} file(s) updated`);
      }
    } catch (error) {
      openSpecStatus = 'failed';
...
async function updateAllIndexedProjects(
  ...
  const runnableProjects: Array<{ projectPath: string; targets: InstalledOwnerTarget[] }> = [];
  let staleRemoved = 0;
  ...
  if (!options.json) {
    log(`  Owner will update ${runnableProjects.length} indexed project(s):`);
...
function resolveSelfUpdateOptions(
  ...
  // The default project/global `owner update` keeps upgrading the package and
  ...
  if (registryProjects.length === 0 && usesImplicitIndexedProjectUpdate) {
    ...
    if (currentProjectTargets.length > 0) {
      ...
      if (options.json) {
        ...
        log('  No indexed Owner projects found. Nothing to update.');
        log('  Run `owner init` in a project first, or use `owner update --scope global`.\n');
      }
      return { status: 'complete' };
    }
  }
...
export type { InstalledOwnerTarget, SkillLanguage, TranslationKey };

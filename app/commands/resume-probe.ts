...
import {
  ...
  type OwnerEntryResumeProbeInput,
  type OwnerEntryResumeProbeResult,
...
function formatText(result: OwnerEntryResumeProbeResult): string {
  const lines = [
...
export async function resumeProbeCommand(
  ...
  const input: OwnerEntryResumeProbeInput = {
    ...
    agent_context: {
      ...
      already_in_owner_flow: options.alreadyInOwnerFlow === true,
    },
  };
  const result = await resolveOwnerEntryResumeProbe(projectPath, input);

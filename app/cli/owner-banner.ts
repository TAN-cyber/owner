...
const CLI_INDENT = '  ';
const LOGO_WIDTH = Math.max(...OWNER_LOGO.map((line) => line.length));
const BANNER_WIDTH = Math.max(LOGO_WIDTH, CLI_INDENT.length + OWNER_TAGLINE.length);
...
const LOGO_CONTENT_RIGHT = Math.max(...OWNER_LOGO.map((line) => line.trimEnd().length));
const LOGO_OFFSET =
  CLI_INDENT.length +
  Math.floor((OWNER_TAGLINE.length - (LOGO_CONTENT_RIGHT - LOGO_CONTENT_LEFT)) / 2) -
  LOGO_CONTENT_LEFT;
...
export const OWNER_BANNER_LINE_COUNT = OWNER_LOGO.length + 1;
...
export function renderOwnerAnimationFrame(phase: AnimationPhase, progress: number): string {
  const amount = Math.max(0, Math.min(1, progress));
  const logo = OWNER_LOGO.map((line, row) => {
  ...
  const tagline = revealTaglineFromCenter(OWNER_TAGLINE, taglineProgress);
  return [...logo, `${ansi(TAGLINE_GOLD)}${tagline}${RESET}`].join('\n');
}

export function renderOwnerBanner(options: { color?: boolean } = {}): string {
  const logo = options.color
    ? OWNER_LOGO.map((line) => `${ansi(BRAND_BLUE)}${logoLineOnCanvas(line)}${RESET}`)
    : OWNER_LOGO.map(logoLineOnCanvas);
  const tagline = options.color
    ? `${ansi(TAGLINE_GOLD)}${taglineOnCanvas(OWNER_TAGLINE)}${RESET}`
    : taglineOnCanvas(OWNER_TAGLINE);
  return [...logo, tagline].join('\n');
}

export function renderOwnerBannerFrame(litColumns: number, particleFrame = 0): string {
  const progress = Math.max(0, Math.min(1, litColumns / BANNER_WIDTH));
  return renderOwnerAnimationFrame('sweep', Math.min(1, progress + particleFrame * 0.01));
}
...
export function canAnimateOwnerBanner(
...
function replaceFrame(frame: string, first: boolean): string {
  const moveUp = first ? '' : `\u001b[${OWNER_BANNER_LINE_COUNT}A`;
  return `${moveUp}${frame
    .split('\n')
...
function clearRenderedFrame(): string {
  const lines = Array.from({ length: OWNER_BANNER_LINE_COUNT }, () => `\r${ERASE_LINE}`).join('\n');
  return `\u001b[${OWNER_BANNER_LINE_COUNT}A${lines}\n`;
}
  ...
  try {
    ...
    for (const frame of timeline()) {
      const rendered = animationFrameForRuntime(
        runtime,
        renderOwnerAnimationFrame(frame.phase, frame.progress),
      );
      await runtime.write(replaceFrame(rendered, !started));
      started = true;
      await runtime.sleep(FRAME_DELAY_MS);
    }
    const stable = animationFrameForRuntime(runtime, renderOwnerBanner({ color: true }));

...
- Pipeline 真实逻辑位于 `domains/owner-pipeline/`，由 `pnpm build:pipeline-runtime` 生成 `assets/skills/owner/scripts/owner-runtime.mjs`；同目录其他 Pipeline launcher 保持薄封装。
...
- 共享入口、selection 与 Hook Router 位于 `domains/owner-entry/`，由 `pnpm build:entry-runtime` 生成 `owner-entry-runtime.mjs` 与 `owner-hook-router.mjs`。
- 每个平台只安装一份 `owner-workflow-guard` Rule；支持 Hook 的平台只安装一个 `owner-hook-router.mjs`。Router 根据 `.owner/current-change.json` 一次只调用当前 Loop 或 Pipeline Guard，两边的阶段、目录、schema 与 Guard 逻辑保持独立。
- `owner-hook-guard.mjs` 与 `owner-loop-hook-guard.mjs` 是各自 runtime 的薄 Guard launcher，不直接作为平台 Hook 安装。
- `owner-env.mjs` 打印自身所在目录（scripts dir），供 skill 样板代码解析同级启动器路径。
...
- 新增/重命名 runtime 入口或生成物必须同步 `assets/manifest.json`、`config/repository-layout.json` 的对应 runtime 映射与 `test/repository/*-runtime-assets.test.ts`；Pipeline launcher 还需同步 `test/domains/owner-pipeline/owner-scripts.test.ts` 的 fixture 列表。
- skill 样板（boilerplate，当前版本 `v3`）在所有 SKILL.md / reference 中重复，改动需全量同步；样板通过 `find` 定位 `owner-env.mjs`，再用 `node "$OWNER_ENV"` 解析路径，命令统一为 `node "$OWNER_STATE" ...` 形式。
...
Pipeline 打包入口 `domains/owner-pipeline/pipeline-cli.ts` 导出 `main` / `runPipelineCli` / `PIPELINE_COMMANDS`；Loop 与 Entry 的入口由 `config/repository-layout.json` 分别声明。esbuild ESM bundle 保留所需 export，启动器直接 import 调用，单进程、无 bash、无二次 node 派生。跨 workflow 的稳定契约放在 `domains/workflow-contract/`，入口归属与路由放在 `domains/owner-entry/`；不要为了复用而合并 Loop 与 Pipeline 的状态机或 Guard。

## .owner.yaml 状态机

每个 change 的状态文件，字段变更需要同步三处（全在 TypeScript 中）：

1. `domains/owner-pipeline/pipeline-state-command.ts` — `set` 白名单 + enum 验证（`SETTABLE_FIELDS` / `MACHINE_OWNED_FIELDS`）
2. `domains/owner-pipeline/pipeline-validate-command.ts` — schema 校验 + 已知字段
3. `test/domains/owner-pipeline/owner-scripts.test.ts` — 测试中的 yaml 字符串

改完 1/2 后 `pnpm build` 重新生成 `owner-runtime.mjs`，否则 `pipeline-runtime.test.ts` 的新鲜度检查会失败。
...
1. **确定比较基线**
   - 确认 `package.json` 当前版本、`origin/main` 版本、上一个发布 tag。
   - 用 `git log <上一个tag>..HEAD --oneline` 只生成候选清单，不等于逐条写入。
   - 如果当前分支已有高于 main 的版本条目，只重写/追加到同一个版本条目，不新增流水账版本。
...
3. **禁止写入开发过程**
   ...
   - “修复刚新增功能里的问题”，除非该问题已经存在于 main / 已发布版本
   - 设计过程、重构过程、命名迁移过程
...
- `Fixed`: 修复已发布版本或 main 中用户会遇到的问题；新功能开发过程中发现并修掉的问题不算 Fixed。

...
`auto_transition` 控制 Owner 工作流在阶段完成后是否自动调用下一个 Skill，还是暂停等待用户手动触发。

**关键区分**：`auto_transition` 不控制阶段推进本身。阶段推进（更新 `.owner.yaml` 中的 `phase` 字段）由 `owner-guard.mjs --apply` 执行，无论 `auto_transition` 如何设置都会发生。该配置仅控制阶段推进后是否自动调用下一个 Skill。
...
```text
owner-guard.mjs --apply                owner-state next <change>
┌──────────────────────┐              ┌──────────────────────┐
...
```bash
# 临时启用自动转场（优先级最高）
export OWNER_AUTO_TRANSITION=true
```

### 初始化时的行为

运行 `owner-state init` 创建新 change 时，`auto_transition` 会从项目级默认值解析并写入 change 的 `.owner.yaml`。之后可以单独修改该 change 的值而不影响其他 change。

## 工作流映射

`owner-state next` 根据当前 phase 和 workflow 类型决定下一个 Skill：

| Phase     | Full 工作流   | Hotfix 工作流 | Tweak 工作流  |
| --------- | ------------- | ------------- | ------------- |
| `open`    | owner-open    | owner-open    | owner-open    |
| `design`  | owner-design  | —             | —             |
| `build`   | owner-build   | owner-hotfix  | owner-tweak   |
| `verify`  | owner-verify  | owner-verify  | owner-verify  |
| `archive` | owner-archive | owner-archive | owner-archive |
...
会。`auto_transition: false` 仅暂停 Skill 调用，`owner-guard.mjs --apply` 仍然会更新 `phase` 字段。暂停后用户需要手动运行下一个 Skill（如 `/owner-build`）继续流程。

### Q: 可以中途切换吗？

可以。在任意时刻修改 change 的 `.owner.yaml` 中的 `auto_transition` 字段即可。下一次 `owner-state next` 调用会使用新值。

### Q: 环境变量和配置文件冲突时以谁为准？

环境变量 `OWNER_AUTO_TRANSITION` 优先级最高，会覆盖项目级和 change 级配置。

...
在不深读所有活动 change 的前提下，Ambient Resume 必须先解析唯一 Loop 目标，再只根据该目标的 portable 状态和正式 Markdown 恢复下一动作。本机 Runtime 缺失不能使可同步的 active change 消失。

## Requirements

### Lightweight candidate discovery

- Ambient Resume MUST 使用已验证的 `loop.artifact_root` 与 bounded、受路径保护的目录规则枚举 active Loop changes。
- 候选发现 MUST 只读取每个 change 的 `owner-state.yaml`，不得读取候选的 brief、Spec、`verification.md`、本机 `state.json`、日志或其他深层产物。
- 可解析 YAML MUST 暴露 change name、phase、status 与稳定 Loop stage；不可读、schema 不兼容或路径无效的状态 MUST 显示为 `invalid`，不得使另一个显式目标失败。
- v4 候选即使没有 `.owner/runtime/loop/changes/<name>` 也 MUST 保持可发现。只读发现不得为了探测健康而创建 overlay。
- Legacy active change MUST 显示 `migration-required`；候选发现不得在读取过程中迁移或删除旧数据。

### Target resolution

- 显式指定的 active change MUST 优先于项目 selection。
- 没有显式名称时，合法的 Loop selection MUST 确定目标；没有有效 selection 时，仅在恰好一个 active change 时 MAY 推断目标。
- 多个 active changes 且没有显式目标或合法 selection 时 MUST 要求调用方选择，并且 MUST NOT 为选择而深读每个 change。
- 只读目标解析 MUST NOT 创建或修改 `.owner/current-change.json`。
...
- 目标确定后，Ambient Resume MUST 只为该目标读取完整 `owner-state.yaml`、正式 brief/Spec，以及按需读取与当前 YAML 版本匹配的本机 `state.json`。
...
- `owner-state.yaml` MUST 决定从哪里继续；`state.json` 只补充当前机器的 in-flight operation，不得覆盖更新的 YAML。
...
- 非默认 artifact root 需要已同步的 `.owner/config.yaml` 或显式 root；Ambient Resume MUST NOT 扫描其他目录猜测。

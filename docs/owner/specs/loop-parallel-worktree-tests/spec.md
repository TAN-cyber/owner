# loop-parallel-worktree-tests

## Purpose

Provide a deterministic CI regression guard for Loop's actual parallel
development flow. The guard must exercise independent Git linked worktrees and
process-level Loop Runtime coordination without requiring a model or network.
...
business source fixture and use the Loop CLI to create two changes with
`worktree` isolation from the repository's primary worktree.

#### Scenario: Two changes are opened in parallel

- **WHEN** the test starts two distinct Loop `new` operations concurrently
  from the primary worktree
- **THEN** Loop MUST create or reuse two distinct linked worktrees
...
- **WHEN** either linked-worktree preparation or its initial Loop command is
  delayed or fails
...
### Requirement: Process-level Loop isolation

The test MUST run Loop CLI commands as separate child processes from each
...
- **WHEN** both linked worktrees concurrently run supported Loop status,
...
Loop change is progressing and then resuming the workflow.
...
- **AND** the user invokes Loop status/resume/continuation from that same
  linked worktree
- **THEN** Loop MUST preserve the manually edited source bytes
- **AND** the command MUST finish within the bounded timeout
- **AND** Loop MUST return either a valid Build continuation or an explicit
...
in the existing cross-platform Runtime smoke job after Loop Runtime assets
are built.

#### Scenario: CI runs without model or network credentials

- **WHEN** the Runtime smoke matrix runs on Ubuntu, macOS, or Windows
- **THEN** the parallel-worktree test MUST use only local Git, Node, Loop
  bundles, and temporary filesystem state
- **AND** it MUST NOT read model credentials or call a model/network endpoint

#### Scenario: Generated Loop entrypoints stay covered

- **WHEN** Loop Runtime source or command entrypoints change
- **THEN** the existing Loop build step MUST regenerate the bundles before
...
- This change does not combine Loop and Pipeline state machines.

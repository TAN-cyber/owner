...
`.owner/config.yaml` 按「全局 / 工作流专属」两层组织：全局字段留在顶层，Classic 工作流专属字段收纳在 `classic:` 嵌套映射下，与 `native:` 嵌套块对称。所有消费者共享同一份类型、规范化和路径边界契约。
...
- 只更新 Owner 管理的 Classic 字段；`classic:` 内未知自定义字段逐项保留。
...
`OwnerProjectConfig.classic` 包含可选 `artifact_layout?: 'legacy' | 'docs'` 以及既有四项字段。Native 配置解析和写入保留完整 Classic 块；Classic merge 保留完整 Native 块。
...
- 不调整 Classic change 级 `.owner.yaml` schema。

...
- 创建新 Native change 前，Skill MUST 在执行 `owner native new` 之前解析工作区模式。
...
- `owner native new` MUST 在创建 change 与初始 `owner-state.yaml` 所使用的同一 Runtime mutation lock 内重新检查当前工作目录的隔离前提，不能只信任 Skill 在加锁前完成的只读判断。
...
- worktree 模式 MUST 在目标 worktree 中运行 `owner native new`，使 portable workspace binding、本机 workspace identity、change 状态和当前 change 记录从创建时就归属于目标项目视图。
...
- Skill MUST NOT 把源工作目录的 `.owner/current-change.json` 复制到目标；目标的当前 change MUST 由目标 worktree 中的 `owner native new` 或 `owner native select` 建立。

...
Loop 必须把 Builder 的“候选完成”与最终验收分开。Runtime 真实执行必要检查，Runner 为当前候选分派新的 Verifier execution，Runtime 只根据完整、结构化且由可信宿主通道绑定的结果决定通过、返回 Build 或等待用户。
...
- Runtime 为已确认验收项分配 `A1`、`A2` 等顺序 ID，并把完整来源与文字保存在 `owner-state.yaml`；ID 只用于结果映射，不由内容计算。
...
- Runtime 为候选生成随机 `candidate_id`，并把精简 Builder handoff 写入 `owner-state.yaml`。该 ID 只关联一次 execution，不代表项目内容或文件身份。
...
- Verifier 是只读角色，不修改实现、`owner-state.yaml` 或最终报告，也不直接推进 phase。
...
- 有效 pass 由 Runtime 原子写入 `owner-state.yaml`，再生成与该 `state_version` 对齐的 `verification.md`。
...
- 中英文 Loop Skill、Loop Runtime 源码和生成资产必须表达同一流程。

# Remove Loop Cryptographic Review

## Goal

Remove the Loop cryptographic external-review subsystem completely. A normal Loop workflow must be able to pass Verify using current acceptance evidence, required checks, implementation scope, snapshot bindings, and the verification report without provisioning cryptographic identities or obtaining an external signature.
...
Existing Loop changes or persisted evidence that depend on these removed schemas are not migrated. Users must recreate the change or regenerate verification evidence with the remaining receipt model.
...
The Loop verification receipt union is reduced to:
...
The generated Loop runtime is rebuilt from `domains/owner-loop/`; generated assets are never edited as the source of truth.
...
5. Rebuild the Loop runtime and run focused Loop tests first.
...
This is a user-visible breaking simplification. The existing unreleased version entry above `origin/master` is updated rather than adding another version. The changelog describes the final behavior only: Loop Verify no longer requires cryptographic external-review provisioning or signed review artifacts.

...
归档摘要和生命周期闭环说明必须使用 `owner state get <name> language` 读取到的 Owner 配置产物语言。
...
- 「需要调整或重新验证」— 不执行归档；运行 `owner state transition <change-name> archive-reopen` 回到 `phase: verify`，再调用 `/owner-verify`。若验证阶段确认需要修复，再按 `/owner-verify` 的验证失败决策回到 `/owner-build`
- 「暂不归档」— 不执行 `archive-confirm` 或归档命令，保留 active change、`phase: archive` 和 `branch_status: pending`，等待用户稍后再次调用 `/owner-archive`
...
如 transition 返回非零退出码，报告错误并停止。只有 transition 成功后，才允许继续 Step 2。用户选择「需要调整或重新验证」后，必须先执行 `archive-reopen` 状态回退，不得手动编辑 `.owner.yaml`。用户选择「暂不归档」后直接停止，不得归档、提交、推送或把 `branch_status` 设为 `handled`。
...
归档后读取 `git status --short`，并以归档前的 dirty-worktree 归因记录为基线。只允许暂存可归因于当前 change 的路径：原 active change 路径、脚本输出的实际 archive 路径、归档目录中已更新为 `branch_status: handled` 的 `.owner.yaml`、被本次 delta 更新的 main specs，以及当前 Design Doc/Plan 的归档元数据。存在无法归因的路径时停止并请求用户处理。

---
name: owner-hotfix
description: "Owner 预设 —— 通过 open-build-verify-archive 短流程修复已有行为 bug。"
---
...
**不适用**：如修复过程命中质变信号（见「升级判定」章节），由用户决定是否升级为完整 `/owner-pipeline` 流程。
...
恢复已有 hotfix change 时，第一项状态操作必须是 `owner state select <change-name>`；创建新 change 时，在 `.owner.yaml` 初始化成功后立即运行该命令，再进入源码写入步骤。

### 1. 快速开启（预设 open）

复用 Owner open 能力创建 change，但使用 hotfix 默认值：不执行 `openspec-explore` 长探索，直接进入精简 change 创建。
...
- B. 创建分支：先创建并切换到 `hotfix/YYYYMMDD/<change-name>`，再运行 `owner state set <name> isolation branch`
- C. 创建 worktree：必须先使用 Skill 工具加载 Superpowers `using-git-worktrees` 技能，由该技能创建隔离工作区；进入 worktree 后运行 `owner state set <name> isolation worktree`
...
使用 hotfix 默认值：`build_mode: direct`、`tdd_mode: direct`、`review_mode: off`。`isolation` 必须沿用 Step 1 中用户已确认的入口工作区隔离方式，不得自行改回 `current`。`direct` 表示不进入完整规划/TDD 编排，不表示可以跳过复现、回归测试或验证。跳过 Superpowers `brainstorming` 和 `writing-plans`；**任务数量本身不触发 `/owner-build`**，任务较多时仍在当前 hotfix 的 tasks.md 中按顺序执行，只有命中后文质变信号或范围 tripwire 才交给用户决定是否升级 full。

继续或开始修改前，按 `owner-pipeline/reference/dirty-worktree.md` 协议处理未提交改动。若归因后发现修复命中质变信号或文件数 tripwire，按本文件「升级判定」处理。
...
1. 读取 `<pipeline-change-dir>/tasks.md`，获取未完成任务列表
2. 对每个未完成任务：
   - 根据任务描述修改代码
...
**如修复影响已有 spec 验收场景**：
- 在 `<pipeline-change-dir>/specs/<capability>/spec.md` 创建 delta spec
...
复用 `/owner-verify`，由 owner-verify 的规模评估决定轻量或完整验证。

**立即执行：** 使用 Skill 工具加载 `owner-verify` 技能。禁止跳过此步骤。

无 delta spec 的小范围 hotfix 通常满足轻量验证条件（≤ 3 tasks、改动文件数低于 scale 阈值），owner-verify 的规模评估会选择轻量验证路径（6 项快速检查；默认 `review_mode: off` 时不自动派发代码审查）。若用户希望增加审查，可在验证前运行 `owner state set <name> review_mode standard` 或 `thorough`。若 hotfix 创建了 delta spec，则根据 owner-verify 的规模评估规则进入完整验证路径。

验证通过后，按 `/owner-verify` 的规则将 `.owner.yaml` 的 `verify_result` 记录为 `pass`，归档前不得跳过该状态。验证通过后仍必须进入 `/owner-archive` 的归档前最终确认，不得自动运行归档脚本。

### 5. 归档（预设 archive）

复用 `/owner-archive`。归档前必须满足 `.owner.yaml` 中 `verify_result: pass`，并等待 `/owner-archive` 的归档前最终确认。

**立即执行：** 使用 Skill 工具加载 `owner-archive` 技能进行归档。禁止跳过此步骤。
如有 delta spec，按 owner-archive 规则同步到 main spec，并处理关联 Design Doc 与 Plan 的归档标注。
...
<IMPORTANT>
Hotfix 流程默认 **一次性连续执行**。调用 `/owner-hotfix` 后，agent 在 hotfix 自有步骤间自动推进，不主动停顿。**例外**：若 `auto_transition: false`，则在每个 phase 边界（build/verify/archive 之间）结束当前调用并按 `HINT` 交还控制权，由用户稍后手动运行下一阶段命令；这是手动衔接，不是新的确认点。无论 `auto_transition` 取何值，以下真正的用户决策仍需暂停：

1. 遇到升级判定信号（见「升级判定」章节），**必须暂停、展示选择并等待用户明确选择**：继续 hotfix 流程，还是升级为完整 `/owner-pipeline` 流程
...
每个阶段完成后立即进入下一阶段。阶段内部仍必须按上文要求调用对应 Owner/OpenSpec/Superpowers skill，被调用的 skill 如有自己的用户决策点，按该 skill 规则执行。
</IMPORTANT>
...
hotfix 的升级判定只决定是否从预设流程转为 full；文件数不自动升级，`owner state scale` 只决定验证轻重。

若由 `/owner-pipeline` 入口传入 intent frame，hotfix 在 build 前只复核 `risk_signal` 和升级信号：新增 capability、public API、schema 变更、跨模块协调或深层架构问题。命中时进入现有升级决策点；不得重新实现入口意图识别。
...
- **阶段守卫**：build → verify 前运行 `owner guard <change-name> build --apply`，verify → archive 前按 `/owner-verify` 规则运行 `owner guard <change-name> verify --apply`
...
- `NEXT: auto` → 调用 `SKILL` 指向的 skill 继续 hotfix 流程（`phase: build` 返回 `owner-hotfix`，`verify` 返回 `owner-verify`，`archive` 返回 `owner-archive`）

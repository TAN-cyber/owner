...
只接受 `schema: owner.classic-layout.v1`。把返回的 `openSpecRoot`、`changesRoot`、`archiveRoot`、`specsRoot`、`superpowersRoot` 分别绑定为 `<classic-open-spec-root>`、`<classic-changes-root>`、`<classic-archive-root>`、`<classic-specs-root>`、`<classic-superpowers-root>`，并把 `<classic-change-dir>` 定义为 `<classic-changes-root>/<name>`。这些逻辑根是本轮事实源；恢复或上下文压缩后必须重新解析。

## 命令规则

- 本 Skill 及其他 Owner-owned Classic Skill 调用官方 OpenSpec CLI 时，必须直接使用：
...
- 普通 init/update 不移动旧产物。运行 `owner classic root move docs --dry-run` 只查看现状；用户确认后运行 `owner classic root move docs --apply` 直接迁移。迁移身份与锁内复检由 Runtime 内部管理。

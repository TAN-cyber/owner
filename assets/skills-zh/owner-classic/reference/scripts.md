...
规范路径：`owner-classic/reference/scripts.md`

本文件是 Classic Skill 调用 Owner Runtime 的单一事实来源。Skill 只使用 PATH 中的公开 `owner` CLI；随包发布的 `owner/scripts/*.mjs` 属于内部安装与 Runtime 资产，不由 Skill 搜索或直接调用。

...
规范路径：`owner-classic/reference/subagent-dispatch.md`
...
主会话必须维护 `<classic-change-dir>/.owner/subagent-progress.md`，并在每次派发、agent 回报、审查结果、修复轮次变化和 task 勾选后立即更新。检查点至少记录：
...
重新加载 Superpowers `subagent-driven-development` 技能并重新阅读本文档。先读取 `<classic-change-dir>/.owner/subagent-progress.md`，再与第一个未勾选 task 和当前工作树核对：

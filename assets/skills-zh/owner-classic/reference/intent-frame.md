...
本文件只在需要解释字段含义时读取。正常 `/owner-classic` 路由只需按主 Skill 的最小骨架填写；runtime 会补齐省略字段并输出最终 `route`。
...
| `utterance` | 触发 `/owner-classic` 的用户原话。 |
...
| 字段 | 含义 |
|------|------|
| `active_changes_count` | `owner classic openspec -- list --json` 得到的未归档 active change 数量。多个 active change 且用户未指定 `change_id` 时会 `ask_user`。 |
| `active_change_names` | active change 名称列表。用户指定 `change_id` 时，runtime 用它检查 change 是否存在。 |
| `dirty_worktree` | 可选状态字段。入口路由最小骨架不需要填写；dirty worktree 由 `owner-classic/reference/dirty-worktree.md` 专门处理。 |

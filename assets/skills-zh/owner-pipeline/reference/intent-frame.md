# OwnerIntentFrame 字段参考

本文件只在需要解释字段含义时读取。正常 `/owner-pipeline` 路由只需按主 Skill 的最小骨架填写；runtime 会补齐省略字段并输出最终 `route`。

## 顶层字段

| 字段 | 含义 |
|------|------|
| `schema_version` | frame 版本，当前固定为 `owner.intent.v1`。 |
| `utterance` | 触发 `/owner-pipeline` 的用户原话。 |
...
| `schema_change` | 是否改变结构化数据格式，例如 `.owner.yaml`、`run-state.json`、eval manifest、bundle manifest、配置 schema。命中时通常倾向 `full`。 |
...
| 字段 | 含义 |
|------|------|
| `active_changes_count` | `owner pipeline openspec -- list --json` 得到的未归档 active change 数量。多个 active change 且用户未指定 `change_id` 时会 `ask_user`。 |
| `active_change_names` | active change 名称列表。用户指定 `change_id` 时，runtime 用它检查 change 是否存在。 |
| `dirty_worktree` | 可选状态字段。入口路由最小骨架不需要填写；dirty worktree 由 `owner-pipeline/reference/dirty-worktree.md` 专门处理。 |
...
| `quote` | 来自用户原话、仓库状态或 `.owner.yaml` 的证据片段。 |

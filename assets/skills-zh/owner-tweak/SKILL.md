---
name: owner-tweak
description: "Owner 预设 —— 处理可收敛为单一 OpenSpec change 的轻量或中等变更。"
---
...
复用 Owner open 能力创建 change，但使用 tweak 默认值：不执行 `openspec-explore` 长探索，直接进入精简 change 创建。
...
**带 delta spec 的验证分流**：tweak 接受 delta spec 作为正常产物。若本次 change 创建了 delta spec，进入 owner-verify 前显式设置完整验证模式，走 OpenSpec 原生验证（`openspec-verify-change`）以覆盖 delta spec 一致性：

```bash
owner state set <change-name> verify_mode full
```

无 delta spec 的 tweak 通常满足轻量验证条件（≤ 3 tasks、改动文件数低于 scale 阈值），由 owner-verify 的规模评估选择轻量验证路径（6 项快速检查）。若用户希望增加审查，可在验证前运行 `owner state set <name> review_mode standard` 或 `thorough`。
...
- `NEXT: auto` → 调用 `SKILL` 指向的 skill 继续 tweak 流程（`phase: build` 返回 `owner-tweak`，`verify` 返回 `owner-verify`，`archive` 返回 `owner-archive`）

...
验证报告必须使用 `owner state get <name> language` 读取到的 Owner 配置产物语言。
...
**覆盖机制**：如 agent 或用户认为自动评估结果不合适，可随时通过 `owner state set <change-name> verify_mode <light|full>` 手动覆盖。

### 1b. 验证失败自动修复与例外决策

先运行 `owner state get <change-name> verify_failures` 读取已持久化的连续失败次数。前 3 次可修复失败自动回到 build：报告失败项后运行 `owner state transition <change-name> verify-fail`，再调用 `/owner-build` 修复，不需要用户确认。
...
```bash
owner state record-check <change-name> verify --command "<实际运行的验证命令>" --exit-code 0
```

`--command` 只记录命令文本，Owner **绝不会执行该文本**。verify 与 build 证据彼此独立，不能互相替代；即使兼容流程使用 `OWNER_SKIP_BUILD=1`，也不能把该绕过标记视为可审计的验证或构建证据。
...
**不通过时**：报告失败项并按 Step 1b 分类。未达到自动修复上限且问题必须或适合修复时，直接执行以下命令回到 build 阶段，然后调用 `/owner-build`：
...
验证不通过时：报告缺失项并按 Step 1b 分类。未达到自动修复上限且缺失项可在当前 change 内补齐时，直接执行以下命令回到 build 阶段，然后调用 `/owner-build`：
...
- 若检查项 6 发现矛盾（delta spec 有内容但 design doc 未体现），**必须以单选题形式暂停、展示处理方式并等待用户选择**，不得自动选择。选项：
  ...
  - 选项 B：用户选择 B 后，运行 `owner state transition <change-name> verify-fail`，然后调用 `/owner-build`；由 `/owner-build` 的 Spec 增量更新规则加载 Superpowers `brainstorming` 更新 Design Doc + delta spec
  - 选项 C：确认偏差可接受，继续验证（归档时 design doc 将标记为 `superseded-by-main-spec`）

### 3. 记录验证证据

验证报告必须落盘，并在 `.owner.yaml` 中记录。不要在 verify 阶段处理、合并或丢弃分支，也不要写入 `branch_status: handled`；归档会产生必须包含在最终提交中的 spec 和元数据改动，分支收尾统一由 `/owner-archive` 在归档提交后执行。不要手动设置 `verify_result: pass`，由阶段守卫 `--apply` 推进。
...
- 验证报告通过
- `.owner.yaml` 中 `verification_report` 指向已存在的验证报告文件
- `.owner.yaml` 中 `branch_status` 仍为 `pending`
- **阶段守卫**：运行 `owner guard <change-name> verify --apply`，全部 PASS 后由守卫通过 `owner state transition verify-pass` 推进到 `phase: archive`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）
...
注意：无论 `NEXT` 为 `auto` 还是 `manual`，`owner-archive` 进入后必须先执行归档前最终确认阻塞点，等待用户明确选择「确认归档」后才允许运行归档脚本。不得因为验证已通过就自动归档。

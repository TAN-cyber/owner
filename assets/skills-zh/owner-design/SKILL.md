...
```bash
owner handoff <change-name> design --write
```

脚本会根据 change `.owner.yaml` 的 `context_compression` 快照生成并记录交接包。
...
```bash
owner handoff <change-name> design --write --full
```
...
```bash
# 记录 design_doc 路径
owner state set <name> design_doc docs/superpowers/specs/YYYY-MM-DD-topic-design.md

# 如有 delta spec 变更，重新生成 handoff（更新 hash）
owner handoff <change-name> design --write

# 阶段守卫推进 phase 到下一阶段
owner guard <change-name> design --apply
```
...
- Design Doc 已创建并保存
- Design Doc frontmatter 包含 `owner_change`、`role: technical-design`、`canonical_spec: openspec`
- `handoff_context` 和 `handoff_hash` 已写入 `.owner.yaml`（由 guard 强制校验）
...
- `design_doc` 已写入 `.owner.yaml`
- **阶段守卫**：运行 `owner guard <change-name> design --apply`，全部 PASS 后由守卫推进到 `phase: build`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）

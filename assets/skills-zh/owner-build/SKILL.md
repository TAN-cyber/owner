...
通过 subagent 创建实施计划，避免 planning skill 占用主 session 上下文。计划文件和执行反馈必须使用 `owner state get <name> language` 读取到的 Owner 配置产物语言。
...
展示联合决策时，只提供本工作流支持的执行方式、TDD 模式和代码审查模式。工作区已经在 Open 阶段准备并绑定；若当前 change 没有有效 isolation，返回 `/owner-open` 修复，不在 Build 创建或切换工作区。
...
如果结果为空，停止 Build 并返回 `/owner-open` 执行 workspace resolve/prepare；不得在本步骤首次选择或创建 current、branch、worktree。
...
- 若用户选择 `executing-plans`：运行 `owner state set <name> subagent_dispatch null`，再运行 `owner state set <name> build_mode executing-plans`
- 若用户选择 `subagent-driven-development`：先运行 `owner state set <name> subagent_dispatch confirmed` 记录已选择子代理执行，再运行 `owner state set <name> build_mode subagent-driven-development`
...
运行 `owner state set <name> tdd_mode <tdd|direct>`
...
运行 `owner state set <name> review_mode <off|standard|thorough>`
...
`subagent_dispatch` 是脚本级硬约束，用于记录用户已选择子代理执行。`build_mode: subagent-driven-development` 离开 build 阶段前必须同时满足 `subagent_dispatch: confirmed`，否则 `owner guard build --apply` 和 `owner state transition build-complete` 都会失败；它不是能力检查。

`tdd_mode` 是脚本级硬约束。full workflow 离开 build 阶段前 `tdd_mode` 必须已选择为 `tdd` 或 `direct`，否则 `owner guard build --apply` 和 `owner state transition build-complete` 都会失败。

`review_mode` 是脚本级硬约束。新建 full workflow 离开 build 阶段前 `review_mode` 必须已选择为 `off`、`standard` 或 `thorough`，否则 `owner guard build --apply` 和 `owner state transition build-complete` 都会失败。旧状态文件若没有该字段，按兼容路径继续，但恢复时应补写该字段。
...
- `requesting-code-review` 技能必须在 `owner guard <change-name> build --apply` 之前加载
...
**用户选择必须包含**：
- 「拆分为新 change」— 通过 `/owner-open` 创建独立 change
...
- **阶段守卫**：运行 `owner guard <change-name> build --apply`，全部 PASS 后由守卫推进到 `phase: verify`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）
...
```bash
owner state record-check <change-name> build --command "<实际运行的构建命令>" --exit-code 0
```

`--command` 只记录命令文本，Owner **绝不会执行该文本**。build 与 verify 证据彼此独立，不能互相替代。`OWNER_SKIP_BUILD=1` 仅是旧流程的兼容绕过方式，不是可审计的构建证据。

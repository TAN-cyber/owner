...
Read this file only when field meanings are needed. Normal `/owner-classic` routing only needs the minimal skeleton in the main Skill; the runtime fills omitted defaults and outputs the final `route`.
...
| `utterance` | The user request that triggered `/owner-classic`. |
...
| Field | Meaning |
|-------|---------|
| `active_changes_count` | Number of unarchived active changes from `owner classic openspec -- list --json`. Multiple active changes without a `change_id` route to `ask_user`. |
| `active_change_names` | Names of active changes. When the user supplies `change_id`, the runtime checks it against this list. |
| `dirty_worktree` | Optional state field. The entry-route minimal skeleton does not need it; dirty worktree handling belongs to `owner-classic/reference/dirty-worktree.md`. |

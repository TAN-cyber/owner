...
Canonical path: `owner-classic/reference/owner-yaml-fields.md`

This file is the field reference for each change-level `.owner.yaml` state file under `<classic-change-dir>/`. `<classic-change-dir>` comes from the resolver binding in `owner-classic/reference/classic-layout.md`.
...
| `language` | Artifact language, `en` or `zh-CN`. Written to `classic.language` in the project or global `.owner/config.yaml` according to install scope, snapshotted into `.owner.yaml` with project-over-global precedence when a change is created, and used as the main-language constraint for OpenSpec / Superpowers artifacts |

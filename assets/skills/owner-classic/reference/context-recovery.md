...
Canonical path: `owner-classic/reference/context-recovery.md`
...
The user may resume the workflow directly from `/owner-open`, `/owner-design`, `/owner-build`, `/owner-verify`, `/owner-archive`, `/owner-hotfix`, or `/owner-tweak`. On entry to any sub-skill, use `owner-classic/reference/scripts.md` to run the public CLI command, then run the entry check or recovery check for that sub-skill's phase. Do not infer phase from conversation history.

```bash
owner state check <change-name> <phase> --recover
```

If the check shows the actual phase, workflow, or evidence belongs to another skill, switch according to script output and `/owner-classic` routing rules; do not keep writing state in the wrong phase. If the worktree has uncommitted changes, attribute them first via `owner-classic/reference/dirty-worktree.md`.

## Recovery Without Explicit `/owner-classic`

If the user did not mention `/owner-classic`, but this repository may have an active change, run the Ambient Resume probe before starting work that may need code changes or investigation. Use `owner-classic/reference/scripts.md` to run the public CLI command, then pass the current user request on stdin:
...
3. Read `<classic-change-dir>/.owner/subagent-progress.md` to recover the current task or final review, implementation commit, RED/GREEN evidence, passed reviews, unresolved feedback, and review-fix round

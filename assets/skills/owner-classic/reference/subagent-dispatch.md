...
Canonical path: `owner-classic/reference/subagent-dispatch.md`
...
The coordinator must maintain `<classic-change-dir>/.owner/subagent-progress.md` and update it immediately after every dispatch, agent return, review result, review-fix round change, and task checkoff. The checkpoint must record at least:
...
owner state task-checkoff "<classic-change-dir>/tasks.md" "<openspec-task-text>"
```
...
Reload the Superpowers `subagent-driven-development` skill and re-read this document. Read `<classic-change-dir>/.owner/subagent-progress.md`, then compare it with the first unchecked task and the current worktree:

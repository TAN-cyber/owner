---
name: owner-pipeline
description: "Owner Pipeline workflow (OpenSpec + Superpowers). Use when the user invokes /owner-pipeline, asks to start or resume it, or resume-probe returns an unambiguous active Pipeline change."
---

# Owner Pipeline — OpenSpec + Superpowers Dual-Star Development Workflow
...
Use the configured Owner artifact language as the output language for every OpenSpec and Superpowers artifact. The configured value is a normalized language id, `en` or `zh-CN`. For an existing change, read `language` from `<pipeline-change-dir>/.owner.yaml` using `owner state get <name> language`. Before `.owner.yaml` exists, read `pipeline.language` from project `.owner/config.yaml`, then fall back to global `~/.owner/config.yaml`; if neither exists, fall back to the current user request language. Include the resolved language explicitly in every prompt or ARGUMENTS passed to external OpenSpec/Superpowers skills.

### Automatic Phase Detection

**Step 0: Active Change Discovery and Intent Resolution**

1. First follow `owner-pipeline/reference/scripts.md` and run the public `owner` CLI command directly.
2. Run `owner pipeline openspec -- list --json` to collect active changes.
3. Fill a `OwnerIntentFrame` from the user request, active change list, and necessary repository state.
4. Prefer `owner pipeline intent route --stdin` to pass the frame JSON and get the runtime-normalized route. `OwnerIntentFrame + runtime scorer` is the source of truth; this prose is only for intent recognition slot extraction.
5. Handle the runtime route:
   ...
   - `ask_user` → pause through `owner-pipeline/reference/decision-point.md` and wait for the user's choice
...
When the user did not explicitly invoke `/owner-pipeline`, but this repository may already have an active Owner change, run the read-only probe before starting work that may need code changes or investigation:
...
See `owner-pipeline/reference/intent-frame.md` for complete field meanings; normal routing only needs the minimal skeleton above.

- `fix_bug` + `existing_behavior: true` + no new capability/public API/schema/cross-module signal → prefer `hotfix`
- User explicitly describes a lightweight/medium change that can fit in a single OpenSpec change, should be executed through OpenSpec apply, and does not need full `/owner-pipeline` deep design/plan → prefer `tweak`
...
| Exactly 1 | `/owner-pipeline <description>` | → **Ask**: continue this change or create a new change |
| Multiple | `/owner-pipeline <description>` | → **Ask**: continue existing or create new; if continuing, list changes for selection |
| Exactly 1 | `/owner-pipeline` with no description | → Auto-select, enter Step 1 |
| Multiple | `/owner-pipeline` with no description | → List changes for user selection |
...
Prefer reading `<pipeline-change-dir>/.owner.yaml`. If not available, fall back to `owner pipeline openspec -- status --change "<name>" --json`, `<pipeline-change-dir>/tasks.md`, and `<pipeline-superpowers-root>/` file checks.

**Resume rules**:
- On every context resume, rerun Step 0 and Step 1; do not trust conversation history for phase detection
- If there is an active change and the worktree has uncommitted changes, handle them through `owner-pipeline/reference/dirty-worktree.md`. That protocol defines checks, attribution, and prohibitions; this file does not repeat them
...
- Upgrade to full `/owner-pipeline` (use `owner state transition <name> preset-escalate` to legally rewind to design and clear preset-only build settings; after the Design Doc, choose the full workflow configuration again in one joint decision)
...
| Scenario | Handling |
|----------|----------|
| `owner pipeline openspec -- list --json` fails | Check whether OpenSpec is installed; if the artifact root is missing or damaged, prompt the user to run `owner update --scope project` or rerun `owner init --scope project` |
...
<IMPORTANT>
A single `/owner-pipeline` invocation starts from the detected phase and advances to the next phase when exit conditions are met.
...
**Decision points are blocking points**: whenever reaching any of the following nodes, the current `/owner-pipeline` invocation must stop, and follow the `owner-pipeline/reference/decision-point.md` protocol to obtain the user's explicit choice. Only after the user explicitly chooses can the corresponding state fields be written and operations executed, then auto-advance resumes.
...
See `owner-pipeline/reference/owner-yaml-fields.md` for complete field reference with examples and descriptions.

### File Structure

See `owner-pipeline/reference/file-structure.md` for the complete directory layout and artifact organization.

### Auto-Transition Protocol

See `owner-pipeline/reference/auto-transition.md` for the complete automatic handoff workflow.

### Context Recovery

See `owner-pipeline/reference/context-recovery.md` for structured recovery after context compression.

### Decision Point Protocol

See `owner-pipeline/reference/decision-point.md` for the complete user decision point protocol.

### Debug Gate Protocol

See `owner-pipeline/reference/debug-gate.md` for the complete debug gate protocol.

### Public CLI

Follow `owner-pipeline/reference/scripts.md` once per session and use only the public `owner` CLI. Do not search for or invoke internal bundles. Key entry points:

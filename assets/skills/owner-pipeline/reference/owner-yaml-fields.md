...
Canonical path: `owner-pipeline/reference/owner-yaml-fields.md`

This file is the field reference for each change-level `.owner.yaml` state file under `<pipeline-change-dir>/`. `<pipeline-change-dir>` comes from the resolver binding in `owner-pipeline/reference/pipeline-layout.md`.
Consult on demand; not loaded inline with skills. Project defaults live in `.owner/config.yaml`, global defaults live in `~/.owner/config.yaml`, and project values take precedence.
...
| `language` | Artifact language, `en` or `zh-CN`. Written to `pipeline.language` in the project or global `.owner/config.yaml` according to install scope, snapshotted into `.owner.yaml` with project-over-global precedence when a change is created, and used as the main-language constraint for OpenSpec / Superpowers artifacts |
...
- These constraints exist in both `owner-guard.mjs build --apply` and `owner-state.mjs transition <name> build-complete`
...
- `preset-escalate` event: only allows `hotfix`/`tweak` workflow at `phase: build`; atomically sets `workflow`/`pipeline_profile` to `full`, rewinds `phase` to `design`, and clears `design_doc` (satisfying the owner-design entry requirement). This is the only legal channel for a preset → full upgrade — direct `set phase design` is hard-blocked by the state machine, and `set pipeline_profile` is a machine-owned field that cannot be set manually

...
Canonical path: `owner-pipeline/reference/file-structure.md`

This file is the Owner project file structure reference. Consult on demand; not loaded inline with skills.

```text
<pipeline-open-spec-root>/              # OpenSpec — WHAT; returned by the Pipeline layout resolver
├── config.yaml
├── changes/
│   ├── <name>/                        # Active change
│   │   ├── .openspec.yaml
...
<pipeline-superpowers-root>/            # Superpowers — HOW; returned by the Pipeline layout resolver

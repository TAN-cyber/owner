...
Canonical path: `owner-pipeline/reference/scripts.md`

This file is the single source of truth for Pipeline Skill calls into the Owner Runtime. Skills use only the public `owner` CLI on PATH. The packaged `owner/scripts/*.mjs` files are internal installation and Runtime assets; Skills do not search for or invoke them directly.

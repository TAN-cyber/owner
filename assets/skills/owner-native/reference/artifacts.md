...
Acceptance criteria must be specific, observable, and non-duplicative. Use simple sequential IDs such as `A1`, `A2`, and `A3`. IDs map results only; they are not derived from content and do not identify files. On Shape confirmation, the Runtime saves the full acceptance text and its source. A Supervisor Change's `children.yaml` uses `owner.native.children.v1`; each child contains only `name`, `depends_on`, and `covers`. Names must be unique, dependencies must exist and be acyclic, and every Supervisor Change acceptance ID must be covered. The Runtime confirms it together with the Supervisor Change acceptance; any change returns the Supervisor Change to Shape.

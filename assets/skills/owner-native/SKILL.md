---
name: owner-native
description: "Owner Native workflow. Use when the user explicitly invokes /owner-native, asks to start or resume a Native change, or the entry router selects Native."
---
...
- The Runtime manages workflow state, local tasks, logs, locks, and transactions. Advance every phase through the public `owner native` commands on PATH; users do not run these commands manually.
- If a command is unavailable, report an incomplete Owner installation and stop. Treat `owner native <command> --help` as authoritative for arguments and output.
...
1. When the change name is known, run `owner native status <change-name> --details --json` directly. Only run `owner native status --json` when the name is unknown, then query the selected change in detail.
...
On `/owner-native` resume, continue from Runtime state and do not duplicate existing children or worktrees.

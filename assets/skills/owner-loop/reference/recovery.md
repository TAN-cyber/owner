...
If the project root, branch, workspace type, or Git state differs from the record in `owner-state.yaml`, the Runtime prevents writes. When it can safely locate or create the declared worktree, follow the returned action. Otherwise it enters `await-user`.
...
`owner-state.yaml` records the last workflow state that can be resumed safely. Local `state.json` only says what this machine is executing. If it is missing, stale, or belongs to an old task, the Runtime rebuilds it from YAML, the brief, and target Specs. Local state cannot overwrite newer YAML.
...
To resume on a new device with no chat history, obtain the same synchronized project code, `owner-state.yaml`, brief, and target Specs. Also synchronize `.owner/config.yaml` when the change uses a non-default artifact directory.
...
After repeated rounds without progress or repeated Verifier task errors, follow the blocker action returned by the Runtime. When failed iterations reach `loop.max_verify_failures`, the workflow enters `await-user` so the user can continue the current target, change the confirmed requirements, or stop.

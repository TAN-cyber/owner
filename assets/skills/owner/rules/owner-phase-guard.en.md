# Owner Phase Awareness (Anti-Drift Rules)

> This rule is injected every round to prevent forgetting Owner workflow state during long context.
> The Hook platform additionally executes `owner-hook-guard.mjs` for hard interception;
...
Reading the `phase` field alone is not enough — you must also confirm **how** that phase was reached. Before writing any source code, self-check whether `.owner.yaml` is in an **illegal jump** state (a prior phase was skipped) using the table below. If any row matches, immediately stop writing source code, go back to the corresponding phase to fill the missing artifact, and do not trust the `phase` field to keep going.

| Detected | Verdict | Action |
|----------|---------|--------|
| `phase: build` + `workflow: full` + `design_doc` empty/null | Skipped design | Stop writing source; run `/owner-design` to create the Design Doc and pass guard |
| `phase: build` + `workflow: full` + `plan` empty/null or its recorded path is invalid | Skipped implementation plan | Stop writing source; return to `/owner-build` Step 1 to create or restore the plan, record its path, and verify with `owner state check <name> build --recover` |
| `phase: build` + any of proposal/design/tasks missing or empty | Skipped open | Return to `/owner-open` to fill the three artifacts |
| `phase: archive` + `verify_result` ≠ `pass` | Skipped verify | Return to `/owner-verify` to complete verification |
...
Upgrade state note: after a preset (hotfix/tweak) hits an upgrade signal and the user confirms upgrading, `owner state transition <name> preset-escalate` legally converts it to `workflow: full` + `phase: design` + `design_doc: null` and clears preset-only build settings. At this point `phase: design` with an empty `design_doc` **is a normal upgrade pre-state**, not an illegal jump — the agent should enter `/owner-design` to supplement the Design Doc, then choose the full workflow configuration again in build. This terminal state does not match the "skipped design" row above (that row only detects `phase: build`).
...
- **Phase exit**: `owner guard <name> <phase> --apply` (must see ALL CHECKS PASSED)
- **Compression recovery**: `owner state check <name> <phase> --recover`
- **State update**: After key operations, update fields through `owner state set`; manually editing .owner.yaml is prohibited
- **Phase advancement only via guard/transition**: directly running `owner state set <name> phase <value>` to jump phases is prohibited. A preset (hotfix/tweak) upgrade to full must use `owner state transition <name> preset-escalate`
- **handoff generation**: `owner handoff <name> design --write` (handwriting summaries is prohibited)
...
1. First script operation = `owner handoff <name> design --write` (loading brainstorming before generating handoff is prohibited)
...
1. First step run `owner state scale <name>` to determine verification level
2. For the first 3 clearly repairable failures, automatically run `owner state transition <name> verify-fail`, return to build, and enter `/owner-build`. CRITICAL/IMPORTANT failures cannot be accepted as deviations
...
```bash
owner state check <name> <phase> --recover
```

...
> 此规则每轮注入，防止长上下文时遗忘 Owner 流程状态。
> Hook 平台额外执行 `owner-hook-guard.mjs` 进行硬性拦截；
> 此 Rule 是所有平台通用的软性防线。
...
仅看 `phase` 字段不够——还必须确认"是如何到达这个阶段的"。每次准备写源代码前，先用下表自检 `.owner.yaml` 是否处于**非法空跳**状态（绕过了前置阶段）。命中任一行，立即停止写源代码，按动作回到对应阶段补齐产物，不得信任 `phase` 字段直接续跑。

| 检测到 | 判定 | 动作 |
|--------|------|------|
| `phase: build` + `workflow: full` + `design_doc` 为空/null | 绕过 design 空跳 | 停止写源代码，运行 `/owner-design` 补 Design Doc 并过 guard |
| `phase: build` + `workflow: full` + `plan` 为空/null 或记录路径失效 | 绕过实施计划 | 停止写源代码，回 `/owner-build` Step 1 创建或恢复 plan，记录路径并用 `owner state check <name> build --recover` 验证 |
| `phase: build` + proposal/design/tasks 任一缺失或为空 | 绕过 open 空跳 | 回 `/owner-open` 补齐三件套 |
| `phase: archive` + `verify_result` ≠ `pass` | 绕过 verify 空跳 | 回 `/owner-verify` 完成验证 |
...
升级态说明：预设（hotfix/tweak）命中升级信号并经用户确认升级后，通过 `owner state transition <name> preset-escalate` 合法地变为 `workflow: full` + `phase: design` + `design_doc: null`，同时清除预设专属 build 配置。此时 `phase: design` + `design_doc` 为空**属正常升级前置态**，不是非法空跳——agent 应进入 `/owner-design` 补 Design Doc，并在 build 重新完成联合工作方式选择。该终态不命中上表「绕过 design 空跳」行（该行仅检测 `phase: build`）。
...
- **阶段退出**: `owner guard <name> <phase> --apply`（必须看到 ALL CHECKS PASSED）
- **压缩恢复**: `owner state check <name> <phase> --recover`
- **状态更新**: 关键操作后通过 `owner state set` 更新字段，禁止手工编辑 .owner.yaml
- **阶段推进只能经 guard/transition**: 禁止用 `owner state set <name> phase <值>` 手动跳阶段；预设升级到 full 必须用 `owner state transition <name> preset-escalate`
- **handoff 生成**: `owner handoff <name> design --write`（禁止手写摘要）
...
1. 第一个脚本操作 = `owner handoff <name> design --write`（未生成 handoff 禁止加载 brainstorming）
...
1. 第一步运行 `owner state scale <name>` 确定验证级别
2. 前 3 次明确可修复失败自动运行 `owner state transition <name> verify-fail` 回到 build，并进入 `/owner-build`；CRITICAL/IMPORTANT 不得接受偏差

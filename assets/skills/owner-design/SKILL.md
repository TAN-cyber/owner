...
The script reads the change `.owner.yaml` `context_compression` snapshot, then generates and records the matching handoff package.
...
- Design Doc frontmatter contains `owner_change`, `role: technical-design`, `canonical_spec: openspec`
- `handoff_context` and `handoff_hash` written to `.owner.yaml` (enforced by guard)
...
- `design_doc` written to `.owner.yaml`
- **Phase guard**: Run `owner guard <change-name> design --apply`; after all PASS, auto-transitions to `phase: build`

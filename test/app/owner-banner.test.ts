...
import {
  OWNER_BANNER_LINE_COUNT,
  OWNER_LOGO,
  OWNER_TAGLINE,
  canAnimateOwnerBanner,
  createBannerStreamWriter,
  printOwnerBanner,
  renderOwnerBanner,
  renderOwnerBannerFrame,
} from '../../app/cli/owner-banner.js';
...
describe('Owner CLI banner rendering', () => {
  it('centers one fixed-grid ASCII logo block over the left-aligned tagline', () => {
    const banner = renderOwnerBanner();
    const lines = banner.split('\n');
    const cliIndent = '  ';
    const canvasWidth = cliIndent.length + OWNER_TAGLINE.length;
    ...
    expect(lines.at(-1)?.startsWith(`${cliIndent}${OWNER_TAGLINE}`)).toBe(true);
    expect(lines.at(-1)?.length).toBe(canvasWidth);

    const logoLines = lines.slice(0, OWNER_LOGO.length);
    const logoLeft = Math.min(...logoLines.map((line) => line.search(/\S/)));
    const logoRight = Math.max(...logoLines.map((line) => line.trimEnd().length));
    const taglineLeft = lines.at(-1)?.indexOf(OWNER_TAGLINE) ?? -1;
    const taglineRight = taglineLeft + OWNER_TAGLINE.length;
  ...
  it('renders the tagline in rgb(208, 151, 53) when color is enabled', () => {
    const banner = renderOwnerBanner({ color: true });

    expect(banner).toContain(`\u001b[38;2;208;151;53m  ${OWNER_TAGLINE}`);
  });

  it('uses deep blue, bright cyan-blue, and rgb(77, 197, 242) across a sweep frame', () => {
    const frame = renderOwnerBannerFrame(24, 1);
    ...
    expect(visibleLines.every((line) => line.length === OWNER_TAGLINE.length + 2)).toBe(true);
  ...
  it.each([
    ...
    [{ isTTY: true, env: {}, getColumns: () => OWNER_TAGLINE.length + 1 }, false],
    ...
    expect(canAnimateOwnerBanner(runtime)).toBe(expected);
  });

  it('prints plain static output for narrow or unknown terminal widths', async () => {
    for (const columns of [OWNER_TAGLINE.length + 1, undefined]) {
      const chunks: string[] = [];
      await printOwnerBanner({
        runtime: {
          isTTY: true,
          env: {},
          getColumns: () => columns,
      ...
      expect(chunks.join('')).toContain(OWNER_TAGLINE);
  ...
  it('keeps the entire static banner left-aligned in a wide terminal', async () => {
    ...
    const baseLines = renderOwnerBanner().split('\n');
  ...
  it('plays a vivid 1.8 second three-act sequence and leaves a stable final frame', async () => {
    ...
    expect(output).toContain('•');
    expect(output).toContain(OWNER_TAGLINE);
    ...
    expect(animationFrames.some((frame) => !frame.includes(OWNER_TAGLINE))).toBe(true);
    const center = Math.floor(OWNER_TAGLINE.length / 2);
    const centerSnippet = OWNER_TAGLINE.slice(center - 5, center + 5);
    expect(
      animationFrames.some(
        (frame) => frame.includes(centerSnippet) && !frame.includes(OWNER_TAGLINE),
      ),
    ).toBe(true);

    const lastParticleFrame = chunks.findLastIndex((chunk) => chunk.includes('·'));
    const stableFrame = chunks.findIndex((chunk) => chunk.includes(OWNER_LOGO[0]));
  ...
  it('prints plain static output when animation is unavailable or fails', async () => {
    ...
    expect(plainChunks.join('')).toContain(OWNER_TAGLINE);
    ...
    const cleanupStart = fallback.indexOf(`\u001b[${OWNER_BANNER_LINE_COUNT}A`);
    const firstStaticLine = renderOwnerBanner().split('\n')[0] ?? '';
    const staticBannerStart = fallback.indexOf(firstStaticLine);
    expect(fallback).not.toContain('\u001b[?25h');
    expect(cleanupStart).toBeGreaterThan(-1);
    expect(fallback.split('\u001b[2K')).toHaveLength(OWNER_BANNER_LINE_COUNT + 1);

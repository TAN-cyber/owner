...
async function installManagedOwnerSkills(baseDir: string, platformDir = '.claude'): Promise<void> {
...
async function state(cwd: string, ...args: string[]) {
  const configPath = path.join(cwd, '.owner', 'config.yaml');
  try {
    await fs.access(configPath);
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error;
    await fs.mkdir(path.join(cwd, '.owner'), { recursive: true });
    await fs.writeFile(
      configPath,
      [
...
describe('doctor command', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `owner-doctor-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('reports a secondary worktree using a complete global fallback without calling it broken', async () => {
    const secondary = path.join(
      os.tmpdir(),
      `owner-doctor-secondary-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    ...
    try {
      ...
      await installManagedOwnerSkills(fakeHome);
      ...
      expect(payload.results.find((result) => result.check === 'Owner skills')).toMatchObject({
        status: 'pass',
        message: expect.stringContaining('secondary worktree'),
      });
      expect(JSON.stringify(payload.results)).not.toContain(
        'not installed in project scope — run: owner init --scope project',
      );
      ...
      await installManagedOwnerSkills(secondary);
  ...
  it('projects the primary worktree Router into a secondary worktree during project repair', async () => {
    const secondary = path.join(
      os.tmpdir(),
      `owner-doctor-hook-secondary-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    const fakeHome = path.join(tmpDir, 'fake-home');
    const git = (...args: string[]) =>
      spawnSync('git', ['-C', tmpDir, ...args], { encoding: 'utf8', timeout: 20_000 });
    try {
      ...
      expect(git('add', 'README.md', '.owner/config.yaml').status).toBe(0);
      expect(git('commit', '-m', 'test').status).toBe(0);
      ...
      expect(hooks.replaceAll('\\', '/')).toContain(
        `${secondary.replaceAll('\\', '/')}/.agents/skills/owner/scripts/owner-hook-router.mjs`,
      );
      await expect(
        fs.access(
          path.join(secondary, '.agents', 'skills', 'owner', 'scripts', 'owner-hook-router.mjs'),
        ),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(secondary, '.owner', 'current-change.json')),
  ...
  it('falls back to a global Router source when the primary worktree has none', async () => {
    const secondary = path.join(
      os.tmpdir(),
      `owner-doctor-global-hook-secondary-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('does not repair a project config transaction while its writer is still active', async () => {
    ...
    expect(
      (await fs.readdir(path.join(tmpDir, '.owner'))).filter(
        (entry) =>
  ...
  it('reports OpenSpec skills and command files nested under docs for Claude and Codex roots', async () => {
    ...
    expect(platformAssets).toMatchObject({
      status: 'fail',
      message: expect.stringMatching(
        /platform directories at the project root.*owner update.*Doctor did not move/iu,
      ),
    });
    for (const platformRoot of platformRoots) {
  ...
  it('fails closed without following a linked platform directory under docs', async () => {
    ...
    try {
      ...
      expect(platformAssets).toMatchObject({
        status: 'fail',
        message: expect.stringMatching(/symbolic link or junction.*owner update/iu),
      });
  ...
  it.each(['configured', 'alternate'] as const)(
    ...
    async (kind) => {
      ...
      try {
        ...
        await fs.writeFile(
          path.join(outsideRoot, 'changes', 'external-marker', '.owner.yaml'),
          'phase: open\n',
          'utf8',
        );
        await fs.mkdir(path.join(tmpDir, '.owner'), { recursive: true });
        ...
        if (kind === 'configured') {
          ...
          expect(layoutResult).toMatchObject({
            status: 'pass',
            message: expect.stringMatching(/standalone OpenSpec root .* ignored by Owner/iu),
          });
        }
  ...
  it('fails working-directory health when the Superpowers root is a directory link', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-doctor-superpowers-link-'));
    try {
      await fs.mkdir(path.join(tmpDir, '.owner'), { recursive: true });
      await fs.writeFile(
        path.join(tmpDir, '.owner', 'config.yaml'),
  ...
  it('fails a Classic change check without reading through its runtime directory link', async () => {
    ...
    const runtimeDir = path.join(changeDir, '.owner');
    await fs.rm(runtimeDir, { recursive: true, force: true });
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-doctor-runtime-link-'));
    ...
    try {
      ...
      const results = await collectDoctorResults(tmpDir);
      const changeCheck = results.find((result) => result.check === '.owner.yaml: runtime-link');
  ...
  it('accepts current Owner state fields while a standalone OpenSpec root coexists', async () => {
    ...
    expect(layoutResult?.message).toContain('ignored by Owner');
    const stateResult = results.find((result) => result.check === '.owner.yaml: current-state');
  ...
  it('prints the current Owner version in text output', async () => {
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    let output: string;
    try {
      await doctorCommand(tmpDir);
    ...
    expect(output).toContain('Owner CLI: installed (');
  });
  ...
  it('explains auto scope and treats global installs as available when project scope is empty', async () => {
    ...
    expect(output).toContain(
      'Project scope: no project-local Owner skills installed; global scope is available',
    );
    expect(output).toContain(
      'run: owner init --scope project only if this project needs its own copy',
    );
    expect(output).not.toContain('skills: Claude Code (project): missing');
  });

  it('does not report non-Owner skill directories as missing Owner installs in auto scope', async () => {
    ...
    expect(output).toContain(
      'Owner skills: not installed in project or global scope — run: owner init',
    );
  });
  ...
  it('reports partial Owner installs with an update command instead of a raw missing dump', async () => {
    await fs.mkdir(path.join(tmpDir, '.claude', 'skills', 'owner'), {
      recursive: true,
    });
    ...
    expect(output).toContain('run: owner update --scope project');
  ...
  it('marks JSON output unhealthy when a diagnostic fails', async () => {
    await writeProjectConfig(tmpDir, defaultProjectConfig('docs'));
    await fs.mkdir(path.join(tmpDir, '.owner'), { recursive: true });
    await fs.writeFile(path.join(tmpDir, '.owner', 'current-change.json'), '{invalid', 'utf8');
  ...
  it('warns when a detected complete Skill install is missing its Rule and Hook', async () => {
    ...
    expect(results.find((result) => result.check === 'rules: Claude Code (project)')).toMatchObject(
      {
        status: 'warn',
        message: expect.stringContaining('owner update --scope project'),
      },
    );
  ...
  it('passes Rule and Hook checks when the managed components are installed', async () => {
    ...
    await installManagedOwnerSkills(tmpDir);
    await copyOwnerRulesForPlatform(tmpDir, claude!, true, 'zh', 'project');
    await installOwnerHooksForPlatform(tmpDir, claude!, 'project');
  ...
  it.each(['claude', 'codex'])(
    'recognizes exactly one healthy Router for the %s platform',
    async (id) => {
      const target = PLATFORMS.find((platform) => platform.id === id)!;
      await installManagedOwnerSkills(tmpDir, target.skillsDir);
      await installOwnerHooksForPlatform(tmpDir, target, 'project');
  ...
  it('detects and repairs an outdated Hook Router runtime', async () => {
    ...
    await expect(fs.readFile(installed)).resolves.toEqual(
      await fs.readFile(
        path.resolve('assets', 'skills', 'owner', 'scripts', 'owner-hook-router.mjs'),
      ),
    );
  ...
  it('uses the Classic-only project language when repairing managed Rules', async () => {
    ...
    const installedRule = await fs.readFile(
      path.join(tmpDir, '.claude', 'rules', 'owner-workflow-guard.md'),
      'utf8',
    );
    expect(installedRule).toContain('# Owner 当前需求阶段规则');
    expect(installedRule).not.toContain('# Owner Current-Change Phase Rule');
  });
  ...
  it('repairs duplicate and legacy managed Hook and Rule state without touching user entries', async () => {
    ...
    settings.hooks.PreToolUse[0].hooks.push(
      ...
      {
        type: 'command',
        command: router.command.replace('owner-hook-router.mjs', 'owner-hook-guard.mjs'),
      },
      { type: 'command', command: 'node user-hook.mjs' },
    );
    await fs.writeFile(hookPath, JSON.stringify(settings), 'utf8');
    const legacyRule = path.join(tmpDir, '.claude', 'rules', 'owner-phase-guard.md');
    ...
    expect(
      commands.filter((command: string) => command?.includes('owner-hook-router.mjs')),
    ).toHaveLength(1);
    expect(commands.some((command: string) => command?.includes('owner-hook-guard.mjs'))).toBe(
      false,
    );
  ...
  it('migrates Classic v1 selection only after a project Router is ready', async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, 'openspec', 'changes', 'legacy-change', '.owner.yaml'),
      [
        'workflow: full',
        'phase: open',
        'design_doc: null',
  ...
  it('clears a missing Native selection after the repaired Router is ready', async () => {
    ...
    await expect(readOwnerCurrentSelection(tmpDir)).resolves.toEqual({ status: 'missing' });
  });
  ...
  it('fails project repair when historical global Hook cleanup is unsafe', async () => {
    ...
    await installManagedOwnerSkills(tmpDir, '.agents');
    await fs.mkdir(path.join(tmpDir, '.codex'), { recursive: true });
  ...
  it('reports an explicitly scoped canonical global Codex install without a detection path', async () => {
    ...
    await installManagedOwnerSkills(fakeHome, '.agents');
          ...
          PreToolUse: [
            {
              ...
              hooks: [
                ...
                {
                  type: 'command',
                  command: `node "${path.join(fakeHome, '.legacy', 'skills', 'owner', 'scripts', 'owner-hook-router.mjs').replaceAll('\\', '/')}" --platform "codex"`,
                },
              ],
            },
          ],
  ...
  it('reports legacy-only Codex skills as requiring update and canonical Codex skills as healthy', async () => {
    ...
    try {
      ...
      expect(legacyOutput).toContain('run: owner update --scope project');
  ...
  it('assigns an explicit project .agents Skill root to Codex', async () => {
    await installManagedOwnerSkills(tmpDir, '.agents');

    const results = await collectDoctorResults(tmpDir, 'project');

    expect(results.filter((result) => result.check === 'skills: Codex (project)')).toHaveLength(1);
  ...
  it('does not infer Codex from the shared .agents root in auto scope without .codex evidence', async () => {
    await installManagedOwnerSkills(tmpDir, '.agents');

    const results = await collectDoctorResults(tmpDir, 'auto');

    expect(results.some((result) => result.check === 'skills: Codex (project)')).toBe(false);
  });
  ...
  it.each(['project', 'auto'] as const)(
    ...
    async (scope) => {
      ...
      const sharedRootChecks = results.filter(
        (result) => result.check === 'skills: Codex (project)',
      );
  ...
  it('uses the shared schema and leaves invalid state untouched', async () => {
    ...
    await fs.appendFile(path.join(invalidChangeDir, '.owner.yaml'), 'unknown_root_field: true\n');
    const before = await fs.readFile(path.join(invalidChangeDir, '.owner.yaml'), 'utf8');
    ...
    expect(
      results.find((result) => result.check === '.owner.yaml: top-level-invalid'),
    ).toMatchObject({
      status: 'fail',
  ...
  it('uses Classic diagnostics for owner yaml validity messages', async () => {
    ...
    const ownerYaml = payload.results.find(
      (item: { check: string }) => item.check === '.owner.yaml: demo',
    );

    expect(ownerYaml.message).toContain('step: legacy:open');
    expect(ownerYaml.message).toContain('mode: legacy-state');
  });

  it('does not synthesize runtime evidence while inspecting a legacy change', async () => {
    await state(tmpDir, 'init', 'demo', 'full');
    const stateFile = path.join(tmpDir, 'openspec', 'changes', 'demo', '.owner.yaml');
    ...
    expect(output).toContain('.owner.yaml: demo: valid (step: legacy:open, mode: legacy-state)');
    expect(output).not.toContain('runtime_check: demo:');
    expect(await fs.readFile(stateFile, 'utf8')).toBe(before);
  });

  it('prints invalid owner yaml errors together with a concrete next step', async () => {
    const invalidChangeDir = path.join(tmpDir, 'openspec', 'changes', 'top-level-invalid');
    await state(tmpDir, 'init', 'top-level-invalid', 'full');
    await fs.appendFile(path.join(invalidChangeDir, '.owner.yaml'), 'unknown_root_field: true\n');

...
async function snapshotChange(changeDir: string): Promise<{ files: string[]; yaml: Buffer }> {
  ...
  return {
    files: files.sort(),
    yaml: await fs.readFile(path.join(changeDir, '.owner.yaml')),
  };
}

async function setOwnerYamlField(
  changeDir: string,
  field: string,
  value: string | null,
): Promise<void> {
...
describe('status command', () => {
  ...
  it('classifies mixed Owner and OpenSpec changes in sorted JSON output', async () => {
    ...
    await fs.writeFile(path.join(changesDir, 'z-owner-ready', 'tasks.md'), '- [ ] ignored\n');

    state(tmpDir, 'init', 'b-invalid-owner', 'full');
    await fs.appendFile(
      path.join(changesDir, 'b-invalid-owner', '.owner.yaml'),
    ...
    expect(changes[1]).toMatchObject({
      name: 'b-invalid-owner',
      ownerManaged: true,
      archiveReady: false,
      recommendedArchiveCommand: 'owner archive b-invalid-owner',
      phase: 'invalid',
      commandChecks: null,
    ...
    expect(changes[3]).toMatchObject({
      ...
      ownerManaged: true,
      archiveReady: true,
      recommendedArchiveCommand: 'owner archive z-owner-ready',
      phase: 'archive',
      verifyResult: 'pass',
      tasksCompleted: 0,
      tasksTotal: 1,
  ...
  it('labels mixed text output and recommends archive commands only for ready changes', async () => {
    ...
    state(tmpDir, 'init', 'owner-not-ready', 'full');
    ...
    expect(output).toContain('owner-ready [Owner] [phase: archive]');
    expect(output).toContain('owner-not-ready [Owner] [phase: open]');
  ...
  it('prints the next command for active changes', async () => {
    ...
    expect(output).toContain('next: /owner-build');
  ...
  it('prints branch-bound workspace modes with bound branch and omits bound suffix for null isolation', async () => {
    ...
    await setOwnerYamlField(path.join(changesDir, 'branch-bound'), 'isolation', 'branch');
    await setOwnerYamlField(path.join(changesDir, 'branch-bound'), 'bound_branch', 'feature-B');

    state(tmpDir, 'init', 'worktree-bound', 'full');
    await setOwnerYamlField(path.join(changesDir, 'worktree-bound'), 'isolation', 'worktree');
    await setOwnerYamlField(path.join(changesDir, 'worktree-bound'), 'bound_branch', 'feature-C');

    state(tmpDir, 'init', 'null-bound', 'full');
    await setOwnerYamlField(path.join(changesDir, 'null-bound'), 'bound_branch', 'feature-D');
  ...
  it('keeps legacy state without a Run byte-for-byte read-only in text and JSON status', async () => {
    ...
    expect(change.nextCommand).toBe('/owner-verify');
  ...
  it('prints actionable runtime-eval recovery guidance for valid changes', async () => {
    ...
    expect(output).toContain(
      'next: run /owner-open or restore missing evidence (openspec.proposal, openspec.tasks), then rerun owner doctor',
    );
  });

...
[
  {
    ...
    "evidence_refs": [
      ...
      "test/domains/owner-pipeline/pipeline-project-config.test.ts",
...
- `pnpm exec vitest run test/domains/owner-pipeline test/domains/skill test/domains/workflow-contract test/app/init.test.ts test/app/update.test.ts`: 47 个测试文件通过，906 条测试通过，13 条按既定条件跳过。
- `pnpm build`: 通过；Pipeline、Loop、Entry runtime、TypeScript 与 Dashboard 均构建成功。
- `pnpm lint`: 通过；ESLint 与架构约束检查均通过。
- `pnpm check:generated`: 通过；Pipeline、Loop 与 Entry 生成物均与源码一致。
...
- `owner loop check pipeline-config-block --json`: 通过；扫描 29 个实现文件，0 个问题，receipt 为 `runtime/evidence/check-receipts/468d987764ca7e1490d67b7cf8d946be11cd502581e91da4708275fb3bc62f61.json`。
...
实现与已批准契约一致：Pipeline 的四项配置统一归入 `pipeline:`；运行时仅读取嵌套配置并保留缺失时的默认值；`init` 生成嵌套块和双语注释；`update` 将旧平铺字段迁移到嵌套块，同时保留其他顶层字段与 `loop:` 配置。

# Known limitations and risks

旧平铺字段在运行 `owner init` 或 `owner update` 前会被 Pipeline 有意忽略，这是契约明确的不兼容边界。全仓格式检查仍受基线中 4 个未修改的 Dashboard 文件影响；本 change 的修改文件格式检查已通过。全量测试中的 Dashboard Markdown 用例出现一次并发超时，单独重跑全部通过。

# Conclusion

通过。五条验收项均有项目内可复核证据，目标回归、构建、lint、架构、生成物一致性和 Loop 内置检查均通过；全量测试的唯一超时已通过隔离重跑确认不是稳定失败。

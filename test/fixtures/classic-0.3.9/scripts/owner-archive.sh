#!/bin/bash
# Owner Archive — automates the archive phase in one command
# Usage: owner-archive.sh <change-name> [--dry-run]
# Exit 0 = archive complete, exit 1 = fatal error

set -euo pipefail

OWNER_BASH="${OWNER_BASH:-${BASH:-bash}}"
OWNER_OPENSPEC="${OWNER_OPENSPEC:-openspec}"
...
YAML="$CHANGE_DIR/.owner.yaml"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd -P)"
STATE_SH="$SCRIPT_DIR/owner-state.sh"
TODAY=$(date -u +%Y-%m-%d)
...
echo "=== Owner Archive: $CHANGE ===" >&2

# --- Step 1: Read .owner.yaml, extract paths ---

yaml_field() {
  local field="$1"
  if [ -f "$STATE_SH" ]; then
    "$OWNER_BASH" "$STATE_SH" get "$CHANGE" "$field" 2>/dev/null
  else
    if [ -f "$YAML" ]; then
      local value
...
if [ "$VERIFY_VAL" != "pass" ]; then
  red "FATAL: verify_result is '$VERIFY_VAL', expected 'pass'. Run owner-verify first."
  exit 1
fi
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  if ! command -v "$OWNER_OPENSPEC" >/dev/null 2>&1; then
    red "FATAL: OpenSpec CLI not found: $OWNER_OPENSPEC"
    red "Install OpenSpec or set OWNER_OPENSPEC to the openspec executable."
    exit 1
  fi

  "$OWNER_OPENSPEC" archive "$CHANGE" --yes >&2
...
# --- Step 7: Mark archived via owner-state transition ---

ARCHIVE_YAML="$ARCHIVE_DIR/.owner.yaml"

if [ "$DRY_RUN" -eq 1 ]; then
  step_dry_run "Would set archived: true in $ARCHIVE_YAML"
else
  if [ -f "$ARCHIVE_YAML" ]; then
    "$OWNER_BASH" "$STATE_SH" transition "$ARCHIVE_NAME" archived >/dev/null
    step_ok "archived: true"
  else
    step_fail "archived: true (.owner.yaml not found after move)"
  fi
fi

#!/bin/bash
# Owner State — unified interface for .owner.yaml state management
# Usage: owner-state.sh <subcommand> <change-name> [args...]
#
# Subcommands:
#   init <change-name> <workflow>  — Initialize .owner.yaml with workflow defaults
#   get <change-name> <field>       — Read a field value from .owner.yaml
...
yaml_file_for() {
  ...
  echo "$change_dir/.owner.yaml"
}

project_context_compression() {
  local value="off"
  local source="default"
  if [ -n "${OWNER_CONTEXT_COMPRESSION:-}" ]; then
    value="$OWNER_CONTEXT_COMPRESSION"
    source="OWNER_CONTEXT_COMPRESSION"
  elif [ -f ".owner/config.yaml" ]; then
    value=$(yaml_field "context_compression" ".owner/config.yaml")
    value="${value:-off}"
...
project_auto_transition_default() {
  local value="true"
  local source="default"
  if [ -n "${OWNER_AUTO_TRANSITION:-}" ]; then
    value="$OWNER_AUTO_TRANSITION"
    source="OWNER_AUTO_TRANSITION"
  elif [ -f ".owner/config.yaml" ]; then
    local raw
    raw=$(yaml_field "auto_transition" ".owner/config.yaml" 2>/dev/null || true)
    if [ -n "$raw" ]; then
      value="$raw"
      source=".owner/config.yaml"
    fi
...
project_review_mode_default() {
  ...
  local source="default"
  if [ -n "${OWNER_REVIEW_MODE:-}" ]; then
    value="$OWNER_REVIEW_MODE"
    source="OWNER_REVIEW_MODE"
  elif [ -f ".owner/config.yaml" ]; then
    local raw
    raw=$(yaml_field "review_mode" ".owner/config.yaml" 2>/dev/null || true)
    if [ -n "$raw" ]; then
      value="$raw"
      source=".owner/config.yaml"
    fi
...
cmd_init() {
  ...
  # Check if .owner.yaml already exists
  if [ -f "$yaml_file" ]; then
    red "ERROR: .owner.yaml already exists at $yaml_file"
    exit 1
  fi
...
cmd_get() {
  ...
  # Check if .owner.yaml exists
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .owner.yaml not found at $yaml_file"
    exit 1
  fi
...
cmd_set() {
  ...
  case "$field" in
    phase)
      ...
      if [ "${_OWNER_IN_TRANSITION:-}" != "1" ] && [ "${OWNER_FORCE_PHASE:-}" != "1" ]; then
        red "ERROR: Setting 'phase' directly is not allowed; it bypasses state machine evidence checks." >&2
        red "  Use: owner-state.sh transition <change-name> <event>" >&2
        red "  Repair-only escape hatch: OWNER_FORCE_PHASE=1 owner-state.sh set <change-name> phase <value>" >&2
        exit 1
      fi
...
cmd_check() {
  ...
  local yaml_file="$change_dir/.owner.yaml"
  local proposal_file="$change_dir/proposal.md"
  local design_file="$change_dir/design.md"
  local tasks_file="$change_dir/tasks.md"

  echo "=== Entry Check: owner-${phase} ==="

  # .owner.yaml must exist for all phases (state machine core)
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .owner.yaml not found at $yaml_file"
    exit 1
  fi
  ...
  case "$phase" in
    open)
      ...
      if [ "$artifacts_done" -eq 3 ]; then
        echo "Recovery action: All artifacts complete. Run /owner-open user confirmation, then guard to transition."
      elif [ "$artifacts_done" -eq 0 ]; then
        echo "Recovery action: No artifacts created yet. Start from /owner-open Step 1 (explore and clarify)."
      else
        echo "Recovery action: Some artifacts incomplete. Resume /owner-open from the first missing artifact."
      fi
      ;;
    design)
      echo "  Artifacts:"
    ...
    verify)
      ...
      if [ "$verify_result" = "pass" ] && [ "$branch_status" = "handled" ]; then
        ...
        echo "Recovery action: Verification failed and rolled back to build. Resume from /owner-build."
      else
    ...
    archive)
      ...
      echo ""
      echo "Recovery action: Run /owner-archive to complete archiving."
      ;;
    *)
...
cmd_scale() {
  ...
  # 3. Changed files: prefer plan base-ref, then .owner.yaml base_ref, fall back to worktree diff
  ...
  if git rev-parse --git-dir > /dev/null 2>&1; then
    ...
    # Fallback to base_ref stored in .owner.yaml (set during init)
  ...
  # Update verify_mode in .owner.yaml
...
cmd_next() {
  ...
  local yaml_file="$change_dir/.owner.yaml"
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .owner.yaml not found at $yaml_file" >&2
    exit 1
  fi
  ...
  case "$phase" in
    ...
    build)
      case "$workflow" in
        hotfix) skill="owner-hotfix" ;;
        tweak)  skill="owner-tweak" ;;
        *)      skill="owner-build" ;;
      esac
      ;;
    verify)
...
case "$SUBCOMMAND" in
  init)
    if [ $# -lt 2 ]; then
      red "Usage: owner-state.sh init <change-name> <workflow>" >&2
  ...
  get)
    if [ $# -lt 2 ]; then
      red "Usage: owner-state.sh get <change-name> <field>" >&2
      exit 1
    fi
    cmd_get "$@"
    ;;
  set)
    if [ $# -lt 3 ]; then
      red "Usage: owner-state.sh set <change-name> <field> <value>" >&2
      exit 1
    fi
    cmd_set "$@"
    ;;
  transition)
    if [ $# -lt 2 ]; then
      red "Usage: owner-state.sh transition <change-name> <event>" >&2
  ...
  check)
    if [ $# -lt 2 ]; then
      red "Usage: owner-state.sh check <change-name> <phase> [--recover]" >&2
  ...
  scale)
    if [ $# -lt 1 ]; then
      red "Usage: owner-state.sh scale <change-name>" >&2
      exit 1
    fi
    cmd_scale "$@"
    ;;
  task-checkoff)
    if [ $# -lt 2 ]; then
      red "Usage: owner-state.sh task-checkoff <file> <task-text>" >&2
      exit 1
    fi
    cmd_task_checkoff "$@"
    ;;
  next)
    if [ $# -lt 1 ]; then
      red "Usage: owner-state.sh next <change-name>" >&2
      exit 1
    fi
    cmd_next "$@"
    ;;
  *)
    ...
    echo "" >&2
    echo "Usage: owner-state.sh <subcommand> <change-name> [args...]" >&2
    echo "" >&2
    echo "Subcommands:" >&2
    echo "  init <change-name> <workflow>  — Initialize .owner.yaml with workflow defaults" >&2
    echo "  get <change-name> <field>       — Read a field value from .owner.yaml" >&2
    echo "  set <change-name> <field> <val> — Update a field value in .owner.yaml" >&2

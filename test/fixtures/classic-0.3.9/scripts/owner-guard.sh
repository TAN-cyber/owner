#!/bin/bash
# Owner Phase Guard — validates exit conditions before phase transitions
# Usage: owner-guard.sh <change-name> <current-phase> [--apply]
...
OWNER_BASH="${OWNER_BASH:-${BASH:-bash}}"
...
if [ "${OWNER_GUARD_SOURCE_ONLY:-0}" = "1" ]; then
  CHANGE="${CHANGE:-}"
  PHASE="${PHASE:-}"
  APPLY="${APPLY:-0}"
...
plan_tasks_all_done() {
  ...
  if [ ! -f "$plan" ]; then
    ...
    echo "Next: restore the Superpowers plan file or update .owner.yaml plan before leaving build." >&2
    return 1
  fi
...
yaml_field_value() {
  local field="$1"
  local yaml="$CHANGE_DIR/.owner.yaml"
  if [ -f "$yaml" ]; then
    local value
...
project_config_value() {
  ...
  for config in ".owner.yaml" "owner.yaml" ".owner.yml" "owner.yml"; do
    if [ -f "$config" ]; then
...
preflight() {
  ...
  if [ ! -f "$CHANGE_DIR/.owner.yaml" ]; then
    red "FATAL: .owner.yaml not found in $CHANGE_DIR"
    exit 1
  fi

  # Schema validation
  local validate_script
  validate_script="$SCRIPT_DIR/owner-yaml-validate.sh"
  if [ -f "$validate_script" ]; then
    if ! "$OWNER_BASH" "$validate_script" "$CHANGE" 2>/dev/null; then
      "$OWNER_BASH" "$validate_script" "$CHANGE" || true
      red "FATAL: .owner.yaml schema validation failed"
      exit 1
    fi
  fi
}
...
isolation_selected() {
  ...
  case "$isolation" in
    ...
    *)
      ...
      echo "  \"\$OWNER_BASH\" \"\$OWNER_STATE\" set $CHANGE isolation <branch|worktree>" >&2
      return 1
      ;;
  esac
}
...
build_mode_selected() {
  ...
  case "$build_mode" in
    ...
    *)
      ...
      echo "  \"\$OWNER_BASH\" \"\$OWNER_STATE\" set $CHANGE build_mode <subagent-driven-development|executing-plans>" >&2
      return 1
      ;;
  esac
}
...
subagent_dispatch_confirmed() {
  ...
  echo "  \"\$OWNER_BASH\" \"\$OWNER_STATE\" set $CHANGE subagent_dispatch confirmed" >&2
  echo "Or ask the user to switch to executing-plans and run:" >&2
  echo "  \"\$OWNER_BASH\" \"\$OWNER_STATE\" set $CHANGE build_mode executing-plans" >&2
  return 1
}
...
tdd_mode_selected() {
  ...
  case "$tdd_mode" in
    ...
    *)
      ...
      echo "  \"\$OWNER_BASH\" \"\$OWNER_STATE\" set $CHANGE tdd_mode <tdd|direct>" >&2
      return 1
      ;;
  esac
}
...
review_mode_selected() {
  ...
  case "$review_mode" in
    ...
    *)
      ...
      echo "  \"\$OWNER_BASH\" \"\$OWNER_STATE\" set $CHANGE review_mode <off|standard|thorough>" >&2
      return 1
      ;;
  esac
}
...
design_handoff_context_valid() {
  ...
  if [ -z "$context" ] || [ "$context" = "null" ]; then
    echo "handoff_context is missing from .owner.yaml" >&2
    echo "Next: run \"\$OWNER_BASH\" \"\$OWNER_HANDOFF\" $CHANGE design --write before invoking Superpowers." >&2
    return 1
  fi
  if [ ! -s "$context" ]; then
    echo "handoff_context does not point to a non-empty file: $context" >&2
    echo "Next: regenerate the design handoff with owner-handoff.sh." >&2
    return 1
  fi
  ...
  if [ "$actual_hash" != "$recorded_hash" ]; then
    ...
    echo "Next: rerun owner-handoff.sh so Superpowers receives the current OpenSpec context." >&2
    return 1
  fi
...
design_handoff_markdown_traceable() {
  ...
  grep -q '^Generated-by: owner-handoff\.sh$' "$markdown" || {
...
design_doc_links_current_change() {
  ...
  design_doc_frontmatter_has "$design_doc" "owner_change" "$CHANGE"
}
...
guard_design() {
  ...
  if [ -n "$design_doc" ] && [ "$design_doc" != "null" ]; then
    ...
    warn "  [WARN] No design_doc recorded in .owner.yaml (optional for hotfix/tweak)"
  fi
}
...
design_doc_recorded() {
  ...
  echo "Next: create the Design Doc and run: \"\$OWNER_BASH\" \"\$OWNER_STATE\" set $CHANGE design_doc <path>" >&2
  return 1
}
...
apply_state_update() {
  local state_sh="$SCRIPT_DIR/owner-state.sh"
  local p="$1"

  if [ -f "$state_sh" ]; then
    case "$p" in
      open)   "$OWNER_BASH" "$state_sh" transition "$CHANGE" open-complete ;;
      design) "$OWNER_BASH" "$state_sh" transition "$CHANGE" design-complete ;;
      build)  "$OWNER_BASH" "$state_sh" transition "$CHANGE" build-complete ;;
      verify) "$OWNER_BASH" "$state_sh" transition "$CHANGE" verify-pass ;;
    esac
  else
    red "FATAL: owner-state.sh not found; cannot apply state transition"
    exit 1
  fi
}
...
if [ "${OWNER_GUARD_SOURCE_ONLY:-0}" = "1" ]; then
  ...
  red "ERROR: OWNER_GUARD_SOURCE_ONLY=1 is only for sourcing, not direct execution" >&2
  ...
  if [ "$APPLY" -eq 1 ]; then
    ...
    case "$PHASE" in
      open)
        ...
        green "  [APPLY] .owner.yaml updated: phase=$new_phase"
        ;;
      design) green "  [APPLY] .owner.yaml updated: phase=build" ;;
      build)  green "  [APPLY] .owner.yaml updated: phase=verify, verify_result=pending" ;;
      verify) green "  [APPLY] .owner.yaml updated: phase=archive, verify_result=pass" ;;
    esac
  fi
  exit 0
fi

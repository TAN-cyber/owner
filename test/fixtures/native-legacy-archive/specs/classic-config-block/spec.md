...
`.owner/config.yaml` 按「全局 / 工作流专属」两层组织：全局字段留在顶层，Classic 工作流专属字段收纳在 `classic:` 嵌套映射下，与已有的 `native:` 嵌套块对称。

## 配置结构

`owner.project.v1` 配置文件结构如下（`classic:` 块为本能力引入，与 `native:` 块同级）：
...
本能力不在 Classic 运行时提供对旧顶层平铺格式的兼容读取；旧格式只能经由 `owner init` / `owner update` 迁移到新格式。

## 类型契约

`OwnerProjectConfig`（`domains/owner-native/native-types.ts`）扩展可选 `classic` 块，字段与值域同上。`OwnerProjectConfig.classic` 缺失时不影响 Native 解析。

## 非目标

- 不改变 `native:` 块、顶层全局字段语义、字段值域。
- 不在运行时兼容旧平铺格式。
- 不调整 Classic change 级 `.owner.yaml`。

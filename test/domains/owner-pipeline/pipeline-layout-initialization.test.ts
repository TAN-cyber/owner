...
import {
  ...
  rollbackPipelineLayoutInitialization,
...
describe('Pipeline layout initialization safety', () => {
  let projectRoot: string;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-layout-init-'));
  });
  ...
  it('allows an explicit desired layout only for a fresh project with both roots missing', async () => {
    await expect(
      assertPipelineLayoutInitializationSafe(projectRoot, 'docs'),
  ...
  it('recovers a prior owned initialization across processes without allowing unmanaged roots', async () => {
    const initialization = await assertPipelineLayoutInitializationSafe(projectRoot, 'docs');
    const owned = await beginPipelineLayoutInitialization(projectRoot, initialization);
    ...
    const resumed = await assertPipelineLayoutInitializationSafe(projectRoot, 'docs');
    expect(resumed.initializationPermit.ownershipId).toBe(owned.initializationPermit.ownershipId);
    await checkpointPipelineLayoutInitialization(projectRoot, resumed.initializationPermit);

    await expect(
      fs.stat(path.join(projectRoot, '.owner', 'pipeline-init-ownership.json')),
  ...
  it('quarantines an unchanged manifest-bound root without deleting its contents', async () => {
    ...
    await fs.mkdir(path.join(owned.openSpecRoot, 'specs'), { recursive: true });
    await fs.writeFile(path.join(owned.openSpecRoot, 'config.yaml'), 'schema: spec-driven\n');
    await checkpointPipelineLayoutInitialization(projectRoot, owned.initializationPermit);

    await expect(
      rollbackPipelineLayoutInitialization(projectRoot, owned.initializationPermit),
    ).resolves.toBe(true);
    ...
    const journal = JSON.parse(
      await fs.readFile(path.join(projectRoot, '.owner', 'pipeline-init-ownership.json'), 'utf8'),
  ...
  it('publishes only one ownership journal when two initializers race', async () => {
    const first = await assertPipelineLayoutInitializationSafe(projectRoot, 'docs');
    const second = await assertPipelineLayoutInitializationSafe(projectRoot, 'docs');

    const results = await Promise.allSettled([
      beginPipelineLayoutInitialization(projectRoot, first),
      beginPipelineLayoutInitialization(projectRoot, second),
    ]);
  ...
  it('initializes Pipeline when the project filesystem does not support hard links', async () => {
    const linkSpy = vi
      .spyOn(fs, 'link')
    ...
    try {
      ...
      await expect(
        fs.readFile(path.join(projectRoot, '.owner', 'pipeline-init-ownership.json'), 'utf8'),
  ...
  it('does not let a stale checkpoint overwrite a successor journal', async () => {
    ...
    await expect(
      checkpointPipelineLayoutInitialization(projectRoot, owned.initializationPermit, {
        testHooks: {
  ...
  it('does not let stale completion delete a successor journal', async () => {
    ...
    await expect(
      completePipelineLayoutInitialization(projectRoot, owned.initializationPermit, {
        testHooks: {
          afterJournalQuarantine: async (operation) => {
  ...
  async function publishSuccessorJournal(id: string): Promise<void> {
    const ownerDir = path.join(projectRoot, '.owner');
    const quarantine = (await fs.readdir(ownerDir)).find((entry) => entry.endsWith('.quarantine'));
    if (!quarantine) throw new Error('expected a quarantined ownership journal');
    const successor = JSON.parse(
      await fs.readFile(path.join(ownerDir, quarantine), 'utf8'),
    ) as Record<string, unknown>;
    successor.id = id;
    successor.stage = 'initializing';
    successor.quarantine = null;
    await fs.writeFile(
      path.join(ownerDir, 'pipeline-init-ownership.json'),
  ...
  it('preserves a legacy root when a pre-layout Pipeline config omits artifact_layout', async () => {
    await writePipelineConfig();
    await fs.mkdir(path.join(projectRoot, 'openspec'), { recursive: true });

    await expect(
      assertPipelineLayoutInitializationSafe(projectRoot, 'legacy'),
    ).resolves.toMatchObject({
  ...
  it('allows a configured Pipeline project to initialize its missing root when both roots are absent', async () => {
    await writePipelineConfig('docs');
  ...
  it('binds an existing configured Pipeline project to the initial config identity', async () => {
    await writePipelineConfig('docs');
    await fs.mkdir(path.join(projectRoot, 'docs', 'openspec'), { recursive: true });
    const initialization = await assertPipelineLayoutInitializationSafe(projectRoot, 'docs');
    ...
    ).rejects.toThrow(/project config changed during Pipeline layout initialization/iu);
  });

  it('allows a Loop-only project to initialize its first explicit Pipeline layout', async () => {
  ...
  it('rejects a desired layout that disagrees with existing configuration', async () => {
    ...
    await expect(assertPipelineLayoutInitializationSafe(projectRoot, 'docs')).rejects.toThrow(
      /configured Pipeline layout is legacy.*requested docs/iu,
    );
  });
  ...
  it('rejects an existing configured project when only the alternate root exists', async () => {
    ...
    await expect(assertPipelineLayoutInitializationSafe(projectRoot, 'docs')).rejects.toThrow(
      /configured Pipeline OpenSpec root is missing/iu,
    );
  });
  ...
  it('rejects a fresh project when the alternate docs root is a directory link', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-init-outside-'));
    try {
      await fs.mkdir(path.join(projectRoot, 'docs'), { recursive: true });
      try {
        await fs.symlink(
      ...
      await expect(assertPipelineLayoutInitializationSafe(projectRoot, 'legacy')).rejects.toThrow(
  ...
  it('rejects a configured OpenSpec root that crosses a junction', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-layout-outside-'));
    try {
      await writePipelineConfig('docs');
      await fs.mkdir(path.join(projectRoot, 'docs'), { recursive: true });
      try {

/**
 * Hook guard tests — split from owner-scripts.test.ts for maintainability.
 *
 * Tests the PreToolUse hook guard (owner-hook-guard.mjs) that blocks or allows
...
describe('hook guard', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-hook-guard-'));
    await preparePipelineLegacyProject(tmpDir);
    hookGuardScript = path.resolve(scriptsDir, 'owner-hook-guard.mjs');
  ...
  it('derives all governed Superpowers artifact directories from the Pipeline layout resolver', async () => {
  ...
  describe('allows source writes during build phase', () => {
    ...
    it('blocks full-workflow build source writes until a plan is recorded', async () => {
      ...
      expect(result.stderr).toContain('ERROR_CODE: pipeline-build-plan-missing');
      expect(result.stderr).toContain('CHANGE: full-build-without-plan');
      expect(result.stderr).toContain('TARGET: src/feature.ts');
      expect(result.stderr).toContain(
        'owner state set full-build-without-plan plan <repository-relative-plan-path>',
      );
      expect(result.stderr).toContain('owner state check full-build-without-plan build --recover');

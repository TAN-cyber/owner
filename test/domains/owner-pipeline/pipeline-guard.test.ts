...
function run(cwd: string, ...args: string[]) {
  ...
  return spawnSync(process.execPath, [scriptByCommand[command], ...rest], {
    ...
    env: {
      ...process.env,
      ...(command === 'state' && rest[2] === 'phase' ? { OWNER_FORCE_PHASE: '1' } : {}),
    },
  });
}

async function makeProject(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'pipeline-guard-'));
  await preparePipelineLegacyProject(dir);
  temporary.push(dir);
  return dir;
}

describe('Pipeline guard command', () => {
  it('blocks the open guard when artifacts are missing and leaves state unchanged', async () => {
    ...
    const stateFile = path.join(dir, 'openspec', 'changes', 'demo', '.owner.yaml');
  ...
  it('passes the open guard and applies the transition when artifacts exist', async () => {
    ...
    expect(result.stderr).toContain('[APPLY] .owner.yaml updated: phase=build');

...
describe('Pipeline OpenSpec adapter', () => {
  ...
  beforeEach(async () => {
    ...
    projectRoot = await fs.mkdtemp(
      path.join(await fs.realpath(os.tmpdir()), 'owner-pipeline-openspec-'),
    );
  ...
  async function seedReadyArchiveChange(): Promise<void> {
    ...
    const previousForcePhase = process.env.OWNER_FORCE_PHASE;
    process.env.OWNER_FORCE_PHASE = '1';
    try {
      expect((await runPipelineCli(['state', 'set', 'demo', 'phase', 'archive'])).exitCode).toBe(0);
    } finally {
      if (previousForcePhase === undefined) delete process.env.OWNER_FORCE_PHASE;
      else process.env.OWNER_FORCE_PHASE = previousForcePhase;
    }
    expect((await runPipelineCli(['state', 'set', 'demo', 'verify_result', 'pass'])).exitCode).toBe(
      0,
    );
    expect(
      (await runPipelineCli(['state', 'transition', 'demo', 'archive-confirm'])).exitCode,
    ).toBe(0);
  }
  ...
  it('runs OpenSpec from the configured base and preserves output, JSON args, and exit code', async () => {
    ...
    const result = await runPipelineCli(['openspec', '--', 'status', '--change', 'demo', '--json']);
  ...
  it('uses the configured docs root when a standalone OpenSpec root also exists', async () => {
    ...
    const result = await runPipelineCli(['openspec', 'status']);
  ...
  it('fails closed when the configured OpenSpec root was not initialized', async () => {
    ...
    expect(result.exitCode).toBe(70);
    expect(result.stderr).toContain('Pipeline OpenSpec root is unhealthy');
  ...
  it('routes archive through the shared adapter and preserves OpenSpec process output', async () => {
    ...
    let result: Awaited<ReturnType<typeof runPipelineCli>>;
    try {
      result = await runPipelineCli(['archive', 'demo']);
    } finally {
  ...
  it('uses the shared missing-executable exit mapping without moving the active change', async () => {
    ...
    await expect(
      fs.access(path.join(projectRoot, 'docs', 'openspec', 'changes', 'demo', '.owner.yaml')),

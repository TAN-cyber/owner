...
function pipelineState(overrides: Record<string, string> = {}): string {
...
describe.sequential('Pipeline protected path consumers', () => {
  let projectRoot: string;
  let outsideRoot: string;
  let previousCwd: string;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-protected-'));
    outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-outside-'));
    previousCwd = process.cwd();
    await fs.mkdir(path.join(projectRoot, '.git'));
    await writeFile(path.join(projectRoot, '.owner', 'config.yaml'), PIPELINE_CONFIG);
    await writeFile(
      path.join(projectRoot, 'openspec', 'changes', 'demo', '.owner.yaml'),
      pipelineState(),
    );
  ...
  it('rejects a state artifact pointer that crosses a directory junction', async () => {
    ...
    if (!linked) return;
    const stateFile = path.join(projectRoot, 'openspec', 'changes', 'demo', '.owner.yaml');
  ...
  it('does not write a Pipeline artifact through a parent junction replaced before commit', async () => {
    const protectedPaths =
      (await import('../../../domains/owner-pipeline/pipeline-protected-path.js')) as unknown as {
    ...
    const parent = path.join(projectRoot, 'openspec', 'changes', 'demo', '.owner', 'handoff');
    ...
    try {
      await expect(
        protectedPaths.writePipelineProjectText(projectRoot, target, 'managed\n', {
          label: 'Pipeline protected write race',
          beforeCommit: async () => {
  ...
  it('rejects a nonempty check when the inspected file is replaced before completion', async () => {
    ...
    try {
      await expect(
        protectedPaths.pipelineProjectFileNonempty(
          projectRoot,
          target,
          'Pipeline nonempty replacement',
          {
            afterOpen: async () => {
              await fs.rename(parent, held);
              await fs.symlink(
  ...
  it('reports a junction-backed artifact pointer as an unsafe validation failure', async () => {
    ...
    await writeFile(
      path.join(projectRoot, 'openspec', 'changes', 'demo', '.owner.yaml'),
      pipelineState({ design_doc: 'linked-artifacts/design.md' }),
    );

    const result = await runPipelineCli(['validate', 'demo']);
  ...
  it('keeps validator path rules aligned with state-set repository-relative pointers', async () => {
    await writeFile(
      path.join(projectRoot, 'openspec', 'changes', 'demo', '.owner.yaml'),
      pipelineState({ design_doc: '../outside/design.md' }),
    );
  ...
  it('rejects an active change runtime junction without writing outside the project', async () => {
    ...
    const runtimeDir = path.join(projectRoot, 'openspec', 'changes', 'demo', '.owner');
    const linked = await linkDirectory(outsideRoot, runtimeDir);
    if (!linked) return;
    const stateFile = path.join(projectRoot, 'openspec', 'changes', 'demo', '.owner.yaml');
    const before = await fs.readFile(stateFile, 'utf8');

    const result = await runPipelineCli(['state', 'set', 'demo', 'review_mode', 'off']);
  ...
  it('blocks guard preflight when an artifact pointer crosses a directory junction', async () => {
    ...
    const result = await runPipelineCli(['guard', 'demo', 'design']);

    expect(result.exitCode).not.toBe(0);
    expect(result.stderr).toContain('.owner.yaml schema validation failed');
    expect(await fs.readFile(path.join(outsideRoot, 'marker.txt'), 'utf8')).toBe('unchanged\n');
  });
  ...
  it('rejects a symlinked project config before guard can inspect removed fields', async () => {
    ...
    await writeFile(outsideConfig, PIPELINE_CONFIG);
    ...
    expect(result.exitCode).not.toBe(0);
    expect(result.stderr).toMatch(/symbolic link or junction/iu);
    expect(await fs.readFile(outsideConfig, 'utf8')).toBe(PIPELINE_CONFIG);
  });
});

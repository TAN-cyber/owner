...
const pipelineRuntimeRoot = path.resolve('assets', 'skills', 'owner', 'runtime', 'pipeline');
const pipelineSkillRoot = pipelineRuntimeRoot;

// Forward-slash absolute path — used for the fake OpenSpec shim (a bash script)
// and the OWNER_OPENSPEC env var it is passed through, both of which need a
...
function canonicalProjectConfig(
  pipelineFields: readonly string[] = [],
  ...
  return [
    ...
    ...pipelineFields.map((field) => `  ${field}`),
    ...topLevelFields,
    '',
  ].join('\n');
}
...
async function createChange(tmpDir: string, name: string, yaml: string, tasks = '- [x] done\n') {
  ...
  await writeFile(path.join(changeDir, '.owner.yaml'), yaml);
...
async function createFakeOpenSpecArchive(
  ...
  // to assign to OWNER_OPENSPEC — already shaped so the runtime's
...
describe('owner script contracts', () => {
  it('keeps all Pipeline command scripts as self-contained bundles', async () => {
    const sources: Record<string, string> = {
      state: await fs.readFile(path.join(scriptsDir, 'owner-state.mjs'), 'utf-8'),
      validate: await fs.readFile(path.join(scriptsDir, 'owner-yaml-validate.mjs'), 'utf-8'),
      guard: await fs.readFile(path.join(scriptsDir, 'owner-guard.mjs'), 'utf-8'),
      handoff: await fs.readFile(path.join(scriptsDir, 'owner-handoff.mjs'), 'utf-8'),
      archive: await fs.readFile(path.join(scriptsDir, 'owner-archive.mjs'), 'utf-8'),
      'hook-guard': await fs.readFile(path.join(scriptsDir, 'owner-hook-guard.mjs'), 'utf-8'),
      intent: await fs.readFile(path.join(scriptsDir, 'owner-intent.mjs'), 'utf-8'),
      'resume-probe': await fs.readFile(path.join(scriptsDir, 'owner-resume-probe.mjs'), 'utf-8'),
    };
    ...
    await expect(fs.access(path.join(scriptsDir, 'owner-runtime.mjs'))).resolves.toBeUndefined();
    for (const source of Object.values(sources)) {
      expect(source).toContain('#!/usr/bin/env node');
      // Self-contained: no longer imports the shared runtime.
      expect(source).not.toContain("from './owner-runtime.mjs'");
      expect(source).not.toMatch(/\b(?:grep|awk|sed)\b/u);
    }
  });

  it('keeps owner-hook-guard blocked messages in English', async () => {
    const hookSource = await fs.readFile(
      path.resolve('domains', 'owner-pipeline', 'pipeline-hook-guard.ts'),
      'utf-8',
    );
...
describe('owner scripts', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `owner-scripts-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    ...
    for (const name of [
      ...
      'owner-hook-guard.mjs',
      'owner-resume-probe.mjs',
    ]) {
    ...
    guardScript = path.join(tmpScriptsDir, 'owner-guard.mjs');
    stateScript = path.join(tmpScriptsDir, 'owner-state.mjs');
    validateScript = path.join(tmpScriptsDir, 'owner-yaml-validate.mjs');
    hookGuardScript = path.join(tmpScriptsDir, 'owner-hook-guard.mjs');
    await writeFile(path.join(tmpDir, '.owner', 'config.yaml'), canonicalProjectConfig());
  ...
  it('initializes a new change directory with workflow defaults', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'new-full-change', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('prints successful initialization to stdout so PowerShell does not surface LoopCommandError', async () => {
    const result = runNode(tmpDir, stateScript, ['init', 'powershell-friendly', 'full']);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain(
      'Initialized: openspec/changes/powershell-friendly/.owner.yaml (workflow=full)',
    );
    expect(result.stderr).toBe('');
  }, 20_000);

  it('keeps hook guard read-only when OWNER_RUNTIME_PIPELINE_ROOT is configured', async () => {
    const init = runNode(tmpDir, stateScript, ['init', 'runtime-root', 'full'], {
      OWNER_RUNTIME_PIPELINE_ROOT: pipelineRuntimeRoot,
      OWNER_PIPELINE_SKILL_ROOT: '',
    });
    const changeDir = path.join(tmpDir, 'openspec', 'changes', 'runtime-root');
    const stateFile = path.join(changeDir, '.owner.yaml');
  ...
  it('keeps OWNER_PIPELINE_SKILL_ROOT as a compatibility fallback', async () => {
    const init = runNode(tmpDir, stateScript, ['init', 'legacy-root', 'full'], {
      OWNER_RUNTIME_PIPELINE_ROOT: '',
      OWNER_PIPELINE_SKILL_ROOT: pipelineRuntimeRoot,
    });
  ...
  it('falls back to the embedded Pipeline runtime package when installed script assets omit internal runtime files', async () => {
    ...
    expect(result.stderr).not.toContain('pipeline runtime package is not installed');
    const runState = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'embedded-runtime', '.owner', 'run-state.json'),
      'utf8',
    );
    expect(JSON.parse(runState)).toMatchObject({ skill: 'owner-pipeline' });
  }, 20_000);
  ...
  it('snapshots language from .owner/config.yaml when initializing a change', async () => {
    await writeFile(
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'language-zh', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('ignores legacy top-level Pipeline settings until init or update migrates them', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'legacy-config-ignored', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('falls back to the global Owner language when project config omits language', async () => {
    const fakeHome = path.join(tmpDir, 'fake-home');
    await writeFile(path.join(fakeHome, '.owner', 'config.yaml'), 'pipeline:\n  language: zh-CN\n');
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'language-global-zh', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('lets project language override the global Owner language', async () => {
    const fakeHome = path.join(tmpDir, 'fake-home');
    await writeFile(path.join(fakeHome, '.owner', 'config.yaml'), 'pipeline:\n  language: zh-CN\n');
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'language-project-over-global', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('rejects an invalid global Owner language when project config omits language', async () => {
    const fakeHome = path.join(tmpDir, 'fake-home');
    await writeFile(
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('pipeline.language must be en or zh-CN');
  }, 20_000);

  it('fails closed when .owner/config.yaml contains malformed unrelated YAML', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
  ...
  it('rejects an explicit empty review_mode instead of silently defaulting', async () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain('pipeline.review_mode must be off, standard, or thorough');
  }, 20_000);
  ...
  it('lets OWNER_LANGUAGE override the project language default', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
      canonicalProjectConfig(['language: zh-CN']),
    );
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'language-env', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('rejects invalid language from .owner/config.yaml when initializing a change', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
  ...
  it('initializes build_pause as null for new changes', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'pause-defaults', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('initializes subagent_dispatch as null for new changes', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'subagent-dispatch-defaults', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('initializes tdd_mode as null for full workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'tdd-defaults', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('initializes review_mode as standard for full workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-defaults', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('initializes review_mode as off for hotfix workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-hotfix', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('initializes tdd_mode as direct for hotfix workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'tdd-hotfix', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('initializes context_compression as off by default', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-defaults', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('snapshots beta context compression from .owner/config.yaml when initializing a change', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-beta', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('snapshots review_mode from .owner/config.yaml when initializing a full change', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-standard', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('rejects invalid review_mode from .owner/config.yaml when initializing a change', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
  ...
  it('lets OWNER_CONTEXT_COMPRESSION override the project context compression default', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
      canonicalProjectConfig(['context_compression: beta']),
    );
    ...
    const result = runNode(tmpDir, stateScript, ['init', 'context-env', 'full'], {
      OWNER_CONTEXT_COMPRESSION: 'off',
    });
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-env', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('initializes auto_transition as true when openspec owner config is absent', async () => {
    const result = runNode(tmpDir, stateScript, ['init', 'auto-transition-defaults', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'auto-transition-defaults', '.owner.yaml'),
      'utf-8',
    );
  ...
  it('initializes auto_transition from .owner/config.yaml when set to false', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'auto-transition-config-false', '.owner.yaml'),
      'utf-8',
    );
    const get = runNode(tmpDir, stateScript, [
      'get',
  ...
  it('validates the language field in .owner.yaml', async () => {
    await createChange(
      tmpDir,
      'language-validate',
      [
    ...
    const valid = runNode(tmpDir, path.join(tmpDir, 'scripts', 'owner-yaml-validate.mjs'), [
      'language-validate',
    ]);
    const setInvalid = runNode(tmpDir, stateScript, [
      'set',
  ...
  it('accepts bound_branch field in .owner.yaml without unknown field errors', async () => {
    await createChange(
      tmpDir,
  ...
  it('rejects a numeric bound_branch value in owner validate', async () => {
    await createChange(
      tmpDir,
      'bound-branch-number',
      [
  ...
  it('rejects array and mapping bound_branch values in owner validate', async () => {
    await createChange(
      tmpDir,
      'bound-branch-array',
      [
    ...
    const validateScript = path.join(tmpDir, 'scripts', 'owner-yaml-validate.mjs');
  ...
  it('accepts a quoted numeric bound_branch string in owner validate', async () => {
    await createChange(
      tmpDir,
      'bound-branch-quoted',
      [
  ...
  it('next resolves auto for full workflow when auto_transition is true', async () => {
    ...
    expect(result.stdout).toContain('SKILL: owner-verify');
  }, 20_000);
  ...
  it('next resolves manual with hint when auto_transition is false', async () => {
    ...
    expect(result.stdout).toContain('SKILL: owner-build');
  ...
  it('next maps hotfix and tweak workflows to their preset skills in build phase', async () => {
    ...
    expect(hotfix.stdout).toContain('SKILL: owner-hotfix');
    expect(tweak.stdout).toContain('SKILL: owner-tweak');
  }, 20_000);
  ...
  it('next maps each non-build phase to the owning skill', async () => {
    ...
    expect(design.stdout).toContain('SKILL: owner-design');
    expect(archive.stdout).toContain('SKILL: owner-archive');
  }, 20_000);

  it('next exits non-zero when .owner.yaml is missing', async () => {
    const result = runNode(tmpDir, stateScript, ['next', 'next-missing']);

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('.owner.yaml not found');
  }, 20_000);
  ...
  it('owner-env.mjs prints the scripts directory it lives in', async () => {
    const envScript = path.join(tmpDir, 'scripts', 'owner-env.mjs');
    const result = runNode(tmpDir, envScript);

    expect(result.status).toBe(0);
    const printedDir = result.stdout.trim();
    // owner-env.mjs prints its own directory — the scripts dir holding every
    // standalone command script.
    expect(printedDir.endsWith('scripts')).toBe(true);
    for (const sibling of ['owner-state.mjs', 'owner-guard.mjs', 'owner-hook-guard.mjs']) {
  ...
  it('uses the global Owner language in guards when change and project values are absent', async () => {
    const fakeHome = path.join(tmpDir, 'fake-home');
    await writeFile(path.join(fakeHome, '.owner', 'config.yaml'), 'pipeline:\n  language: zh-CN\n');
    await createChange(
      tmpDir,
  ...
  it('fails closed before language checks when .owner/config.yaml is malformed', async () => {
    await writeFile(
      path.join(tmpDir, '.owner', 'config.yaml'),
      `${canonicalProjectConfig(['language: en'])}unrelated_field: [unterminated\n`,
    );
  ...
  it('excludes fenced code blocks from the language dominance check', async () => {
    ...
    const bodyWithLargeCodeBlock = [
      ...
      '',
      '```bash',
      'npx vitest run test/domains/owner-pipeline/owner-scripts.test.ts --reporter verbose --coverage --watch=false',
      'export OWNER_LANGUAGE=en OWNER_CONTEXT_COMPRESSION=beta OWNER_AUTO_TRANSITION=true',
  ...
  it('generates a design handoff and requires minimal design doc linkage before leaving design', async () => {
    const handoffScript = path.join(tmpDir, 'scripts', 'owner-handoff.mjs');
    await createChange(
      tmpDir,
      'handoff-change',
      [
    ...
    expect(contextPath).toBe('openspec/changes/handoff-change/.owner/handoff/design-context.json');
  ...
  it('accepts handoff source paths containing regular expression metacharacters', async () => {
    ...
    await writeFile(
      ...
      [
        '---',
        'owner_change: regex-source-path',
        'role: technical-design',
        'canonical_spec: openspec',
        '---',
        '',
  ...
  it('generates a beta spec projection handoff with verbatim spec content', async () => {
    ...
    expect(handoff.status).toBe(0);
    expect(contextPath).toBe('openspec/changes/beta-context/.owner/handoff/spec-context.json');
    ...
    expect(contextMarkdown).toContain('Generated-by: owner-handoff.sh');
  ...
  it('blocks beta design exit when spec-context.json is malformed JSON', async () => {
    ...
    await writeFile(
      ...
      [
        '---',
        'owner_change: beta-malformed-json',
        'role: technical-design',
        'canonical_spec: openspec',
        '---',
        '',
  ...
  it('reads owner yaml fields without including trailing comments', async () => {
    const handoffScript = path.join(tmpDir, 'scripts', 'owner-handoff.mjs');
    const validateScript = path.join(tmpDir, 'scripts', 'owner-yaml-validate.mjs');
    await createChange(
      tmpDir,
  ...
  it('accepts design doc frontmatter after a BOM and leading blank lines', async () => {
    ...
    await writeFile(
      ...
      [
        ...
        '',
        '---',
        'owner_change: frontmatter-prefix',
        'role: technical-design',
        'canonical_spec: openspec',
        '---',
        '',
  ...
  it('warns when --full is passed in beta mode', async () => {
    ...
    expect(contextPath).toBe('openspec/changes/beta-full-warn/.owner/handoff/spec-context.json');
  }, 20_000);
  ...
  it('blocks design exit when design doc frontmatter is missing required fields', async () => {
    ...
    // Design doc with wrong owner_change
    await writeFile(
      path.join(tmpDir, 'docs', 'superpowers', 'specs', 'bad-design.md'),
      [
        '---',
        'owner_change: wrong-change',
        'role: technical-design',
        'canonical_spec: openspec',
        '---',
        '',
  ...
  it('blocks build completion until isolation and build mode are selected', async () => {
    ...
    expect(guard.stderr).toContain(
      'owner state set missing-build-decisions isolation <current|branch|worktree>',
    );
  ...
  it('rejects invalid build_pause values during schema validation', async () => {
    ...
    expect(result.stderr).toContain('FATAL: .owner.yaml schema validation failed');
  }, 20_000);
  ...
  it('removes legacy command fields from existing change state before guard checks', async () => {
    ...
    const migrated = await fs.readFile(path.join(changeDir, '.owner.yaml'), 'utf8');
  ...
  it('treats repo-root owner.yaml as absent when running inferred build checks', async () => {
    await createChange(
      tmpDir,
    ...
    await writeFile(path.join(tmpDir, 'owner.yaml'), 'build_command: node root-build-check.js\n');
    await writeFile(
  ...
  it('does not silently pass when a malformed project config still references a removed command field', async () => {
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).not.toContain('[PASS] Verification passes');
    expect(result.stderr).toContain('Invalid .owner/config.yaml');
  }, 20_000);
  ...
  it('resolves OpenSpec date-prefixed archive directories from the original change name', async () => {
    ...
    expect(validate.stderr).toContain(
      '[VALIDATE] openspec/changes/archive/2026-05-21-resolved-archive/.owner.yaml',
    );
  }, 20_000);

  it('reports accurate archive step counts when syncing and annotating', async () => {
    const archiveScript = path.join(tmpDir, 'scripts', 'owner-archive.mjs');
  ...
  it('scale uses base_ref from .owner.yaml when plan has no base-ref header', async () => {
    execFileSync('git', ['init'], { cwd: tmpDir, stdio: 'ignore' });
    execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd: tmpDir });
    execFileSync('git', ['config', 'user.name', 'Test User'], { cwd: tmpDir });
    await writeFile(path.join(tmpDir, 'README.md'), 'base\n');
  ...
  it('blocks direct phase writes but allows the OWNER_FORCE_PHASE escape hatch', async () => {
    await createChange(
      tmpDir,
      'phase-jump',
      ['workflow: full', 'phase: open', 'design_doc: null', 'archived: false', ''].join('\n'),
  ...
  it('escalates preset workflows from build to design via preset-escalate', async () => {
    for (const workflow of ['hotfix', 'tweak'] as const) {
      ...
      const profile = runNode(tmpDir, stateScript, ['get', name, 'pipeline_profile']);
  ...
  it('reports error for malformed .owner.yaml on get', async () => {
    const changeDir = path.join(tmpDir, 'openspec', 'changes', 'bad-yaml');
    await fs.mkdir(changeDir, { recursive: true });
    await writeFile(path.join(changeDir, '.owner.yaml'), 'workflow: full\nphase: [\n  broken\n');

    const result = runNode(tmpDir, stateScript, ['get', 'bad-yaml', 'phase']);

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('Invalid .owner.yaml');
  });
  ...
  describe('check --recover', () => {
    ...
    it('requires subagent dispatch confirmation when recovering subagent build mode', async () => {
      ...
      expect(result.stdout).toContain('owner state set <change-name> subagent_dispatch confirmed');
    ...
    it('outputs recovery context for design phase with handoff but no design doc', async () => {
      await createChange(
        ...
        [
          ...
          'plan: null',
          'verify_result: pending',
          'handoff_context: openspec/changes/recover-design/.owner/handoff/design-context.json',
    ...
    it('outputs recovery context for archive phase', async () => {
      ...
      expect(result.stdout).toContain('/owner-archive');
      expect(result.stdout).toContain('End Recovery Context');
    });
  ...
  describe('review fix: build-complete conditional reset', () => {
    it('preserves verification_report on re-verify cycle (H1)', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'reverify-test', '.owner.yaml'),
        'utf-8',
      );
  ...
  describe('verify-fail invalidates premature branch handling', () => {
    it('resets branch_status so archive owns final branch handling', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'branch-preserve', '.owner.yaml'),
        'utf-8',
      );
  ...
  describe('owner-hook-guard.mjs — phase write guard', () => {
    it('allows all writes when no active owner change exists', async () => {
    ...
    it('does not treat root .owner.yaml as a supported Owner config file', async () => {
      await createChange(
        tmpDir,
        'test-hook',
        [
      ...
      const result = runHookGuard(
        tmpDir,
        hookGuardScript,
        hookStdin(path.join(tmpDir, '.owner.yaml')),
      );
    ...
    it('allows new change artifact writes while another change is in archive phase (no state file yet)', async () => {
      ...
      // Brand-new change being created: artifacts written before .owner.yaml exists
    ...
    it('requires selection before a standard specs write with multiple active changes', async () => {
      ...
      expect(result.stderr).toContain('owner state select <change-name>');
    }, 20_000);
  ...
  describe('workspace mode branch binding', () => {
    const stateScript = path.join(scriptsDir, 'owner-state.mjs');
    ...
    it.each(['current', 'branch', 'worktree'])(
      ...
      async (isolation) => {
        ...
        const yaml = await fs.readFile(
          path.join(tmpDir, 'openspec', 'changes', 'bind-workspace', '.owner.yaml'),
          'utf-8',
        );
    ...
    it.each(['current', 'branch', 'worktree'])(
      ...
      async (isolation) => {
        ...
        expect(result.status).toBe(0);
        const yaml = await fs.readFile(
          path.join(tmpDir, 'openspec', 'changes', 'rebind-test', '.owner.yaml'),
          'utf-8',
        );
        expect(yaml).toContain(`isolation: ${isolation}`);
    ...
    it('re-points bound_branch when switching between workspace modes', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'mode-switch', '.owner.yaml'),
        'utf-8',
      );
    ...
    it('rejects select while the bound branch has drifted', async () => {
      ...
      expect(init.status).toBe(0);
      const stateFile = path.join(tmpDir, 'openspec', 'changes', 'drift-select', '.owner.yaml');
      await fs.writeFile(
        stateFile,
      ...
      await expect(fs.access(path.join(tmpDir, '.owner', 'current-change.json'))).rejects.toThrow();
    }, 20_000);
    ...
    it.each(['current', 'branch', 'worktree'])(
      ...
      async (isolation) => {
        ...
        const yaml = await fs.readFile(
          path.join(tmpDir, 'openspec', 'changes', 'detached-test', '.owner.yaml'),
          'utf-8',
        );
    ...
    it.each(['current', 'branch', 'worktree'])(
      ...
      async (isolation) => {
        ...
        expect(result.stderr).toContain(`[SET] isolation=${isolation}`);
        const yaml = await fs.readFile(
          path.join(tmpDir, 'openspec', 'changes', 'non-git-workspace', '.owner.yaml'),
          'utf-8',
        );
        expect(yaml).toContain(`isolation: ${isolation}`);
        expect(yaml).toContain('bound_branch: null');
    ...
    it.each(['current', 'branch', 'worktree'])(
      ...
      async (isolation) => {
        ...
        expect(init.status).toBe(0);
        const stateFile = path.join(tmpDir, 'openspec', 'changes', 'legacy-select', '.owner.yaml');
  ...
  describe('state check bound_branch drift', () => {
    ...
    it('drift is still detected after the .owner/ sidecar is deleted (reads .owner.yaml, not the sidecar)', async () => {
      execFileSync('git', ['init', '-b', 'feature-A'], { cwd: tmpDir, stdio: 'ignore' });
      execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd: tmpDir });
      execFileSync('git', ['config', 'user.name', 'Test User'], { cwd: tmpDir });
      await writeFile(path.join(tmpDir, 'README.md'), 'test\n');
      ...
      await fs.rm(path.join(tmpDir, 'openspec', 'changes', 'check-drift-no-sidecar', '.owner'), {
        recursive: true,
        force: true,
      });
  ...
  describe('state rebind', () => {
    ...
    it('rebind updates bound_branch, passes state check, and appends a rebind audit event', async () => {
      ...
      const eventsLog = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'rebind-success', '.owner', 'state-events.jsonl'),
        'utf8',
      );
  ...
  describe('guard bound_branch drift', () => {
    const guardScript = path.join(scriptsDir, 'owner-guard.mjs');

    it.each(['current', 'branch', 'worktree'])(
      'drifted %s-bound change: owner-guard archive returns non-zero with BLOCKED and drift message',
      async (isolation) => {
        execFileSync('git', ['init', '-b', 'feature-A'], { cwd: tmpDir, stdio: 'ignore' });
        execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd: tmpDir });
        execFileSync('git', ['config', 'user.name', 'Test User'], { cwd: tmpDir });

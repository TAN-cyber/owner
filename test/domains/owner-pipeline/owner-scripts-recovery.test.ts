/**
 * Recovery tests — split from owner-scripts.test.ts for maintainability.
 *
...
describe('check --recover', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-recovery-'));
  ...
  it('returns full build recovery to plan creation when configuration is already selected', async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain('ERROR_CODE: pipeline-build-plan-missing');
    expect(result.stdout).toContain('CHANGE: recover-missing-plan');
    expect(result.stdout).toContain(
      'owner state set recover-missing-plan plan <repository-relative-plan-path>',
    );
    expect(result.stdout).toContain('owner state check recover-missing-plan build --recover');
  ...
  it('reports a recorded but missing full build plan as broken', async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain('ERROR_CODE: pipeline-build-plan-broken');
    expect(result.stdout).toContain('RECORDED_PLAN: docs/superpowers/plans/missing-plan.md');
    expect(result.stdout).toContain(
      'owner state set recover-broken-plan plan <new-repository-relative-plan-path>',
    );
    expect(result.stdout).toContain('owner state check recover-broken-plan build --recover');
  });

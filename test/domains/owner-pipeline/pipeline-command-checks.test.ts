...
import {
  latestCommandCheck,
  recordCommandCheck,
} from '../../../domains/owner-pipeline/pipeline-command-checks.js';
...
describe('Pipeline command check evidence', () => {
  let projectRoot: string;
  let changeDir: string;
  let run: RunState;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-command-check-'));

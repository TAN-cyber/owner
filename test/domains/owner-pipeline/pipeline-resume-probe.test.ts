...
import { resolveOwnerResumeProbe } from '../../../domains/owner-pipeline/pipeline-resume-probe.js';
...
async function createChange(
  ...
  await writeFile(path.join(root, '.owner.yaml'), yaml);
...
describe('resolveOwnerResumeProbe', () => {
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-resume-probe-'));
    await writeFile(
  ...
  it('returns none when no active Owner changes exist', async () => {
    const result = await resolveOwnerResumeProbe(tmpDir, {
  ...
  it('ignores non-change files in the Pipeline changes directory', async () => {
  ...
  it('auto resumes a single active change for resume-like work', async () => {
    ...
    expect(result).toMatchObject({
      ...
      phase: 'build',
      nextCommand: '/owner-build',
    });
  });

  it('resumes the configured Pipeline root without scanning the standalone OpenSpec root', async () => {
    ...
    await expect(
      resolveOwnerResumeProbe(tmpDir, {
        schema_version: 'owner.resume_probe.v1',
        utterance: '继续刚才的任务',
        agent_context: { non_trivial_work: true, already_in_owner_flow: false },
      }),
  ...
  it('fails closed when an active change directory is a junction', async () => {
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-resume-probe-outside-'));
    try {
      const outsideChange = path.join(outside, 'cache-ttl');
      await writeFile(path.join(outsideChange, '.owner.yaml'), buildYaml);
  ...
  it('does not rewrite legacy command fields while probing', async () => {
    ...
    const yamlPath = path.join(tmpDir, 'openspec', 'changes', 'cache-ttl', '.owner.yaml');
  ...
  it('asks when an OpenSpec change exists without Owner state', async () => {
    ...
    expect(result.reason).toContain('missing Owner state');
  });
  ...
  it('returns out_of_scope for pure questions', async () => {
    ...
    const result = await resolveOwnerResumeProbe(tmpDir, {
      schema_version: 'owner.resume_probe.v1',
      utterance: '这个探针靠谱吗',
      agent_context: { non_trivial_work: false, already_in_owner_flow: false },
    });

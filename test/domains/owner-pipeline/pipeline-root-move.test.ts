...
import {
  applyPipelineRootMove,
  inspectPipelineRootMove,
  planPipelineRootMove,
  repairPipelineRootMove,
} from '../../../domains/owner-pipeline/pipeline-root-move.js';
import { formatPipelineRootMoveReport } from '../../../domains/owner-pipeline/pipeline-root-command.js';
import {
...
describe('Pipeline root move', () => {
  ...
  async function applyPlannedRootMove(
    options: NonNullable<Parameters<typeof applyPipelineRootMove>[1]> = {},
  ) {
    const plan = await planPipelineRootMove(projectRoot);
    return applyPipelineRootMove(projectRoot, {
      ...options,
      planId: plan.planId,
    } as Parameters<typeof applyPipelineRootMove>[1]);
  }
  ...
  async function writeJournal(
    ...
    const planned = await planPipelineRootMove(projectRoot);
    const configPath = String(overrides.configPath ?? '.owner/config.yaml');
    ...
    const staging = String(
      overrides.staging ?? `.owner/transactions/pipeline-root-move/${transactionId}/openspec`,
    );
  ...
  async function externalDirectory(): Promise<string> {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-root-move-outside-'));
  ...
  async function writeArchivedState(
    ...
    if (options.runStatus) {
      ...
      await fs.writeFile(
        path.join(changeDir, '.owner', 'run-state.json'),
        `${JSON.stringify(run, null, 2)}\n`,
      );
      if (options.checkpoint) {
        await fs.writeFile(
          path.join(changeDir, '.owner', 'checkpoint.json'),
          `${JSON.stringify({
            runId: run.runId,
            stateVersion: 1,
            trajectoryOffset: 0,
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-root-move-'));
  ...
  it('reports a dry run without modifying files or config', async () => {
    ...
    const report = formatPipelineRootMoveReport(plan, 'dry-run');
    ...
    expect(report).toContain('config path: .owner/config.yaml');
    ...
    const reportWithBlockers = formatPipelineRootMoveReport(
    ...
    const chineseReport = formatPipelineRootMoveReport(
      {
        ...plan,
        conflicts: ['Pipeline docs target is not empty'],
        blockers: ['pending Pipeline root move: transaction-id at locked'],
      },
      'dry-run',
      'zh-CN',
    );
    ...
    expect(chineseReport).toContain(
      '阻塞项:\n- 存在待恢复的 Pipeline 根目录迁移：transaction-id，阶段 locked',
    );
    expect(report).toContain('historical pointers preserved:');
    expect(report).toContain('allowed recovery strategies: none');
    expect((await readProjectConfig(projectRoot))?.pipeline?.artifact_layout).toBe('legacy');
  ...
  it('derives and locks the executable plan during apply', async () => {
    const applied = await applyPipelineRootMove(projectRoot);
    expect(applied.readyToApply).toBe(true);

    expect((await readProjectConfig(projectRoot))?.pipeline?.artifact_layout).toBe('docs');
    ...
    await expect(
      fs.stat(path.join(projectRoot, '.owner', 'pipeline-root-move.json')),
    ).rejects.toMatchObject({ code: 'ENOENT' });
  });

  it('rejects source and config drift against the approved dry-run plan', async () => {
    const sourcePlan = await planPipelineRootMove(projectRoot);
    await fs.appendFile(path.join(projectRoot, 'openspec', 'specs', 'demo', 'spec.md'), 'drift\n');

    await expect(
      applyPipelineRootMove(projectRoot, {
        planId: sourcePlan.planId,
      } as Parameters<typeof applyPipelineRootMove>[1]),
    ).rejects.toThrow(/plan changed since dry-run/iu);

    await fs.writeFile(path.join(projectRoot, 'openspec', 'specs', 'demo', 'spec.md'), '# Demo\n');
    const configPlan = await planPipelineRootMove(projectRoot);
    await fs.appendFile(path.join(projectRoot, '.owner', 'config.yaml'), '# drift\n');
  ...
  it('rejects a same-content replacement of the source root', async () => {
    ...
    const replacementPlan = await planPipelineRootMove(projectRoot);
  ...
  it('does not invalidate the plan for an unrelated project-root file', async () => {
    ...
    const current = await planPipelineRootMove(projectRoot);
  ...
  it('does not let a stale apply replace or unlink a successor published after quarantine', async () => {
    ...
    const staleApply = applyPipelineRootMove(projectRoot, {
      planId: plan.planId,
      testHooks: {
        afterJournalQuarantine: async (operation) => {
          ...
          successorApply = applyPipelineRootMove(projectRoot, {
            planId: plan.planId,
            testHooks: {
    ...
    const persisted = JSON.parse(
      await fs.readFile(path.join(projectRoot, '.owner', 'pipeline-root-move.json'), 'utf8'),
    ) as { id: string; stage: string };
  ...
  it('moves a quiescent legacy root and switches config last', async () => {
    ...
    expect((await readProjectConfig(projectRoot))?.pipeline?.artifact_layout).toBe('docs');
    expect(digest(await fs.readFile(path.join(projectRoot, '.owner', 'config.yaml'), 'utf8'))).toBe(
  ...
  it('moves active and incomplete archived changes with the complete OpenSpec tree', async () => {
    ...
    await applyPipelineRootMove(projectRoot);
  ...
  it('refuses a non-empty docs target without merging roots', async () => {
    ...
    const plan = await planPipelineRootMove(projectRoot);
    expect(plan.conflicts).toContain('Pipeline docs target is not empty');
    expect(plan.readyToApply).toBe(false);
    await expect(applyPlannedRootMove()).rejects.toThrow('Pipeline docs target is not empty');
    await expect(
  ...
  it.each([
    ...
    async ({ label, target, prepare }) => {
      ...
      try {
        await expect(
          planPipelineRootMove(projectRoot, {
            testHooks: {
  ...
  it('rejects a legacy source root that is a directory link outside the project', async () => {
    ...
    await expect(planPipelineRootMove(projectRoot)).rejects.toThrow(
  ...
  it('rejects a transaction ancestor that is a directory link outside the project', async () => {
    const outside = await externalDirectory();
    await fs.mkdir(path.join(projectRoot, '.owner', 'transactions'), { recursive: true });
    await fs.rmdir(path.join(projectRoot, '.owner', 'transactions'));
    await directoryLink(outside, path.join(projectRoot, '.owner', 'transactions'));
  ...
  it('does not create the migration journal through a replaced .owner parent', async () => {
    const outside = await externalDirectory();
    const originalOwner = path.join(outside, 'original-owner');
    const externalOwner = path.join(outside, 'external-owner');
    await fs.mkdir(externalOwner);
    await fs.writeFile(path.join(externalOwner, 'marker.txt'), 'external marker\n');
      ...
      applyPlannedRootMove({
        testHooks: {
          beforeMutation: async (operation) => {
            ...
            await fs.rename(path.join(projectRoot, '.owner'), originalOwner);
            await directoryLink(externalOwner, path.join(projectRoot, '.owner'));
          },
        },
      }),
    ).rejects.toThrow(/changed|symbolic link|junction/iu);

    await expect(fs.readdir(externalOwner)).resolves.toEqual(['marker.txt']);
    await expect(fs.stat(path.join(originalOwner, 'config.yaml'))).resolves.toBeDefined();
  });

  it('does not update the migration journal through a replaced .owner parent', async () => {
    const outside = await externalDirectory();
    const originalOwner = path.join(outside, 'original-update-owner');
    const externalOwner = path.join(outside, 'external-update-owner');
    await fs.mkdir(externalOwner);
    await fs.writeFile(path.join(externalOwner, 'marker.txt'), 'external marker\n');
    ...
    await expect(fs.readdir(externalOwner)).resolves.toEqual(['marker.txt']);
    const persisted = JSON.parse(
      await fs.readFile(path.join(originalOwner, 'pipeline-root-move.json'), 'utf8'),
    ) as { stage: string };
  ...
  it('rejects a persisted staging root that is a directory link outside the project', async () => {
    ...
    await expect(repairPipelineRootMove(projectRoot, 'continue')).rejects.toThrow(
      /physical path .*symbolic link or junction/u,
    );
    await expect(
      fs.readFile(path.join(outside, 'openspec', 'specs', 'demo', 'spec.md'), 'utf8'),
  ...
  it.each([
    ...
    await expect(repairPipelineRootMove(projectRoot, 'continue')).rejects.toThrow(
      /invalid Pipeline root move journal|must stay inside/u,
    );
  ...
  it('rejects a journal whose parent becomes an external junction after inspection', async () => {
    ...
    const externalOwner = path.join(outside, 'external-owner');
    const originalOwner = path.join(outside, 'original-owner');
    await fs.mkdir(externalOwner, { recursive: true });
    await fs.writeFile(
      path.join(externalOwner, 'pipeline-root-move.json'),
      '{"external":"untrusted"',
      'utf8',
    );
    let replaced = false;
    ...
    await expect(
      inspectPipelineRootMove(projectRoot, {
        testHooks: {
          afterJournalInspect: async () => {
            if (replaced) return;
            replaced = true;
            const ownerDirectory = path.join(projectRoot, '.owner');
            await fs.rename(ownerDirectory, originalOwner);
            await directoryLink(externalOwner, ownerDirectory);
          },
        },
      }),
    ).rejects.toThrow(/symbolic link or junction|changed while/iu);
    ...
    await expect(
      fs.readFile(path.join(externalOwner, 'pipeline-root-move.json'), 'utf8'),
    ).resolves.toBe('{"external":"untrusted"');
  });

  it('rejects a root-move journal that exceeds the 16 MiB read budget', async () => {
    const journal = path.join(projectRoot, '.owner', 'pipeline-root-move.json');
    await fs.writeFile(journal, '{}', 'utf8');
    await fs.truncate(journal, 16 * 1024 * 1024 + 1);

    await expect(inspectPipelineRootMove(projectRoot)).rejects.toThrow(
  ...
  it('rejects traversal inside a persisted manifest before copying', async () => {
    ...
    await expect(repairPipelineRootMove(projectRoot, 'continue')).rejects.toThrow(
      /invalid Pipeline root move journal|manifest path/u,
    );
  });
  ...
  it('continues a verified ready transaction only when explicitly requested', async () => {
    ...
    const recoveryPlan = await planPipelineRootMove(projectRoot);
    expect(recoveryPlan).toMatchObject({
      staging: `.owner/transactions/pipeline-root-move/${transactionId}/openspec`,
      blockers: [`pending Pipeline root move: ${transactionId} at ready`],
    ...
    await expect(applyPlannedRootMove()).rejects.toThrow(
      /use owner doctor --repair --strategy continue\|rollback/u,
    );
    await expect(repairPipelineRootMove(projectRoot)).rejects.toThrow(/strategy is required/u);
    expect((await inspectPipelineRootMove(projectRoot))?.allowedStrategies).toEqual([
      'continue',
      'rollback',
    ]);
    expect((await inspectPipelineRootMove(projectRoot))?.staging).toBe(
      `.owner/transactions/pipeline-root-move/${transactionId}/openspec`,
    );

    await expect(repairPipelineRootMove(projectRoot, 'continue')).resolves.toBe(true);
    expect((await readProjectConfig(projectRoot))?.pipeline?.artifact_layout).toBe('docs');
  });
  ...
  it('keeps legacy v1 journals explicitly recoverable after v2 is introduced', async () => {
    ...
    await expect(inspectPipelineRootMove(projectRoot)).resolves.toMatchObject({
      stage: 'ready',
      allowedStrategies: ['continue', 'rollback'],
    });
    await expect(repairPipelineRootMove(projectRoot, 'rollback')).resolves.toBe(true);
    await expect(fs.stat(path.join(projectRoot, 'openspec'))).resolves.toBeDefined();
    await expect(
      fs.stat(path.join(projectRoot, '.owner', 'pipeline-root-move.json')),
    ).rejects.toMatchObject({ code: 'ENOENT' });
  ...
  it('does not recursively remove rollback staging through a replacement junction', async () => {
    ...
    await expect(
      repairPipelineRootMove(projectRoot, 'rollback', {
        testHooks: {
          beforeMutation: async (operation) => {
  ...
  it('preserves all trees when project config drifts after the journal is written', async () => {
    await stageSource();
    await writeJournal('ready');
    await fs.appendFile(path.join(projectRoot, '.owner', 'config.yaml'), 'extension: drifted\n');

    await expect(repairPipelineRootMove(projectRoot, 'continue')).rejects.toThrow(
      /config changed after migration preflight/u,
    );
    await expect(repairPipelineRootMove(projectRoot, 'rollback')).rejects.toThrow(
      /config changed after migration preflight/u,
    );
    await expect(fs.stat(path.join(projectRoot, 'openspec'))).resolves.toBeDefined();
    await expect(
    ...
    const expectedSource = renderStructuredProjectConfig(
      {
        ...parsed.value,
        pipeline: {
          ...(parsed.value.pipeline as Record<string, unknown>),
  ...
  it('does not overwrite config drift after the root-move identity check and before commit', async () => {
    ...
    await expect(
      writePipelineArtifactLayout(projectRoot, 'docs', {
        expectedIdentity: {
          exists: true,
  ...
  it('continues an interrupted manifest-bound quarantine cleanup idempotently', async () => {
    ...
    expect((await inspectPipelineRootMove(projectRoot))?.allowedStrategies).toEqual(['continue']);
    await expect(repairPipelineRootMove(projectRoot, 'continue')).resolves.toBe(true);
  ...
  it('blocks ordinary Pipeline writes while a root move is pending', async () => {
    await writeJournal('copying');

    await expect(assertPipelineLayoutWritable(projectRoot)).rejects.toThrow(
      /Pipeline root move .* is incomplete/u,
    );
  });
});

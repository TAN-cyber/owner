...
import * as pipelinePaths from '../../../domains/owner-pipeline/pipeline-paths.js';

const { resolvePipelineChangeDirectory } = pipelinePaths;
...
function exportedFunction<T>(name: string): T | undefined {
  return (pipelinePaths as unknown as Record<string, T | undefined>)[name];
}

describe('Pipeline change paths', () => {
  let projectRoot: string;
  let outsideRoot: string;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-paths-'));
    outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-pipeline-paths-outside-'));
  ...
  it('fails closed when an active change directory is a directory link', async () => {
    await fs.writeFile(path.join(outsideRoot, '.owner.yaml'), 'workflow: full\n', 'utf8');
    await fs.symlink(
      outsideRoot,
    ...
    await expect(resolvePipelineChangeDirectory('demo', projectRoot)).rejects.toThrow(
  ...
  it('does not resolve a longer archived change that merely shares the requested suffix', async () => {
    ...
    await fs.writeFile(path.join(wrong, '.owner.yaml'), 'workflow: full\n', 'utf8');

    await expect(resolvePipelineChangeDirectory('demo', projectRoot)).resolves.toEqual({
      label: 'openspec/changes/demo',
      directory: path.join(projectRoot, 'openspec', 'changes', 'demo'),
    });
  });
  ...
  it('fails closed when an archived change state is not a real file', async () => {
    ...
    await fs.mkdir(path.join(archived, '.owner.yaml'), { recursive: true });
  ...
  it('selects the latest strict date-prefixed archive for the exact change name', async () => {
    for (const date of ['2026-07-27', '2026-07-28']) {
      ...
      await fs.writeFile(path.join(archived, '.owner.yaml'), 'workflow: full\n', 'utf8');
    }
  ...
  it('prefers the exact archive produced by the current archive operation', async () => {
    ...
    await expect(
      pipelinePaths.findPipelineArchiveChangeDirectory('demo', projectRoot, {
  ...
  it('does not fall back to an old archive when the preferred archive is missing', async () => {
    ...
    await fs.writeFile(path.join(exact, '.owner.yaml'), 'workflow: full\n', 'utf8');

...
import { pipelineResumeProbeCommand } from '../../../domains/owner-pipeline/pipeline-resume-probe-command.js';
...
async function createActiveChange(name: string): Promise<string> {
  ...
  await writeFile(path.join(changeDir, '.owner.yaml'), buildYaml);
...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-resume-probe-command-'));
  await preparePipelineLegacyProject(tmpDir);
...
describe('pipelineResumeProbeCommand', () => {
  it('prints usage for missing probe subcommand input', async () => {
    const result = await pipelineResumeProbeCommand([], { json: false });

    expect(result.exitCode).toBe(64);
    expect(result.stderr).toContain('Usage: owner-resume-probe.mjs probe <input-json>');
  });

  it('reports invalid JSON with exit code 1', async () => {
    const result = await pipelineResumeProbeCommand(['probe', '{'], { json: false });
  ...
  it('returns parsed probe result for `probe <input-json>`', async () => {
    const result = await pipelineResumeProbeCommand(['probe', SAMPLE_INPUT], { json: false });
  ...
  it('returns parsed probe result for `probe --stdin`', async () => {
    const result = await runWithStdin(SAMPLE_INPUT, () =>
      pipelineResumeProbeCommand(['probe', '--stdin'], { json: false }),
    );
    const payload = JSON.parse(result.stdout ?? '');
  ...
  it('emits a pipeline --json envelope that contains the probe stdout payload', async () => {
    const result = await runPipelineCli(['resume-probe', 'probe', SAMPLE_INPUT, '--json']);
  ...
  it('is registered in the shared Pipeline CLI dispatcher', async () => {
    const result = await runPipelineCli(['resume-probe'], {

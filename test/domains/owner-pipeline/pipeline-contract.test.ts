...
// behavior (stdout/stderr/exit/.owner.yaml), not the invocation mechanism.
...
const referenceRoot = path.resolve('test', 'fixtures', 'pipeline-0.3.9');
const referenceScripts = path.join(referenceRoot, 'scripts');
const activeScripts = path.resolve('assets', 'skills', 'owner', 'scripts');
const temporaryRoots: string[] = [];

const FROZEN_VARIANT: ScriptVariant = {
  names: [...scriptNames],
  state: 'owner-state.sh',
  guard: 'owner-guard.sh',
  handoff: 'owner-handoff.sh',
  hookGuard: 'owner-hook-guard.sh',
  executor: 'bash',
};

const ACTIVE_VARIANT: ScriptVariant = {
  names: activeScriptNames,
  state: 'owner-state.mjs',
  guard: 'owner-guard.mjs',
  handoff: 'owner-handoff.mjs',
  hookGuard: 'owner-hook-guard.mjs',
  executor: 'node',
};
...
async function configureActivePipelineLegacyLayout(
  root: string,
...
function runScript(
  ...
  const env = {
    ...
    LANG: 'C',
    LC_ALL: 'C',
    OWNER_RUNTIME_PIPELINE_ROOT: path.resolve('assets', 'skills', 'owner', 'runtime', 'pipeline'),
    OWNER_PIPELINE_SKILL_ROOT: path.resolve('assets', 'skills', 'owner', 'runtime', 'pipeline'),
  };
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      (/ENOENT: no such file or directory, open /u.test(nextLine) ||
        /Pipeline language-check artifact .* does not exist/u.test(nextLine))
    ) {
      index += 1;
      continue;
    }
...
async function observeState(
  ...
  const root = await fs.mkdtemp(
    path.join(await fs.realpath(os.tmpdir()), `owner-pipeline-${profile}-`),
  );
  temporaryRoots.push(root);
  const scripts = path.join(root, 'scripts');
  await copyScripts(sourceScripts, scripts, variant.names);
  await configureActivePipelineLegacyLayout(root, sourceScripts);
  ...
  const result = followUp
    ...
    : init;
  const yamlPath = path.join(root, 'openspec', 'changes', name, '.owner.yaml');
...
async function observeGuard(
  ...
  const variant = variantOf(sourceScripts);
  const root = await fs.mkdtemp(
    path.join(await fs.realpath(os.tmpdir()), `owner-guard-${profile}-`),
  );
  temporaryRoots.push(root);
  const scripts = path.join(root, 'scripts');
  await copyScripts(sourceScripts, scripts, variant.names);
...
async function observeHandoff(
  ...
  const variant = variantOf(sourceScripts);
  const root = await fs.mkdtemp(
    path.join(await fs.realpath(os.tmpdir()), `owner-handoff-${profile}-`),
  );
  temporaryRoots.push(root);
  const scripts = path.join(root, 'scripts');
  await copyScripts(sourceScripts, scripts, variant.names);
  ...
  const yaml = parse(await fs.readFile(path.join(changeDir, '.owner.yaml'), 'utf8')) as Record<
    string,
    unknown
  >;
...
async function observeHook(sourceScripts: string): Promise<GuardObservation> {
  const variant = variantOf(sourceScripts);
  const root = await fs.mkdtemp(path.join(await fs.realpath(os.tmpdir()), 'owner-hook-'));
  temporaryRoots.push(root);
  const scripts = path.join(root, 'scripts');
  await copyScripts(sourceScripts, scripts, variant.names);
  await configureActivePipelineLegacyLayout(root, sourceScripts);
...
describe('frozen Pipeline 0.3.9 reference', () => {
...
describeBash('Pipeline 0.3.9 differential contract', () => {

/**
 * Guard tests — split from owner-scripts.test.ts for maintainability.
 *
 * Tests owner-guard.mjs phase guards: open guard (design.md skip for preset),
...
describe('owner guard', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-guard-'));
  ...
  it('keeps the configured guard active when a standalone OpenSpec root exists', async () => {
    ...
    await expect(
      fs.stat(path.join(tmpDir, 'openspec', 'changes', 'dual-root', '.owner')),
  ...
  describe('recorded command-check evidence', () => {
    ...
    it('keeps a bound_branch healed during guard --apply', async () => {
      ...
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'apply-heal', '.owner.yaml'),
        'utf-8',
      );
    ...
    it('explains how to recover when a commandless build has no recorded evidence', async () => {
      ...
      expect(result.stderr).toContain(
        'owner state record-check commandless-build build --command "<command>" --exit-code 0',
      );
    });
    ...
    it('uses the latest same-run evidence even when an older check succeeded', async () => {
      ...
      expect(result.stderr).toContain(
        'owner state record-check latest-build build --command "pnpm lint" --exit-code 0',
      );
    });
    ...
    it('ignores evidence recorded for another run', async () => {
      ...
      const run = JSON.parse(
        await fs.readFile(path.join(changeDir, '.owner', 'run-state.json'), 'utf8'),
    ...
    it('makes OWNER_SKIP_BUILD visible instead of reporting an ordinary silent pass', async () => {
      ...
      expect(result.status).toBe(0);
      expect(result.stderr).toContain('SKIPPED via OWNER_SKIP_BUILD=1');
    });
  });
});

...
import {
  ...
  selectCurrentChange,
} from '../../../domains/owner-pipeline/pipeline-current-change.js';
...
describe('Pipeline current change selection', () => {
  let root: string;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-current-change-'));
  ...
  it('atomically selects an active change and records the selection branch', async () => {
    ...
    expect((await fs.readdir(path.join(root, '.owner'))).sort()).toEqual([
      'config.yaml',
      'current-change.json',
    ]);
  });
  ...
  it('rejects an active change directory junction without selecting external state', async () => {
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-current-change-outside-'));
    try {
  ...
  it.each(['current', 'branch', 'worktree'])(
    ...
    async (isolation) => {
      ...
      expect(
        await fs.readFile(
          path.join(root, 'openspec', 'changes', 'change-a', '.owner.yaml'),
          'utf8',
        ),
  ...
  it('does not write bound_branch while resolving an unbound selection', async () => {
    ...
    expect(
      await fs.readFile(path.join(root, 'openspec', 'changes', 'change-a', '.owner.yaml'), 'utf8'),
  ...
  it('reads the released v1 selection as Pipeline compatibility input', async () => {
    await seedActiveChange(root, 'change-a', false);
    await fs.mkdir(path.dirname(currentChangeFile(root)), { recursive: true });
    await fs.writeFile(
      currentChangeFile(root),

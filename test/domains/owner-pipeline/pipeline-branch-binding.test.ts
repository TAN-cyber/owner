...
import {
  ...
  driftBlockedMessage,
  driftStaleReason,
} from '../../../domains/owner-pipeline/pipeline-branch-binding.js';
...
describe('resolveBranchBinding', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-branch-binding-'));
  ...
  it('heals an unbound binding-isolation change when heal is enabled', async () => {
    ...
    expect(await fs.readFile(path.join(changeDir, '.owner.yaml'), 'utf8')).toContain(
      'bound_branch: main',
    );
  });
  ...
  it('reports needs-heal without writing when heal is disabled', async () => {
    ...
    expect(await fs.readFile(path.join(changeDir, '.owner.yaml'), 'utf8')).not.toContain(
      'bound_branch',
    );
  });

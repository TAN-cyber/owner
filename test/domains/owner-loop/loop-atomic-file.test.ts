...
describe('Loop atomic file containment', () => {
  let root: string;
  let outside: string;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-atomic-'));
    outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-atomic-outside-'));
  });

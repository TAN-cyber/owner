...
import { migrateLoopLegacyChangeToPortable } from '../../../domains/owner-loop/loop-portable-migration-runtime.js';
...
describe('Loop portable migration Runtime', () => {
  const roots: string[] = [];
  ...
  async function legacyChange(): Promise<LoopProjectPaths> {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-migrate-runtime-'));
    roots.push(root);
  ...
  it('commits portable YAML before cleaning legacy Runtime and is idempotent', async () => {
    ...
    const migrated = await migrateLoopLegacyChangeToPortable({ paths, name: 'legacy-change' });
    ...
    expect(await migrateLoopLegacyChangeToPortable({ paths, name: 'legacy-change' })).toEqual(
      migrated,
    );
  });
  ...
  it('finishes cleanup and rebuilds local state when the portable YAML outlives its transaction', async () => {
    const paths = await legacyChange();
    const migrated = await migrateLoopLegacyChangeToPortable({
      paths,
      name: 'legacy-change',
    });
    ...
    const recovered = await migrateLoopLegacyChangeToPortable({
      paths,
      name: 'legacy-change',
    });
  ...
  it('recognizes only valid portable migration journals and lets that migration resume itself', async () => {
    ...
    await expect(
      withLoopMutationLock(paths, 'unrelated file probe', async () => 'allowed'),
    ...
    await expect(
      withLoopMutationLock(paths, 'invalid transaction probe', async () => 'blocked'),
    ...
    await fs.writeFile(
      transactionFile,
      `${JSON.stringify({
        schema: 'owner.loop.portable-migration.v1',
        id: randomUUID(),
        change: 'legacy-change',
        fromSchema: 'owner.loop.v3',
        status: 'prepared',
    ...
    await expect(
      createLoopPortableChange({ paths, name: 'blocked-by-migration', language: 'en' }),
    ).rejects.toThrow('transaction recovery is required');
    const migrated = await migrateLoopLegacyChangeToPortable({
      paths,
      name: 'legacy-change',
    });
    expect(migrated).toMatchObject({ schema: 'owner.loop.v4', name: 'legacy-change' });
    ...
    await expect(
      withLoopMutationLock(paths, 'completed transaction probe', async () => 'allowed'),
    ).resolves.toBe('allowed');
    await expect(
      migrateLoopLegacyChangeToPortable({ paths, name: 'legacy-change' }),
  ...
  it('refuses to migrate an isolated legacy change from the wrong checkout', async () => {
    ...
    await writeLoopWorkspaceIdentity({
      ...
      binding: {
        isolation: 'branch',
        changeBranch: 'owner/legacy-change',
        targetBranch: 'main',
      },
    });
    ...
    await expect(
      fs.readFile(path.join(paths.changesDir, 'legacy-change', 'owner-state.yaml'), 'utf8'),
    ).resolves.toContain('schema: owner.loop.v3');
  });

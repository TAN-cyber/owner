...
import {
  readLoopRunState,
  readLoopTrajectoryText,
  replaceLoopTrajectoryText,
  writeLoopRunState,
} from '../../../domains/owner-loop/loop-run-store.js';
import { LOOP_LEGACY_RUNTIME_IDENTITIES } from '../../../domains/owner-loop/loop-runtime-package.js';
import type { LoopProjectPaths } from '../../../domains/owner-loop/loop-types.js';
import { inspectLoopVerificationFreshness } from '../../../domains/owner-loop/loop-verification-runtime.js';
...
# Decisions
Use the Loop evidence envelope.
# Open questions
None.
...
describe('Loop evidence-bound phase transitions', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-evidence-transition-'));
  ...
  it('binds Build scope and Verify evidence, then marks implementation drift stale', async () => {
    ...
    const built = await advanceLoopChange({
      paths,
      name: 'evidence-change',
    ...
    await expect(inspectLoopStatus(paths, 'evidence-change', { details: true })).resolves.toEqual(
      expect.objectContaining({
    ...
    await expect(
      inspectLoopVerificationFreshness({ paths, state: verified.change }),
    ...
    const stale = await inspectLoopVerificationFreshness({ paths, state: verified.change });
    expect(stale).toMatchObject({
      freshness: 'stale',
      findingCodes: ['verification-implementation-stale'],
    });
    const preflight = await inspectLoopArchivePreflight({ paths, name: 'evidence-change' });
    expect(preflight).toMatchObject({ ready: false });
    expect(preflight.findingCodes).toContain('verification-evidence-stale');
    await expect(inspectLoopStatus(paths, 'evidence-change')).resolves.toMatchObject({
      phase: 'archive',
      nextCommand: 'owner loop next evidence-change --summary "<summary>"',
      continuation: {
        disposition: 'continue',
        action: 'advance-phase',
        command: 'owner loop next evidence-change --summary "<summary>"',
      },
    });

    const retreated = await advanceLoopChange({
      paths,
      name: 'evidence-change',
  ...
  it('retreats a stale Verify scope to Build before requiring contract re-confirmation', async () => {
    ...
    const status = await inspectLoopStatus(paths, 'evidence-change', { details: true });
    expect(status).toMatchObject({
      ...
      findings: expect.arrayContaining([
        expect.objectContaining({
          ...
          retryCommand: 'owner loop next evidence-change --summary "<summary>"',
          requiresUserDecision: false,
        }),
      ]),
    });
    ...
    const blocked = await advanceLoopChange({
      paths,
      name: 'evidence-change',
      evidence: {
    ...
    const confirmed = await advanceLoopChange({
      paths,
      name: 'evidence-change',
      evidence: {
  ...
  it('continues an active v1 Run through the compatible v2 iteration-budget upgrade', async () => {
    ...
    await writeLoopRunState(runtimeDir, {
      ...run!,
      ...LOOP_LEGACY_RUNTIME_IDENTITIES[0],
    });
    await fs.writeFile(path.join(projectRoot, 'src', 'feature.ts'), 'export const value = 2;\n');
    ...
    expect(built.change.phase).toBe('verify');
    await expect(readLoopRunState(runtimeDir)).resolves.toMatchObject(
      LOOP_LEGACY_RUNTIME_IDENTITIES[0],
    );
  });
  ...
  it('does not persist verification evidence before Run consistency accepts the transition', async () => {
    ...
    const beforeState = await readLoopChange(paths, 'evidence-change');
    const run = await readLoopRunState(runtimeDir);
    expect(run).not.toBeNull();
    await writeLoopRunState(runtimeDir, { ...run!, currentStep: 'build' });
    ...
    await expect(readLoopChange(paths, 'evidence-change')).resolves.toMatchObject({
  ...
  it('does not persist verification evidence when the protected trajectory is invalid', async () => {
    ...
    const run = await readLoopRunState(runtimeDir);
    expect(run).not.toBeNull();
    const trajectory = await readLoopTrajectoryText(runtimeDir, run!.trajectoryRef);
  ...
  it('does not retreat fresh Archive evidence through an ordinary next command', async () => {
    ...
    const result = await advanceLoopChange({
      paths,
      name: 'evidence-change',
    ...
    expect(result).toMatchObject({
      next: 'manual',
      nextCommand: 'owner loop archive evidence-change --dry-run',
      change: { phase: 'archive' },
      continuation: {
        disposition: 'continue',
        action: 'archive',
        command: 'owner loop archive evidence-change --dry-run',
      },
    });
  });
  ...
  it('keeps a failed envelope for repair history and clears it on the next Build capture', async () => {
    ...
    const failed = await advanceLoopChange({
      paths,
      name: 'evidence-change',
      evidence: {
  ...
  it('requires one exact user confirmation before attaching a partial scope', async () => {
    ...
    const partial = await advanceLoopChange({
      paths,
      name: 'evidence-change',
    ...
    expect((await readLoopChange(paths, 'evidence-change')).partial_allowance).toBe(

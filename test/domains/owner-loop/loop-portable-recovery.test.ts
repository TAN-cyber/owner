...
import { recoverLoopPortableChange } from '../../../domains/owner-loop/loop-portable-recovery.js';
import { applyLoopRunnerInput } from '../../../domains/owner-loop/loop-runner-input.js';
import {
...
import { compareAndSwapLoopPortableState } from '../../../domains/owner-loop/loop-portable-state.js';
...
describe('Loop portable recovery', () => {
  let root: string;
  let paths: LoopProjectPaths;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-recovery-v4-'));
  ...
  async function createBuild(name: string): Promise<LoopPortableState> {
    ...
    return confirmLoopPortableShape({ paths, name });
  }
  ...
  it.each(['shape', 'build'] as const)(
    ...
    async (phase) => {
      ...
      await fs.rm(path.dirname(loopLocalExecutionFile(paths, name)), {
        recursive: true,
        force: true,
      });
      const recovered = await recoverLoopPortableChange({ paths, name });
  ...
  it.each(['missing', 'stale'] as const)(
    ...
    async (localState) => {
      ...
      const archiveReady: LoopPortableState = {
        ...state,
        phase: 'archive',
        ...
        acceptance: state.acceptance.map((entry) => ({
          ...entry,
          result: 'passed',
          reason: toLoopPortableText('Passed before transfer.'),
        })),
        builder_handoff: {
          ...
          iteration: 1,
          summary: toLoopPortableText('Built.'),
        ...
        verification: {
          ...
          summary: toLoopPortableText('Passed.'),
          risks: [],
      ...
      await compareAndSwapLoopPortableState({
        file: loopPortableStateFile(paths, state.name),
        expectedStateVersion: state.state_version,
        next: archiveReady,
        containedRoot: paths.loopRoot,
      });
      if (localState === 'missing') {
        await fs.rm(path.dirname(loopLocalExecutionFile(paths, state.name)), {
          recursive: true,
          force: true,
        });
      }

      const recovered = await recoverLoopPortableChange({ paths, name: state.name });
      expect(recovered).toMatchObject({
        action: 'reverify',
        state: {
          phase: 'verify',
  ...
  it('abandons a lost running Verifier and returns to a fresh Verify boundary', async () => {
    ...
    const resolved = await executeLoopPortableCheckPlan({
      paths,
      name: state.name,
      plans: [],
    });
    ...
    const rebound = await applyLoopRunnerInput({
      paths,
      name: state.name,
  ...
  it('rebuilds a missing verification report from matching portable state', async () => {
    ...
    const report = path.join(loopPortableChangeDir(paths, state.name), 'verification.md');
  ...
  it('discards a stale non-repeatable check without changing newer portable state', async () => {
    ...
    const file = loopLocalExecutionFile(paths, state.name);

...
describe('Loop contract snapshot', () => {
  ...
  it('changes the contract hash for brief, target spec, operation, or canonical base changes', () => {
    const baseline = buildLoopContractSnapshot({ briefMarkdown: brief, specs: [replaceSpec()] });
    const variants = [
  ...
  it('normalizes line endings without changing the contract hash', () => {
    const lf = buildLoopContractSnapshot({ briefMarkdown: brief, specs: [replaceSpec()] });
    const crlf = buildLoopContractSnapshot({

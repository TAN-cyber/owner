...
describe('Loop portable Doctor', () => {
  let projectRoot: string;
  let paths: LoopProjectPaths;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-portable-doctor-'));
  ...
  it('keeps a fresh portable change healthy in named and project-wide Doctor', async () => {
    ...
    await expect(loopDoctorCommand(['fresh-portable'], projectRoot)).resolves.toMatchObject({
      exitCode: 0,
      data: { healthy: true, workflow: 'loop-portable', change: 'fresh-portable' },
    });
    await expect(loopDoctorCommand([], projectRoot)).resolves.toMatchObject({
      exitCode: 0,
      data: {
        healthy: true,
  ...
  it('merges portable statuses with legacy migration findings in a mixed project', async () => {
    ...
    const result = await loopDoctorCommand([], projectRoot);
    ...
    expect(result).toMatchObject({
      ...
      data: {
        healthy: false,
        workflow: 'loop-mixed',
        changes: [{ name: 'portable-change', schema: 'owner.loop.status.v2' }],
  ...
  it('reports an active and archived portable change with the same name as unhealthy', async () => {
    ...
    const result = await loopDoctorCommand([name], projectRoot);
  ...
  it('finishes an incomplete portable migration through named Doctor repair', async () => {
    ...
    const inspected = await loopDoctorCommand([name], projectRoot);
    ...
    const repaired = await loopDoctorCommand([name, '--repair'], projectRoot);
    expect(repaired).toMatchObject({
      exitCode: 0,
      data: {
        healthy: true,
        repaired: true,
        migration: { recovered: true, to: 'owner.loop.v4', stateVersion: state.state_version },
      },
    });
  ...
  it('reports and resumes a persisted portable migration transaction', async () => {
    ...
    await expect(loopDoctorCommand([name], projectRoot)).resolves.toMatchObject({
      exitCode: 65,
      data: {
        findings: [
          expect.objectContaining({
    ...
    await expect(loopDoctorCommand([name, '--repair'], projectRoot)).resolves.toMatchObject({
      exitCode: 0,
      data: {
        healthy: true,
        repaired: true,
        migration: { recovered: true, to: 'owner.loop.v4' },
      },
    });
  ...
  it('repairs a persisted portable transaction from project-wide Doctor', async () => {
    ...
    const repaired = await loopDoctorCommand(['--repair'], projectRoot);

...
import { canonicalHash, canonicalJson } from '../../../domains/owner-loop/loop-canonical-hash.js';

describe('Loop canonical hashing', () => {
  ...
  it('hashes the domain tag, newline, and canonical JSON exactly', () => {
    const value = { b: 2, a: 1 };
    const expected = createHash('sha256').update('owner.loop.test.v1\n{"a":1,"b":2}').digest('hex');

    expect(canonicalHash('owner.loop.test.v1', value)).toBe(expected);
    expect(canonicalHash('owner.loop.other.v1', value)).not.toBe(expected);
  });

...
// compatibility. The v4 CLI contract is covered by loop-cli-v4-surface.test.ts.
describe('Loop Wave B CLI contract (legacy)', () => {
  let projectRoot: string;
  const projectArgs = () => ['--project-root', projectRoot] as const;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-wave-b-cli-'));
  ...
  it.skip('keeps one envelope and returns the same continuation shape from mutations and status', async () => {
    ...
    const status = json(await runLoopCli(['status', 'cold-resume', '--json', ...projectArgs()]));
    expect(status.data).toMatchObject({
      revision: 2,
      inspection: { freshness: 'fresh' },
      checkpoint: { summary: 'Parser work is complete' },
      detailsCommand: 'owner loop status cold-resume --details',
  ...
  it.skip('puts bounded finding and manifest details behind status --details', async () => {
    const details = json(
      await runLoopCli(['status', 'cold-resume', '--details', '--json', ...projectArgs()]),
    );
    ...
    const invalid = json(await runLoopCli(['status', '--details', '--json', ...projectArgs()]));
        ...
        findings: [
          {
            ...
            severity: 'error',
            path: 'docs/owner/changes/cold-resume/brief.md',
            requiredAction: 'answer-blocking-question',
            retryCommand: 'owner loop next cold-resume --summary "<summary>"',
            repairCommand: null,
  ...
  it('rejects the retired checkpoint command instead of invoking the v4 Runtime', async () => {
    const result = json(
      await runLoopCli(['checkpoint', 'cold-resume', '--json', ...projectArgs()]),
    );

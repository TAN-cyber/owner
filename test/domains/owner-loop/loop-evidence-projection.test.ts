...
import {
  LOOP_EVIDENCE_PROJECTION_REF,
  LOOP_EVIDENCE_PROJECTION_LIMITS,
  renderLoopEvidenceProjectionMarkdown,
  writeLoopEvidenceProjection,
} from '../../../domains/owner-loop/loop-evidence-projection.js';
import {
...
describe('Loop evidence projection rendering', () => {
  describe('renderLoopEvidenceProjectionMarkdown (pure)', () => {
    it('writes the boundary declaration and generator fingerprint', () => {
      ...
      expect(markdown).toContain('Generated-by: owner-loop');
    ...
    it('is deterministic: identical inputs produce identical bytes', () => {
      ...
      expect(renderLoopEvidenceProjectionMarkdown(input)).toBe(
        renderLoopEvidenceProjectionMarkdown({ ...input }),
      );
    });
  });
});

describe('Loop evidence projection write', () => {
  let projectRoot: string;
  let paths: LoopProjectPaths;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-projection-'));
  ...
  function buildFixtureEvidence() {
    ...
    const acceptanceReceipt = buildLoopVerificationReceipt({
  ...
  it('is a no-op when no evidence is recorded (shape phase)', async () => {
    await writeLoopEvidenceProjection(paths, 'secure-login');
    const file = path.join(
      loopChangeDir(paths, 'secure-login'),
      ...LOOP_EVIDENCE_PROJECTION_REF.split('/'),
    );
  ...
  it('writes a human-readable projection after scope evidence is recorded', async () => {
    ...
    const state = await readLoopChange(paths, 'secure-login');
  ...
  it('writes scope, verification, and receipt sections for a verified change', async () => {
    ...
    const { writeLoopPartialAllowance } =
      await import('../../../domains/owner-loop/loop-evidence-storage.js');
    const allowanceRef = await writeLoopPartialAllowance({
      paths,
      name: 'secure-login',
      allowance,
    });
    const receiptRef = await writeLoopVerificationReceipt({
      paths,
      name: 'secure-login',
  ...
  it('truncates long unresolved scope lists with a truncation marker', async () => {
    // The scope builder collapses changes beyond MAX_LOOP_DETAILED_SCOPE_CHANGES
    ...
    const overflowCount = LOOP_EVIDENCE_PROJECTION_LIMITS.maxUnresolvedReasons + 5;
    ...
    const bigSnapshot: LoopContentSnapshotManifest = {
      schema: 'owner.loop.content-snapshot.v1',
      origin: 'explicit',
      createdAt: '2026-07-17T00:00:00.000Z',
      complete: true,
  ...
  it('reads back through the content-addressed readers (tampered evidence surfaces as an error)', async () => {
    ...
    const scopeFile = loopRuntimeRefFile(loopChangeRuntimeDir(paths, 'secure-login'), scopeRef);
    ...
    await expect(writeLoopEvidenceProjection(paths, 'secure-login')).rejects.toThrow();
    // And the underlying reader agrees.
    await expect(readLoopImplementationScope(paths, 'secure-login', scopeRef)).rejects.toThrow();
  });
});

...
function overflowGitSelection(combinedHash: string) {
  return {
    schema: 'owner.loop.git-selection.v1' as const,
...
function changedPhysicalSelection(afterHash: string) {
  return {
    schema: 'owner.loop.physical-selection.v1' as const,
...
function entry(entryPath: string, hash: string, size = 1): LoopSnapshotEntry {
...
function manifest(
  options: {
    ...
    omittedCount?: number;
    overflow?: LoopContentSnapshotManifest['omissionOverflow'];
...
describe('Loop implementation scope', () => {
  it('derives sorted added, modified, and removed content changes', () => {
    const result = buildLoopImplementationScope({
      baseline: manifest({
  ...
  it('does not infer removals when current omissions exist only in overflow metadata', () => {
    const overflow = {
      ref: `loop-snapshot://omitted-overflow/${HASH_C}`,
      hash: HASH_C,
      count: 1,
    } as const;
    const result = buildLoopImplementationScope({
  ...
  it('preserves a root-level Git enumeration omission as bounded partial evidence', () => {
    ...
    expect(parseLoopImplementationScopeBundle(bundle)).toEqual(bundle);
  });
  ...
  it('binds Git selection evidence into the snapshot projection and scope hashes', () => {
    ...
    const first = buildLoopImplementationScopeBundle({
      ...authority,
      current: current(HASH_B),
    });
    const second = buildLoopImplementationScopeBundle({
      ...authority,
    ...
    expect(parseLoopImplementationScopeBundle(first)).toEqual(first);
  });
  ...
  it('never compacts away a root-level physical selection blocker', () => {
    ...
    const rootSelectionBlocker: LoopSnapshotOmission = {
      path: '.',
      size: null,
      type: 'directory',
      reason: 'physical-selection-changed',
  ...
  it('keeps a complete Git scope stable when only staging state changes', () => {
    ...
    const beforeStaging = buildLoopImplementationScopeBundle({
      ...authority,
      current: unchangedWorkingTree,
    });
    const afterStaging = buildLoopImplementationScopeBundle({
      ...authority,
  ...
  it('summarizes oversized change details into a stable bounded unresolved scope', () => {
    ...
    const largeManifest = (values: LoopSnapshotEntry[]): LoopContentSnapshotManifest => ({
      ...manifest(),
      limits: {
        maxFiles: 1_000,
        maxFileBytes: 1_000,
    ...
    const first = buildLoopImplementationScopeBundle(input);
    const second = buildLoopImplementationScopeBundle(input);
  ...
  it('bounds long omission details by serialized bytes and folds the remainder stably', () => {
    ...
    const largeOmissionManifest: LoopContentSnapshotManifest = {
      ...manifest(),
      complete: false,
      limits: {
        maxFiles: 10_000,
  ...
  it('binds the content hashes of entries folded into projection overflow', () => {
    ...
    const oversizedManifest = (values: LoopSnapshotEntry[]): LoopContentSnapshotManifest => ({
      ...manifest(),
      limits: {
        maxFiles: 10_000,
        maxFileBytes: 1_000,
  ...
  it('creates stable unresolved IDs for every omission, incompleteness, and overflow', () => {
    const omission: LoopSnapshotOmission = {
      path: 'large.bin',
      size: 2_000,
      type: 'file',
      reason: 'file-size',
    ...
    const first = buildLoopImplementationScope(input);
    const second = buildLoopImplementationScope({
      ...input,
      baseline: manifest({
  ...
  it('requires a non-empty reason before a genuinely unchanged scope is complete', () => {
    ...
    const missing = buildLoopImplementationScope({
      baseline: unchanged,
    ...
    const explained = buildLoopImplementationScope({
      baseline: unchanged,
      current: { ...unchanged, createdAt: '2026-07-18T00:00:00.000Z' },
      contractHash: HASH_B,
      declaredArtifacts: [],
  ...
  it('does not let a self-rehashed persisted document erase unresolved implementation work', () => {
    const original = buildLoopImplementationScope({
      baseline: manifest(),
      current: manifest({ entries: [entry('src/changed.ts', HASH_A)] }),
      contractHash: HASH_B,
      declaredArtifacts: [],
    ...
    expect(() => parseLoopImplementationScope(forged)).toThrow(
  ...
  it('rebuilds snapshot omissions instead of trusting a self-rehashed complete scope', () => {
    ...
    expect(() => parseLoopImplementationScope(forged.scope)).not.toThrow();
    expect(() => parseLoopImplementationScopeBundle(forged)).toThrow(
  ...
  it('is invariant to timestamps and input array order', () => {
    ...
    const firstBundle = buildLoopImplementationScopeBundle({
      baseline: manifest({
    ...
    const reorderedBundle = buildLoopImplementationScopeBundle({
      baseline: manifest({
        createdAt: '2030-01-01T00:00:00.000Z',
  ...
  it('changes the content address when content, contract, or ownership changes', () => {
    ...
    const original = buildLoopImplementationScope(baseInput);
    const contentChanged = buildLoopImplementationScope({
      ...baseInput,
      current: manifest({ entries: [entry('src/a.ts', HASH_C)] }),
    });
    const contractChanged = buildLoopImplementationScope({
      ...baseInput,
      contractHash: HASH_C,
    });
    const ownershipChanged = buildLoopImplementationScope({
      ...baseInput,
      declaredArtifacts: [{ path: 'src', kind: 'directory' }],
    });
  ...
  it('keeps Git paths advisory and never uses them to decide completeness', () => {
    const withoutGit = buildLoopImplementationScope({
      baseline: manifest(),
      current: manifest(),
      contractHash: HASH_B,
      declaredArtifacts: [],
    ...
    const withGit = buildLoopImplementationScope({
      baseline: manifest(),
      current: manifest(),
      contractHash: HASH_B,
      declaredArtifacts: [],
  ...
  it('covers persisted scope parser validation branches', () => {
    const valid = buildLoopImplementationScope({
    ...
    for (const [label, value, message] of cases) {
      expect(() => parseLoopImplementationScope(value), label).toThrow(message);
    }
  });
});

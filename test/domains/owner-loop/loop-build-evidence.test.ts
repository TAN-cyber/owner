...
import {
  inspectLoopBuildEvidence,
  persistLoopBuildEvidence,
  prepareLoopBuildEvidence,
} from '../../../domains/owner-loop/loop-build-evidence.js';
import { executeLoopCheckReceipt } from '../../../domains/owner-loop/loop-check-receipt.js';
import {
  createLoopChange,
  loopChangeDir,
  writeLoopChange,
...
const snapshotMock = vi.hoisted(() => ({
  next: null as LoopContentSnapshotManifest | null,
  filtered: null as LoopContentSnapshotManifest | null,
}));

vi.mock('../../../domains/owner-loop/loop-snapshot.js', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('../../../domains/owner-loop/loop-snapshot.js')>();
  return {
    ...actual,
    createLoopCurrentContentSnapshot: async (
      ...args: Parameters<typeof actual.createLoopCurrentContentSnapshot>
    ) => {
      ...
      return actual.createLoopCurrentContentSnapshot(...args);
    },
    filterLoopContentSnapshotToProjectScope: async (
      ...args: Parameters<typeof actual.filterLoopContentSnapshotToProjectScope>
    ) => {
      ...
      return actual.filterLoopContentSnapshotToProjectScope(...args);
    },
  };
});
...
function changedGitSelectionSnapshot(
  origin: LoopContentSnapshotManifest['origin'] = 'explicit',
): LoopContentSnapshotManifest {
  return {
...
describe('Loop Build evidence preparation', () => {
  ...
  beforeEach(async () => {
    snapshotMock.next = null;
    snapshotMock.filtered = null;
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-build-evidence-'));
  ...
  it('derives a complete content-addressed scope from declared project artifacts', async () => {
    ...
    const result = await prepareLoopBuildEvidence({
      paths,
      state,
    ...
    await expect(readLoopImplementationScope(paths, state.name, result.scopeRef)).resolves.toEqual(
      result.bundle.scope,
    );
  });

  it('automatically classifies proven fast-forward target-branch drift as external', async () => {
    const gitRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-external-drift-'));
    try {
      ...
      await execFileAsync('git', ['config', 'user.name', 'Owner Test'], { cwd: gitRoot });
      await execFileAsync('git', ['config', 'user.email', 'owner@example.test'], { cwd: gitRoot });
      await execFileAsync('git', ['add', 'src/baseline.ts'], { cwd: gitRoot });
      await execFileAsync('git', ['commit', '-m', 'baseline'], { cwd: gitRoot });

      const gitPaths = await loopProjectPaths(gitRoot, '.');
      const created = await createLoopChange({
        paths: gitPaths,
        verificationProtocol: 'legacy-v1',
        name: 'external-drift',
      ...
      await fs.writeFile(path.join(loopChangeDir(gitPaths, created.name), 'brief.md'), brief);
      ...
      const result = await inspectLoopBuildEvidence({
        paths: gitPaths,
        state: { ...created, phase: 'build', approval: 'implicit' },
        artifactRefs: ['src/feature.ts'],
      });
      ...
      await expect(
        readLoopImplementationScopeBundle(gitPaths, created.name, result.scopeRef),
      ).resolves.toMatchObject({
        authority: { externalDrift: { paths: ['src/parallel.ts'] } },
        scope: { externalDrift: { paths: ['src/parallel.ts'] } },
      });
      const checked = await executeLoopCheckReceipt({
        paths: gitPaths,
        state: {
          ...created,
          phase: 'verify',
      ...
      await expect(
        readLoopImplementationScope(gitPaths, created.name, result.scopeRef),
  ...
  it('persists deterministic partial scope IDs and only allows their exact confirmed set', async () => {
    ...
    const partial = await prepareLoopBuildEvidence({
      paths,
      state,
      artifactRefs: ['src/feature.ts'],
    });
  ...
  it('preserves a Git baseline removal after the deleted path becomes ignored', async () => {
    ...
    await writeLoopChange(paths, gitState);
  ...
  it.each(['.env.local', '.npmrc', 'owner/runtime/run-state.json', 'missing.ts'])(
  ...
  it('rejects a complete oversized baseline when its 1 MiB evidence projection becomes partial', async () => {
    ...
    const error = await inspectLoopBuildEvidence({
      paths,
      state,
      artifactRefs: [],
      allowPartialScopeHash: 'f'.repeat(64),
  ...
  it('returns and persists bounded partial evidence for a large unrelated tree', async () => {
    ...
    const preparation = await inspectLoopBuildEvidence({
      paths,
      state,
      artifactRefs: [],
    });
    ...
    await expect(persistLoopBuildEvidence({ paths, state, preparation })).resolves.toBeUndefined();
    await expect(
      readLoopImplementationScope(paths, state.name, preparation.scopeRef),

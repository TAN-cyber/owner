...
} from '../../../domains/owner-loop/loop-portable-acceptance.js';

describe('Loop portable acceptance', () => {
  it('assigns readable sequential IDs without content hashes', () => {
    const acceptance = buildLoopPortableAcceptance({
...
`,
    ...
    expect(() => assertLoopPortableAcceptanceIds(acceptance)).not.toThrow();
  });

  it('rejects empty and duplicate acceptance definitions', () => {
    expect(() =>
      buildLoopPortableAcceptance({ briefMarkdown: '# Outcome\nNothing yet.\n' }),
...
`,
  ...
  it('compares confirmed acceptance by source and full text', () => {
    ...
    expect(sameLoopPortableAcceptance(first, [...first])).toBe(true);
    expect(sameLoopPortableAcceptance(first, [first[1], first[0]])).toBe(false);
    expect(sameLoopPortableAcceptance(first, [{ ...first[0], text: 'Changed.' }, first[1]])).toBe(
      false,
    );
  });

  it('requires a contiguous A1..An sequence', () => {
    expect(() => assertLoopPortableAcceptanceIds([{ id: 'A1' }, { id: 'A3' }])).toThrow(
      'contiguous',
    );
  });

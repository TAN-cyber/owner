...
import {
  ...
  inspectLoopChildren,
  parseLoopChildrenContract,
} from '../../../domains/owner-loop/loop-children.js';
import {
...
import { loopNewCommand } from '../../../domains/owner-loop/loop-new-command.js';
import { loopNextCommand } from '../../../domains/owner-loop/loop-next-command.js';
...
import { inspectLoopPortableStatus } from '../../../domains/owner-loop/loop-portable-status.js';
import { loopShowCommand } from '../../../domains/owner-loop/loop-show-command.js';
import type {
...
const CHILDREN = `schema: owner.loop.children.v1
children:
  - name: child-a
    depends_on: []
    covers: [A1]
...
const REORDERED_CHILDREN = `schema: owner.loop.children.v1
children:
  - name: child-a
    depends_on: []
    covers: [A1]
...
async function verifyChild(options: {
  ...
  const executed = await executeLoopPortableCheckPlan({ paths, name, plans: [] });
  const dispatched = await dispatchLoopPortableVerifier({
    paths,
    name,
...
async function writeMergedChildProjection(options: {
  ...
  await writeLoopPortableState(
    path.join(directory, 'owner-state.yaml'),
    {
      ...options.source,
      name: options.name,
      workspace: {
        isolation: 'worktree',
        change_branch: `owner/${options.name}`,
...
describe('Loop parent and child changes', () => {
...
`,
    ...
    expect(hashLoopParentContract({ acceptance, children: parsed })).toBe(
      hashLoopParentContract({ acceptance, children: reformatted }),
    );
    expect(
      hashLoopParentContract({
        acceptance,
    ...
    ).not.toBe(hashLoopParentContract({ acceptance, children: parsed }));
  ...
  it('gates the parent on real child merges and starts dependents from the integrated HEAD', async () => {
    const repository = await fs.mkdtemp(
      path.join(await fs.realpath(os.tmpdir()), 'owner-loop-children-'),
    );
    repositories.push(repository);
    git(repository, ['init', '-b', 'integration']);
    git(repository, ['config', 'user.email', 'loop@example.test']);
    git(repository, ['config', 'user.name', 'Loop Test']);
    ...
    await fs.writeFile(
      path.join(repository, 'openspec', 'changes', 'child-a', '.owner.yaml'),
      'phase: build\n',
    );
    git(repository, ['add', '.']);
    git(repository, ['commit', '-m', 'seed parent integration branch']);

    const parentCreated = await loopNewCommand(['parent'], repository);
    expect(parentCreated.exitCode).toBe(0);
    const parentPaths = await loopProjectPaths(repository, 'docs');
    await ensureLoopDirectories(parentPaths);
    const parentDir = loopPortableChangeDir(parentPaths, 'parent');
    ...
    const parentConfirmed = await loopNextCommand(
    ...
    const initialChildren = await inspectLoopChildren({ paths: parentPaths, state: parentState });
    ...
    await expect(
      inspectLoopPortableStatus({ paths: parentPaths, name: 'parent' }),
    ...
    const gated = await loopNextCommand(
    ...
    await expect(loopShowCommand(['parent'], repository)).resolves.toMatchObject({
      data: {
        continuation: { action: 'advance-children', runnerAction: { kind: 'none' } },
      },
    });
    await expect(loopDoctorCommand(['parent', '--repair'], repository)).resolves.toMatchObject({
      data: {
        continuation: { action: 'advance-children', runnerAction: { kind: 'none' } },
      },
    });
    const parentRunner = createLoopRunnerChannel();
    await expect(
      submitLoopPortableBuilderCandidate({
        paths: parentPaths,
        name: 'parent',
    ...
    git(repository, ['add', 'docs/owner/changes/parent']);
    git(repository, ['commit', '-m', 'confirm parent child plan']);
    expect(git(repository, ['status', '--short'])).toBe('');

    const earlyDependentCreated = await loopNewCommand(
    ...
    await expect(
      inspectLoopChildren({
        paths: parentPaths,
        state: await readLoopPortableState(path.join(parentDir, 'owner-state.yaml')),
      }),
    ).resolves.toMatchObject({
    ...
    const childCreated = await loopNewCommand(
    ...
    const childPaths = await loopProjectPaths(childRoot!, childConfig!.loop.artifact_root);
    const childDir = loopPortableChangeDir(childPaths, 'child-a');
    await fs.writeFile(path.join(childDir, 'brief.md'), CHILD_BRIEF);
    const childShaped = await confirmLoopPortableShape({ paths: childPaths, name: 'child-a' });
    ...
    const archivePreview = await loopArchiveCommand(
    ...
    const archived = await loopArchiveCommand(['child-a', '--confirmed'], childRoot!);
    ...
    const parentAfterMerge = await readLoopPortableState(path.join(parentDir, 'owner-state.yaml'));
    const afterA = await inspectLoopChildren({ paths: parentPaths, state: parentAfterMerge });
    ...
    git(repository, ['branch', '-D', 'owner/child-b']);
    await expect(
      inspectLoopChildren({ paths: parentPaths, state: parentAfterMerge }),
    ).resolves.toMatchObject({
      children: expect.arrayContaining([
    ...
    const dependentCreated = await loopNewCommand(
      ['child-b', '--isolation', 'worktree', '--target-branch', 'integration'],
      repository,
    );
    ...
    const archivedA = await readLoopPortableState(path.join(archivedDirectory, 'owner-state.yaml'));
    ...
    const uncommittedProjection = await inspectLoopChildren({
      paths: parentPaths,
      state: parentAfterMerge,
    });
    expect(uncommittedProjection?.allDone).toBe(false);
    git(repository, ['add', 'docs/owner/archive']);
    git(repository, ['commit', '-m', 'record merged child projections']);
    const allDone = await inspectLoopChildren({ paths: parentPaths, state: parentAfterMerge });
    expect(allDone?.allDone).toBe(true);

    const integrated = await loopNextCommand(
    ...
    const parentChecks = await executeLoopPortableCheckPlan({
      paths: parentPaths,
      name: 'parent',
      plans: [],
    });
    const parentDispatched = await dispatchLoopPortableVerifier({
      paths: parentPaths,
      name: 'parent',
      checks: parentChecks.checks,
    });
    const verifier = createLoopRunnerChannel();
    const failed = await submitLoopPortableVerifierResult({
      paths: parentPaths,
      name: 'parent',
      checks: parentChecks.checks,
      maxVerifyFailures: 3,
    ...
    const repairRequired = await loopNextCommand(
    ...
    await expect(
      ...
      continuation: {
        ...
        commandArgs: ['owner', 'loop', 'next', 'parent', '--summary', '<summary>'],
        requiredInputs: ['summary'],
        runnerAction: { kind: 'none' },
      },
    });
    const bypassAttempt = await loopNextCommand(
...
`,
    ...
    const replanned = await loopNextCommand(

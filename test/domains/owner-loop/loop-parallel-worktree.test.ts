...
interface LoopProcessResult extends LoopEnvelope {
  stdout: string;
  stderr: string;
  timedOut: boolean;
}

const LOOP_SCRIPTS = path.resolve('assets/skills/owner-loop/scripts');
const COMMAND_TIMEOUT_MS = 20_000;
const CHANGE_NAMES = ['parallel-alpha', 'parallel-beta'] as const;
const BUSINESS_SOURCE = 'src/business.ts';
const BRIEF = `# Outcome
Keep Loop parallel worktrees recoverable.
# Scope
...
# Decisions
Use process-level Loop CLI commands.
# Open questions
None.
# Verification expectations
Run the deterministic parallel-worktree regression test.
`;
const SPEC = `# loop parallel worktree probe

## Requirements

### Requirement: Keep independent worktrees usable

Loop parallel worktrees MUST progress without crossing Runtime boundaries.
...
function commandScript(command: string): string {
  return path.join(LOOP_SCRIPTS, `owner-loop-${command}.mjs`);
}
...
function runLoopCommand(
  ...
  return new Promise((resolve) => {
    ...
    child.once('close', (code) => {
      ...
      try {
        envelope = JSON.parse(stdout) as LoopEnvelope;
      } catch {
...
function assertCompleted(result: LoopProcessResult, label: string): void {
  expect(result.timedOut, `${label} timed out\n${result.stderr}`).toBe(false);
  expect(result.exitCode, `${label}\n${result.stdout}\n${result.stderr}`).toBe(0);
}

async function submitBuilderHandoff(name: string, projectRoot: string): Promise<LoopProcessResult> {
  ...
  try {
    return await runLoopCommand('next', [name, '--runner-input', input], projectRoot);
  } finally {
...
describe('Loop parallel linked-worktree Runtime', () => {
  ...
  it('keeps concurrent linked worktrees usable after a manual business-source edit', async () => {
    const repository = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-parallel-'));
    repositories.push(repository);
    execFileSync('git', ['init'], { cwd: repository, stdio: 'ignore' });
    execFileSync('git', ['config', 'user.email', 'loop@example.test'], {
      cwd: repository,
      stdio: 'ignore',
    });
    ...
    const initialized = await runLoopCommand(
      'init',
      ['--root', 'docs', '--language', 'en'],
      repository,
    );
    assertCompleted(initialized, 'initialize Loop fixture');
    ...
    execFileSync('git', ['add', '.owner/config.yaml', BUSINESS_SOURCE], {
      cwd: repository,
      stdio: 'ignore',
    });
    execFileSync('git', ['commit', '-m', 'seed Loop parallel fixture'], {
      cwd: repository,
      stdio: 'ignore',
    });
    ...
    await Promise.all(
      worktrees.map(async ({ name, root }) => {
        const changeDir = path.join(root, 'docs', 'owner', 'changes', name);
        ...
        const shaped = await runLoopCommand(
          'next',
    ...
    const guard = await runLoopCommand('hook-guard', [], editedWorktree.root, COMMAND_TIMEOUT_MS, {
      ...process.env,
      FILE_PATH: BUSINESS_SOURCE,
    });
    ...
    const [editedStatus, untouchedStatus] = await Promise.all([
      runLoopCommand('status', [editedWorktree.name], editedWorktree.root),
      runLoopCommand('status', [untouchedWorktree.name], untouchedWorktree.root),
    ]);
    ...
    for (const { root } of worktrees) {
      const locks = path.join(root, '.owner', 'runtime', 'loop', 'locks');
      ...
      expect(
        await readDirectoryOrEmpty(path.join(root, '.owner', 'runtime', 'loop', 'transactions')),
      ).toEqual([]);
      expect(
        execFileSync('git', ['branch', '--show-current'], { cwd: root, encoding: 'utf8' }).trim(),
      ).toBe(`owner/${path.basename(root)}`);
      const selection = JSON.parse(
        await fs.readFile(path.join(root, '.owner', 'current-change.json'), 'utf8'),
      ) as { workflow: string; change: string };
      expect(selection).toMatchObject({ workflow: 'loop', change: path.basename(root) });
    }
    await expect(
      fs.access(path.join(repository, 'docs', 'owner', 'changes', 'parallel-alpha')),
    ).rejects.toMatchObject({ code: 'ENOENT' });
    await expect(
      fs.access(path.join(repository, 'docs', 'owner', 'changes', 'parallel-beta')),
    ).rejects.toMatchObject({ code: 'ENOENT' });
  }, 120_000);

  it('rejects the retired checkpoint bundle on a linked-worktree-capable host', async () => {
    const repository = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-retired-command-'));
    repositories.push(repository);
    execFileSync('git', ['init'], { cwd: repository, stdio: 'ignore' });
    const result = await runLoopCommand('runtime', ['checkpoint', 'legacy-change'], repository);

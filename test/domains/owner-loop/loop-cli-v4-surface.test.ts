...
import { loopLocalExecutionFile } from '../../../domains/owner-loop/loop-portable-runtime.js';
...
describe('Loop v4 public CLI surface', () => {
  let projectRoot: string;
  let runnerInputSequence: number;
  const projectArgs = () => ['--project-root', projectRoot] as const;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(await fs.realpath(os.tmpdir()), 'owner-loop-v4-cli-'));
  ...
  async function runnerStep(name: string, input: unknown): Promise<JsonEnvelope> {
    ...
    if (
      ...
      const current = json(await runLoopCli(['show', name, '--json', ...projectArgs()]));
      const state = current.data?.state as {
        state_version: number;
        loop: { iteration: number; attempt: number };
      };
      const local = await readLoopLocalExecution(
        loopLocalExecutionFile(await loopProjectPaths(projectRoot, 'docs'), name),
      );
      payload = {
    ...
    const file = path.join(projectRoot, `.loop-runner-input-${runnerInputSequence}.json`);
    await fs.writeFile(file, JSON.stringify(payload));
    try {
      return json(
        await runLoopCli(['next', name, '--runner-input', file, '--json', ...projectArgs()]),
      );
    } finally {
  ...
  async function prepareBuild(name: string, acceptance: string[] = ['First behavior works.']) {
    await runLoopCli(['new', name, ...projectArgs()]);
    const brief = `# Outcome
...
`;
    await fs.writeFile(path.join(projectRoot, 'docs', 'owner', 'changes', name, 'brief.md'), brief);
  ...
  it('documents stable-boundary and honestly labeled Skill coordination operations', async () => {
    const root = await runLoopCli(['--help']);
    const next = await runLoopCli(['next', '--help']);
    const archive = await runLoopCli(['archive', '--help']);
    const status = await runLoopCli(['status', '--help']);
    ...
    const retiredSpec = json(await runLoopCli(['spec', 'rebase', '--help', '--json']));
  ...
  it('returns v4 continuations and rejects retired Agent-authored verification inputs', async () => {
    const created = json(await runLoopCli(['new', 'surface-test', '--json', ...projectArgs()]));
    ...
    for (const command of ['show', 'status', 'doctor']) {
      const result = json(await runLoopCli([command, 'surface-test', '--json', ...projectArgs()]));
    ...
    const archive = json(
      await runLoopCli(['archive', 'surface-test', '--dry-run', '--json', ...projectArgs()]),
    );
    ...
    for (const args of [
      ...
      const result = json(await runLoopCli([...args, '--json', ...projectArgs()]));
    ...
    const projectRepair = json(
      await runLoopCli(['doctor', '--repair', '--json', ...projectArgs()]),
    );
    expect(projectRepair).toMatchObject({
      exitCode: 0,
      data: { healthy: true, workflow: 'loop-portable', repaired: true },
    });

    for (const command of ['checkpoint', 'check', 'evidence', 'receipt']) {
      const result = json(await runLoopCli([command, '--json', ...projectArgs()]));
      expect(result).toMatchObject({
        command,
        exitCode: 64,
        error: { code: 'usage', message: `Unknown Loop command: ${command}` },
      });
    }
  });
  ...
  it('rejects caller-supplied identity, provider, execution, and candidate bindings', async () => {
    ...
    expect(
      json(await runLoopCli(['status', 'reject-forged-runner-fields', '--json', ...projectArgs()]))
        .data,
  ...
  it('drives a complete skill-coordinated CLI loop to Archive with an explicit empty check plan', async () => {
    ...
    expect(built).toMatchObject({
      ...
      data: {
        ...
        continuation: {
          ...
          commandArgs: ['owner', 'loop', 'next', name, '--runner-input', '<temporary-json-file>'],
        },
      },
    });
    ...
    const pendingReport = await fs.readFile(
      path.join(projectRoot, 'docs', 'owner', 'changes', name, 'verification.md'),
      'utf8',
    );
  ...
  it('rejects delayed generic Verifier errors and unavailable messages from an older attempt', async () => {
    ...
    const before = json(await runLoopCli(['show', name, '--json', ...projectArgs()]));
    ...
    const after = json(await runLoopCli(['show', name, '--json', ...projectArgs()]));
  ...
  it('rejects verifier-unavailable while a resolved Runtime check failed', async () => {
    ...
    expect(json(await runLoopCli(['show', name, '--json', ...projectArgs()])).data).toMatchObject({
  ...
  it('executes Verifier-requested checks and resumes the same programmatic attempt', async () => {
    ...
    const changeDir = path.join(projectRoot, 'docs', 'owner', 'changes', name);
    const portableYaml = await fs.readFile(path.join(changeDir, 'owner-state.yaml'), 'utf8');
  ...
  it('resolves a semantic Verifier blocker with a new attempt and retained checks', async () => {
    ...
    const localFile = loopLocalExecutionFile(paths, name);
    expect(await readLoopLocalExecution(localFile)).toMatchObject({

...
import { readLoopBoundedTextFile } from '../../../domains/owner-loop/loop-bounded-file.js';

describe('Loop bounded artifact reader', () => {
  let root: string;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-artifact-'));
  });
  ...
  it('rejects a partial Windows identity when the available inode differs', async () => {
    ...
    await expect(readLoopBoundedTextFile({ root, ref: 'report.md' })).rejects.toThrow(
  ...
  it('returns only a normalized ref, bounded content identity, and UTF-8 text', async () => {
    ...
    await expect(readLoopBoundedTextFile({ root, ref: 'specs/auth/spec.md' })).resolves.toEqual({
  ...
  it.each(['../secret', '.env', '.git/config', 'runtime/evidence.json', 'cache/.pytest_cache/x'])(
    ...
    async (ref) => {
      await expect(readLoopBoundedTextFile({ root, ref })).rejects.toThrow(
  ...
  it('rejects size growth and path replacement while a file is open', async () => {
    ...
    await expect(readLoopBoundedTextFile({ root, ref: 'report.md', maxBytes: 2 })).rejects.toThrow(
      'exceeds',
    );

    await expect(
      readLoopBoundedTextFile({
        root,
        ref: 'report.md',
        hooks: {
          afterOpen: async () => {
  ...
  it('rejects a symlink or junction in the parent chain', async () => {
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-artifact-outside-'));
    try {
      ...
      await expect(readLoopBoundedTextFile({ root, ref: 'linked/brief.md' })).rejects.toThrow(

...
import {
  ...
  takeMany,
  takeOption,
} from '../../../domains/owner-loop/loop-cli-shared.js';
import {
  LoopArchivePreflightError,
  LoopSpecConflictError,
...
import { LoopVerificationReceiptBindingError } from '../../../domains/owner-loop/loop-verification-runtime.js';

describe('Loop CLI shared helpers', () => {
  ...
  it('reads bounded evidence files and reports non-regular and oversized inputs', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-cli-shared-'));
  ...
  it('maps domain errors to stable envelopes and renders JSON or text output', () => {
    ...
    const cases = [
      errorResult('cmd', new LoopUsageError('bad usage')),
      errorResult('cmd', new LoopSpecConflictError('capability', 'expected', 'actual', 'spec.md')),
      errorResult('cmd', new LoopArchivePreflightError(preflight)),
      errorResult('cmd', new LoopChangeRevisionConflictError('change', 1, 2)),
      errorResult(
        'cmd',
        new LoopBaselineIncompleteError('change', 1, { 'file-size': 1 }, ['a'], false),
      ),
      errorResult(
        'cmd',
        new LoopVerificationReceiptBindingError([

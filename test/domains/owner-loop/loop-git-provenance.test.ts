...
import { detectLoopGitExternalDrift } from '../../../domains/owner-loop/loop-git-provenance.js';
import type { LoopSnapshotProjection } from '../../../domains/owner-loop/loop-verification-scope.js';

const projection = (entry: { path: string; gitObjectId: string }): LoopSnapshotProjection => ({
  schema: 'owner.loop.snapshot-projection.v1',
...
describe('Loop Git provenance', () => {
  let projectRoot: string;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-provenance-'));

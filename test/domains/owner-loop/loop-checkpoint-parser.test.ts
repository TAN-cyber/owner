...
import {
  ...
  normalizeLoopCheckpointArtifactRef,
  parseLoopCheckpointManifestValue,
  parseLoopProgressCheckpointValue,
...
describe('Loop checkpoint parser branches', () => {
  it('normalizes safe artifact paths and hashes a canonical manifest', () => {
    expect(normalizeLoopCheckpointArtifactRef('src\\app.ts')).toBe('src/app.ts');
    expect(normalizeLoopCheckpointArtifactRef('src/./app.ts')).toBe('src/app.ts');
    const parsed = parseLoopCheckpointManifestValue(validManifest(), 'checkpoint-change');
    expect(parsed.artifacts[0]?.path).toBe('src/app.ts');
    expect(hashLoopCheckpointManifest(parsed)).toMatch(/^[a-f0-9]{64}$/u);
  });
  ...
  it.each([
    ...
    expect(() => normalizeLoopCheckpointArtifactRef(value)).toThrow(message);
  });
  ...
  it.each([
    ...
    expect(() =>
      parseLoopCheckpointManifestValue(validManifest(patch), 'checkpoint-change'),
    ).toThrow(message);
  });

  it('parses a progress checkpoint and rejects revision, reference, and credential mismatches', () => {
    expect(parseLoopProgressCheckpointValue(validCheckpoint(), 'checkpoint-change')).toMatchObject({
      phase: 'build',
      stateRevision: 2,
      artifactCount: 1,
    });
    ...
    for (const [patch, message] of invalidCases) {
      expect(() =>
        parseLoopProgressCheckpointValue(validCheckpoint(patch), 'checkpoint-change'),
      ).toThrow(message);
    }
  });
});

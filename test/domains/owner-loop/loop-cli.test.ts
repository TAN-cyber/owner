...
import { projectRootFrom } from '../../../domains/owner-loop/loop-cli-shared.js';
...
describe('Owner Loop CLI dispatcher', () => {
  let projectRoot: string;
  const projectArgs = () => ['--project-root', projectRoot] as const;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(await fs.realpath(os.tmpdir()), 'owner-loop-cli-'));
  ...
  async function initializeAndCommit(): Promise<string> {
    expect(await runLoopCli(['init', ...projectArgs()])).toMatchObject({ exitCode: 0 });
    configureGit();
    execFileSync('git', ['add', '-A'], { cwd: projectRoot, stdio: 'ignore' });
    execFileSync('git', ['commit', '-m', 'initialize Loop'], {
      cwd: projectRoot,
      stdio: 'ignore',
    });
  ...
  it('initializes docs as the default Loop artifact root and allowlists only project config', async () => {
    const initialized = json(await runLoopCli(['init', '--json', ...projectArgs()]));
    ...
    await expect(
      fs.stat(path.join(projectRoot, 'docs', 'owner', 'changes')),
    ).resolves.toBeDefined();
    await expect(fs.access(path.join(projectRoot, 'owner'))).rejects.toMatchObject({
      code: 'ENOENT',
    });
    await expect(
    ...
    expect(gitignore).toContain('/.owner/*');
    expect(gitignore).toContain('!/.owner/config.yaml');
    expect(gitignore).not.toContain('!/.owner/runtime');
  });

  it('preserves an existing Loop root and language when init is repeated', async () => {
    await writeProjectConfig(projectRoot, defaultProjectConfig('artifacts/loop', 'zh-CN'));

    const initialized = json(await runLoopCli(['init', '--json', ...projectArgs()]));

    expect(initialized).toMatchObject({
      exitCode: 0,
      data: { artifactRoot: 'artifacts/loop', language: 'zh-CN' },
    });
    const saved = await readProjectConfig(projectRoot);
    expect(saved?.loop).toMatchObject({ artifact_root: 'artifacts/loop', language: 'zh-CN' });
  });
  ...
  it('creates v4 portable artifacts without scanning or snapshotting an unrelated large project file', async () => {
    ...
    const created = json(await runLoopCli(['new', 'portable-change', '--json', ...projectArgs()]));
    ...
    expect((await fs.readdir(changeDir)).sort()).toEqual(['brief.md', 'owner-state.yaml', 'specs']);
    const runtimeDir = loopChangeRuntimeDir(paths, 'portable-change');
    expect(await fs.readdir(runtimeDir)).toEqual(['state.json']);

    const yaml = await fs.readFile(path.join(changeDir, 'owner-state.yaml'), 'utf8');
    expect(yaml).toContain('schema: owner.loop.v4');
  ...
  it('binds one current-workspace change and requires isolation for a second change', async () => {
    const branch = currentBranch();
    const first = json(await runLoopCli(['new', 'first-change', '--json', ...projectArgs()]));
    ...
    expect(
      JSON.parse(
        await fs.readFile(path.join(projectRoot, '.owner', 'current-change.json'), 'utf8'),
      ),
    ).toEqual({
    ...
    const second = json(await runLoopCli(['new', 'second-change', '--json', ...projectArgs()]));
    expect(second).toMatchObject({
      exitCode: 73,
      data: {
        requestedIsolation: 'current',
        activeChanges: ['first-change'],
        requiredAction: 'create-loop-worktree',
      },
  ...
  it('fails closed when a current-workspace change is opened from another branch', async () => {
    ...
    expect(
      json(await runLoopCli(['new', 'current-bound', '--json', ...projectArgs()])),
    ).toMatchObject({
      exitCode: 0,
      data: {
        workspace: {
    ...
    expect(
      json(await runLoopCli(['status', 'current-bound', '--json', ...projectArgs()])),
    ).toMatchObject({
      data: {
  ...
  it('atomically allows only one current-workspace creation when two sessions race', async () => {
    const results = await Promise.all(
      ['race-alpha', 'race-beta'].map(async (name) => ({
        name,
        result: json(await runLoopCli(['new', name, '--json', ...projectArgs()])),
      })),
    );
  ...
  it('persists portable branch ownership and projects branch drift as await-user', async () => {
    ...
    execFileSync('git', ['switch', '-c', 'owner/branch-owned'], {
      cwd: projectRoot,
      stdio: 'ignore',
    });
    ...
    expect(created).toMatchObject({
      ...
      data: {
        workspace: {
          isolation: 'branch',
          change_branch: 'owner/branch-owned',
    ...
    const paths = await loopProjectPaths(projectRoot, 'docs');
    await expect(
      fs.access(path.join(loopChangeRuntimeDir(paths, 'branch-owned'), 'workspace.json')),
    ).rejects.toMatchObject({ code: 'ENOENT' });

    execFileSync('git', ['switch', targetBranch], { cwd: projectRoot, stdio: 'ignore' });
    const status = json(await runLoopCli(['status', 'branch-owned', '--json', ...projectArgs()]));
    expect(status).toMatchObject({
      ...
      data: {
        ...
        workspace: {
          ...
          changeBranch: 'owner/branch-owned',
          targetBranch,
  ...
  it('rechecks an explicit Git binding under the Loop creation lock', async () => {
    const targetBranch = await initializeAndCommit();
    execFileSync('git', ['switch', '-c', 'owner/locked-binding'], {
      cwd: projectRoot,
      stdio: 'ignore',
    });
    const paths = await loopProjectPaths(projectRoot, 'docs');
    const lock = await acquireLoopLock(paths, 'root-move', 'hold new under test');
    let pending: ReturnType<typeof runLoopCli>;
    try {
      pending = runLoopCli([
        'new',
        'locked-binding',
    ...
    await expect(
      fs.access(path.join(projectRoot, 'docs', 'owner', 'changes', 'locked-binding')),
    ).rejects.toMatchObject({ code: 'ENOENT' });
  });
  ...
  it('persists worktree binding and reports drift from the linked worktree', async () => {
    ...
    const secondary = path.join(
      ...
      `owner-loop-cli-worktree-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    try {
      execFileSync('git', ['worktree', 'add', '-b', 'owner/worktree-owned', secondary, 'HEAD'], {
        cwd: projectRoot,
        stdio: 'ignore',
      });
      const created = json(
      ...
      expect(created).toMatchObject({
        ...
        data: {
          workspace: {
            isolation: 'worktree',
            change_branch: 'owner/worktree-owned',
            target_branch: targetBranch,
          },
          preparation: {
            isolation: 'worktree',
      ...
      expect(
        json(await runLoopCli(['status', 'worktree-owned', '--json', '--project-root', secondary]))
          .data,
      ).toMatchObject({
        workspace: {
          projectRoot: path.resolve(secondary),
          ...
          changeBranch: 'owner/worktree-owned',
          targetBranch,
        },
        continuation: { disposition: 'continue' },
      });

      execFileSync('git', ['switch', '-c', 'owner/worktree-drift'], {
        cwd: secondary,
        stdio: 'ignore',
      });
      expect(
        ...
        workspace: { bindingState: 'mismatch', changeBranch: 'owner/worktree-owned' },
        continuation: {
          disposition: 'await-user',
          requiredInputs: ['return-to-bound-workspace'],
        },
  ...
  it('rejects an incomplete root move before preparing an isolated workspace', async () => {
    ...
    expect(
      execFileSync('git', ['branch', '--list', 'owner/blocked-root-move'], {
        cwd: projectRoot,
        encoding: 'utf8',
      }).trim(),
    ).toBe('');
  ...
  it('keeps a linked worktree authoritative when a host passes the primary root', async () => {
    ...
    execFileSync('git', ['add', '.owner/config.yaml'], { cwd: projectRoot, stdio: 'ignore' });
    execFileSync('git', ['commit', '-m', 'seed Loop config'], {
      cwd: projectRoot,
      stdio: 'ignore',
    });
    const secondary = path.join(
      await fs.realpath(os.tmpdir()),
      `owner-loop-cli-root-route-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    try {
      execFileSync('git', ['worktree', 'add', '-b', 'owner/root-route', secondary, 'HEAD'], {
        cwd: projectRoot,
        stdio: 'ignore',
      });
      ...
      try {
        ...
        expect(
          json(await runLoopCli(['new', 'root-routed', '--json', '--project-root', projectRoot])),
      ...
      await expect(
        fs.access(path.join(loopChangeRuntimeDir(secondaryPaths, 'root-routed'), 'state.json')),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(
          path.join(secondary, 'docs', 'owner', 'changes', 'root-routed', 'owner-state.yaml'),
        ),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(projectRoot, 'docs', 'owner', 'changes', 'root-routed')),
      ).rejects.toMatchObject({ code: 'ENOENT' });
    } finally {
      execFileSync('git', ['worktree', 'remove', '--force', secondary], {
        cwd: projectRoot,
  ...
  it('runs a custom-root change through every CLI-reachable stable boundary', async () => {
    ...
    const created = json(
      await runLoopCli(['new', 'sentence-counting', '--json', ...projectArgs()]),
    );
    expect(created).toMatchObject({
      exitCode: 0,
      data: {
    ...
    const paths = await loopProjectPaths(projectRoot, 'artifacts/loop');
    ...
    expect(json(await runLoopCli(['status', '--json', ...projectArgs()])).data).toMatchObject({
    ...
    expect(
      json(await runLoopCli(['show', 'sentence-counting', '--json', ...projectArgs()])).data,
    ).toMatchObject({
      state: { schema: 'owner.loop.v4', language: 'zh-CN', phase: 'shape' },
      brief,
    ...
    const status = json(
      await runLoopCli(['status', 'sentence-counting', '--details', '--json', ...projectArgs()]),
    );
    ...
    expect((await fs.readdir(loopChangeRuntimeDir(paths, 'sentence-counting'))).sort()).toEqual([
      'state.json',
    ]);
  });

  it('returns every A1..An acceptance item directly instead of paginating derived IDs', async () => {
    await runLoopCli(['new', 'complete-acceptance', ...projectArgs()]);
    const paths = await loopProjectPaths(projectRoot, 'docs');
    ...
    const status = json(
      await runLoopCli(['status', 'complete-acceptance', '--details', '--json', ...projectArgs()]),
    );
  ...
  it('creates default config from new and keeps Pipeline paths untouched', async () => {
    const result = json(await runLoopCli(['new', 'default-root', '--json', ...projectArgs()]));
    expect(result).toMatchObject({
      exitCode: 0,
      data: { schema: 'owner.loop.v4', name: 'default-root', phase: 'shape' },
    });
    expect(await fs.readFile(path.join(projectRoot, '.owner', 'config.yaml'), 'utf8')).toContain(
      'artifact_root: docs',
    );
    await expect(
      fs.stat(path.join(projectRoot, 'docs', 'owner', 'changes', 'default-root')),
    ).resolves.toBeDefined();
    await expect(fs.access(path.join(projectRoot, 'owner'))).rejects.toMatchObject({
      code: 'ENOENT',
    });
  ...
  it('uses stable usage, data, and conflict exit codes and retires the old public commands', async () => {
    const usage = await runLoopCli(['unknown', '--json', ...projectArgs()]);
    ...
    const help = await runLoopCli(['--help', ...projectArgs()]);
    expect(help.stdout).toContain('next <change-name>');
    const nextHelp = await runLoopCli(['next', '--help', ...projectArgs()]);
    ...
    const specHelp = await runLoopCli(['spec', 'remove', '--help', ...projectArgs()]);
    expect(specHelp.stdout).toContain('spec remove <change-name> <capability>');
    expect(
      json(await runLoopCli(['spec', 'rebase', '--help', '--json', ...projectArgs()])),
    ).toMatchObject({ exitCode: 64, error: { code: 'usage' } });
    for (const retired of ['checkpoint', 'check', 'evidence', 'receipt']) {
      expect(help.stdout).not.toContain(`\n  ${retired}`);
      expect(json(await runLoopCli([retired, '--json', ...projectArgs()]))).toMatchObject({
        command: retired,
        exitCode: 64,
        error: { code: 'usage', message: `Unknown Loop command: ${retired}` },
      });
    }

    const missing = await runLoopCli(['status', '--json', ...projectArgs()]);
    expect(missing.exitCode).toBe(65);
    expect(json(missing)).toMatchObject({ error: { code: 'invalid-data' } });

    await runLoopCli(['init', '--root', '.', ...projectArgs()]);
    const paths = await loopProjectPaths(projectRoot, '.');
    const lock = await acquireLoopLock(paths, 'root-move', 'archive concurrent-change');
    try {
      const conflict = await runLoopCli(['root', 'move', 'docs', '--json', ...projectArgs()]);
  ...
  it('returns the v2 confirmation continuation when Shape has not been confirmed', async () => {
    await runLoopCli(['new', 'blocked-shape', ...projectArgs()]);
    const result = json(
      await runLoopCli([
        'next',
        'blocked-shape',
    ...
    expect(result).toMatchObject({
      ...
      data: {
        state: { schema: 'owner.loop.v4', phase: 'shape' },
        continuation: {
          schema: 'owner.loop.continuation.v2',
          disposition: 'continue',
          action: 'confirm-shape',
  ...
  it('records explicit Shape confirmation in portable state and advances to Builder handoff', async () => {
    await runLoopCli(['new', 'confirmed-shape', ...projectArgs()]);
    const paths = await loopProjectPaths(projectRoot, 'docs');
  ...
  it('returns Verify to Build only after a trusted Runner submitted the Builder candidate', async () => {
    await runLoopCli(['new', 'return-to-build', ...projectArgs()]);
    const paths = await loopProjectPaths(projectRoot, 'docs');
    ...
    expect(
      json(await runLoopCli(['status', 'return-to-build', '--json', ...projectArgs()])).data,
    ).toMatchObject({
      phase: 'verify',
  ...
  it('keeps the same explicit Shape confirmation boundary in Sequential and Batch modes', async () => {
    await runLoopCli(['init', '--root', 'docs', ...projectArgs()]);
    ...
    await writeProjectConfig(projectRoot, {
      ...initialConfig!,
      loop: { ...initialConfig!.loop, clarification_mode: 'sequential' },
    });
    await runLoopCli(['new', 'mode-boundary', ...projectArgs()]);
    const paths = await loopProjectPaths(projectRoot, 'docs');
    ...
    await writeProjectConfig(projectRoot, {
      ...config!,
      loop: { ...config!.loop, clarification_mode: 'batch' },
    });
    const batch = json(
      await runLoopCli([
        'next',
  ...
  it('records a complete remove intent without a canonical hash', async () => {
    await runLoopCli(['new', 'remove-capability', ...projectArgs()]);
    const paths = await loopProjectPaths(projectRoot, 'docs');
  ...
  it('shows a brief larger than the retired per-file read budget', async () => {
    await runLoopCli(['new', 'large-brief', ...projectArgs()]);
    const paths = await loopProjectPaths(projectRoot, 'docs');
    ...
    const result = json(await runLoopCli(['show', 'large-brief', '--json', ...projectArgs()]));
  ...
  it('shows complete proposed Specs beyond the retired count, aggregate, and output budgets', async () => {
    await runLoopCli(['new', 'large-spec-set', ...projectArgs()]);
    const paths = await loopProjectPaths(projectRoot, 'docs');
    ...
    const result = json(await runLoopCli(['show', 'large-spec-set', '--json', ...projectArgs()]));
  ...
  it('repairs a stale selection without requiring a transaction strategy', async () => {
    await runLoopCli(['init', ...projectArgs()]);
    await fs.writeFile(
      path.join(projectRoot, '.owner', 'current-change.json'),
      JSON.stringify({
        schema: 'owner.selection.v2',
        workflow: 'loop',
    ...
    const repaired = await runLoopCli(['doctor', '--repair', '--json', ...projectArgs()]);
  ...
  it.each([
    ...
    const result = await runLoopCli([...args, '--json', ...projectArgs()]);
  ...
  it.each([
    ...
    async (_command, args, retryCreatesChange) => {
      ...
      const retried = await runLoopCli([...args, '--json', ...projectArgs()]);
  ...
  it('migrates a legacy active change on its first mutating CLI command', async () => {
    ...
    await fs.writeFile(
      path.join(changeDir, 'owner-state.yaml'),
      stringify({
        schema: 'owner.loop.v3',
    ...
    const runtimeDir = loopChangeRuntimeDir(paths, 'legacy-change');
    ...
    expect(
      json(await runLoopCli(['status', 'legacy-change', '--json', ...projectArgs()])),
    ).toMatchObject({
      exitCode: 0,
      data: {
        schema: 'owner.loop.status.v2',
    ...
    expect(await readLoopPortableChange(paths, 'legacy-change')).toMatchObject({
      phase: 'build',
      schema: 'owner.loop.v4',
    });
    await expect(
      fs.access(loopLocalExecutionFile(paths, 'legacy-change')),
    ).resolves.toBeUndefined();
  });
});

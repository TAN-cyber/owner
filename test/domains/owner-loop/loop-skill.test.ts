...
const roots = {
  en: path.resolve('assets', 'skills', 'owner-loop'),
  zh: path.resolve('assets', 'skills-zh', 'owner-loop'),
};
...
describe('Owner Loop Skills', () => {
  it('keeps valid bilingual entry Skills with progressive references', async () => {
    for (const language of ['en', 'zh'] as const) {
      ...
      expect(metadata.name).toBe('owner-loop');
      expect(metadata.description).toContain('Loop');
  ...
  it('bounds permanent Loop context and keeps bilingual file structure aligned', async () => {
    for (const language of ['en', 'zh'] as const) {
  ...
  it('keeps the main Skill on decisions while delegating mechanics to public CLI output', async () => {
    ...
    for (const variant of variants) {
      ...
      expect(skill).not.toContain('owner doctor --repair --scope project');
      expect(skill).not.toContain('scripts/owner-loop-runtime.mjs');
      expect(skill).not.toContain('owner-loop-<cmd>.mjs');
    }
  });
  ...
  it('lets the Loop Skill assess and coordinate large changes without a new mode', async () => {
    const variants = [
      {
        language: 'zh' as const,
        required: [
  ...
  it('keeps Agent-authored formal artifacts separate from Runtime state and reports', async () => {
    for (const language of ['en', 'zh'] as const) {
      ...
      for (const required of [
        '<artifact-root>/owner/changes/<change-name>/',
        'brief.md',
        'children.yaml',
        'owner.loop.children.v1',
      ...
      for (const RuntimeDetail of [
        'owner.loop.workspace.v3',
        'baselineProjectionRef',
        'loop-controller-trust.json',
        'events.jsonl',
        'transition.json',
        '512 KiB',
      ]) {
  ...
  it('uses the command reference for semantic exceptions instead of duplicating CLI syntax', async () => {
    for (const language of ['en', 'zh'] as const) {
      ...
      expect(commands).toContain('owner loop <command> --help');
      expect(commands).toContain('owner loop <group> <command> --help');
      for (const field of [
        'commandArgs',
        'inputOptions',
        'workspace',
      ...
      expect(commands.match(/owner loop/gu)?.length ?? 0).toBeLessThanOrEqual(4);
      ...
      expect(commands).not.toContain('owner loop receipt automated <change-name>');
      expect(commands).not.toContain('owner loop checkpoint <change-name>');
    }
  });
  ...
  it('keeps recovery focused on exceptional safety decisions and exact Runtime actions', async () => {
    ...
    for (const variant of variants) {
      ...
      expect(recovery).not.toContain('owner loop spec rebase <change-name>');
    ...
    const variants = [
      ...
      },
      {
        ...
        required: [
          ...
          'Another active Loop change already exists',
  ...
  it('contains no forbidden wording or external prescriptive workflow dependency', async () => {
    for (const language of ['en', 'zh'] as const) {
      ...
      expect(content).not.toMatch(/owner\s+(state|guard|handoff)\b/iu);

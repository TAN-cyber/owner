...
const fixture = fileURLToPath(
  new URL('../../fixtures/loop-lock-process-worker.ts', import.meta.url),
);
...
describe('Loop lock process concurrency', () => {
  ...
  beforeAll(async () => {
    buildRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-lock-worker-'));
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-lock-process-'));
  ...
  it('prevents an old owner release from removing a replacement owner', async () => {
    const old = await acquireLoopLock(paths, 'archive', 'old archive owner');
    const oldSnapshot = path.join(projectRoot, 'old-lock.json');
    await fs.writeFile(oldSnapshot, JSON.stringify(old));
    await releaseLoopLock(old);
    const replacement = await acquireLoopLock(paths, 'archive', 'replacement archive owner');
    ...
    await releaseLoopLock(replacement);
  });

...
import {
  ...
  writeLoopRunState as writeRunStateAt,
...
import {
  ...
  inspectLoopTransitionJournalValue,
  LOOP_TRANSITION_JOURNAL_MAX_BYTES,
  loopTransitionJournalFile,
} from '../../../domains/owner-loop/loop-transition-journal.js';
import { appendLoopTrajectoryEvent } from '../../../domains/owner-loop/loop-trajectory.js';
import { repairLoopTrajectoryTail } from '../../../domains/owner-loop/loop-trajectory-recovery.js';
import type {
  LoopChangeState,
  LoopProjectPaths,
...
function legacyState(state: LoopChangeState): Record<string, unknown> {
  ...
  return { ...legacy, schema: LOOP_LEGACY_CHANGE_SCHEMA };
}

function legacyTransition(journal: LoopTransitionJournal): Record<string, unknown> {
...
function v2State(state: LoopChangeState): Record<string, unknown> {
...
function v2Transition(journal: LoopTransitionJournal): Record<string, unknown> {
...
describe('Loop transition recovery', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-transition-recovery-'));
    ...
    runtimeDir = loopChangeRuntimeDir(paths, state.name);
  ...
  it('bounds the protected transition journal before JSON parsing', async () => {
    ...
    await fs.writeFile(
      loopTransitionJournalFile(paths, 'recover-transition'),
      'x'.repeat(LOOP_TRANSITION_JOURNAL_MAX_BYTES + 1),
    );

    await expect(inspectPendingLoopTransitionSchema(paths, 'recover-transition')).rejects.toThrow(
      `exceeds ${LOOP_TRANSITION_JOURNAL_MAX_BYTES} bytes`,
    );
  });
  ...
  it.each<{
    ...
    expect(await fs.stat(loopTransitionJournalFile(paths, 'recover-transition'))).toBeDefined();

    const recovered = await continueLoopTransition(paths, 'recover-transition');
    expect(recovered).toMatchObject({ phase: 'build', revision: 2 });
    expect(await readLoopChange(paths, 'recover-transition')).toMatchObject({
      phase: 'build',
      revision: 2,
    });
    ...
    await expect(
      fs.access(loopTransitionJournalFile(paths, 'recover-transition')),
  ...
  it.each(['v1', 'v2', 'v3'] as const)(
    ...
    async (generation) => {
      ...
      const currentJournal = (await inspectPendingLoopTransition(paths, 'recover-transition'))!;
      const stateAtCrash = await readLoopChange(paths, 'recover-transition');
      const changeFile = path.join(changeDir, 'owner-state.yaml');
      const transitionFile = loopTransitionJournalFile(paths, 'recover-transition');
      ...
      if (generation === 'v3') {
        await expect(continueLoopTransition(paths, 'recover-transition')).rejects.toThrow(
          /status must be running/iu,
        );
      } else {
        await expect(migrateLoopChange({ paths, name: 'recover-transition' })).rejects.toThrow(
          /status must be running/iu,
        );
      }
  ...
  it.each(['v1', 'v2', 'v3'] as const)(
    ...
    async (generation) => {
      ...
      const current = (await inspectPendingLoopTransition(paths, 'recover-transition'))!;
      ...
      for (const mutation of mutations) {
        ...
        expect(
          () => inspectLoopTransitionJournalValue(journal, 'recover-transition'),
          mutation.label,
        ).toThrow(/Loop transition journal/iu);
      }
    },
  );
  ...
  it('requires a typed v3 operation and does not let spec-rebase bypass advance semantics', async () => {
    ...
    expect(() => inspectLoopTransitionJournalValue(missing, 'recover-transition')).toThrow(
      /operation is invalid/iu,
    );
    const spoofed = structuredClone(current);
    spoofed.operation = 'spec-rebase';
    expect(() => inspectLoopTransitionJournalValue(spoofed, 'recover-transition')).toThrow(
  ...
  it.each(['v1', 'v2'] as const)(
    ...
    async (generation) => {
      ...
      const state = await readLoopChange(paths, 'recover-transition');
      await fs.writeFile(
      ...
      await migrateLoopChange({ paths, name: 'recover-transition' });
      const migrated = (await inspectPendingLoopTransition(paths, 'recover-transition'))!;
      ...
      await continueLoopTransition(paths, 'recover-transition');
      const run = (await readRunStateAt(runtimeDir))!;
  ...
  it('preserves a pending journal when the same-revision change content diverges', async () => {
    ...
    const transitionFile = loopTransitionJournalFile(paths, 'recover-transition');
    const changeFile = path.join(changeDir, 'owner-state.yaml');
    const current = await readLoopChange(paths, 'recover-transition');
  ...
  it('preserves a pending journal when a different valid Run is present', async () => {
    ...
    const journal = (await inspectPendingLoopTransition(paths, 'recover-transition'))!;
    const transitionFile = loopTransitionJournalFile(paths, 'recover-transition');
    await writeRunStateAt(runtimeDir, { ...journal.nextRun, runId: 'other-valid-run' });
    const [changeBefore, journalBefore] = await Promise.all([
      fs.readFile(path.join(changeDir, 'owner-state.yaml'), 'utf8'),
      fs.readFile(transitionFile, 'utf8'),
    ]);
    ...
    expect(await fs.readFile(path.join(changeDir, 'owner-state.yaml'), 'utf8')).toBe(changeBefore);
    expect(await fs.readFile(transitionFile, 'utf8')).toBe(journalBefore);
  ...
  it('rejects an existing transition event whose full content does not match the journal', async () => {
    ...
    expect(await fs.readFile(transitionFile, 'utf8')).toBe(journalBefore);
    expect(await readRunStateAt(runtimeDir)).toBeNull();
    expect((await readLoopChange(paths, 'recover-transition')).phase).toBe('shape');
  });
  ...
  it.each<{
    ...
    async ({ hooks, prewriteEvents }) => {
      ...
      if (prewriteEvents) {
        const started = await appendLoopTrajectoryEvent({
          changeDir: runtimeDir,
        ...
        const transitioned = await appendLoopTrajectoryEvent({
          changeDir: runtimeDir,
          run: currentJournal.nextRun,
          type: 'state_transitioned',
      ...
      legacyNextRun.skillHash = sha256Text('owner-loop-runtime:v1');
      await fs.writeFile(changeFile, stringify(legacyState(stateAtCrash)));
      await fs.writeFile(transitionFile, JSON.stringify(legacyJournal, null, 2) + '\n');
      await fs.rm(loopBaselineManifestFile(paths, 'recover-transition'), { force: true });
      const [changeBefore, transitionBefore] = await Promise.all([
        fs.readFile(changeFile, 'utf8'),
        fs.readFile(transitionFile, 'utf8'),
      ]);

      expect(await inspectLoopStatus(paths, 'recover-transition')).toMatchObject({
      ...
      ).rejects.toThrow('run owner loop doctor recover-transition --repair before mutating it');
      const inspected = await doctorLoopProject({ paths, name: 'recover-transition' });
      ...
      expect(await readLoopChange(paths, 'recover-transition')).toMatchObject({
        ...
        revision: 2,
        phase: 'build',
        run_id: 'loop-recovery-eval-run',
      });
      expect(await readLoopBaselineManifest(paths, 'recover-transition')).toMatchObject({
      ...
      await doctorLoopProject({
        paths,
        name: 'recover-transition',
        repair: true,
        recoveryStrategy: 'continue',
      });
      expect((await readLoopChange(paths, 'recover-transition')).revision).toBe(2);
  ...
  it('migrates and continues a directly persisted v2 transition exactly once', async () => {
    ...
    await fs.writeFile(
      changeFile,
      stringify(v2State(await readLoopChange(paths, 'recover-transition'))),
    );
    await fs.writeFile(
      transitionFile,
  ...
  it('recovers a v1 transition migration interrupted after writing the v2 journal', async () => {
    ...
    await fs.writeFile(
      changeFile,
      stringify(legacyState(await readLoopChange(paths, 'recover-transition'))),
    );
    await fs.writeFile(
      transitionFile,
    ...
    expect(await inspectPendingLoopSchemaMigration(paths, 'recover-transition')).toMatchObject({
      fromSchema: LOOP_LEGACY_CHANGE_SCHEMA,
      toSchema: LOOP_V2_CHANGE_SCHEMA,
      transition: {
        nextJournal: { schema: LOOP_V2_TRANSITION_SCHEMA },
      },
    });
  ...
  it('supersedes a durable v2 Archive transition through the schema-migration journal', async () => {
    ...
    const status = await inspectLoopStatus(paths, 'recover-transition', { details: true });
  ...
  it.each<{
    ...
    async ({ slug, hook }) => {
      ...
      const pending = (await inspectPendingLoopTransition(paths, 'recover-transition'))!;
      await appendLoopTrajectoryEvent({
        changeDir: runtimeDir,
        run: pending.nextRun,
        type: 'state_transitioned',
      ...
      const recovered = await migrateLoopChange({ paths, name: 'recover-transition' });
      ...
      await expect(fs.access(transitionFile)).rejects.toMatchObject({ code: 'ENOENT' });
      await expect(
        fs.access(loopSchemaMigrationJournalFile(paths, 'recover-transition')),
  ...
  it('serializes concurrent transitions for the same change', async () => {
    ...
    const second = advanceLoopChange({
      paths,
      name: 'recover-transition',
  ...
  it('doctor atomically removes only an incomplete final trajectory line before continuing', async () => {
    ...
    const runFile = loopRuntimeRefFile(runtimeDir, LOOP_RUN_STORAGE.stateRef);
    const transitionFile = loopTransitionJournalFile(paths, 'recover-transition');
    const trajectoryFile = loopRuntimeRefFile(runtimeDir, LOOP_RUN_STORAGE.trajectoryRef);
    ...
    const events = await readTrajectory(runtimeDir, LOOP_RUN_STORAGE.trajectoryRef);
    expect(events.map((event) => event.type)).toEqual(['run_started', 'state_transitioned']);
    expect((await readLoopChange(paths, 'recover-transition')).revision).toBe(2);
  });
  ...
  it('never truncates a malformed middle trajectory line', async () => {
    ...
    ).rejects.toThrow('Loop trajectory is invalid');
    const repaired = await doctorLoopProject({
      paths,
      name: 'recover-transition',
      repair: true,
  ...
  it('refuses to overwrite trajectory events appended after tail inspection', async () => {
    ...
    const appended =
      ...
      '\n';
    await expect(
      repairLoopTrajectoryTail(paths, 'recover-transition', {
  ...
  it('requires explicit doctor takeover before continuing stale transition locks', async () => {
    ...
    expect((await readLoopChange(paths, 'recover-transition')).phase).toBe('build');
    await Promise.all(
  ...
  it('requires pending Verify recovery before archive preview and commit', async () => {
    ...
    await expect(
      inspectLoopArchivePreflight({ paths, name: 'recover-transition', now }),

...
import {
  advanceLoopChange,
  formatLoopReceiptBindingMismatchMessage,
} from '../../../domains/owner-loop/loop-transitions.js';
...
describe('Loop guarded transitions', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-transitions-'));
  ...
  it('uses the concrete change name in receipt binding recovery guidance', () => {
    const message = formatLoopReceiptBindingMismatchMessage({
      change: 'advance-change',
      detail: 'verification.json -> sourceRevision: expected 3, got 2',
    });

    expect(message).toContain('owner loop receipt refresh advance-change --apply');
  ...
  it('does not write Run files when Shape guard fails', async () => {
    ...
    expect((await readLoopChange(paths, 'advance-change')).phase).toBe('shape');
    expect((await readLoopChange(paths, 'advance-change')).spec_changes).toEqual([]);
  ...
  it('blocks Shape before Build when a legacy baseline is incomplete', async () => {
    const baseline = await readLoopBaselineManifest(paths, 'advance-change');
    await writeLoopBaselineManifest(paths, 'advance-change', {
      ...baseline,
      complete: false,
      omitted: [
        {
  ...
  it('advances Shape and Build with Engine state and idempotent evidence', async () => {
    const first = await advanceLoopChange({
      paths,
      name: 'advance-change',
    ...
    expect((await readLoopRunState(runtimeDir))?.currentStep).toBe('build');

    const retry = await advanceLoopChange({
      paths,
      name: 'advance-change',
      evidence: { summary: 'shape done', confirmed: true },
    });
  ...
  it('rebuilds a missing Build Runtime without pretending verification survived', async () => {
    ...
    expect(await readLoopRunState(runtimeDir)).toMatchObject({
      runId: 'rebuilt-runtime-run',
      currentStep: 'build',
    });
    expect(await readLoopBaselineManifest(paths, 'advance-change')).not.toBeNull();
  });
  ...
  it.each(['verify', 'archive'] as const)(
    ...
    async (phase) => {
      ...
      await fs.writeFile(path.join(projectRoot, 'feature.ts'), 'export const feature = true;\n');
      const verifying = await advanceLoopChange({
        paths,
        name: 'advance-change',
        evidence: { summary: 'implemented', artifacts: ['feature.ts'] },
      });
      ...
      expect(blocked).toMatchObject({
        ...
        findings: [
          expect.objectContaining({
            ...
            retryCommand: 'owner loop next advance-change --summary "<summary>" --confirmed',
  ...
  it('requires a legacy implicit Build state to be confirmed before Verify', async () => {
    const shaped = await advanceLoopChange({
      paths,
      name: 'advance-change',
      evidence: { summary: 'shared understanding confirmed', confirmed: true },
      clarificationMode: 'batch',
    });
    await writeLoopChange(paths, { ...shaped.change, approval: 'implicit' });
    expect(await inspectLoopStatus(paths, 'advance-change')).toMatchObject({
      phase: 'build',
      nextCommand:
        'owner loop next advance-change --summary "<summary>" --artifact "<project-relative-path>" --confirmed',
      continuation: {
        command:
          'owner loop next advance-change --summary "<summary>" --artifact "<project-relative-path>" --confirmed',
        commandArgs: [
  ...
  it('records explicit confirmation from Shape and rejects it in Verify', async () => {
    ...
    const verify = await advanceLoopChange({
      paths,
      name: 'advance-change',
      evidence: {
  ...
  it('explicitly returns a fresh Verify change to Build without counting a failure', async () => {
    ...
    const returned = await advanceLoopChange({
      paths,
      name: 'advance-change',
      evidence: {
    ...
    const trajectory = await readLoopTrajectory(runtimeDir, run!.trajectoryRef);
  ...
  it('advances Verify pass to the Loop archive command without reasoning fields', async () => {
    await advanceLoopChange({
      paths,
      name: 'advance-change',
      evidence: { summary: 'shape done', confirmed: true },
    ...
    expect(result.nextCommand).toBe('owner loop archive advance-change --dry-run');
    const run = (await readLoopRunState(runtimeDir))!;
    const source = await fs.readFile(loopRuntimeRefFile(runtimeDir, run.trajectoryRef), 'utf8');

...
vi.mock('../../../domains/owner-loop/loop-verification-receipt-runtime.js', () => ({
  issueLoopManualEvidenceReceipt: mocks.issueManual,
  issueLoopAutomatedCheckReceipt: mocks.issueAutomated,
  MAX_LOOP_AUTOMATED_COMMAND_TIMEOUT_MS: 60 * 60 * 1_000,
}));

vi.mock('../../../domains/owner-loop/loop-receipt-refresh.js', () => ({
  refreshLoopVerificationReceipts: mocks.refresh,
}));
...
import { loopReceiptCommand } from '../../../domains/owner-loop/loop-receipt-command.js';
import { loopSpecCommand } from '../../../domains/owner-loop/loop-spec-command.js';

describe('Loop command modules', () => {
  let projectRoot: string;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-command-modules-'));
  ...
  it('passes repeated manual receipt options to the issuance runtime', async () => {
    ...
    const result = await loopReceiptCommand(
      [
        'manual',
        'demo-change',
        '--acceptance',
    ...
    expect(result).toEqual({
      ...
      exitCode: 0,
      data: { ref: 'manual-ref' },
      text: 'Loop manual receipt: manual-ref\n',
    });
  ...
  it.each([
    ...
    async (status, exitCode) => {
      ...
      expect(result).toMatchObject({
        ...
        text: `Loop automated receipt ${status}: automated-${status}\n`,
      });
  ...
  it('rejects incomplete automated receipt commands and invalid timeouts', async () => {
    await expect(
      loopReceiptCommand(['automated', 'demo-change', '--acceptance', 'acceptance-a'], projectRoot),
  ...
  it('renders every receipt refresh report section and the applied outcome', async () => {
    ...
    const result = await loopReceiptCommand(['refresh', 'demo-change', '--apply'], projectRoot);
    ...
    expect(result.text).toContain('Re-run `owner loop check demo-change`');
  ...
  it('reports a clean refresh and rejects conflicting refresh flags', async () => {
    ...
    await expect(
      loopReceiptCommand(['refresh', 'demo-change', '--dry-run'], projectRoot),
    ).resolves.toMatchObject({
      text: 'No stale receipts found.\n',
    });
    await expect(
      loopReceiptCommand(['refresh', 'demo-change', '--apply', '--dry-run'], projectRoot),
    ).rejects.toThrow('cannot be combined');
    await expect(loopReceiptCommand(['unknown', 'demo-change'], projectRoot)).rejects.toThrow(
      'Unknown receipt command',
    );
  });

  it('rejects retired Loop spec mutation variants', async () => {
    await expect(
      loopSpecCommand(
    ...
    await expect(loopSpecCommand(['unknown', 'demo-change'], projectRoot)).rejects.toThrow(

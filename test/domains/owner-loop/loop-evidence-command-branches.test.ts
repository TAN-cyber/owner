...
vi.mock('../../../domains/owner-loop/loop-cli-shared.js', () => ({
  assertNoArguments: (args: string[]) => {
    if (args.length > 0) throw new Error(`Unexpected argument: ${args[0]}`);
  },
  LoopUsageError: class LoopUsageError extends Error {},
...
describe('Loop evidence command stdin branches', () => {
  afterEach(() => {
  ...
  it('rejects stdin formatting when stdin is interactive', async () => {
    ...
    await expect(loopEvidenceCommand(['format'], 'project')).rejects.toThrow(
  ...
  it('formats a valid non-interactive stdin array and rejects JSON objects', async () => {
    ...
    await expect(loopEvidenceCommand(['format'], 'project')).resolves.toMatchObject({
      command: 'evidence format',
      exitCode: 0,
    });
    await expect(loopEvidenceCommand(['format'], 'project')).rejects.toThrow('JSON array');
  });
});

...
import { archiveLoopPortableChange } from '../../../domains/owner-loop/loop-portable-archive.js';
import {
...
describe('Loop v4 verification-loop regression eval', () => {
  let root: string;
  let paths: LoopProjectPaths;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-v4-regression-'));
...
`,
    ...
    let state = await confirmLoopPortableShape({ paths, name });
    ...
    await expect(archiveLoopPortableChange({ paths, name })).rejects.toThrow('not archive-ready');

    const firstChecks = await executeLoopPortableCheckPlan({
      paths,
      name,
      plans: [outputPlan('candidate-1-check', 0)],
    });
    await dispatchLoopPortableVerifier({
      paths,
      name,
    ...
    state = await readLoopPortableChange(paths, name);
    expect(state).toMatchObject({
      phase: 'verify',
      verification_result: 'pending',
    ...
    await expect(
      fs.readFile(path.join(loopPortableChangeDir(paths, name), 'verification.md'), 'utf8'),
    ...
    const finalChecks = await executeLoopPortableCheckPlan({
      paths,
      name,
      plans: finalPlans,
    });
    ...
    const localBeforeArchive = await readLoopLocalExecution(loopLocalExecutionFile(paths, name));
    ...
    for (const check of localBeforeArchive?.checks ?? []) {
      ...
      const log = path.join(
        path.dirname(loopLocalExecutionFile(paths, name)),
        ...check.log.split('/'),
      );
    ...
    const reusedChecks = await executeLoopPortableCheckPlan({ paths, name, plans: finalPlans });
    ...
    const passed = await submitLoopPortableVerifierResult({
      paths,
      name,
      checks: finalChecks.checks,
      maxVerifyFailures: 5,
    ...
    const confirmed = await confirmLoopPortableSkillCoordinatedPass({ paths, name });
    ...
    await expect(fs.stat(loopPreferredChangeRuntimeDir(paths, name))).rejects.toMatchObject({
      code: 'ENOENT',
    });
    await expectNoLegacyVerificationArtifacts();
  });

...
import {
  inspectLoopRepairHistory,
  loopRepairFailedCheckIdsFromReceipts,
} from '../../../domains/owner-loop/loop-repair-integration.js';
import { LOOP_TRAJECTORY_MAX_TEXT_CHARACTERS } from '../../../domains/owner-loop/loop-trajectory-limits.js';
import type {
  LoopProjectPaths,
  LoopTransitionHooks,
...
import {
  loopVerificationFixtureFailedReceipt,
...
describe('Loop repair stagnation transitions', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-repair-transition-'));
  ...
  async function failVerify(
    ...
    const config = await readProjectConfig(projectRoot);
    const current = await inspectLoopStatus(paths, 'repair-change');
      ...
      try {
        for (const identity of identities) {
          failedReceiptRefs.push(
            (
              await loopVerificationFixtureFailedReceipt({
                paths,
                name: 'repair-change',
  ...
  async function reachManualStop() {
    ...
    await expect(
      inspectLoopStatus(paths, 'repair-change', {
        details: true,
        maxVerifyFailures: 5,
      }),
  ...
  it('derives failed check IDs from non-passing required-check receipts', () => {
    const receipt = buildLoopVerificationReceipt({
      ...
      acceptanceIds: [],
      actor: 'loop-runtime:scoped-text-safety',
    ...
    expect(loopRepairFailedCheckIdsFromReceipts([receipt])).toEqual([loopFailedCheckId(receipt)]);
  });
  ...
  it.skip('persists a third-failure stop and records exactly one explicit override', async () => {
    ...
    await expect(inspectLoopStatus(paths, 'repair-change')).resolves.toMatchObject({
      nextCommand: null,
      repair: { disposition: 'manual-stop', signatureHash: signature },
      continuation: { disposition: 'blocked' },
    });

    const unchanged = await runLoopCli([
      'next',
      'repair-change',
      '--summary',
    ...
    const historyAfterOverride = await inspectLoopRepairHistory(paths, overridden.change);
  ...
  it('rejects oversized trajectory text before state or evidence mutation', async () => {
    ...
    const beforeStatus = await inspectLoopStatus(paths, 'repair-change');
    await expect(
      advanceLoopChange({
        paths,
        name: 'repair-change',
        evidence: { summary: 'x'.repeat(LOOP_TRAJECTORY_MAX_TEXT_CHARACTERS + 1) },
      }),
    ...
    const buildStatus = await inspectLoopStatus(paths, 'repair-change');
    await expect(
      advanceLoopChange({
        paths,
        name: 'repair-change',
        evidence: {
          ...
          noCodeReason: 'x'.repeat(LOOP_TRAJECTORY_MAX_TEXT_CHARACTERS + 1),
        },
      }),
    ).rejects.toThrow('4096 characters');
  ...
  it('recovers an interrupted manual stop exactly once', async () => {
    ...
    const history = await inspectLoopRepairHistory(paths, recovered.change);
  ...
  it.skip('hard-stops at five alternating failures and preserves the count when config increases', async () => {
    ...
    config.loop.max_verify_failures = 6;
    await writeProjectConfig(projectRoot, config);
    const progressed = await runLoopCli([
      'next',
      'repair-change',
      '--summary',

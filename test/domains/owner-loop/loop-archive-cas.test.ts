...
describe('Loop Archive target CAS', () => {
  let projectRoot: string;
  let paths: LoopProjectPaths;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-archive-cas-'));
  ...
  async function preparedChange(
    ...
    const hash = await readyLoopArchivePreflight({ paths, name, now });
  ...
  async function preparedRemoveChange(
    ...
    await prepareLoopArchiveFixture({ paths, name, specChanges });
  ...
  async function archiveCasQuarantines(canonical: string): Promise<string[]> {
    return (await fs.readdir(path.dirname(canonical)))
      .filter((entry) => entry.includes('.owner-archive-cas-'))
  ...
  it('continues after crashing immediately after the original target was quarantined', async () => {
    ...
    expect((await readLoopArchiveTransactionV2(paths, transactionId)).status).toBe('committed');
  ...
  it('resumes rollback after crashing with the transaction write quarantined', async () => {
    ...
    let crashed = false;
    await expect(
      rollbackLoopArchiveTransactionV2(paths, journal, {

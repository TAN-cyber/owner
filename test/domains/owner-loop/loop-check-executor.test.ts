...
import {
  ...
  resolveLoopCheckCwd,
} from '../../../domains/owner-loop/loop-check-executor.js';

describe('Loop check executor', () => {
  let root: string;
  let runtimeDir: string;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-check-'));
    runtimeDir = path.join(root, '.owner', 'runtime', 'loop', 'changes', 'example');
  ...
  it('derives failed and interrupted states from exit and timeout', async () => {
    const failed = await executeLoopCheck({
      projectRoot: root,
      runtimeDir,
    ...
    const timedOut = await executeLoopCheck({
      projectRoot: root,
      runtimeDir,
  ...
  it('keeps cwd inside the project and redacts portable argv displays', () => {
    expect(resolveLoopCheckCwd(root, '.')).toBe(root);
    expect(() => resolveLoopCheckCwd(root, '../outside')).toThrow('project-relative');
    expect(loopPortableArgvDisplay(['--token=secret-value'])).not.toContain('secret-value');
    expect(
      loopPortableArgvDisplay([
        '--token',
        'token-value',

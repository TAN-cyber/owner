...
import { readLoopVerificationEvidence } from '../../../domains/owner-loop/loop-evidence-storage.js';
import { readLoopVerificationReceipt } from '../../../domains/owner-loop/loop-evidence-storage.js';
...
# Decisions
Use the built-in Loop check.
# Open questions
None.
# Verification expectations
Run owner loop check.
`;
...
// See loop-check-executor.test.ts and loop-portable-runtime.test.ts.
describe('Loop check public seam (legacy)', () => {
  let projectRoot: string;
  const name = 'safe-check';
  const projectArgs = () => ['--project-root', projectRoot] as const;

  function runtimeDir(
    paths: Awaited<ReturnType<typeof loopProjectPaths>>,
    changeDir: string,
  ): string {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-loop-check-cli-'));
  ...
  async function prepareVerifyChange(
    ...
    const changeDir = loopChangeDir(paths, changeName);
    ...
    await expect(readLoopChange(paths, changeName)).resolves.toMatchObject({
      phase: 'verify',
  ...
  it.skip('runs the internal scoped check through a whitelist-only CLI projection', async () => {
    ...
    const stateBefore = await fs.readFile(path.join(changeDir, 'owner-state.yaml'), 'utf8');
    ...
    const checked = json(await runLoopCli(['check', name, '--json', ...projectArgs()]));
    ...
    const firstReceipt = await readLoopVerificationReceipt(paths, name, firstRef);
    expect(firstReceipt).toMatchObject({
      schema: 'owner.loop.verification-receipt.v3',
    ...
    const repeated = json(await runLoopCli(['check', name, '--json', ...projectArgs()]));
    ...
    const changed = json(await runLoopCli(['check', name, '--json', ...projectArgs()]));
    ...
    expect(await fs.readFile(path.join(changeDir, 'owner-state.yaml'), 'utf8')).toBe(stateBefore);
    expect(
  ...
  it.skip('runs and binds a fresh required check when Verify passes', async () => {
    ...
    const envelope = await readLoopVerificationEvidence(paths, name, state.verification_evidence!);
  ...
  it.skip('validates the Verify report before running the automatic required check', async () => {
    ...
    await expect(readLoopChange(paths, name)).resolves.toMatchObject({
      phase: 'verify',
  ...
  it.skip('returns CLI exit 1 and persists bounded text issues without changing workflow state', async () => {
    ...
    const before = await readLoopChange(paths, name);
    const trajectoryBefore = await fs.readFile(
      path.join(runtimeDir(paths, changeDir), 'trajectory.jsonl'),
      'utf8',
    );
    const result = json(await runLoopCli(['check', name, '--json', ...projectArgs()]));
    ...
    const receipt = await readLoopVerificationReceipt(paths, name, result.data!.ref as string);
    expect(receipt).toMatchObject({
      kind: 'static-inspection',
      status: 'failed',
    });
    await expect(readLoopChange(paths, name)).resolves.toEqual(before);
    expect(
      await fs.readFile(path.join(runtimeDir(paths, changeDir), 'trajectory.jsonl'), 'utf8'),
    ).toBe(trajectoryBefore);
  });
  ...
  it.skip('rejects non-Verify use and any attempt to supply a command or path', async () => {
    ...
    const shape = json(await runLoopCli(['check', shapeName, '--json', ...projectArgs()]));
    ...
    for (const extras of [
      ...
      const rejected = json(
        await runLoopCli(['check', name, ...extras, '--json', ...projectArgs()]),
      );

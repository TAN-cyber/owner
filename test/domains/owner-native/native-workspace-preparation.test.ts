...
vi.mock('../../../domains/owner-native/native-config.js', () => config);

import {
  NativeWorkspacePreparationError,
  prepareNativeWorkspace,
} from '../../../domains/owner-native/native-workspace-preparation.js';
import type { OwnerProjectConfig } from '../../../domains/owner-native/native-types.js';

describe('Native workspace preparation', () => {
  let projectRoot: string;
  const sourceConfig = { native: { artifact_root: 'docs' } } as OwnerProjectConfig;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-workspace-preparation-'));

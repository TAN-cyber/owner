...
import { nativeArchiveCommand } from '../../../domains/owner-native/native-archive-command.js';
import {
  hashNativeParentContract,
  inspectNativeChildren,
  parseNativeChildrenContract,
} from '../../../domains/owner-native/native-children.js';
import {
  defaultProjectConfig,
  readProjectConfig,
  writeProjectConfig,
} from '../../../domains/owner-native/native-config.js';
import { nativeDoctorCommand } from '../../../domains/owner-native/native-doctor-command.js';
import { nativeNewCommand } from '../../../domains/owner-native/native-new-command.js';
import { nativeNextCommand } from '../../../domains/owner-native/native-next-command.js';
import {
...
} from '../../../domains/owner-native/native-portable-state.js';
import { inspectNativePortableStatus } from '../../../domains/owner-native/native-portable-status.js';
import { nativeShowCommand } from '../../../domains/owner-native/native-show-command.js';
import type {
  NativePortableAcceptanceState,
  NativePortableState,
} from '../../../domains/owner-native/native-portable-types.js';
...
const CHILDREN = `schema: owner.native.children.v1
children:
  - name: child-a
    depends_on: []
    covers: [A1]
...
const REORDERED_CHILDREN = `schema: owner.native.children.v1
children:
  - name: child-a
    depends_on: []
    covers: [A1]
...
schema: owner.native.children.v1
children:
  - covers:
      - A1
    name: child-a
...
`,
  ...
  it('gates the parent on real child merges and starts dependents from the integrated HEAD', async () => {
    const repository = await fs.mkdtemp(
      path.join(await fs.realpath(os.tmpdir()), 'owner-native-children-'),
    );
    ...
    await expect(
      inspectNativeChildren({
        paths: parentPaths,
        state: await readNativePortableState(path.join(parentDir, 'owner-state.yaml')),
      }),
    ).resolves.toMatchObject({
    ...
    const parentAfterMerge = await readNativePortableState(
      path.join(parentDir, 'owner-state.yaml'),
    );
    ...
    const archivedA = await readNativePortableState(
      path.join(archivedDirectory, 'owner-state.yaml'),
    );
    ...
    await expect(
      ...
      continuation: {
        ...
        commandArgs: ['owner', 'native', 'next', 'parent', '--summary', '<summary>'],

...
} from '../../../domains/owner-native/native-local-execution.js';
import { createNativePortableState } from '../../../domains/owner-native/native-portable-state.js';
import type { NativeLocalExecutionState } from '../../../domains/owner-native/native-portable-types.js';
...
describe('Native local execution overlay', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-local-execution-'));
    runtimeRoot = path.join(root, '.owner', 'runtime', 'native');

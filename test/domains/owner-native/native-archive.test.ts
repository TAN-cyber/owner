...
import { nativeArchiveTransactionPaths } from '../../../domains/owner-native/native-archive-transaction.js';
import {
  createNativeChange,
...
describe('Native archive', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-archive-'));
  ...
  it('applies create, replace, and remove specs before archiving the active change', async () => {
    ...
    expect(
      await readNativeChangeFile(path.join(result.archiveDir, 'owner-state.yaml')),
    ).toMatchObject({
      archived: true,
      phase: 'archive',
    });
    ...
    expect(await readNativeTransaction(paths, result.transactionId)).toMatchObject({
      schema: 'owner.native.transaction.v2',
      kind: 'archive',
      status: 'committed',
  ...
  it('supports an archive with no spec changes', async () => {
    ...
    ).resolves.toContain('# Owner Native Evidence Projection');
  });
  ...
  it('refuses a transactions junction that would stage archive data outside owner', async () => {
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-transactions-outside-'));
    try {
      const specChanges: NativeSpecChange[] = [
        {
          capability: 'sessions',
  ...
  it('refuses canonical spec junctions that would write outside owner', async () => {
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-spec-outside-'));
    try {

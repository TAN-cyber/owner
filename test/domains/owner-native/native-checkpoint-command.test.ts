...
vi.mock('../../../domains/owner-native/native-progress-checkpoint.js', () => ({
  checkpointNativeChange,
}));
vi.mock('../../../domains/owner-native/native-diagnostics.js', () => ({ inspectNativeStatus }));

import { nativeCheckpointCommand } from '../../../domains/owner-native/native-checkpoint-command.js';
import {
...
describe('Native checkpoint command argument branches', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-checkpoint-command-'));

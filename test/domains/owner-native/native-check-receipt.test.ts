...
} from '../../../domains/owner-native/native-check-receipt.js';
...
describe('Native scoped check receipts', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-check-receipt-'));
  ...
  it('scans only scoped text with no command execution and records bounded issues', async () => {
    ...
    expect(result.receipt).toMatchObject({
      schema: 'owner.native.check-receipt.v1',
      status: 'failed',
      stale: false,
      counts: {
        filesSelected: 1,
  ...
  it('fails closed on a scoped parent redirected through a symlink or junction', async () => {
    const state = await prepareState('export const value = 2;\n');
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-check-outside-'));
    try {
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const state = await prepareState('export const value = 2;\n');
      const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-check-file-link-'));
      try {

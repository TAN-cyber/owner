...
describe('Native artifact root recovery', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-root-recovery-'));
  ...
  it('continues an interruption in the copying stage and blocks normal discovery meanwhile', async () => {
    ...
    expect(recovered.activeNativeRoot).toBe(path.join(projectRoot, 'docs', 'owner'));
  ...
  it('continues a transaction-bound source quarantine after a removal crash', async () => {
    ...
    expect(path.basename(quarantine)).toMatch(/^\.owner-native-source-.+\.removing$/u);
    await expect(fs.access(source)).rejects.toMatchObject({ code: 'ENOENT' });
  ...
  it('stops without deleting either tree when staged hashes changed', async () => {
    ...
    const staging = path.join(projectRoot, 'docs', `.owner-native-move-${transactionId}`);

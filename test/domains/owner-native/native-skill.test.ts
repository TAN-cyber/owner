...
const roots = {
  en: path.resolve('assets', 'skills', 'owner-native'),
  zh: path.resolve('assets', 'skills-zh', 'owner-native'),
};
...
describe('Owner Native Skills', () => {
  it('keeps valid bilingual entry Skills with progressive references', async () => {
    for (const language of ['en', 'zh'] as const) {
      ...
      expect(metadata.name).toBe('owner-native');
  ...
  it('keeps the main Skill on decisions while delegating mechanics to public CLI output', async () => {
    ...
    for (const variant of variants) {
      ...
      expect(skill).not.toContain('scripts/owner-native-runtime.mjs');
      expect(skill).not.toContain('owner-native-<cmd>.mjs');
    }
  });
  ...
  it('uses the command reference for semantic exceptions instead of duplicating CLI syntax', async () => {
    for (const language of ['en', 'zh'] as const) {
      ...
      expect(commands).toContain('owner native <command> --help');
      expect(commands).toContain('owner native <group> <command> --help');
      for (const field of [
        'commandArgs',
        'inputOptions',
        'workspace',
      ...
      expect(commands.match(/owner native/gu)?.length ?? 0).toBeLessThanOrEqual(4);
      ...
      expect(commands).not.toContain('owner native receipt automated <change-name>');
      expect(commands).not.toContain('owner native checkpoint <change-name>');
    }
  });
  ...
  it('keeps recovery focused on exceptional safety decisions and exact Runtime actions', async () => {
    ...
    for (const variant of variants) {
      ...
      expect(recovery).not.toContain('owner native spec rebase <change-name>');

...
const NATIVE_SCRIPTS = path.resolve('assets/skills/owner-native/scripts');
...
function commandScript(command: string): string {
  return path.join(NATIVE_SCRIPTS, `owner-native-${command}.mjs`);
}
...
describe('Native parallel linked-worktree Runtime', () => {
  ...
  it('keeps concurrent linked worktrees usable after a manual business-source edit', async () => {
    const repository = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-parallel-'));
    ...
    for (const { root } of worktrees) {
      const locks = path.join(root, '.owner', 'runtime', 'native', 'locks');
      ...
      expect(
        await readDirectoryOrEmpty(path.join(root, '.owner', 'runtime', 'native', 'transactions')),
      ).toEqual([]);
      expect(
  ...
  it('rejects the retired checkpoint bundle on a linked-worktree-capable host', async () => {
    const repository = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-retired-command-'));
    repositories.push(repository);
    execFileSync('git', ['init'], { cwd: repository, stdio: 'ignore' });

...
import { NATIVE_LEGACY_RUNTIME_IDENTITIES } from '../../../domains/owner-native/native-runtime-package.js';
import type { NativeProjectPaths } from '../../../domains/owner-native/native-types.js';
import { inspectNativeVerificationFreshness } from '../../../domains/owner-native/native-verification-runtime.js';
...
describe('Native evidence-bound phase transitions', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-evidence-transition-'));
  ...
  it('binds Build scope and Verify evidence, then marks implementation drift stale', async () => {
    ...
    await expect(inspectNativeStatus(paths, 'evidence-change')).resolves.toMatchObject({
      phase: 'archive',
      nextCommand: 'owner native next evidence-change --summary "<summary>"',
      continuation: {
        disposition: 'continue',
        action: 'advance-phase',
        command: 'owner native next evidence-change --summary "<summary>"',
      },
    });
    ...
    expect(status).toMatchObject({
      ...
      findings: expect.arrayContaining([
        expect.objectContaining({
          ...
          retryCommand: 'owner native next evidence-change --summary "<summary>"',
          requiresUserDecision: false,
        }),
      ]),
    });
  ...
  it('does not retreat fresh Archive evidence through an ordinary next command', async () => {
    ...
    expect(result).toMatchObject({
      next: 'manual',
      nextCommand: 'owner native archive evidence-change --dry-run',
      change: { phase: 'archive' },
      continuation: {
        disposition: 'continue',
        action: 'archive',
        command: 'owner native archive evidence-change --dry-run',
      },
    });
  });

...
import {
  ...
  isNativeArchiveHash,
} from '../../../domains/owner-native/native-archive-content.js';

describe('Native Archive content identity budgets', () => {
  let root: string;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-archive-content-'));
  });
  ...
  it('produces a stable content identity without embedding the absolute root', async () => {
    const other = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-archive-content-copy-'));
    try {

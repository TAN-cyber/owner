...
import { loadNativeVerificationReceiptContext } from '../../../domains/owner-native/native-verification-receipt-runtime.js';
...
describe('Native verification receipt fence recovery (legacy)', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-receipt-fence-'));
  ...
  it.skip('stops before command execution and returns a bounded self-healing recovery payload', async () => {
    ...
    expect(result).toMatchObject({
      ...
      data: {
        ...
        nextCommand:
          'owner native next receipt-fence --summary "Implementation changed after Build; return to Build and refresh scope"',

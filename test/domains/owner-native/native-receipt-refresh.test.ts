...
import { parseNativeVerificationMachineBlock } from '../../../domains/owner-native/native-acceptance.js';
import { replaceAcceptanceEvidenceBlock } from '../../../domains/owner-native/native-receipt-refresh.js';
import { atomicWriteText } from '../../../domains/owner-native/native-atomic-file.js';
import { prepareNativeBuildEvidence } from '../../../domains/owner-native/native-build-evidence.js';
import {
  createNativeChange,
  nativeChangeDir,
...
import { collectNativeContractFiles } from '../../../domains/owner-native/native-contract-files.js';
import {
  readNativeImplementationScopeBundle,
  readNativeVerificationReceipt,
  writeNativeVerificationReceipt,
} from '../../../domains/owner-native/native-evidence-storage.js';
...
} from '../../../domains/owner-native/native-receipt-refresh.js';
import {
  inspectNativeVerificationEvidence,
  prepareNativeVerificationEvidence,
  NativeVerificationReceiptBindingError,
} from '../../../domains/owner-native/native-verification-runtime.js';
import { persistNativeStaticInspectionReceipt } from '../../../domains/owner-native/native-verification-receipt-runtime.js';
import {
  buildNativeVerificationReceipt,
  nativeArtifactBindingHash,
  type NativeVerificationReceiptBindings,
} from '../../../domains/owner-native/native-verification-receipt.js';
import { buildNativeCheckReceipt } from '../../../domains/owner-native/native-check-receipt-model.js';
import {
  readNativeCheckReceipt,
  writeNativeCheckReceipt,
} from '../../../domains/owner-native/native-check-receipt-storage.js';
...
describe('Native receipt refresh', () => {
  ...
  async function seedFixtureWithAcceptanceRevision(acceptanceRevision: number): Promise<string> {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-receipt-refresh-'));
  ...
  it('returns a no-op when Verify has no evidence envelope yet', async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-receipt-refresh-empty-'));
    paths = await nativeProjectPaths(projectRoot, '.');
    const created = await createNativeChange({
      paths,
  ...
  it('rejects reports without both acceptance-evidence markers', () => {
    ...
    expect(() =>
      replaceAcceptanceEvidenceBlock(
        '<!-- owner-native:acceptance-evidence:start -->',
        'new block',
      ),

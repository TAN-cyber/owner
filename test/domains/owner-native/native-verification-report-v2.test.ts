...
} from '../../../domains/owner-native/native-loop-runtime.js';
import { createNativePortableState } from '../../../domains/owner-native/native-portable-state.js';
...
} from '../../../domains/owner-native/native-verification-report-v2.js';
...
describe('Native verification report projection', () => {
  ...
  it('rebuilds a missing or stale report without rerunning verification', async () => {
    const state = passedState();
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-report-'));
    roots.push(root);

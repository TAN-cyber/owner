...
vi.mock('../../../domains/owner-native/native-root-move.js', () => ({ moveNativeRoot }));

import { nativeRootCommand } from '../../../domains/owner-native/native-root-command.js';
import {
...
describe('Native root command branches', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-root-command-'));

...
describe('Native Wave B CLI contract (legacy)', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-wave-b-cli-'));
  ...
  it.skip('keeps one envelope and returns the same continuation shape from mutations and status', async () => {
    ...
    expect(status.data).toMatchObject({
      ...
      detailsCommand: 'owner native status cold-resume --details',
        ...
        findings: [
          {
            ...
            retryCommand: 'owner native next cold-resume --summary "<summary>"',
            repairCommand: null,

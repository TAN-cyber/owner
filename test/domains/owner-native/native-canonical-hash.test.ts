...
import {
  canonicalHash,
  canonicalJson,
} from '../../../domains/owner-native/native-canonical-hash.js';
...
describe('Native canonical hashing', () => {
  ...
  it('hashes the domain tag, newline, and canonical JSON exactly', () => {
    ...
    const expected = createHash('sha256')
      .update('owner.native.test.v1\n{"a":1,"b":2}')
      .digest('hex');

    expect(canonicalHash('owner.native.test.v1', value)).toBe(expected);
    expect(canonicalHash('owner.native.other.v1', value)).not.toBe(expected);
  });

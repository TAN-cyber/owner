...
} from '../../../domains/owner-native/native-archive.js';
import {
  createNativeChange,
  nativeChangeDir,
  readNativeChange,
...
import { inspectNativeChangeConflicts } from '../../../domains/owner-native/native-conflict-inspection.js';
import { readProjectConfig } from '../../../domains/owner-native/native-config.js';
import { doctorNativeProject } from '../../../domains/owner-native/native-doctor.js';
import { sha256File } from '../../../domains/owner-native/native-hash.js';
import { nativeProjectPaths } from '../../../domains/owner-native/native-paths.js';
import {
  moveNativeRoot,
  recoverNativeRootMove,
} from '../../../domains/owner-native/native-root-move.js';
import type {
  NativeProjectPaths,
  NativeSpecChange,
...
describe('Owner Native Phase 1 behavior matrix', () => {
  ...
  async function project(): Promise<string> {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-matrix-'));
    projects.push(root);
    execFileSync('git', ['init'], { cwd: root, stdio: 'ignore' });
    return root;
  }

  it('supports both project/owner and docs/owner without creating Classic trees', async () => {
    const defaultProject = await project();
    const docsProject = await project();

    expect((await initialize(defaultProject)).nativeRoot).toBe(path.join(defaultProject, 'owner'));
  ...
  it('fails closed for malformed config/state and never scans an openspec fixture', async () => {
    ...
    await fs.writeFile(
      path.join(nativeChangeDir(paths, 'malformed-state'), 'owner-state.yaml'),
      'schema: owner.native.v1\nphase: [broken\n',
    );
    ...
    await fs.writeFile(path.join(projectRoot, '.owner', 'config.yaml'), 'native: [broken\n');

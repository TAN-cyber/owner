...
import { withNativeMutationLock } from '../../../domains/owner-native/native-mutation-lock.js';
import { migrateNativeLegacyChangeToPortable } from '../../../domains/owner-native/native-portable-migration-runtime.js';
import { createNativePortableChange } from '../../../domains/owner-native/native-portable-runtime.js';
import type { NativeProjectPaths } from '../../../domains/owner-native/native-types.js';
import { writeNativeWorkspaceIdentity } from '../../../domains/owner-native/native-workspace.js';
...
describe('Native portable migration Runtime', () => {
  ...
  async function legacyChange(): Promise<NativeProjectPaths> {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-migrate-runtime-'));
    roots.push(root);
  ...
  it('recognizes only valid portable migration journals and lets that migration resume itself', async () => {
    ...
    await fs.writeFile(
      transactionFile,
      `${JSON.stringify({
        schema: 'owner.native.portable-migration.v1',
        id: randomUUID(),
        change: 'legacy-change',
        fromSchema: 'owner.native.v3',
        status: 'prepared',
    ...
    expect(migrated).toMatchObject({ schema: 'owner.native.v4', name: 'legacy-change' });
  ...
  it('refuses to migrate an isolated legacy change from the wrong checkout', async () => {
    ...
    ).resolves.toContain('schema: owner.native.v3');
  });

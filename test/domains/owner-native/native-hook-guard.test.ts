...
} from '../../../domains/owner-native/native-hook-guard.js';
...
describe('Native phase Hook guard', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-hook-'));
  ...
  it('does not let the allowlist bypass Native Runtime-owned files', async () => {
    ...
    await expect(
      inspectNativeHookGuard(projectRoot, writeRequest('.owner/runtime/extra.json')),
  ...
  it('allows portable Build implementation writes while protecting Runtime-owned state', async () => {
    ...
    const stateRef = path
      .relative(
        projectRoot,
        path.join(nativePortableChangeDir(paths, state.name), 'owner-state.yaml'),
      )
      .replaceAll('\\', '/');
    await expect(
  ...
  it('allows Native artifacts and projects without an active change', async () => {
    ...
    await expect(
      inspectNativeHookGuard(projectRoot, writeRequest('docs/owner/changes/example/brief.md')),

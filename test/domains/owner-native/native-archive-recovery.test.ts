...
} from '../../../domains/owner-native/native-transaction.js';
import {
  nativeArchiveTransactionPaths,
  readNativeArchiveTransactionV2,
} from '../../../domains/owner-native/native-archive-transaction.js';
import type {
  NativeProjectPaths,
  NativeSpecChange,
...
describe('Native archive recovery', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-recovery-'));
  ...
  it('finalizes when the active change moved before the journal committed', async () => {
    ...
    await expect(fs.access(changeDir)).rejects.toMatchObject({ code: 'ENOENT' });
    expect((await readNativeChangeFile(path.join(archiveDir, 'owner-state.yaml'))).archived).toBe(
      false,
    );

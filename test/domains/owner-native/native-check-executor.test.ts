...
} from '../../../domains/owner-native/native-check-executor.js';
...
describe('Native check executor', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-check-'));
    runtimeDir = path.join(root, '.owner', 'runtime', 'native', 'changes', 'example');

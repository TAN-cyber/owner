...
} from '../../../domains/owner-native/native-findings.js';
...
describe('Native structured findings', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-findings-'));
  ...
  it('normalizes project-relative paths and emits stable metadata order', () => {
    ...
    expect(findings[0]).toMatchObject({
      ...
      retryCommand: 'owner native next finding-shape --summary "<summary>"',
      repairCommand: null,
  ...
  it('reserves user-decision pauses for explicit clarification decisions', () => {
    ...
    expect(
      ...
      retryCommand: 'owner native next finding-shape --summary "<summary>" --confirmed',
      requiresUserDecision: true,
    });

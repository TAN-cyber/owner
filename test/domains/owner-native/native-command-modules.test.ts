...
vi.mock('../../../domains/owner-native/native-receipt-refresh.js', () => ({
...
import { nativeReceiptCommand } from '../../../domains/owner-native/native-receipt-command.js';
import { nativeSpecCommand } from '../../../domains/owner-native/native-spec-command.js';

describe('Native command modules', () => {
  let projectRoot: string;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-command-modules-'));
  ...
  it('renders every receipt refresh report section and the applied outcome', async () => {
    ...
    expect(result.text).toContain('Re-run `owner native check demo-change`');

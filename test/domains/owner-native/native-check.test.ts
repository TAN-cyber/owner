...
import { readNativeVerificationEvidence } from '../../../domains/owner-native/native-evidence-storage.js';
import { readNativeVerificationReceipt } from '../../../domains/owner-native/native-evidence-storage.js';
import {
...
describe('Native check public seam (legacy)', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-check-cli-'));
  ...
  it.skip('runs the internal scoped check through a whitelist-only CLI projection', async () => {
    ...
    expect(firstReceipt).toMatchObject({
      schema: 'owner.native.verification-receipt.v3',

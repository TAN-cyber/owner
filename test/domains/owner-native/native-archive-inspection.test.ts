...
import { nativeTransitionJournalFile } from '../../../domains/owner-native/native-transition-journal.js';
...
import { prepareNativeVerificationEvidence } from '../../../domains/owner-native/native-verification-runtime.js';
import {
...
} from '../../../domains/owner-native/native-verification-receipt-runtime.js';
import {
...
describe('Native Archive inspection', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-archive-inspection-'));
  ...
  it('returns a stable ready preview without writing project state', async () => {
    const changeFile = path.join(nativeChangeDir(paths, state.name), 'owner-state.yaml');

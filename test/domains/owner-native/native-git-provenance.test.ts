...
import { detectNativeGitExternalDrift } from '../../../domains/owner-native/native-git-provenance.js';
import type { NativeSnapshotProjection } from '../../../domains/owner-native/native-verification-scope.js';

const projection = (entry: { path: string; gitObjectId: string }): NativeSnapshotProjection => ({
  schema: 'owner.native.snapshot-projection.v1',
...
describe('Native Git provenance', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-provenance-'));

...
describe('Native workspace identity', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-workspace-'));
  ...
  it('does not treat legacy target-branch provenance as a change-branch binding', async () => {
    ...
    identity.schema = 'owner.native.workspace.v2';
  ...
  it('reports a copied identity as root drift without executing VCS commands', async () => {
    ...
    const otherRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-workspace-copy-'));
    try {

...
describe('Native portable state', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-portable-state-'));
  ...
  it('round-trips the v4 portable schema and rejects unknown fields at every level', async () => {
    ...
    expect(await fs.readFile(file, 'utf8')).toContain('schema: owner.native.v4');
    ...
    await fs.writeFile(file, 'schema: owner.native.v4\nschema: owner.native.v4\n', 'utf8');

...
describe('Native project configuration', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-config-'));
  ...
  it('does not write Native project config through a linked .owner directory', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-config-outside-'));
    try {
  ...
  it('does not replace a Native project config symlink', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-link-outside-'));
  ...
  it('renders Chinese comments for a Chinese project config', async () => {
    ...
    expect(source).toContain('# Native 规格和 change 的存放根目录；运行时数据始终位于 .owner');
  ...
  it('discovers the nearest configured project from a nested directory', async () => {
    ...
    expect(resolved.paths.nativeRoot).toBe(path.join(projectRoot, 'docs', 'owner'));
  ...
  it.each([
    ...
    ['missing Native root', 'schema: owner.project.v1\ndefault_workflow: native\nnative: {}\n'],
    [
      'bad pending move',

...
describe('Native artifact root safety', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-paths-'));
    outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-outside-'));
  });
  ...
  it('rejects a configured owner root that is itself a junction', async () => {
    ...
    await expect(nativeProjectPaths(projectRoot, '.')).rejects.toThrow(
      'Native owner root must not be a symbolic link',
    );
  });
  ...
  it('keeps Runtime project-local when the document artifact root changes', async () => {
    ...
    expect(rootPaths.runtimeDir).toBe(path.join(projectRoot, '.owner', 'runtime', 'native'));
    ...
    expect(nativePreferredChangeRuntimeDir(docsPaths, 'focused-change')).toBe(
      path.join(projectRoot, '.owner', 'runtime', 'native', 'changes', 'focused-change'),
    );
  });

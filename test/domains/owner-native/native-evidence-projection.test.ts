...
} from '../../../domains/owner-native/native-evidence-projection.js';
import {
...
import { buildNativeVerificationReceipt } from '../../../domains/owner-native/native-verification-receipt.js';
...
describe('Native evidence projection rendering', () => {
  describe('renderNativeEvidenceProjectionMarkdown (pure)', () => {
    it('writes the boundary declaration and generator fingerprint', () => {
      ...
      expect(markdown).toContain('Generated-by: owner-native');
...
describe('Native evidence projection write', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-projection-'));
  ...
  it('writes scope, verification, and receipt sections for a verified change', async () => {
    ...
    const { writeNativePartialAllowance } =
      await import('../../../domains/owner-native/native-evidence-storage.js');

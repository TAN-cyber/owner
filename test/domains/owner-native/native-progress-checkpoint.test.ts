...
} from '../../../domains/owner-native/native-checkpoint-storage.js';
import {
  nativeChangeRuntimeDir,
  nativeProjectPaths,
} from '../../../domains/owner-native/native-paths.js';
import { checkpointNativeChange } from '../../../domains/owner-native/native-progress-checkpoint.js';
import { markNativeSpecRemoval } from '../../../domains/owner-native/native-specs.js';
...
describe('Native progress checkpoints', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-progress-'));
...
`,
  ...
  it('records progress without changing phase and treats an identical retry exactly once', async () => {
    ...
    expect(first).toMatchObject({
      ...
      revision: 2,
      outcome: 'recorded',
      continuation: { schema: 'owner.native.continuation.v1', revision: 2 },
    });
  ...
  it('rejects an internal symlink or junction in the checkpoint manifest parent chain', async () => {
    ...
    const manifest = {
      schema: 'owner.native.checkpoint-manifest.v1' as const,
      change: 'resume-work',
      artifacts: [],
      totalBytes: 0,
    };

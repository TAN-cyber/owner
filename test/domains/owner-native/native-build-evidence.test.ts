...
} from '../../../domains/owner-native/native-build-evidence.js';
import { executeNativeCheckReceipt } from '../../../domains/owner-native/native-check-receipt.js';
import {
  createNativeChange,
  nativeChangeDir,
  writeNativeChange,
...
vi.mock('../../../domains/owner-native/native-snapshot.js', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('../../../domains/owner-native/native-snapshot.js')>();
  return {
    ...actual,
...
describe('Native Build evidence preparation', () => {
  ...
  beforeEach(async () => {
    snapshotMock.next = null;
    snapshotMock.filtered = null;
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-build-evidence-'));
  ...
  it('automatically classifies proven fast-forward target-branch drift as external', async () => {
    const gitRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-external-drift-'));
    try {

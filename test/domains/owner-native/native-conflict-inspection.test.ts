...
} from '../../../domains/owner-native/native-conflict-inspection.js';
...
describe('Native conflict radar collection', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-conflict-inspection-'));
  ...
  it('does not traverse a symlinked change directory', async () => {
    ...
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-conflict-outside-'));
    try {

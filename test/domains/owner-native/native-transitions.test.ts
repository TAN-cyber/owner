...
import { readNativeImplementationScopeBundle } from '../../../domains/owner-native/native-evidence-storage.js';
import {
  advanceNativeChange,
  formatNativeReceiptBindingMismatchMessage,
} from '../../../domains/owner-native/native-transitions.js';
...
describe('Native guarded transitions', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-transitions-'));
  ...
  it('uses the concrete change name in receipt binding recovery guidance', () => {
    ...
    expect(message).toContain('owner native receipt refresh advance-change --apply');
      ...
      expect(blocked).toMatchObject({
        ...
        findings: [
          expect.objectContaining({
            ...
            retryCommand: 'owner native next advance-change --summary "<summary>" --confirmed',
  ...
  it('requires a legacy implicit Build state to be confirmed before Verify', async () => {
    ...
    expect(await inspectNativeStatus(paths, 'advance-change')).toMatchObject({
      phase: 'build',
      nextCommand:
        'owner native next advance-change --summary "<summary>" --artifact "<project-relative-path>" --confirmed',
      continuation: {
        command:
          'owner native next advance-change --summary "<summary>" --artifact "<project-relative-path>" --confirmed',
        commandArgs: [
  ...
  it('advances Verify pass to the Native archive command without reasoning fields', async () => {
    ...
    expect(result.nextCommand).toBe('owner native archive advance-change --dry-run');
    const run = (await readNativeRunState(runtimeDir))!;

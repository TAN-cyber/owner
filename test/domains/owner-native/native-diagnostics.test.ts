...
} from '../../../domains/owner-native/native-diagnostics.js';
import { nativeContinuation } from '../../../domains/owner-native/native-continuation.js';
import {
...
describe('Native status diagnostics', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-status-'));
  ...
  it('keeps workspace advisories visible without blocking an otherwise ready Archive', () => {
      ...
      findings: [
        {
          ...
          retryCommand: 'owner native status ready-change',
          repairCommand: null,
    ...
    expect(continuation).toMatchObject({
      ...
      action: 'archive',
      command: 'owner native archive ready-change --dry-run',
    });
  ...
  it('blocks status when a workspace binding cannot be parsed safely', async () => {
    ...
    await fs.writeFile(
      ...
      '{"schema":"owner.native.workspace.v3","isolation":"invalid"}\n',
    );
  ...
  it('returns policy-aware continuation after a ready Archive preview', () => {
    ...
    expect(
      ...
      disposition: 'continue',
      action: 'archive',
      command: `owner native archive ready-change --expect-preflight ${preflightHash}`,
      commandArgs: [
        'owner',
        'native',
        'archive',
  ...
  it('sorts multiple active changes and projects only Native next commands', async () => {
    ...
    expect(statuses[0]).toMatchObject({
      phase: 'shape',
      selected: false,
      nextCommand: 'owner native next alpha-change --summary "<summary>" --confirmed',
    });
    expect(statuses[1]).toMatchObject({ selected: true });
    expect(JSON.stringify(statuses)).not.toMatch(/openspec|superpowers|owner classic/iu);
  });
    ...
    expect(status).toMatchObject({
      ...
      findings: [
        expect.objectContaining({
          ...
          retryCommand: 'owner native next contract-drift --summary "<summary>" --confirmed',
        }),
      ],
    });
  });
  ...
  it('only marks Archive ready after brief, spec, and verification checks pass', async () => {
    ...
    expect(readyStatus).toMatchObject({
      archiveReady: true,
      nextCommand: 'owner native archive ready-change --dry-run',
    });
    ...
    expect(await inspectNativeStatus(paths, 'ready-change')).toMatchObject({
      archiveReady: false,
      nextCommand: 'owner native next ready-change --summary "<summary>"',
    });
  });
  ...
  it('keeps a change discoverable when its whole local Runtime is missing', async () => {
    ...
    expect(await inspectNativeStatus(paths, 'missing-runtime', { details: true })).toMatchObject({
      ...
      nextCommand: 'owner native next missing-runtime --summary "<summary>"',

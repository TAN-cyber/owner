...
vi.mock('../../../domains/owner-native/native-diagnostics.js', () => ({ inspectNativeStatus }));
vi.mock('../../../domains/owner-native/native-portable-status.js', () => ({
  inspectNativePortableStatus,
}));
vi.mock('../../../domains/owner-native/native-portable-runtime.js', () => ({
  isNativePortableChange,
}));
vi.mock('../../../domains/owner-native/native-selection.js', () => ({ selectNativeChange }));
...
describe('Native select command branches', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-select-command-'));

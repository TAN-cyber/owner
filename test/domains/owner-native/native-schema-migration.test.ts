...
describe('Native schema compatibility and journalized migration', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-schema-'));
  ...
  async function seedLegacyChange(name: string): Promise<string> {
    ...
    const file = path.join(nativeChangeDir(paths, name), 'owner-state.yaml');
  ...
  async function downgradeToV2(state: NativeChangeState): Promise<string> {
    const file = path.join(nativeChangeDir(paths, state.name), 'owner-state.yaml');
  ...
  it.each<NativePhase>(['verify', 'archive'])(
    ...
    async (phase) => {
      ...
      expect(
        await readNativeChangeFile(path.join(archived.archiveDir, 'owner-state.yaml')),
      ).toMatchObject({
        phase: 'archive',
        archived: true,
      });

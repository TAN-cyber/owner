...
import { DEFAULT_NATIVE_SNAPSHOT_CONFIG } from '../../../domains/owner-native/native-config.js';
import {
  createNativeChange,
  nativeChangeDir,
...
import { buildNativeImplementationScopeBundle } from '../../../domains/owner-native/native-verification-scope.js';
import type {
...
describe('Native VCS-independent content snapshots', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-snapshot-'));
    outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-snapshot-outside-'));
  ...
  it('records only safe project-relative metadata and excludes secrets, caches, Native state, and links', async () => {
    ...
    expect(manifest).toMatchObject({
      schema: 'owner.native.content-snapshot.v1',
      origin: 'explicit',
  ...
  it('prunes default platform Skill directories before bounded physical enumeration', async () => {
    await Promise.all([
      fs.mkdir(path.join(projectRoot, 'src'), { recursive: true }),
      fs.mkdir(path.join(projectRoot, '.agents', 'skills', 'owner-native'), { recursive: true }),
    ]);
    await fs.writeFile(path.join(projectRoot, 'src', 'app.ts'), 'export const app = true;\n');
    await Promise.all(
      Array.from({ length: 32 }, (_, index) =>
        fs.writeFile(
          path.join(projectRoot, '.agents', 'skills', 'owner-native', `bundle-${index}.mjs`),
          'export {};\n',
        ),
      ),
    );
  ...
  it('unions a path staged after combined selection and marks the index race incomplete', async () => {
    ...
    expect(manifest.capture?.gitSelection).toMatchObject({
      schema: 'owner.native.git-selection.v1',
      status: 'changed',
  ...
  it('marks a physical directory node-budget overflow as hard incomplete evidence', async () => {
    ...
    expect(manifest.capture?.physicalSelection).toMatchObject({
      schema: 'owner.native.physical-selection.v1',
      status: 'overflow',
  ...
  it('covers Native snapshot manifest validation and optional metadata branches', () => {
    ...
    const policyHash = sha256Text(
      `owner.native.snapshot-policy.v1\n${JSON.stringify({
    ...
    const parsed = parseNativeContentSnapshotManifest({
      ...base,
      policy: {
        schema: 'owner.native.snapshot-policy.v1',
        include: policyInclude,
        exclude: policyExclude,
        hash: policyHash,
      },
    ...
    const invalidCases: Array<[string, unknown, string]> = [
      ...
      [
        ...
        {
          ...base,
          policy: { schema: 'owner.native.snapshot-policy.v1', include: 'all', exclude: [] },
        },
        'policy patterns',
      ],
      [
        ...
        {
          ...base,
          policy: { schema: 'owner.native.snapshot-policy.v1', include: [], exclude: [] },
        },
...
describe('Native incremental content snapshots', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-incremental-'));
    paths = await nativeProjectPaths(projectRoot, '.');
    await fs.mkdir(paths.nativeRoot, { recursive: true });
  });

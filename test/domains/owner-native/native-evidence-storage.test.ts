...
} from '../../../domains/owner-native/native-verification-scope.js';
import {
...
describe('Native evidence storage', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-evidence-'));
  ...
  it('reads legacy archived acceptance counters and validates their envelope', async () => {
    ...
    await fs.writeFile(
      file,
      JSON.stringify({
        schema: 'owner.native.verification-evidence.v1',
        change: 'secure-login',
        envelopeHash: hash,
        acceptanceTrace: {
          schema: 'owner.native.acceptance-trace.v1',
          total: 3,
          evidenced: 2,
          skipped: 1,
        },

...
import { moveNativeRoot } from '../../../domains/owner-native/native-root-move.js';
import { readNativeTransaction } from '../../../domains/owner-native/native-transaction.js';
import {
  assertNativeWorkspaceBinding,
  inspectNativeWorkspaceAdvisory,
  readNativeWorkspaceIdentity,
  setNativeWorkspaceFinish,
} from '../../../domains/owner-native/native-workspace.js';
...
describe('Native artifact root moves', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-root-move-'));
  ...
  it('stops before moving a root that contains an invalid workspace binding', async () => {
    ...
    await expect(fs.access(path.join(projectRoot, 'artifacts/native/owner'))).rejects.toMatchObject(
      { code: 'ENOENT' },
    );
  });
  ...
  it('refreshes a valid v3 binding even when the change schema needs a newer runtime', async () => {
    ...
    await fs.writeFile(
      stateFile,
      state
        .replace('schema: owner.native.v3', 'schema: owner.native.v4')
    ...
    ).resolves.toMatchObject({ schema: 'owner.native.workspace.v3' });
  });
  ...
  it.each([
    ...
    if (legacy) {
      ...
      const workspace = JSON.parse(await fs.readFile(workspaceFile, 'utf8')) as Record<
        ...
        unknown
      >;
      workspace.schema = 'owner.native.workspace.v2';
    ...
    const secondary = path.join(
      os.tmpdir(),
      `owner-native-root-move-foreign-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    try {
      execFileSync(
        'git',
      ...
      ).resolves.toMatchObject({ toNativeRoot: path.join(secondary, 'artifacts/native/owner') });
  ...
  it('quarantines first and rejects a child rewritten by the pre-quarantine hook', async () => {
    ...
    const quarantine = path.join(projectRoot, `.owner-native-source-${pending!.id}.removing`);
    await expect(fs.access(source)).rejects.toMatchObject({ code: 'ENOENT' });

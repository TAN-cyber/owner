...
import { nativeSelectionFile } from '../../../domains/owner-native/native-selection.js';
...
describe('Native doctor', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-doctor-'));
...
`,
  ...
  it('fails closed when a managed Native directory escapes through a symlink', async () => {
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-doctor-outside-'));
    try {

...
import { canonicalHash } from '../../../domains/owner-native/native-canonical-hash.js';
import type { NativeContentSnapshotManifest } from '../../../domains/owner-native/native-types.js';
...
describe('Native acceptance evidence trace', () => {
  ...
  it('bounds missing-coverage diagnostics instead of echoing every acceptance ID', () => {
    ...
    let message = '';
    try {
      buildNativeAcceptanceEvidenceTrace(criteria, [], { nativeRootRef: 'owner' });
    } catch (error) {
  ...
  it('rejects sensitive refs and deeply invalid traces even when their self-hash is refreshed', () => {
    ...
    malformed.traceHash = canonicalHash('owner.native.acceptance-trace.v1', content);

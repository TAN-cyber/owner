...
import { readNativeBaselineManifest } from '../../../domains/owner-native/native-snapshot.js';
import {
...
describe('Native change store', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-change-'));
  ...
  it('reports migration, incompatibility, and current inspection states', () => {
    ...
    expect(
      inspectNativeChangeValue({ schema: 'owner.native.unknown', minimum_runtime_version: 9 }),
    ).toMatchObject({
    ...
    expect(() => parseNativeChangeValue(legacy)).toThrow('run owner native doctor');
    expect(() =>
      parseNativeChangeValue({
        ...legacy,
        schema: NATIVE_V2_CHANGE_SCHEMA,
    ...
    ).toThrow('run owner native doctor');
  });

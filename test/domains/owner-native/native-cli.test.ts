...
import { runNativeCli } from '../../../domains/owner-native/native-cli.js';
import {
  defaultProjectConfig,
  readProjectConfig,
  writeProjectConfig,
} from '../../../domains/owner-native/native-config.js';
import { projectRootFrom } from '../../../domains/owner-native/native-cli-shared.js';
import { acquireNativeLock, releaseNativeLock } from '../../../domains/owner-native/native-lock.js';
import {
...
import { createNativeRunnerChannel } from '../../../domains/owner-native/native-runner-protocol.js';
...
describe('Owner Native CLI dispatcher', () => {
  let projectRoot: string;
  const projectArgs = () => ['--project-root', projectRoot] as const;

  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(await fs.realpath(os.tmpdir()), 'owner-native-cli-'));
  ...
  it('creates v4 portable artifacts without scanning or snapshotting an unrelated large project file', async () => {
    ...
    expect(created).toMatchObject({
      ...
      data: {
        ...
        continuation: {
          schema: 'owner.native.continuation.v2',
    ...
    expect(yaml).toContain('schema: owner.native.v4');
    ...
    expect(local).toMatchObject({
      schema: 'owner.native.local-execution.v4',
  ...
  it('persists worktree binding and reports drift from the linked worktree', async () => {
    ...
    const secondary = path.join(
      ...
      `owner-native-cli-worktree-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    try {
  ...
  it('keeps a linked worktree authoritative when a host passes the primary root', async () => {
    ...
    const secondary = path.join(
      await fs.realpath(os.tmpdir()),
      `owner-native-cli-root-route-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    try {
  ...
  it('runs a custom-root change through every CLI-reachable stable boundary', async () => {
    ...
    expect(json(await runNativeCli(['status', '--json', ...projectArgs()])).data).toMatchObject({
      schema: 'owner.native.status-page.v2',
      total: 1,
      items: [
    ...
    expect(
      ...
      state: { schema: 'owner.native.v4', language: 'zh-CN', phase: 'shape' },
      brief,
  ...
  it('creates default config from new and keeps Classic paths untouched', async () => {
    ...
    expect(result).toMatchObject({
      exitCode: 0,
      data: { schema: 'owner.native.v4', name: 'default-root', phase: 'shape' },
    });
  ...
  it('returns the v2 confirmation continuation when Shape has not been confirmed', async () => {
    ...
    expect(result).toMatchObject({
      ...
      data: {
        state: { schema: 'owner.native.v4', phase: 'shape' },
        continuation: {
          schema: 'owner.native.continuation.v2',
          disposition: 'continue',
          action: 'confirm-shape',

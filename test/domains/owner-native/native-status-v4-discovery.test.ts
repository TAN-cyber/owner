...
import { listDiscoveredNativeStatusPage } from '../../../domains/owner-native/native-status-discovery.js';
...
async function createRepository(): Promise<RepositoryFixture> {
  const root = await fs.mkdtemp(
    path.join(await fs.realpath(os.tmpdir()), 'owner-native-v4-discovery-'),
  );

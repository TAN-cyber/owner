...
import { readNativeBoundedTextFile } from '../../../domains/owner-native/native-bounded-file.js';

describe('Native bounded artifact reader', () => {
  let root: string;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-artifact-'));
  });
  ...
  it('rejects a symlink or junction in the parent chain', async () => {
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-artifact-outside-'));
    try {

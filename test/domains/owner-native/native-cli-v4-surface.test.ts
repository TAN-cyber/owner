...
import { readNativeLocalExecution } from '../../../domains/owner-native/native-local-execution.js';
import { nativeProjectPaths } from '../../../domains/owner-native/native-paths.js';
import { nativeLocalExecutionFile } from '../../../domains/owner-native/native-portable-runtime.js';
...
describe('Native v4 public CLI surface', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(
      path.join(await fs.realpath(os.tmpdir()), 'owner-native-v4-cli-'),
    );
    ...
    expect(built).toMatchObject({
      ...
      data: {
        ...
        continuation: {
          ...
          commandArgs: ['owner', 'native', 'next', name, '--runner-input', '<temporary-json-file>'],
        },
      },
    });

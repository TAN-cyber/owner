...
import { nativeEvidenceCommand } from '../../../domains/owner-native/native-evidence-command.js';
...
describe('Native evidence command', () => {
  ...
  it('formats canonical evidence entries from a file', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-evidence-command-'));
    roots.push(root);

...
} from '../../../domains/owner-native/native-acceptance.js';
...
describe('Native verification evidence runtime', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-native-verification-runtime-'));
    await fs.mkdir(path.join(projectRoot, 'src'), { recursive: true });
    await fs.mkdir(path.join(projectRoot, 'domains', 'owner-native'), { recursive: true });
    await fs.writeFile(path.join(projectRoot, 'src', 'feature.ts'), 'export const value = 1;\n');
    await fs.writeFile(
      path.join(projectRoot, 'domains', 'owner-native', 'policy.ts'),
...
<!-- owner-native:acceptance-evidence:end -->
`,
    );
    await expect(
      prepareNativeVerificationEvidence({

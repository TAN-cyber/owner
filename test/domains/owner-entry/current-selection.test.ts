...
describe('shared Owner current selection', () => {
  let root: string;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-selection-v2-'));
  });
  ...
  it('writes and reads one workflow owner atomically', async () => {
    ...
    await writeOwnerCurrentSelection(root, selection);

    await expect(readOwnerCurrentSelection(root)).resolves.toEqual({
      status: 'selected',
      selection,
      legacy: false,
    });
  ...
  it('fails closed on malformed and invalid workflow records', async () => {
    ...
    await expect(readOwnerCurrentSelection(root)).rejects.toThrow('invalid JSON');
  ...
  it('bounds and regular-file checks the shared selection before parsing', async () => {
    ...
    await expect(readOwnerCurrentSelection(root)).rejects.toThrow('exceeds 16384 bytes');

    await fs.rm(file);
    await fs.mkdir(file);
    await expect(readOwnerCurrentSelection(root)).rejects.toThrow('regular file');
  });
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await expect(readOwnerCurrentSelection(root)).rejects.toThrow(/regular file|symbolic link/);
    },
  );
  ...
  it('never returns content read past the byte limit when the file is swapped for a bigger one mid-read', async () => {
    ...
    const pending = readOwnerCurrentSelection(root);

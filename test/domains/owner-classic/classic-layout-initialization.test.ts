...
describe('Classic layout initialization safety', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-layout-init-'));
  });
  ...
  it('recovers a prior owned initialization across processes without allowing unmanaged roots', async () => {
    ...
    await expect(
      fs.stat(path.join(projectRoot, '.owner', 'classic-init-ownership.json')),
  ...
  it('quarantines an unchanged manifest-bound root without deleting its contents', async () => {
    ...
    const journal = JSON.parse(
      await fs.readFile(path.join(projectRoot, '.owner', 'classic-init-ownership.json'), 'utf8'),
  ...
  it('initializes Classic when the project filesystem does not support hard links', async () => {
    ...
    try {
      ...
      await expect(
        fs.readFile(path.join(projectRoot, '.owner', 'classic-init-ownership.json'), 'utf8'),
  ...
  async function publishSuccessorJournal(id: string): Promise<void> {
    ...
    await fs.writeFile(
      path.join(ownerDir, 'classic-init-ownership.json'),
  ...
  it('rejects a fresh project when the alternate docs root is a directory link', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-init-outside-'));
    try {
      await fs.mkdir(path.join(projectRoot, 'docs'), { recursive: true });
      try {
        await fs.symlink(
  ...
  it('rejects a configured OpenSpec root that crosses a junction', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-layout-outside-'));
    try {
      await writeClassicConfig('docs');
      await fs.mkdir(path.join(projectRoot, 'docs'), { recursive: true });
      try {

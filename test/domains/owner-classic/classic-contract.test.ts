...
function runScript(
  ...
  const env = {
    ...
    LANG: 'C',
    LC_ALL: 'C',
    OWNER_RUNTIME_CLASSIC_ROOT: path.resolve('assets', 'skills', 'owner', 'runtime', 'classic'),
    OWNER_CLASSIC_SKILL_ROOT: path.resolve('assets', 'skills', 'owner', 'runtime', 'classic'),
  };
...
async function observeState(
  ...
  const root = await fs.mkdtemp(
    path.join(await fs.realpath(os.tmpdir()), `owner-classic-${profile}-`),
  );

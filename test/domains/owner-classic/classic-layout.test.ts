...
async function project(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-layout-'));
  roots.push(root);
...
async function externalDirectory(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-layout-outside-'));
  roots.push(root);
  return root;
}
...
describe('Classic artifact layout', () => {
  ...
  it('fails closed when unrelated project-config YAML is malformed', async () => {
    ...
    await expect(readClassicArtifactLayout(root)).rejects.toThrow('Invalid .owner/config.yaml');
  });
  ...
  it('allows read-only layout inspection while a root move journal is pending', async () => {
    ...
    await fs.mkdir(path.join(root, 'openspec'), { recursive: true });
    await fs.writeFile(path.join(root, '.owner', 'classic-root-move.json'), '{}\n');

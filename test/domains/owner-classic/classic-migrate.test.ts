...
import { ensureClassicRun } from '../../../domains/owner-classic/classic-migrate.js';
import {
  readClassicState,
  writeClassicState,
} from '../../../domains/owner-classic/classic-store.js';
import type { ClassicState } from '../../../domains/owner-classic/classic-state.js';
import {
  readArtifacts,
  readCheckpoint,
  readContext,
...
describe('Classic legacy migration', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-migrate-'));
  ...
  it('rejects migration when the handoff parent is replaced during the protected read', async () => {
    ...
    const outsideRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), 'owner-classic-migrate-handoff-outside-'),
    );

...
describe.sequential('Classic protected path consumers', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-protected-'));
    outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-outside-'));
    previousCwd = process.cwd();
    await fs.mkdir(path.join(projectRoot, '.git'));
    await writeFile(path.join(projectRoot, '.owner', 'config.yaml'), CLASSIC_CONFIG);
    await writeFile(
  ...
  it('does not write a Classic artifact through a parent junction replaced before commit', async () => {
    const protectedPaths =
      (await import('../../../domains/owner-classic/classic-protected-path.js')) as unknown as {

...
const classicRuntimeRoot = path.resolve('assets', 'skills', 'owner', 'runtime', 'classic');
...
describe('owner script contracts', () => {
  ...
  it('keeps owner-hook-guard blocked messages in English', async () => {
    const hookSource = await fs.readFile(
      path.resolve('domains', 'owner-classic', 'classic-hook-guard.ts'),
      'utf-8',
    );
...
describe('owner scripts', () => {
  ...
  it('keeps hook guard read-only when OWNER_RUNTIME_CLASSIC_ROOT is configured', async () => {
    const init = runNode(tmpDir, stateScript, ['init', 'runtime-root', 'full'], {
      OWNER_RUNTIME_CLASSIC_ROOT: classicRuntimeRoot,
      OWNER_CLASSIC_SKILL_ROOT: '',
    });
  ...
  it('keeps OWNER_CLASSIC_SKILL_ROOT as a compatibility fallback', async () => {
    const init = runNode(tmpDir, stateScript, ['init', 'legacy-root', 'full'], {
      OWNER_RUNTIME_CLASSIC_ROOT: '',
      OWNER_CLASSIC_SKILL_ROOT: classicRuntimeRoot,
    });
  ...
  it('falls back to the embedded Classic runtime package when installed script assets omit internal runtime files', async () => {
    ...
    expect(JSON.parse(runState)).toMatchObject({ skill: 'owner-classic' });
  }, 20_000);
  ...
  it('falls back to the global Owner language when project config omits language', async () => {
    ...
    await writeFile(path.join(fakeHome, '.owner', 'config.yaml'), 'classic:\n  language: zh-CN\n');
  ...
  it('excludes fenced code blocks from the language dominance check', async () => {
    ...
    const bodyWithLargeCodeBlock = [
      ...
      '',
      '```bash',
      'npx vitest run test/domains/owner-classic/owner-scripts.test.ts --reporter verbose --coverage --watch=false',

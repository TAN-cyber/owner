...
import { readClassicConfigValue } from '../../../domains/owner-classic/classic-project-config.js';
...
describe('Classic project config', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-config-project-'));
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-config-home-'));
  });
  ...
  it('rejects a linked project config instead of following it', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-config-outside-'));
    try {

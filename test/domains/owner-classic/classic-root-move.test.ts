...
import { formatClassicRootMoveReport } from '../../../domains/owner-classic/classic-root-command.js';
import {
...
describe('Classic root move', () => {
  ...
  async function writeJournal(
    ...
    const staging = String(
      overrides.staging ?? `.owner/transactions/classic-root-move/${transactionId}/openspec`,
    );
    ...
    const journal = {
      schema: 'owner.classic-root-move.v2',
      id: transactionId,
      stage,
      source,
      target,
  ...
  async function externalDirectory(): Promise<string> {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-root-move-outside-'));
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-root-move-'));
  ...
  it('derives and locks the executable plan during apply', async () => {
    ...
    await expect(
      fs.stat(path.join(projectRoot, '.owner', 'classic-root-move.json')),
  ...
  it('does not let a stale apply replace or unlink a successor published after quarantine', async () => {
    ...
    const persisted = JSON.parse(
      await fs.readFile(path.join(projectRoot, '.owner', 'classic-root-move.json'), 'utf8'),
    ) as { id: string; stage: string };
  ...
  it('does not update the migration journal through a replaced .owner parent', async () => {
    ...
    await expect(fs.readdir(externalOwner)).resolves.toEqual(['marker.txt']);
    const persisted = JSON.parse(
      await fs.readFile(path.join(originalOwner, 'classic-root-move.json'), 'utf8'),
    ) as { stage: string };
  ...
  it('rejects a journal whose parent becomes an external junction after inspection', async () => {
    ...
    await fs.writeFile(
      path.join(externalOwner, 'classic-root-move.json'),
      '{"external":"untrusted"',
      'utf8',
    );
    let replaced = false;
    ...
    await expect(
      fs.readFile(path.join(externalOwner, 'classic-root-move.json'), 'utf8'),
    ).resolves.toBe('{"external":"untrusted"');
  });

  it('rejects a root-move journal that exceeds the 16 MiB read budget', async () => {
    const journal = path.join(projectRoot, '.owner', 'classic-root-move.json');
  ...
  it('continues a verified ready transaction only when explicitly requested', async () => {
    ...
    expect((await inspectClassicRootMove(projectRoot))?.staging).toBe(
      `.owner/transactions/classic-root-move/${transactionId}/openspec`,
    );

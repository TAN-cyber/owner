...
const chinesePackageRoot = path.resolve('assets', 'skills-zh', 'owner', 'runtime', 'classic');
const englishPackageRoot = path.resolve('assets', 'skills', 'owner', 'runtime', 'classic');
const stableSteps = [
  'full.open',
  'full.design.handoff',
...
describe('Chinese owner-classic package', () => {
  ...
  it('declares every invoked public Skill and never invokes itself', async () => {
    ...
    expect(invoked).not.toContain('owner-classic');
  });
...
describe('English owner-classic package', () => {

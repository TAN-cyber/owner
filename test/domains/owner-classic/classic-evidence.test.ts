...
import {
  ...
  evidenceSatisfied,
} from '../../../domains/owner-classic/classic-evidence.js';
import type { ClassicStateProjection } from '../../../domains/owner-classic/classic-state.js';
...
describe('Classic evidence collection', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-evidence-'));
  ...
  it('does not satisfy linked evidence with a project traversal pointer', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-outside-'));
    try {
  ...
  it('resolves a docs-layout handoff pointer inside the same configured docs archive', async () => {
    ...
    projection.classic!.handoffContext = 'docs/openspec/changes/demo/.owner/handoff/context.json';
  ...
  it.each([
    ...
    projection.classic!.handoffContext = `openspec/changes/${pointerChange}/.owner/handoff/context.json`;
  ...
  it('rejects traversal in a legacy handoff pointer instead of applying archive fallback', async () => {
    ...
    projection.classic!.handoffContext = 'openspec/changes/demo/.owner/handoff/../../outside.json';
  ...
  it('rejects a junction in the mapped archived handoff path', async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-classic-handoff-outside-'));
    changeDir = path.join(projectRoot, 'docs', 'openspec', 'changes', 'archive', '2026-06-14-demo');
    try {
      await fs.rm(path.join(projectRoot, 'openspec'), { recursive: true, force: true });
      await fs.mkdir(path.join(changeDir, '.owner'), { recursive: true });

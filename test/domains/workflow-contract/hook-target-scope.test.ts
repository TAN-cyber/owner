...
import { scopeOwnerHookTargets } from '../../../domains/workflow-contract/hook-target-scope.js';

describe('scopeOwnerHookTargets', () => {
  let root: string;
  let externalRoot: string;

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-hook-scope-project-'));
    externalRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-hook-scope-external-'));
  });
  ...
  it('treats a project path redirected through a symlink or junction as external', async () => {
    ...
    await expect(
      scopeOwnerHookTargets(root, [path.join('linked-memory', 'note.md')]),
    ).resolves.toEqual({
      projectTargets: [],

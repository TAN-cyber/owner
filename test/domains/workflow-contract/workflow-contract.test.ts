...
describe('workflow contract normalization', () => {
  ...
  it('keeps generated project-file reads bounded and rejects a post-inspection symlink swap', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-generated-config-race-'));
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-generated-config-outside-'));
    ...
    await fs.writeFile(configPath, 'schema: owner.project.v1\n', 'utf8');
  ...
  it('keeps YAML parsing ownership out of project-config consumers', async () => {
    const consumers = [
      ...
      'domains/owner-entry/project-status.ts',
      'domains/owner-entry/resume-probe.ts',
  ...
  it('rejects protected project paths that traverse or cross a junction', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-protected-path-'));
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-protected-outside-'));
    try {
      await expect(
  ...
  it('does not write project config through a linked .owner directory', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-write-'));
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-write-outside-'));
    try {
      try {
        await fs.symlink(
          outsideRoot,
  ...
  it('publishes project config when the project filesystem does not support hard links', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-copy-publish-'));
    const linkSpy = vi
      .spyOn(fs, 'link')
  ...
  it('does not replace a project config file symlink', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-link-'));
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-link-outside-'));
    const outsideConfig = path.join(outsideRoot, 'config.yaml');
    try {
  ...
  it('does not commit project config through a parent junction replaced after inspection', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-parent-race-'));
    const outsideRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), 'owner-config-parent-race-outside-'),
    );
    const linkProbe = path.join(projectRoot, 'link-probe');
    try {
      try {
      ...
      await expect(
        writeWorkflowProjectConfigSource(
          ...
          {
            beforeCommit: async () => {
              const managedDirectory = path.join(projectRoot, '.owner');
              ...
              await fs.rename(managedDirectory, path.join(projectRoot, '.owner-held'));
  ...
  it('does not read project config through a symlink', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-read-link-'));
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-read-outside-'));
    const outsideConfig = path.join(outsideRoot, 'config.yaml');
    try {
      await fs.mkdir(path.join(projectRoot, '.owner'), { recursive: true });
      await fs.writeFile(
  ...
  it.each(['existing', 'missing'] as const)(
    ...
    async (initialState) => {
      const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-identity-'));
      try {
        ...
        await fs.writeFile(path.join(projectRoot, '.owner', 'config.yaml'), externalSource, 'utf8');
  ...
  it('does not overwrite a successor config published after the expected config is quarantined', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-publish-race-'));
    try {
  ...
  it('recovers the previous config after a process exits with it quarantined', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-crash-recovery-'));
    try {
      await writeWorkflowProjectConfig(projectRoot, defaultWorkflowProjectConfig('before-crash'));
      const previous = await fs.readFile(path.join(projectRoot, '.owner', 'config.yaml'), 'utf8');
      ...
      expect(
        (await fs.readdir(path.join(projectRoot, '.owner'))).filter(
  ...
  it('does not unlink a same-path successor published while an owned transaction file is cleaned', async () => {
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-config-cleanup-race-'));
    try {
      await writeWorkflowProjectConfig(
        projectRoot,
  ...
  it('normalizes the Owner five-phase template into Nodes with Output Schemas', () => {
    const workflow = normalizeWorkflowDefinition(
      builtinOwnerFivePhaseWorkflow({
        name: 'team-owner',
        goal: 'Use the project component library in Owner execution.',
      }),
    );

    expect(workflow.protocol.schemaVersion).toBe(1);
    expect(workflow.protocol.kind).toBe('owner-five-phase-overlay');
    expect(workflow.protocol.nodes.map((node) => node.id)).toEqual([
      'open',
      'design',
      'plan',
    ...
    expect(workflow.protocol.outputSchemas.map((schema) => schema.id)).toEqual(
      expect.arrayContaining(['owner.plan.v1', 'owner.handoff.v1', 'owner.review.v1']),
    );
    expect(workflow.protocol.state).toEqual({
      kind: 'owner-overlay',
  ...
  it('allows required Skill calls without replacing Node implementations', () => {
    ...
    expect(workflow.protocol.nodes.find((node) => node.id === 'execute')).toMatchObject({
      implementation: { skill: 'owner-build', operation: 'default' },
  ...
  it('normalizes Required Skill Call and augmentation enforcement levels', () => {
    const workflow = normalizeWorkflowDefinition({
      ...builtinOwnerFivePhaseWorkflow({
        name: 'enforced-owner',
        goal: 'Require and augment a Owner Node.',
      }),
      nodes: {
        execute: {
  ...
  it('attaches custom Output Schemas through Node patches', () => {
    const workflow = normalizeWorkflowDefinition({
      ...
      nodes: {
        design: { outputSchemas: ['owner.grill-me.v1'] },
        plan: { outputSchemas: ['owner.grill-me.v1'] },
        review: { outputSchemas: ['owner.grill-me.v1'] },
      },
      outputSchemas: [
        {
          id: 'owner.grill-me.v1',
    ...
    expect(workflow.protocol.evals[0]?.requiredOutputSchemas).toEqual(
      expect.arrayContaining(['owner.grill-me.v1']),
    );
  });
  ...
  it('rejects ordinary override of Owner control Nodes', () => {
    expect(() =>
      normalizeWorkflowDefinition({
        ...builtinOwnerFivePhaseWorkflow({
          name: 'unsafe-owner',
        ...
        nodes: {
          execute: {
            ...
            satisfies: ['owner.execution-evidence.v1'],
          },
        },
      }),
  ...
  it('hashes protocols deterministically', () => {
    const workflow = normalizeWorkflowDefinition(
      builtinOwnerFivePhaseWorkflow({ name: 'hashable-owner', goal: 'Hash protocol.' }),
    );

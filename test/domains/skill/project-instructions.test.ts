...
import {
  installOwnerProjectInstructions,
  removeOwnerProjectInstructions,
  syncOwnerProjectInstructions,
} from '../../../domains/skill/project-instructions.js';

let tmpDir: string;

describe('Owner project instructions', () => {
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-project-instructions-'));
  });

  it('creates AGENTS.md and CLAUDE.md with managed XML blocks', async () => {
    const result = await installOwnerProjectInstructions(tmpDir, 'zh');
    ...
    for (const name of ['AGENTS.md', 'CLAUDE.md']) {
      ...
      expect(content).toContain('owner.resume_probe.v2');
  ...
  it('preserves existing user rules and updates only the managed block', async () => {
    ...
    await installOwnerProjectInstructions(tmpDir, 'zh');

    const content = await fs.readFile(agents, 'utf8');
    expect(content.startsWith('# User Rules\n\n必须中文回答。')).toBe(true);
    expect(content.match(/<owner-ambient-resume>/gu)).toHaveLength(1);
    expect(content).toContain('开始处理需要改动或调查的任务前');
  ...
  it.each([
    [
      'zh' as const,
      '用户通过宿主明确调用任意 Owner Skill',
      '不要运行 resume probe',
      '当前请求未明确调用 Owner Skill',
    ],
    [
      'en' as const,
      'user explicitly invokes any Owner Skill through the host',
      'do not run the resume probe',
      'current request did not explicitly invoke a Owner Skill',
    ],
  ])('gives explicit Owner Skill invocation precedence in %s', async (language, ...markers) => {
    await installOwnerProjectInstructions(tmpDir, language);
  ...
  it('removes only the managed block', async () => {
    ...
    const result = await removeOwnerProjectInstructions(tmpDir);

    expect(result.removed).toBeGreaterThan(0);
    expect(await fs.readFile(agents, 'utf8')).toContain('Keep me.');
    expect(await fs.readFile(agents, 'utf8')).not.toContain('<owner-ambient-resume>');
  });
  ...
  it('removes managed blocks when Ambient Resume is disabled', async () => {
    ...
    const result = await syncOwnerProjectInstructions(tmpDir, 'en', false);
  ...
  it('updates the in-project target without replacing an AGENTS.md alias', async () => {
    ...
    const result = await installOwnerProjectInstructions(tmpDir, 'en');

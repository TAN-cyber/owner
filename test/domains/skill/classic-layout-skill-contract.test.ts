...
const LANGUAGE_CASES = [
  {
    ...
    ruleFiles: ['owner-phase-guard.md', 'owner-workflow-guard.md'],
    allowedLayoutDescriptionLines: new Set([
      '- 新 Classic 项目默认使用 `docs/openspec/`。',
      '- 缺少 `classic.artifact_layout` 时默认使用 `docs/openspec/`；`owner update` 检测到已有根目录 `openspec/` 产物时会显式补为 `legacy`，不会移动产物。',
    ]),
  },
  {
    label: 'English',
    languageRoot: 'skills',
    ruleFiles: ['owner-phase-guard.en.md', 'owner-workflow-guard.en.md'],
    allowedLayoutDescriptionLines: new Set([
...
const BARE_OPENSPEC_COMMAND =
  /(?<!owner classic )\bopenspec\s+(?:--version|<args\.\.\.>|[a-z][a-z0-9-]*)/u;
...
async function classicGuidanceFiles(
  ...
  files.push(...(await markdownFiles(path.join(assetRoot, 'owner-classic', 'reference'))));
  files.push(...ruleFiles.map((rule) => path.resolve('assets', 'skills', 'owner', 'rules', rule)));
  return files;
}
  ...
  it.each(['skills-zh', 'skills'])(
    ...
    async (languageRoot) => {
      const reference = await fs.readFile(
        path.resolve('assets', languageRoot, 'owner-classic', 'reference', 'classic-layout.md'),
        'utf8',
      );
      expect(reference).toContain('owner classic root show');
      expect(reference).toContain('owner classic openspec -- <args...>');
      expect(reference).toContain('openSpecRoot');
      expect(reference).toContain('superpowersRoot');
      expect(reference).toContain('owner classic root move docs --dry-run');
      ...
      for (const skill of CLASSIC_SKILLS) {
        ...
        expect(source, skill).toContain('owner-classic/reference/classic-layout.md');
      }
    },
  );

  describe.each(LANGUAGE_CASES)(
    '$label Owner-owned guidance',

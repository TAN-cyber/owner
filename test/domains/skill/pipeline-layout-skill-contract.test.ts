...
const BARE_OPENSPEC_COMMAND =
  /(?<!owner pipeline )\bopenspec\s+(?:--version|<args\.\.\.>|[a-z][a-z0-9-]*)/u;
...
async function pipelineGuidanceFiles(
  languageRoot: string,
  ruleFiles: readonly string[],
): Promise<string[]> {
  const assetRoot = path.resolve('assets', languageRoot);
  const files = PIPELINE_SKILLS.map((skill) => path.join(assetRoot, skill, 'SKILL.md'));
  files.push(...(await markdownFiles(path.join(assetRoot, 'owner-pipeline', 'reference'))));
  files.push(...ruleFiles.map((rule) => path.resolve('assets', 'skills', 'owner', 'rules', rule)));
  return files;
}

describe('Pipeline layout Skill contract', () => {
  it.each(['skills-zh', 'skills'])(
    'ships the layout resolver and adapter protocol in %s',
    async (languageRoot) => {
      const reference = await fs.readFile(
        path.resolve('assets', languageRoot, 'owner-pipeline', 'reference', 'pipeline-layout.md'),
        'utf8',
      );
      expect(reference).toContain('owner pipeline root show');
      expect(reference).toContain('owner pipeline openspec -- <args...>');
      expect(reference).toContain('openSpecRoot');
      expect(reference).toContain('superpowersRoot');
      expect(reference).toContain('owner pipeline root move docs --dry-run');
      ...
      for (const skill of PIPELINE_SKILLS) {
        ...
        expect(source, skill).toContain('owner-pipeline/reference/pipeline-layout.md');
      }
    },
  );
  ...
  describe.each(LANGUAGE_CASES)(
    ...
    ({ languageRoot, ruleFiles, allowedLayoutDescriptionLines }) => {
      it('uses the adapter and resolver-backed logical roots everywhere', async () => {
        const files = await pipelineGuidanceFiles(languageRoot, ruleFiles);
        const violations: string[] = [];

        for (const file of files) {
          const source = await fs.readFile(file, 'utf8');
          const isLayoutReference =
            path.basename(file) === 'pipeline-layout.md' &&

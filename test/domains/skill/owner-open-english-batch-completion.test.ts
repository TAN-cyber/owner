...
describe('owner-open English batch completion protocol', () => {
  it('uses the OpenSpec status graph instead of a hard-coded artifact order', async () => {
    const skill = await readFile(
      path.resolve('assets', 'skills', 'owner-open', 'SKILL.md'),
      'utf8',
    );
  ...
  it('requires every split item to pass the CLI completion checks', async () => {
    ...
    expect(skill).toContain('owner state check <name> design');
  });

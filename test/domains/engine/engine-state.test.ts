...
describe('engine state projection', () => {
  ...
  beforeEach(async () => {
    changeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-engine-state-'));
    outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-engine-state-outside-'));
    await fs.writeFile(
  ...
  it('creates .owner/run-state.json when the state file is absent', async () => {
  ...
  it('does not replace .owner/run-state.json when writing fails', async () => {

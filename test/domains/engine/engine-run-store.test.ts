...
describe('run store', () => {
  ...
  beforeEach(async () => {
    changeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-run-store-'));
    outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-run-store-outside-'));
  });
  ...
  it('appends trajectory events and atomically round-trips run files', async () => {
    ...
    await appendTrajectory(changeDir, '.owner/trajectory.jsonl', event);
    await writeArtifacts(changeDir, '.owner/artifacts.json', { report: 'report.md' });
    await writeContext(changeDir, '.owner/context.md', '# Context\n');
    await writePendingAction(changeDir, '.owner/pending-action.json', action);
    await writeCheckpoint(changeDir, '.owner/checkpoint.json', checkpoint);

    expect(await readArtifacts(changeDir, '.owner/artifacts.json')).toEqual({
      report: 'report.md',
    });
    expect(await readPendingAction(changeDir, '.owner/pending-action.json')).toEqual(action);
    await clearPendingAction(changeDir, '.owner/pending-action.json');
    expect(await readPendingAction(changeDir, '.owner/pending-action.json')).toBeNull();
    expect(await readContext(changeDir, '.owner/context.md')).toBe('# Context\n');
    expect(await readCheckpoint(changeDir, '.owner/checkpoint.json')).toEqual(checkpoint);
    expect(await readTrajectory(changeDir, '.owner/trajectory.jsonl')).toEqual([event]);
  });

  it('returns empty recovery values when optional Run files do not exist', async () => {
    expect(await readArtifacts(changeDir, '.owner/artifacts.json')).toEqual({});
    expect(await readContext(changeDir, '.owner/context.md')).toBeNull();
    expect(await readPendingAction(changeDir, '.owner/pending-action.json')).toBeNull();
    expect(await readCheckpoint(changeDir, '.owner/checkpoint.json')).toBeNull();
    expect(await readTrajectory(changeDir, '.owner/trajectory.jsonl')).toEqual([]);
  });

  it('reports the malformed Trajectory line during recovery', async () => {
    const file = path.join(changeDir, '.owner', 'trajectory.jsonl');
    await fs.mkdir(path.dirname(file), { recursive: true });
    await fs.writeFile(file, '{"sequence":1}\nnot-json\n');

    await expect(readTrajectory(changeDir, '.owner/trajectory.jsonl')).rejects.toThrow(
  ...
  it('does not append a trajectory through a parent replaced before commit', async () => {
    ...
    await appendTrajectory(changeDir, '.owner/trajectory.jsonl', original);
    const parent = path.join(changeDir, '.owner');
    ...
    try {
      await expect(
        appendWithHook(changeDir, '.owner/trajectory.jsonl', next, {
          beforeCommit: async () => {
  ...
  it('rejects a checkpoint read when its parent changes after open', async () => {
    ...
    try {
      await expect(
        readWithHooks(changeDir, '.owner/checkpoint.json', {
          hooks: {
            afterOpen: async () => {
              await fs.rename(parent, held);
              await fs.writeFile(
  ...
  it('does not clear a pending action through a replaced parent', async () => {
    ...
    try {
      await expect(
        clearWithHook(changeDir, '.owner/pending-action.json', {
          beforeRemove: async () => {
            await fs.rename(parent, held);

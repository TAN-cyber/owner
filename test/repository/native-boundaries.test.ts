...
describe('Owner Native isolation boundaries', () => {
  it('keeps the Native domain independent from Classic and OpenSpec execution', async () => {
    const files = (await filesUnder(path.resolve('domains', 'owner-native'))).filter((file) =>
      file.endsWith('.ts'),
    );
    const source = await combined(files);

    expect(source).not.toMatch(/\bfrom\s+['"][^'"]*owner-classic[^'"]*['"]/u);
    ...
    expect(new Set(source.match(/\.owner\/[A-Za-z0-9._/-]+/gu) ?? [])).toEqual(
      new Set(['.owner/config.yaml', '.owner/current-change.json', '.owner/runtime/native']),
    );
  });

  it('ships a self-contained Skill and runtime with no external workflow invocation', async () => {
    const skillFiles = [
      ...(await filesUnder(path.resolve('assets', 'skills', 'owner-native'))),
      ...(await filesUnder(path.resolve('assets', 'skills-zh', 'owner-native'))),
  ...
  it('keeps both workflow domains independent below the entry seam', async () => {
    const [nativeSource, classicSource] = await Promise.all([
      combined(
        (await filesUnder(path.resolve('domains', 'owner-native'))).filter((file) =>
          file.endsWith('.ts'),
        ),
      ),
      combined(
        (await filesUnder(path.resolve('domains', 'owner-classic'))).filter((file) =>
          file.endsWith('.ts'),
        ),
      ),
    ]);

    expect(nativeSource).not.toMatch(/\bfrom\s+['"][^'"]*owner-classic[^'"]*['"]/u);
    expect(classicSource).not.toMatch(/\bfrom\s+['"][^'"]*owner-native[^'"]*['"]/u);

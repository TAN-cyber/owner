...
describe('CI workflows', () => {
  it('runs the required CI contract for every pull request', async () => {
    ...
    expect(workflow).not.toMatch(/dashboard|\beval\b|uv sync|pytest/iu);
  ...
  it('runs dependency review and CodeQL with least-privilege permissions', async () => {
    ...
    expect(dependabot).not.toContain('package-ecosystem: pip');
    expect(dependabot).toContain('package-ecosystem: github-actions');
  });

  it('defines PR title linting with Owner-specific semantic scopes', async () => {

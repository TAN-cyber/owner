...
describe('Native runtime release asset', () => {
  it('publishes the Native Skill, references, and runtime from the manifest', () => {
    for (const relative of [
      ...
      'owner-native/reference/artifacts.md',
      'owner-native/reference/clarification.md',
      'owner-native/reference/commands.md',
      'owner-native/reference/recovery.md',
      'owner-native/reference/workspace.md',
  ...
  it('ships one fresh self-contained Node runtime', async () => {
    ...
    expect(source).not.toMatch(/domains\/owner-classic|openspec|superpowers|requiredSkillCalls/iu);
    ...
    expect(source).not.toContain('owner.native.controller-trust-store.v1');
    expect(source).not.toContain('owner.native.creation-authorization.v1');
    expect(source).not.toContain('owner.native.review-trust-policy.v2');
  ...
  it('ships one self-contained bundle per command launcher', async () => {
    const scriptsDir = path.resolve('assets', 'skills', 'owner-native', 'scripts');
    ...
    const commandScripts = [
      'owner-native-hook-guard.mjs',
      'owner-native-init.mjs',
      'owner-native-root.mjs',
      'owner-native-new.mjs',
      'owner-native-spec.mjs',
      'owner-native-show.mjs',
      'owner-native-status.mjs',
      'owner-native-select.mjs',
      'owner-native-next.mjs',
      'owner-native-archive.mjs',
      'owner-native-doctor.mjs',
    ];
    for (const script of commandScripts) {
      const source = await fs.readFile(path.join(scriptsDir, script), 'utf8');
      expect(source.startsWith('#!/usr/bin/env node\n')).toBe(true);
      expect(source).not.toMatch(/from\s+['"]\.\/owner-native-runtime\.mjs['"]/u);
    }
    for (const retired of [
      'owner-native-checkpoint.mjs',
      'owner-native-check.mjs',
      'owner-native-evidence.mjs',
      'owner-native-receipt.mjs',
    ]) {
      expect(manifest.skills).not.toContain(`owner-native/scripts/${retired}`);
  ...
  it('keeps command syntax and defaults in CLI help instead of Skill references', async () => {
    const help = await fs.readFile(
      path.resolve('domains', 'owner-native', 'native-cli-help.ts'),
      'utf8',
    );
    const english = await fs.readFile(
      path.resolve('assets', 'skills', 'owner-native', 'reference', 'commands.md'),
      'utf8',
    );
    const chinese = await fs.readFile(
      path.resolve('assets', 'skills-zh', 'owner-native', 'reference', 'commands.md'),
      'utf8',
    );

    expect(help).toContain('owner native new <change-name> [--language en|zh-CN]');
    expect(help).toContain('defaults to docs');
    for (const reference of [english, chinese]) {
      expect(reference).toContain('owner native <command> --help');
      expect(reference).not.toContain('owner native new <change-name> [--language en|zh-CN]');
    }
  });

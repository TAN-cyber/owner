...
describe('Native self-contained runtime boundary (legacy)', () => {
  it.skip('keeps process execution inside the bounded snapshot and receipt providers', async () => {
    const root = path.resolve('domains', 'owner-native');
    ...
    expect(receiptRuntime).toContain('OWNER_NATIVE_COMMAND_PAYLOAD');
    ...
    expect(combined).not.toMatch(
      /(?:^|[/'"])(?:owner-classic|openspec)(?:[/'"]|$)|superpowers|assets\/skills|\.agents\/skills|\.owner\/skills/imu,
    );
  });

  it.skip('pins Native snapshot processes to read-only Git arguments and bounded termination', async () => {
    const source = await fs.readFile(
      path.resolve('domains', 'owner-native', 'native-snapshot.ts'),
      'utf8',
    );
  ...
  it('routes v4 checks through the platform process adapter', async () => {
    const executor = await fs.readFile(
      path.resolve('domains', 'owner-native', 'native-check-executor.ts'),
      'utf8',
    );
    const runtime = await fs.readFile(
      path.resolve('domains', 'owner-native', 'native-portable-runtime.ts'),
      'utf8',
    );

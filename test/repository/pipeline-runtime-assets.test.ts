...
const pipelineReferenceDirectory = path.resolve('assets', 'skills', 'owner-pipeline', 'reference');
...
describe('Pipeline runtime release assets', () => {
  it('ships bilingual Pipeline references under the Pipeline entry', async () => {
    const manifest = await readJson<AssetsManifest>(path.resolve('assets', 'manifest.json'));
    const englishFiles = (await fs.readdir(pipelineReferenceDirectory))
      .filter((fileName) => fileName.endsWith('.md'))
      .sort();
    const chineseFiles = (
      await fs.readdir(path.resolve('assets', 'skills-zh', 'owner-pipeline', 'reference'))
    )
      .filter((fileName) => fileName.endsWith('.md'))
      .sort();
    const manifestReferences = manifest.skills
      .filter((skillPath) => skillPath.startsWith('owner-pipeline/reference/'))
      .sort();

    expect(chineseFiles).toEqual(englishFiles);
    expect(manifestReferences).toEqual(
      englishFiles.map((fileName) => `owner-pipeline/reference/${fileName}`),
    );
  ...
  it('lists every shipped Pipeline script in the assets manifest', async () => {
    const manifest = await readJson<AssetsManifest>(path.resolve('assets', 'manifest.json'));

...
describe('Git worktree inspection', () => {
  ...
  beforeEach(async () => {
    ...
    primary = await fs.mkdtemp(path.join(temporaryRoot, 'owner-git-primary-'));
    secondary = path.join(
      temporaryRoot,
      `owner-git-secondary-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('returns a stable non-Git result outside a repository', async () => {
    const outside = await fs.mkdtemp(path.join(await fs.realpath(os.tmpdir()), 'owner-no-git-'));
    try {

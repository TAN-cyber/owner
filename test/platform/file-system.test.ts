...
describe('file-system utils', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `owner-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe('copyFile', () => {
    ...
    it('follows broken symlinks by resolving readlink target', async () => {
      ...
      // Simulate: ~/.claude/skills/owner -> ../../.agents/skills/owner
      const agentDir = path.join(tmpDir, '.agents', 'skills', 'owner');
      await fs.mkdir(agentDir, { recursive: true });

      const claudeSkillsDir = path.join(tmpDir, '.claude', 'skills');
      await fs.mkdir(claudeSkillsDir, { recursive: true });
      const symlinkPath = path.join(claudeSkillsDir, 'owner');
      await fs.symlink(path.join('..', '..', '.agents', 'skills', 'owner'), symlinkPath);

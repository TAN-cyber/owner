...
describe('Git process helpers', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'owner-git-process-'));
    ...
    await fs.writeFile(path.join(projectRoot, '.owner', 'current-change.json'), '{}\n');
    execFileSync('git', ['add', '.owner/current-change.json'], {
      cwd: projectRoot,
      stdio: 'ignore',
    });
  ...
  it('preserves a leading dot when a tracked path is deleted', async () => {
    await fs.rm(path.join(projectRoot, '.owner', 'current-change.json'));

    expect(gitStatusPaths(projectRoot)).toEqual(['.owner/current-change.json']);
  });
});

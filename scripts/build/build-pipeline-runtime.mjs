...
const runtimeOutput = layout.pipelineRuntime.outputs.runtime;
const runtimeEntry = layout.pipelineRuntime.entries.runtime;

if (!runtimeOutput || !runtimeEntry) {
  throw new Error('Pipeline runtime requires entries.runtime and outputs.runtime');
}

const commandEntries = Object.entries(layout.pipelineRuntime.entries).filter(
  ([name]) => name !== 'runtime',
);
const commandOutputByName = new Map(
  Object.entries(layout.pipelineRuntime.outputs).filter(([name]) => name !== 'runtime'),
);

const banner = {
  js: [
    '#!/usr/bin/env node',
    "import { createRequire as __ownerCreateRequire } from 'module';",
    'const require = __ownerCreateRequire(import.meta.url);',
  ].join('\n'),
};
...
async function bundledRuntime(entry) {
  ...
  if (result.outputFiles.length !== 1) {
    throw new Error(`Expected one Pipeline runtime output, got ${result.outputFiles.length}`);
  }
...
async function checkFreshness(outputRelative, outputFile, expected) {
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      console.error(`Pipeline runtime script is missing: ${outputRelative}`);
      process.exitCode = 1;
      return;
    }
    throw error;
  ...
  if (!actual.equals(expected)) {
    console.error(
      `Pipeline runtime script is stale: ${outputRelative}; run node scripts/build/build-pipeline-runtime.mjs`,
    );
    process.exitCode = 1;
  }
}
...
// the former thin launcher, so running e.g. `owner-state.mjs` only loads the
// state command's dependency graph instead of the whole Pipeline domain.
const outputs = [
  {
...
for (const [name, entry] of commandEntries) {
  ...
  if (!outputRelative) {
    throw new Error(`Pipeline runtime entry '${name}' has no matching output`);
  }
  outputs.push({
    outputRelative,

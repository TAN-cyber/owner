...
const runtimeEntry = layout.loopRuntime?.entries?.runtime;
const runtimeOutput = layout.loopRuntime?.outputs?.runtime;

if (!runtimeEntry || !runtimeOutput) {
  throw new Error('Loop runtime requires entries.runtime and outputs.runtime');
}

const commandEntries = Object.entries(layout.loopRuntime.entries).filter(
  ([name]) => name !== 'runtime',
);
const commandOutputByName = new Map(
  Object.entries(layout.loopRuntime.outputs).filter(([name]) => name !== 'runtime'),
);
...
async function bundledRuntime(entry) {
  ...
  if (result.outputFiles.length !== 1) {
    throw new Error(`Expected one Loop runtime output, got ${result.outputFiles.length}`);
  }
...
async function checkFreshness(outputRelative, outputFile, expected) {
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      console.error(`Loop runtime script is missing: ${outputRelative}`);
      process.exitCode = 1;
      return;
    }
    throw error;
  ...
  if (!actual.equals(expected)) {
    console.error(
      `Loop runtime script is stale: ${outputRelative}; run node scripts/build/build-loop-runtime.mjs`,
    );
    process.exitCode = 1;
  }
}
...
// the former thin launcher, so running e.g. `owner-loop-hook-guard.mjs` only
// loads the hook-guard command's dependency graph instead of the whole Loop
// domain.
const outputs = [
  {
...
for (const [name, entry] of commandEntries) {
  ...
  if (!outputRelative) {
    throw new Error(`Loop runtime entry '${name}' has no matching output`);
  }
  outputs.push({
    outputRelative,
